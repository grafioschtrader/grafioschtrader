...
```bash
./update.sh 0.37.0     # or: ./update.sh latest
```
...
sed -i 's/^GT_VERSION=.*/GT_VERSION=0.37.0/' .env
...
(the installer's default) tracks the newest release; set `GT_VERSION=0.37.0` to
