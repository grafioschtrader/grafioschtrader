...
AS_MEAN_REVERSION_DIP((byte) 68)          // Complex strategy with JSON config
AS_MA_CROSSING_ALERT((byte) 69)           // Watchlist alarm: price vs moving average
AS_RSI_THRESHOLD_ALERT((byte) 70)         // Watchlist alarm: RSI threshold
AS_EXPRESSION_ALERT((byte) 71)            // Watchlist alarm: EvalEx expression

// New bindings in StrategyHelper (DONE)
// AS_MEAN_REVERSION_DIP uses complexConfigClass (4th constructor param) instead of level-specific models
strategyBindingMap.put(AS_MEAN_REVERSION_DIP,
    new StrategyClassBindingDefinition(
        AS_MEAN_REVERSION_DIP,
        null, null, null, StrategyConfig.class, null, false));

// New indicator-based alerts (security level only, repeatable)
strategyBindingMap.put(AS_MA_CROSSING_ALERT,
    new StrategyClassBindingDefinition(
        AS_MA_CROSSING_ALERT, null, null, MaCrossingAlert.class, null, true));
strategyBindingMap.put(AS_RSI_THRESHOLD_ALERT,
    new StrategyClassBindingDefinition(
        AS_RSI_THRESHOLD_ALERT, null, null, RsiThresholdAlert.class, null, true));
strategyBindingMap.put(AS_EXPRESSION_ALERT,
    new StrategyClassBindingDefinition(
        AS_EXPRESSION_ALERT, null, null, ExpressionAlert.class, null, true));
...
| `frontend/.../algo.strategy.implementation.type.ts` | Added `AS_MEAN_REVERSION_DIP = 68` |
...
| `frontend/.../en.json` | Added `AS_MEAN_REVERSION_DIP`, `STRATEGY_CONFIG_YAML`, `YAML_PARSE_ERROR` |
| `frontend/.../de.json` | Added `AS_MEAN_REVERSION_DIP`, `STRATEGY_CONFIG_YAML`, `YAML_PARSE_ERROR` |
...
7. Register all new strategy/alert types in StrategyHelper (AS_MEAN_REVERSION_DIP, AS_MA_CROSSING_ALERT, AS_RSI_THRESHOLD_ALERT, AS_EXPRESSION_ALERT)
8. Add NLS messages
...
2. Add `AS_MEAN_REVERSION_DIP = 68` to frontend enum **(DONE)**
...
11. Add NLS translations: `AS_MEAN_REVERSION_DIP`, `STRATEGY_CONFIG_YAML`, `YAML_PARSE_ERROR` (en + de) **(DONE)**
...
- `frontend/src/app/shared/types/algo.strategy.implementation.type.ts` - added AS_MEAN_REVERSION_DIP = 68 (DONE)
...
- `frontend/src/assets/i18n/en.json` - added ACTIVATABLE, ALERT_CONTEXT, STANDALONE, AS_MEAN_REVERSION_DIP, STRATEGY_CONFIG_YAML, YAML_PARSE_ERROR
- `frontend/src/assets/i18n/de.json` - added ACTIVATABLE, ALERT_CONTEXT, STANDALONE, AS_MEAN_REVERSION_DIP, STRATEGY_CONFIG_YAML, YAML_PARSE_ERROR
...
- Editing an existing simple strategy (e.g., `AS_EXPRESSION_ALERT`) → verify dynamic fields still work correctly
