    ...
    >
      ...
      >
        <ng-template pTemplate="caption">
          ...
          <h5 *ngIf="!gtNetMyEntryId" style="color:red;">
            {{ 'GTNET_COMM_REQUIREMENT' | translate }}
          </h5>
        </ng-template>
        ...
        >
          <tr [pSelectableRow]="el">
            <td>
              <a
                *ngIf="gtNetMessageMap[el.idGtNet]"
                href="#"
                [pRowToggler]="el"
              >
                <i
            ...
            >
              <ng-container [ngSwitch]="field.templateName">
                <ng-container *ngSwitchCase="'myEntry'">
                  <span
                    [style]="
                      el.idGtNet === gtNetMyEntryId ? 'font-weight:500' : null
                    "
                  >
          ...
          <tr>
            ...
            >
              <gtnet-message-treetable
                [gtNetMessages]="gtNetMessageMap[el.idGtNet]"
...
export class GTNetSetupTableComponent extends TableCrudSupportMenu<GTNet> {
  ...
  protected override readData(): void {
    const observable = [
      ...
      ...(!this.formDefinitions
        ? [this.gtNetMessageService.getAllFormDefinitionsWithClass()]
        : []),
    ];
    ...
    combineLatest(observable).subscribe((data) => {
      ...
      this.formDefinitions ??= <
        { [type: string]: ClassDescriptorInputAndShow }
      >data[1];
      this.prepareTableAndTranslate();
    });
  }
  ...
  public override getEditMenuItems(): MenuItem[] {
    ...
    menuItems.push({
      label: 'GTNET_MESSAGE_SEND',
      command: (e) => this.sendMsgSelected(),
