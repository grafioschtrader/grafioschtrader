...
export class GTNetMessageEditComponent
  extends SimpleEntityEditBase<GTNetMessage>
  implements OnInit
{
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_GTNET,
      AppSettings.GTNETMESSAGE.toUpperCase(),
      translateService,
      gps,
      messageToastService,
      gtNetMessageService
  ...
  protected override initialize(): void {
    ...
    if (this.msgCallParam.gtNetMessage) {
      ...
      this.configObject[this.MESSAGE_CODE].formControl.setValue(
        this.msgCallParam.gtNetMessage[this.MESSAGE_CODE]
      );
    } else {
      this.configObject[this.MESSAGE_CODE].valueKeyHtmlOptions =
        SelectOptionsHelper.createHtmlOptionsFromEnum(
          this.translateService,
          GTNetMessageCodeType,
          Object.keys(GTNetMessageCodeType)
            .filter(
              (key) =>
                !isNaN(Number(GTNetMessageCodeType[key])) && key.endsWith('_C')
            )
            .map((k) => GTNetMessageCodeType[k]),
          false
        );
    }
  }
  ...
  private valueChangedOnMessageCode(): void {
    this.messageCodeSubscription = this.configObject[
      this.MESSAGE_CODE
  ...
  private createViewFromSelectedEnum(
    gtNetMessageCodeType: string | GTNetMessageCodeType
  ): void {
    this.classDescriptorInputAndShows =
      this.msgCallParam.formDefinitions[gtNetMessageCodeType];
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    this.classDescriptorInputAndShows.fieldDescriptorInputAndShows.forEach(
      (fDIAS) =>
        (gtNetMessage.gtNetMessageParamMap[fDIAS.fieldName] = new BaseParam(
          valuesFlatten[fDIAS.fieldName]
        ))
    );
    return gtNetMessage;
  }
