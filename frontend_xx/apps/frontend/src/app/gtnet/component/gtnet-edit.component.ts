...
export class GTNetEditComponent
  extends SimpleEntityEditBase<GTNet>
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldCheckboxHeqF('lastpriceUseDetailLog', {
        fieldsetName: this.LAST_PRICE,
      }),
  ...
  protected override initialize(): void {
    this.gps
      .getTimezones()
      .subscribe((timezones: ValueKeyHtmlSelectOptions[]) => {
        this.configObject.timeZone.valueKeyHtmlOptions = timezones;
        this.configObject.entityServerState.valueKeyHtmlOptions =
          SelectOptionsHelper.createHtmlOptionsFromEnum(
            this.translateService,
            GTNetServerStateTypes
          );
        this.configObject.lastpriceServerState.valueKeyHtmlOptions =
          this.configObject.entityServerState.valueKeyHtmlOptions;
      });
  }
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    const gtNet: GTNet = this.copyFormToPrivateBusinessObject(
      new GTNet(),
      this.callParam
    );
    return gtNet;
  ...
  override onHide(event): void {
    this.domainRemoteNameSubscripe &&
      this.domainRemoteNameSubscripe.unsubscribe();
    super.onHide(event);
  }
}
