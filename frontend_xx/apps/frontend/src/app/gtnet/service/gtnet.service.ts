...
@Injectable()
export class GTNetService
  extends AuthServiceWithLogout<GTNet>
  implements ServiceEntityUpdate<GTNet>
{
  constructor(
  ...
  getAllGTNetsWithMessages(): Observable<GTNetWithMessages> {
    return <Observable<GTNetWithMessages>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.GTNET_KEY}` +
            `/gtnetwithmessage`,
          this.getHeaders()
        )
  ...
  checkRemoteDomainWithApplicationInfo(
  ...
  deleteEntity(idGtNet: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.GTNET_KEY}/${idGtNet}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
