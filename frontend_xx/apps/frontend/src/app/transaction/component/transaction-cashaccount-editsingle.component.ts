...
@Component({
  ...
  template: `
    <p-dialog
      header="{{ 'SINGLE_ACCOUNT_TRANSACTION' | translate }}"
...
export class TransactionCashaccountEditSingleComponent
  ...
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldCurrencyNumberHeqF(
        ...
        AppSettings.FID_STANDARD_FRACTION_DIGITS,
        false,
        { ...this.gps.getNumberCurrencyMask(), allowNegative: false },
        true,
        { invisible: true }
      ),
  ...
  selectSingleOptions(fieldConfig: FieldConfig, force: boolean) {
    if (
      fieldConfig.valueKeyHtmlOptions.length === 1 ||
      (force && fieldConfig.valueKeyHtmlOptions.length > 0)
    ) {
  ...
  valueChangedOnTransactionType() {
    this.transactionTypeChangedSub =
      this.configObject.transactionType.formControl.valueChanges.subscribe(
        (data: string) => {
          switch (TransactionType[data]) {
            case TransactionType.FEE:
              AppHelper.enableAndVisibleInput(
            ...
            default:
              ...
              this.setAmountValidator(
                false,
                gteWithMask(this.minAmount),
                'gte'
              );
          }

          this.configObject.cashaccountAmount.labelKey =
            this.configObject.transactionType.valueKeyHtmlOptions.find(
              (vkho) => vkho.key === data
            ).value;
          const invisibleWithdrawal =
            TransactionType[data] !== TransactionType.WITHDRAWAL;
  ...
  valueChangedOnCashaccount(): void {
    this.chashaccountChangedSub =
      this.configObject.idCashaccount.formControl.valueChanges.subscribe(
        (data: string) => {
          const cp: { cashaccount: Cashaccount; portfolio: Portfolio } =
            this.getCashaccountByIdCashaccountFromPortfolios(
              this.portfolios,
          ...
          const precision = this.gps.getCurrencyPrecision(
            this.cashaccountCurrency
          );
          this.adjustNumberInputFractions(
            this.configObject.cashaccountAmount,
            AppSettings.FID_MAX_INTEGER_DIGITS,
            precision
          );
          this.adjustNumberInputFractions(
  ...
  onHide(event) {
    this.transactionTypeChangedSub &&
      this.transactionTypeChangedSub.unsubscribe();
  ...
  submit(value: { [name: string]: any }) {
    ...
    transaction.cashaccount = this.getCashaccountByIdCashaccountFromPortfolios(
      this.portfolios,
    ...
    if (
      ...
      transaction.cashaccountAmount =
        transaction.cashaccountAmount + transaction.transactionCost;
    }
  ...
  private adjustNumberInputFractions(
    fieldConfig: FieldConfig,
    integerDigits: number,
    precision: number
  ): void {
    fieldConfig.currencyMaskConfig.prefix = AppHelper.addSpaceToCurrency(
      this.cashaccountCurrency
    );
  ...
  private getSinglePortfolioByIdPortfolio(): void {
    ...
    this.portfolioService
      .getPortfolioByIdPortfolio(
        this.transactionCallParam.portfolio.idPortfolio
      )
  ...
  private setExistingTransactionToView(): void {
    if (this.transactionCallParam.transaction === null) {
      this.configObject.idCashaccount.formControl.setValue(
      ...
      this.configObject.cashaccountAmount.formControl.setValue(
        BusinessHelper.getTotalAmountFromTransaction(
          this.transactionCallParam.transaction
        ) -
          (this.transactionCallParam.transaction.transactionCost === null
            ? 0
  ...
  private setAmountValidator(
    allowNegativ: boolean,
    validator: ValidatorFn,
    validatorKey: string
  ) {
    this.configObject.cashaccountAmount.currencyMaskConfig.allowNegative =
      allowNegativ;
    this.configObject.cashaccountAmount.validation = [
      Validators.required,
      validator,
    ];
    this.configObject.cashaccountAmount.formControl.setValidators(
      this.configObject.cashaccountAmount.validation
    );
    this.configObject.cashaccountAmount.errors = [
      this.errorRequired,
      {
        name: validatorKey,
        keyi18n: validatorKey,
        param1: this.minAmount,
        rules: ['dirty'],
      },
    ];
    TranslateHelper.translateMessageError(
      this.translateService,
      this.configObject.cashaccountAmount
    );
