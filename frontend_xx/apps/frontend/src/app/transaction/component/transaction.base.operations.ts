...
export abstract class TransactionBaseOperations {
    ...
    if (idSecuritycashaccount != null) {
      for (const portfolio of portfolios) {
        const found: { cashaccount: Cashaccount; portfolio: Portfolio } =
          this.getCashaccountByIdCashaccountFromPortfolio(
            portfolio,
  ...
  getCashaccountByIdCashaccountFromPortfolios(
    ...
    for (const portfolio of portfolios) {
      const found = this.getCashaccountByIdCashaccountFromPortfolio(
        portfolio,
        idSecuritycashaccount
      );
      if (found) {
  ...
  oneOverX(event): void {
    this.configObject.currencyExRate.formControl.setValue(
      1 / this.configObject.currencyExRate.formControl.value
    );
  }
  ...
  protected disableEnableExchangeRateButtons() {
    ...
    this.configObject.oneOverX.disabled =
      this.configObject.currencyExRate.formControl.disabled;
  }
}
