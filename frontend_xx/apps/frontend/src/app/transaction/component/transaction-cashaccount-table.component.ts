  ...
  implements ChildPreservePage, OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.addColumn(
      DataType.String,
      'transaction.currencypair.fromCurrency',
      'CURRENCY',
      true,
      false
    );
      ...
      if (
        !transactions.find(
          (transaction) =>
            ...
            this.cashaccountTransactionPositionSelected.idTransaction
        )
      ) {
        this.cashaccountTransactionPositionSelected =
          this.cashaccountTransactionPositions.find(
  ...
  onRowSelect(event): void {
    ...
    this.selectedTransaction =
      this.cashaccountTransactionPositionSelected.transaction;
  }
  ...
  protected initialize(): void {
    if (this.portfolio) {
      ...
      this.portfolioService
        .getPortfolioByIdSecuritycashaccount(this.idSecuritycashAccount)
        .subscribe((portfolio) => {
  ...
  private loadData() {
    const transactionsObserable: Observable<CashaccountTransactionPosition[]> =
      this.transactionService.getTransactionsWithSaldoForCashaccount(
        this.idSecuritycashAccount
      );
    const currencypairObservable: Observable<Currencypair[]> =
      this.currencypairService.getCurrencypairByPortfolioId(
        this.portfolio.idPortfolio
      );

    combineLatest([transactionsObserable, currencypairObservable]).subscribe(
      (result) => {
        this.cashaccountTransactionPositions =
          this.addCurrencypairToCashaccountTransactionPosition(
            result[0],
            result[1]
          );
  ...
  private addCurrencypairToCashaccountTransactionPosition(
    ...
    currencypairs.forEach((currencypair) =>
      currencypairMap.set(currencypair.idSecuritycurrency, currencypair)
    );

    for (const cashaccountTransactionPosition of cashaccountTransactionPositions) {
      cashaccountTransactionPosition['idTransaction'] =
        cashaccountTransactionPosition.transaction.idTransaction;
      if (cashaccountTransactionPosition.transaction.idCurrencypair != null) {
        cashaccountTransactionPosition.transaction.currencypair =
          currencypairMap.get(
    ...
    this.createTranslatedValueStoreAndFilterField(
      cashaccountTransactionPositions
    );
