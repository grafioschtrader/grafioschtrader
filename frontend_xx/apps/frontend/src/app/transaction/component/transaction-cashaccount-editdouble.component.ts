...
@Component({
  ...
  template: `
    <p-dialog
      header="{{ 'ACCOUNT_TRANSFER' | translate }}"
      [(visible)]="visibleCashaccountTransactionDoubleDialog"
      [responsive]="true"
      [style]="{ width: '550px' }"
      (onShow)="onShow($event)"
      (onHide)="onHide($event)"
...
export class TransactionCashaccountEditDoubleComponent
  extends TransactionCashaccountBaseOperations
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    const calcGroupConfig: FieldConfig[] = [
      DynamicFieldHelper.createFieldCurrencyNumberVSParam(
        'currencyExRate',
        'EXCHANGE_RATE',
        true,
        AppSettings.FID_MAX_FRACTION_DIGITS - 2,
      ...
      */
      DynamicFieldHelper.createFieldCurrencyNumberVSParamHeqF(
        'creditAmount',
        true,
        8,
        8,
  ...
  ////////////////////////////////////////////////
  onHide(event): void {
    this.debitChashaccountChangedSub &&
      this.debitChashaccountChangedSub.unsubscribe();
    this.creditChashaccountChangedSub &&
      this.creditChashaccountChangedSub.unsubscribe();
    this.transactionTimeChangeSub &&
      this.transactionTimeChangeSub.unsubscribe();
    super.close();
  }

  valueChangedOnCalcFields(): void {
    this.valueChangedOnValueCalcFieldsSub =
  ...
  setCalculatedDebitAmount(): number {
    ...
    if (this.currencypair) {
      ...
      this.configObject.debitAmount.formControl.setValue(
        values.creditAmount +
          (values.transactionCost ? values.transactionCost : 0)
      );
    }
  ...
  calcDebitAmount(values: any): number {
    return (
      BusinessHelper.divideMultiplyExchangeRate(
        values.creditAmount,
        values.currencyExRate,
  ...
  valueChangedOnTransactionTime() {
    this.transactionTimeChangeSub =
      this.configObject.transactionTime.formControl.valueChanges.subscribe(() =>
  ...
  valueChangedOnDebitCashaccount(): void {
    this.debitChashaccountChangedSub =
      this.configObject.idDebitCashaccount.formControl.valueChanges.subscribe(
        (data: string) => {
          if (Helper.hasValue(data)) {
            const cp: { cashaccount: Cashaccount; portfolio: Portfolio } =
              this.getCashaccountByIdCashaccountFromPortfolios(
                this.portfolios,
                +data
              );
            this.debitCashaccount = cp.cashaccount;
            this.filterCreditHtmlOptions(
              this.configObject.idDebitCashaccount.valueKeyHtmlOptions,
              cp.cashaccount.idSecuritycashAccount
            );
            this.configObject.debitAmount.currencyMaskConfig.prefix =
              AppHelper.addSpaceToCurrency(this.debitCashaccount.currency);
            this.configObject.transactionCost.currencyMaskConfig.prefix =
              AppHelper.addSpaceToCurrency(this.debitCashaccount.currency);
  ...
  valueChangedOnCreditCashaccount(): void {
    this.creditChashaccountChangedSub =
      this.configObject.idCreditCashaccount.formControl.valueChanges.subscribe(
        (data: string) => {
          if (data) {
            const cp: { cashaccount: Cashaccount; portfolio: Portfolio } =
              this.getCashaccountByIdCashaccountFromPortfolios(
            ...
            this.configObject.creditAmount.currencyMaskConfig.prefix =
              AppHelper.addSpaceToCurrency(this.creditCashaccount.currency);
            this.changeCurrencyExRateState();
          }
        }
      );
  ...
  helpLink(): void {
    BusinessHelper.toExternalHelpWebpage(
      ...
      HelpIds.HELP_TRANSACTION_ACCOUNT
    );
  }

  submit(value: { [name: string]: any }) {
    let cashAccountTransfer: CashAccountTransfer =
      this.createOrCopyExistingTransaction();
  ...
  saveTransaction(cashAccountTransfer: CashAccountTransfer) {
    this.transactionService
      .updateCreateDoubleTransaction(cashAccountTransfer)
      .subscribe({
        next: (newCashAccountTransfer: CashAccountTransfer) => {
          ...
          this.closeDialog.emit(
            new ProcessedActionData(
              cashAccountTransfer.depositTransaction.idTransaction
                ? ProcessedAction.UPDATED
                : ProcessedAction.CREATED,
              [
                newCashAccountTransfer.withdrawalTransaction,
                newCashAccountTransfer.depositTransaction,
              ]
            )
          );
        },
    ...
    this.portfolioService
      ...
      .subscribe((data: Portfolio[]) => {
        ...
        if (this.transactionCallParam.transaction) {
          this.setupModifyExistingTransaction(
            this.transactionCallParam.transaction
          );
        } else {
          this.setupCashaccountSelect(
            this.transactionCallParam.cashaccount.idSecuritycashAccount,
            null
          );
        }
      });
  ...
  private adjustNumberInputFractions(): void {
    const fromCurrencyFraction = this.gps.getCurrencyPrecision(
      this.currencypair
        ? this.currencypair.fromCurrency
        : this.creditCashaccount.currency
    );
    const toCurrencyFraction = this.gps.getCurrencyPrecision(
      this.currencypair
        ? this.currencypair.toCurrency
        : this.debitCashaccount.currency
    );
    DynamicFieldHelper.adjustNumberFraction(
      this.configObject.creditAmount,
      AppSettings.FID_MAX_INTEGER_DIGITS,
      toCurrencyFraction
    );
    DynamicFieldHelper.adjustNumberFraction(
  ...
  private setupCashaccountSelect(
    debitIdCashaccount: number,
    creditIdCashaccount: number
  ) {
    this.configObject.idDebitCashaccount.valueKeyHtmlOptions =
      this.prepareCashaccountOptions(this.portfolios);
    this.filterCreditHtmlOptions(
      this.configObject.idDebitCashaccount.valueKeyHtmlOptions,
      creditIdCashaccount
    );
    ...
    this.configObject.idDebitCashaccount.formControl.setValue(
      debitIdCashaccount
    );
  ...
  private setupModifyExistingTransaction(transaction1: Transaction) {
    this.transactionService
      .getTransactionByIdTransaction(transaction1.connectedIdTransaction)
      .subscribe((data) => {
        this.transactions = [];
        this.transactions.push(transaction1);
        if (
          TransactionType[data.transactionType] === TransactionType.WITHDRAWAL
        ) {
  ...
  private setFormValues(): void {
    this.configObject.transactionTime.formControl.setValue(
      new Date(this.transactions[this.WithIndex].transactionTime)
    );
    this.configObject.currencyExRate.formControl.setValue(
      this.transactions[this.WithIndex].currencyExRate
    );
    this.configObject.transactionCost.formControl.setValue(
      this.transactions[this.WithIndex].transactionCost
    );
    this.configObject.creditAmount.formControl.setValue(
      this.transactions[this.DepIndex].cashaccountAmount
    );
    this.configObject.note.formControl.setValue(
      this.transactions[this.WithIndex].note
    );
  ...
  private setCurrencypair(): void {
    if (
      this.transactionCallParam.transaction &&
      this.transactions[this.DepIndex].cashaccount.currency ===
        this.debitCashaccount.currency &&
      this.transactions[this.WithIndex].cashaccount.currency ===
        this.creditCashaccount.currency
    ) {
    ...
    return (
      ...
      (t !== null &&
        ((this.configObject.idCreditCashaccount.formControl.value &&
          this.transactions[this.DepIndex].cashaccount.idSecuritycashAccount !==
            this.configObject.idCreditCashaccount.formControl.value) ||
          (this.transactions[this.WithIndex].cashaccount
            .idSecuritycashAccount &&
            this.transactions[this.WithIndex].cashaccount
              .idSecuritycashAccount !==
              this.configObject.idDebitCashaccount.formControl.value)))
    );
  }
  ...
  private filterCreditHtmlOptions(
    debitHtmlOptions: ValueKeyHtmlSelectOptions[],
    idCashaccountCredit: number
  ): void {
    const valueKeyHtmlOptions = debitHtmlOptions.filter(
      (debitHtmlOption) => debitHtmlOption.key !== idCashaccountCredit
    );

    this.configObject.idCreditCashaccount.valueKeyHtmlOptions =
      valueKeyHtmlOptions;
    this.configObject.idCreditCashaccount.valueKeyHtmlOptions.unshift(
      new ValueKeyHtmlSelectOptions('', '')
    );
    if (valueKeyHtmlOptions.length === 2) {
      this.configObject.idCreditCashaccount.formControl.setValue(
        valueKeyHtmlOptions[1].key
      );
    } else {
      this.configObject.idCreditCashaccount.formControl.setValue('');
    }
  ...
  private createOrCopyExistingTransaction(): CashAccountTransfer {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (this.transactionCallParam.transaction) {
        ...
        newTransactions[i].transactionType =
          TransactionType[
            i === this.WithIndex
  ...
  private viewToBusinessModel(
    cashAccountTransfer: CashAccountTransfer
  ): CashAccountTransfer {
    const values: any = {};
    this.form.cleanMaskAndTransferValuesToBusinessObject(values, true);

    cashAccountTransfer.withdrawalTransaction.cashaccount =
      this.getCashaccountByIdCashaccountFormPortfolios(
        this.portfolios,
        values.idDebitCashaccount
      ).cashaccount;

    cashAccountTransfer.withdrawalTransaction.transactionCost =
      values.transactionCost;
    cashAccountTransfer.depositTransaction.cashaccount =
      this.getCashaccountByIdCashaccountFormPortfolios(
        this.portfolios,
        values.idCreditCashaccount
      ).cashaccount;
    cashAccountTransfer.depositTransaction.cashaccountAmount =
      values.creditAmount;

    if (this.currencypair != null) {
      cashAccountTransfer.withdrawalTransaction.idCurrencypair =
        this.currencypair.idSecuritycurrency;
      cashAccountTransfer.depositTransaction.idCurrencypair =
        this.currencypair.idSecuritycurrency;
    }

    cashAccountTransfer.withdrawalTransaction.cashaccountAmount =
      this.calcDebitAmount(values);
