  ...
  template: `
    ...
    >
      <p-header>
        ...
        <div
          *ngIf="isOpenMarginInstrument && !transactionCallParam.transaction"
        >
...
export class TransactionSecurityEditComponent
  extends TransactionBaseOperations
  implements OnInit
{
  ...
  ngOnInit(): void {
    if (this.transactionCallParam.transaction) {
      ...
      this.transactionCallParam.transaction.security =
    ...
    const calcGroupConfig: FieldConfig[] = [
      DynamicFieldHelper.createFieldCurrencyNumber(
        'currencyExRate',
        'EXCHANGE_RATE',
        true,
        7,
        AppSettings.FID_MAX_FRACTION_DIGITS,
        false,
      ...
      DynamicFieldHelper.createFieldCurrencyNumberHeqF(
        'taxCost',
        false,
        AppSettings.FID_STANDARD_INTEGER_DIGITS,
        AppSettings.FID_STANDARD_FRACTION_DIGITS,
        false,
        this.gps.getNumberCurrencyMask(),
        true,
        { userDefinedValue: 'S', usedLayoutColumns: 8 }
      ...
      DynamicFieldHelper.createFieldCurrencyNumber(
        ...
        false,
        AppSettings.FID_STANDARD_INTEGER_DIGITS,
        AppSettings.FID_MAX_FRACTION_DIGITS - 3,
        true,
        this.gps.getNumberCurrencyMask(),
        true,
        { userDefinedValue: 'S' }
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldPcalendarHeqF(
        DataType.DateNumeric,
        'exDate',
        false,
 ...
 */
      DynamicFieldHelper.createFieldTextareaInputStringHeqF(
        'note',
        AppSettings.FID_MAX_LETTERS,
        false
      ),
      DynamicFieldHelper.createSubmitButton(),
    ];

    this.configObject = TranslateHelper.prepareFieldsAndErrors(
      this.translateService,
      this.config
    );
    this.translateService
      .get('VOLUME')
      .subscribe((text) => (this.volumei18n = text));
    this.setCurrencyPrefixOnFields(
      null,
      this.transactionCallParam.transaction?.cashaccount.currency
    );
  }
  ...
  valueChangedOnCalcFields(): void {
    this.subObj[this.valueChangedOnValueCalcFields] =
      this.configObject.calcGroup.formControl.valueChanges.subscribe((data) => {
  ...
  valueChangedOnCashaccountOrSecurity(): void {
    ...
    const obs2 =
      this.configObject.idSecuritycurrency.formControl.valueChanges.pipe(
        map((v) => ({
    ...
    this.subObj[this.cashaccountSecurity] = merge(obs1, obs2).subscribe(
      (controlValue) => {
        if (this.configObject.idSecuritycurrency.formControl.value != null) {
          this.selectedSecurity = this.getSecurityById(
            +this.configObject.idSecuritycurrency.formControl.value
          );
          if (this.selectedSecurity) {
            ...
            if (
              ...
              this.configObject.idSecuritycurrency.formControl
            ) {
  ...
  valueChangedOnTransactionTimeAndExDate(): void {
    this.timeChangedSubscribe(
      this.valueChangedOnTransactionTime,
      this.configObject.transactionTime.formControl,
      false
    );
    this.timeChangedSubscribe(
      this.valueChangedOnExDate,
      this.configObject.exDate.formControl,
      true
    );
  }
  ...
  valueChangedOnSecurityaccount(): void {
    this.subObj[this.valueChangedOnIdSecurityaccount] =
      this.configObject.idSecurityaccount.formControl.valueChanges.subscribe(
        (value) => {
          ...
          if (portfolio != null) {
            ...
            if (
              ...
              const cashaccounts: Cashaccount[] =
                portfolio.cashaccountList.filter(
                  (cashaccount) =>
              ...
              if (cashaccounts.length > 0) {
                this.setValueToControl(
                  ...
                  cashaccounts[0].idSecuritycashAccount
                );
  ...
  prepareSecurityOptions(): void {
    if (this.transactionCallParam.security) {
      ...
      this.configObject.idSecuritycurrency.valueKeyHtmlOptions =
        securitiesHtmlSelect;
      this.selectSecurity();
    } else {
  ...
  onHide(event) {
    Object.keys(this.subObj).forEach(
      (key) => this.subObj[key] && this.subObj[key].unsubscribe()
    );
  ...
  submit(value: { [name: string]: any }) {
    ...
    if (this.currencypair != null) {
      this.currencypairService
        .findOrCreateCurrencypairByFromAndToCurrency(
          this.currencypair.fromCurrency,
          this.currencypair.toCurrency
        )
  ...
  helpLink(): void {
    BusinessHelper.toExternalHelpWebpage(
      this.gps.getUserLang(),
      this.isMarginInstrument
        ? HelpIds.HELP_TRANSACTION_MARGIN_BASED
        : HelpIds.HELP_TRANSACTION_CASH_BASED
    );
  }
  ...
  private createQuotationField(): FieldConfig {
    if (
      ...
      return DynamicFieldHelper.createFieldCurrencyNumber(
        'units',
        'NUMBER_OF_DAYS',
        true,
        4,
  ...
  private calcPosTotalOnChanges(data: any): void {
    ...
    if (data.units && data.quotation) {
      if (!this.configObject.securityRisk.invisible) {
        this.setValueToControl(
          this.configObject.securityRisk,
          this.calcPosTotal(data, true) * -1
        );
      }
  ...
  private calcPosTotal(data: any, calcSecurityRisk): number {
    const currencyExRate =
      ...
      Helper.hasValue(data.currencyExRate)
        ? data.currencyExRate
        : 1.0;
        ...
        if (this.securityOpenPositionPerSecurityaccount) {
          quotation -=
            this.transactionCallParam.closeMarginPosition
              .quotationOpenPosition /
            this.securityOpenPositionPerSecurityaccount.securityPositionSummary
    ...
    const accruedInterest =
      !this.configObject.assetInvestmentValue1.invisible &&
      !this.isMarginInstrument &&
    ...
    return BusinessHelper.divideMultiplyExchangeRate(
      this.transactionEditType.calcPosTotal(
        quotation,
        units,
        taxCost,
        transactionCost,
  ...
  private setCurrencyPrefixOnFields(
    ...
    currencySecurity &&
      Object.values(this.configObject)
        ...
        .map((fc) => {
          fc.currencyMaskConfig.prefix =
            AppHelper.addSpaceToCurrency(currencySecurity);
          /*
    ...
    currencyCashaccount &&
      Object.values(this.configObject)
        ...
        .map((fc) => {
          fc.currencyMaskConfig.prefix = AppHelper.addSpaceToCurrency(
            this.currencyCashaccount
          );
          /*
                DynamicFieldHelper.adjustNumberFraction(fc, AppSettings.FID_MAX_INTEGER_DIGITS,
  ...
  private setVisibilityOnFields(): void {
    ...
    if (!this.configObject.assetInvestmentValue1.invisible) {
      this.configObject.assetInvestmentValue1.labelKey = this
        .isOpenMarginInstrument
        ? 'DAILY_CFD_HOLDING_COST'
        : 'ACCRUED_INTEREST';
      this.configObject.assetInvestmentValue1.currencyMaskConfig.allowNegative =
        this.isOpenMarginInstrument;
      this.configObject.assetInvestmentValue1.currencyMaskConfig.precision =
        this.isOpenMarginInstrument
          ? AppSettings.FID_MAX_FRACTION_DIGITS
          : AppSettings.FID_STANDARD_FRACTION_DIGITS;
    }
    ...
    FormHelper.disableEnableFieldConfigsWhenAlreadySet(
      !this.isOpenMarginInstrument,
  ...
  private isBondOrConvertibleBondAndDirectInvestment(): boolean {
    return (
      (this.selectedSecurity.assetClass.categoryType ===
        AssetclassType[AssetclassType.FIXED_INCOME] ||
        this.selectedSecurity.assetClass.categoryType ===
          AssetclassType[AssetclassType.CONVERTIBLE_BOND]) &&
  ...
  private timeChangedSubscribe(
    ...
    this.subObj[propertySub] = control.valueChanges.subscribe((value) => {
      if (!this.configObject.exDate.formControl.value || hasPriority) {
        if (
          value != null &&
          !moment(value).isBefore(DynamicFieldHelper.minDateCalendar)
        ) {
  ...
  private readAllPortfoliosForTenant(): void {
    this.portfolioService
      .getPortfoliosForTenantOrderByName()
  ...
  private setHoldingsForTime(requiredTransactionTime: number): void {
    ...
    if (
      this.holdingCheck.hasChanged(
        this.transactionCallParam.idSecuritycurrency,
        requiredTransactionTime
      )
    ) {
      ...
      holdingsSecurityaccountObserable.subscribe(
        (sopps: SecurityOpenPositionPerSecurityaccount) => {
  ...
  private createSecurityaccountHtmlSelect(): void {
    ...
    if (securityaccountsHtmlSelect.length === 1) {
      ...
      this.setValueToControl(
        ...
        securityaccountsHtmlSelect[0].key
      );
    } else {
      // For Edge a empty Option is needed, otherwise the first Account would be selected
      securityaccountsHtmlSelect.splice(
        0,
        0,
        new ValueKeyHtmlSelectOptions('', '')
      );
      this.configObject.idSecurityaccount.formControl.enable();
      this.selectAccumulteSecurityaccountWhenAvailable(
        securityaccountsHtmlSelect
      );
    }
    this.transactionCallParam.transaction &&
      this.setCurrencyOnSecurityAndCashaccount(
        this.transactionCallParam.transaction.security
      );
  }
  ...
  private prepareAsyncActiveSecurityOptions(): void {
    if (this.configObject.transactionTime.formControl.value) {
      ...
      if (this.securities.length === 0) {
        ...
        SelectOptionsHelper.securitiesEnableDisableOptionsByActiveDate(
          this.securities,
          this.configObject.idSecuritycurrency,
          timeValue
        );
  ...
  private getExDate(): number {
    return this.transactionCallParam.transactionType ===
      TransactionType.DIVIDEND && this.configObject.exDate.formControl.value
  ...
  private loadSecuritiesOnce(timeValue: number): void {
    this.securityService
      .getTradableSecuritiesByTenantAndIdWatschlist(
        this.transactionCallParam.idWatchList
      )
      .subscribe((securities: Security[]) => {
        ...
        SelectOptionsHelper.securityCreateValueKeyHtmlSelectOptions(
          securitiesF,
          this.configObject.idSecuritycurrency
        );
        SelectOptionsHelper.securitiesEnableDisableOptionsByActiveDate(
  ...
  private getTransactionByForm(): Transaction {
    ...
    if (transaction.idCashaccount && this.portfolios) {
      transaction.cashaccount =
        this.getCashaccountByIdCashaccountFormPortfolios(
          this.portfolios,
          +transaction.idCashaccount
        ).cashaccount;
    }
    transaction.security = this.getSecurityById(
      +transaction.idSecuritycurrency
    );
    transaction.transactionType = BusinessHelper.getTransactionTypeAsName(
      this.transactionCallParam.transactionType
    );
    return transaction;
  }
  ...
  private initialize(): void {
    ...
    if (
      ...
      this.transactionEditType = new TransactionSecurityEditFinanceCost(
        this.transactionCallParam
      );
    } else if (
      this.transactionCallParam.transactionType === TransactionType.ACCUMULATE
    ) {
      this.transactionEditType = new TransactionSecurityEditAccumulate(
        this.transactionCallParam
      );
      this.isAccumaulteOrReduce = true;
    } else {
      this.isAccumaulteOrReduce =
        this.transactionCallParam.transactionType === TransactionType.REDUCE;
      this.transactionEditType = new TransactionSecurityEditDividendReduce(
        this.transactionCallParam
      );
    }
    ...
    this.maxTransactionDate =
      this.transactionCallParam.security &&
      new Date(this.transactionCallParam.security.activeToDate).getTime() <
        new Date().getTime()
    ...
    this.setValueToControl(
      this.configObject.transactionTime,
      this.maxTransactionDate
    );
    this.configObject.transactionType.valueKeyHtmlOptions =
  ...
  private changeExistingOpenMarginPos() {
    if (this.transactionCallParam.transaction && this.isOpenMarginInstrument) {
      this.transactionService
        .getConnectedMarginPositionByIdTransaction(
          this.transactionCallParam.transaction.idTransaction
        )
        .subscribe((cmp) => {
  ...
  private getAndSetQuotationSecurity(security: Security): void {
    if (
      ...
      const transactionTime: number =
        +this.configObject.transactionTime.formControl.value;
      if (
        transactionTime > security.sTimestamp ||
  ...
  private getPortfolioByIdSecurityaccount(
  ...
  private getSecurityById(idSecuritycurrency: number): Security {
    return this.securities.filter(
      (security) => security.idSecuritycurrency === idSecuritycurrency
    )[0];
  }
   ...
   */
  private selectAccumulteSecurityaccountWhenAvailable(
    securityaccountsHtmlSelect: ValueKeyHtmlSelectOptions[]
  ) {
    if (this.transactionCallParam.portfolio) {
      const securityaccount =
        this.transactionCallParam.portfolio.securityaccountList.filter((sa) =>
  ...
  private enableDisableCurrencyExRate(enable: boolean): void {
    if (enable) {
      ...
      this.valueChangedOnCA(
        this.valueChangedOnquotationCA,
        this.configObject.quotationCA,
  ...
  private valueChangedOnCA(
    ...
    if (!this.subObj[propertySub]) {
      this.subObj[propertySub] = sourceField.formControl.valueChanges.subscribe(
        (value) =>
  ...
  private currencyExRateHasChanged(): void {
    if (
      !this.configObject.currencyExRate.formControl.disabled &&
      this.oldCurrencyExChange !==
        this.configObject.currencyExRate.formControl.value
    ) {
      this.oldCurrencyExChange =
        this.configObject.currencyExRate.formControl.value;
      this.calculateBaseValueFromCurrencyValue(
        this.configObject.quotationCA,
        this.configObject.quotation
      );
      this.calculateBaseValueFromCurrencyValue(
  ...
  private calculateBaseValueFromCurrencyValue(
    sourceField: FieldConfig,
    targetField: FieldConfig
  ): void {
    if (
      sourceField.formControl.value &&
      this.configObject.currencyExRate.formControl.value
    ) {
  ...
  private hasChangedOnExistingTransaction(): boolean {
    ...
    return (
      t === null ||
      (t !== null &&
        (t.cashaccount.idSecuritycashAccount !==
          this.configObject.idCashaccount.formControl.value ||
          t.security.idSecuritycurrency !==
            this.configObject.idSecuritycurrency.formControl.value))
    );
  }
  ...
  private createHtmlSelectKeyValue(
    ...
    const securityaccountOpenPositionUnits: SecurityaccountOpenPositionUnits[] =
      this.securityOpenPositionPerSecurityaccount
        ?.securityaccountOpenPositionUnitsList || [];

    securitycashaccounts.forEach(
      (securityaccount: Securityaccount | Cashaccount) => {
        if (
  ...
  private getUnitsForSecurityAccountAsString(
    ...
    if (
      ...
      if (this.transactionCallParam?.transaction.idTransaction) {
        existingUnits = Math.min(
          existingUnits,
          this.transactionCallParam?.transaction.units
        );
      }
  ...
  private getUnitsForSecurityAccount(securityaccount: Securityaccount): number {
    const securityaccountOpenPositionUnits: SecurityaccountOpenPositionUnits[] =
      this.securityOpenPositionPerSecurityaccount
        .securityaccountOpenPositionUnitsList;
    const securityaccountOpenPositionUnit =
      securityaccountOpenPositionUnits.find(
        (pos) => securityaccount.idSecuritycashAccount === pos.idSecurityaccount
      );
    return securityaccountOpenPositionUnit
      ? securityaccountOpenPositionUnit.units
      : null;
  }
}
