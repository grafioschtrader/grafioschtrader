...
export class TransactionSecurityFieldDefinition {
  static getFieldDefinition(
    ...
    if (isMarginInstrument) {
      tcm.addColumn(
        DataType.Numeric,
        'transaction.assetInvestmentValue2',
        'VALUE_PER_POINT',
        true,
        false
      );
    ...
    if (
      tsop &&
      tsop.indexOf(TransactionSecurityOptionalParam.SHOW_TAXABLE_COLUMN) >= 0
    ) {
      tcm.addColumnFeqH(
        DataType.Boolean,
        'transaction.taxableInterest',
        true,
        false,
        { templateName: 'check' }
      );
