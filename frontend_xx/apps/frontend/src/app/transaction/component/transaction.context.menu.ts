 ...
 */
@Directive()
export abstract class TransactionContextMenu
  ...
  implements IGlobalMenuAttach
{
  ...
  constructor(
    ...
    this.pageFirstRowSelectedRow =
      this.parentChildRegisterService.getRowPostion(null);
  }
    ...
    AppHelper.confirmationDialog(
      ...
      () => {
        this.transactionService
          .deleteTransaction(transaction.idTransaction)
          .subscribe((response) => {
            this.messageToastService.showMessageI18n(
              InfoLevelType.SUCCESS,
              transaction.connectedIdTransaction != null
                ? 'MSG_DELETE_TRANSACTIONS'
                : 'MSG_DELETE_RECORD',
              {
                i18nRecord:
                  transaction.connectedIdTransaction != null
  ...
  setMenuItemsToActivePanel(): void {
    ...
    this.contextMenuItems = this.getMenuItemsOnTransaction(
  ...
  protected handleEditTransaction(
  ...
  protected handleSecurityEditTransaction(
    transaction: Transaction,
    closeMarginPosition?: CloseMarginPosition
  ): void {
    // this.menuedit.hide();
    ...
    if (Transaction.isSecurityTransaction(transaction.transactionType)) {
      ...
      Transaction.isOnlyCashAccountTransaction(transaction.transactionType)
    ) {
  ...
  protected abstract prepareTransactionCallParam(
    transactionCallParam: TransactionCallParam
  );
  ...
  protected goToFirsRowPosition(id: number): void {
    setTimeout(() => {
      this.pageFirstRowSelectedRow =
        this.parentChildRegisterService.getRowPostion(id);
  ...
  protected getMenuItemsOnTransaction(transaction: Transaction): MenuItem[] {
    ...
    if (transaction && transaction.idTransaction >= 0) {
      menuItems.push({
        ...
        command: (e) =>
          transaction ? this.handleEditTransaction(transaction) : null,
      ...
      menuItems.push({
        ...
        command: (e) =>
          transaction ? this.handleDeleteTransaction(transaction) : null,
        disabled:
          !transaction ||
          !transaction.idTransaction ||
          !this.canDeleteTransaction(transaction),
      });
      ...
      menuItems.push({
        ...
        command: (e) =>
          transaction
            ? this.handleConnectDebitCreditTransaction(transaction)
            : null,
        disabled:
          !transaction ||
          !transaction.idTransaction ||
