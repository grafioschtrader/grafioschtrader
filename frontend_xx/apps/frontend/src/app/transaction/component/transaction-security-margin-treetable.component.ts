...
import {
  CloseMarginPosition,
  TransactionCallParam,
} from './transaction.call.parm';
              ...
              >
                ...
                <ng-container [ngSwitch]="field.templateName">
                  <ng-container *ngSwitchCase="'greenRed'">
                    <span
                      [style.color]="
                        isValueByPathMinus(rowData, field) ? 'red' : 'inherit'
                      "
                    >
...
export class TransactionSecurityMarginTreetableComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  readonly HYPOTHETICAL_TRANSACTION_PROPERTY =
    'hypotheticalTransactionProperty';
  ...
  securityTransactionSummary: SecurityTransactionSummary =
    new SecurityTransactionSummary(null, null);
  ...
  ngOnInit(): void {
    this.currencyColumnConfigMC =
      TransactionSecurityFieldDefinition.getFieldDefinition(
        this,
        this.idTenant,
        true,
  ...
  onNodeSelect(event): void {
    const securityTransactionPosition: SecurityTransactionPosition =
      event.node.data;
  ...
  protected initialize(): void {
    BusinessHelper.setSecurityTransactionSummary(
      ...
      this.createTranslatedValueStoreAndFilterField(
        this.securityTransactionSummary.transactionPositionList
      );
      this.transactionPositionList =
        this.securityTransactionSummary.transactionPositionList;
      this.currencyColumnConfigMC.forEach((cc) => {
        cc.headerSuffix =
          this.securityTransactionSummary.securityPositionSummary.mainCurrency;
  ...
  protected getMenuItemsOnTransaction(transaction: Transaction): MenuItem[] {
      ...
      localContextMenu.push({
        ...
        command: (e) =>
          this.handleClosePosition(
            transaction,
            this.selectedNode.data[this.HYPOTHETICAL_TRANSACTION_PROPERTY]
          ),
        disabled:
          !this.selectedNode.data[this.HYPOTHETICAL_TRANSACTION_PROPERTY],
      });
    ...
    return localContextMenu.concat(
      super.getMenuItemsOnTransaction(transaction)
    );
  }
  ...
  protected canDeleteTransaction(transaction): boolean {
    return (
      ...
      !this.selectedNode.data[this.OPEN_HAS_CLOSE_POSITION]
    );
  }

  protected prepareTransactionCallParam(
    transactionCallParam: TransactionCallParam
  ) {}
  ...
  private addTreeNodes(): void {
    ...
    this.transactionPositionList.forEach((stp: SecurityTransactionPosition) => {
      ...
      if (
        ...
        parentNode.data.transaction.idTransaction ===
          stp.transaction.connectedIdTransaction
      ) {
        // Close position or hypothetical sell/buy
        parentNode.children.push(treeNode);
        if (
          stp.transaction.transactionType ===
            TransactionType[TransactionType.HYPOTHETICAL_BUY] ||
          stp.transaction.transactionType ===
            TransactionType[TransactionType.HYPOTHETICAL_SELL]
        ) {
          parentNode.data[this.HYPOTHETICAL_TRANSACTION_PROPERTY] =
            stp.transaction;
        } else {
  ...
  private handleFinanceCost(openTransaction: Transaction): void {
    this.transactionService
      ...
      .subscribe((pMFC: ProposedMarginFinanceCost) => {
        ...
        fcTransaction.quotation =
          pMFC.financeCost && pMFC.daysToPay
            ? pMFC.financeCost / pMFC.daysToPay
            : 0;
        fcTransaction.transactionType =
          TransactionType[TransactionType.FINANCE_COST];
        if (pMFC.untilDate) {
  ...
  private getNewTransaction(openTransaction: Transaction): Transaction {
    ...
    newTransaction.idCashaccount =
      openTransaction.cashaccount.idSecuritycashAccount;
