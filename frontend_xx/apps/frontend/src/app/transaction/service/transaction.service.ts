...
export class TransactionService extends AuthServiceWithLogout<Transaction> {
  ...
  ): Observable<ClosedMarginPosition> {
    return <Observable<ClosedMarginPosition>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.TRANSACTION_KEY}/` +
            `connectedmargin/${idTransaction}`,
          this.getHeaders()
        )
  ...
  getTransactionByIdTransaction(
    ...
    return <Observable<Transaction>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.TRANSACTION_KEY}/${idTransaction}`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  getTransactionsWithSaldoForCashaccount(
  ...
  getEstimatedMarginFinanceCost(
    idTransaction: number
  ): Observable<ProposedMarginFinanceCost> {
  ...
  updateCreateSecurityTrans(transaction: Transaction): Observable<Transaction> {
    return this.updateEntity(
      transaction,
      transaction.idTransaction,
      `${AppSettings.TRANSACTION_KEY}/securitytrans`
    );
  }

  updateCreateSingleCashTrans(
    transaction: Transaction
  ): Observable<Transaction> {
    return this.updateEntity(
      transaction,
      transaction.idTransaction,
      `${AppSettings.TRANSACTION_KEY}/singlecashtrans`
    );
  }

  updateCreateDoubleTransaction(
    cashAccountTransfer: CashAccountTransfer
  ): Observable<CashAccountTransfer> {
    return <Observable<CashAccountTransfer>>(
      this.httpClient
        .post(
          `${AppSettings.API_ENDPOINT}` +
            `${AppSettings.TRANSACTION_KEY}/cashaccounttransfer`,
          cashAccountTransfer,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
