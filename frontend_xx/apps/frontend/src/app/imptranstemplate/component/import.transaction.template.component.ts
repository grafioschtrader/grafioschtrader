    ...
    >
      <div class="flex-two-columns">
        <h4 class="ui-widget-header singleRowTableHeader">
          {{ 'IMPORTTRANSACTIONGROUP' | translate }}
        </h4>
        <div class="right-half" *ngIf="selectedEntity">
          <ngx-file-drop
            dropZoneLabel="{{ 'DROP_TEMPLATE_HERE' | translate }}"
...
export class ImportTransactionTemplateComponent
  extends SingleRecordMasterViewBase<
  ...
  implements
    OnInit,
    OnDestroy,
    ParentChildRowSelection<ImportTransactionTemplate>
{
  private static readonly MAIN_FIELD = 'idTransactionImportPlatform';

  // Access child components
  @ViewChild(ImportTransactionTemplateTableComponent)
  ittdc: ImportTransactionTemplateTableComponent;
  ...
  constructor(
    ...
    super(
      gps,
      HelpIds.HELP_BASEDATA_IMPORT_TRANSACTION_TEMPLATE_GROUP,
      ImportTransactionTemplateComponent.MAIN_FIELD,
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldInputString(
        'idCsvImportImplementation',
        'TRANSACTION_IMPLEMENTATION',
        32,
        false,
        { disabled: true, usedLayoutColumns: 6 }
      ),
    ];
      ...
      .subscribe((platformTransactionImports: IPlatformTransactionImport[]) => {
        this.platformTransactionImportHtmlOptions =
          SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromArray(
            'id',
            'readableName',
            platformTransactionImports,
            true
          );
        this.configObject.idCsvImportImplementation.valueKeyHtmlOptions =
          this.platformTransactionImportHtmlOptions;
        setTimeout(() => {
  ...
  prepareEditMenu(): MenuItem[] {
    ...
    menuItems = menuItems.concat(
      this.getBaseEditMenu('IMPORTTRANSACTIONGROUP')
    );
    menuItems.push({
  ...
  rowSelectionChanged(
    childEntityList: ImportTransactionTemplate[],
    childSelectedEntity: ImportTransactionTemplate
  ) {
  ...
  public dropped(files: NgxFileDropEntry[]) {
    AppHelper.processDroppedFiles(
      files,
      this.messageToastService,
      'tmpl',
      this.uploadTemplateFiles.bind(this)
    );
  }
  ...
  */
  ...
  private async exportAllTemplates(
    itp: ImportTransactionPlatform
  ): Promise<void> {
    const blob = await this.importTransactionTemplateService
      .getTemplatesByPlatformPlanAsZip(
        this.selectedEntity.idTransactionImportPlatform
      )
      .catch((error) =>
        this.messageToastService.showMessageI18n(
          InfoLevelType.ERROR,
          'DOWNLOAD_TEMPLATE_DATA_FAILED'
        )
      );
    if (blob) {
      ...
      this.messageToastService.showMessageI18n(
        ...
        'DOWNLOAD_TEMPLATE_DATA_SUCCESS'
      );
    }
  }
  ...
  private uploadTemplateFiles(formData: FormData): void {
    this.importTransactionTemplateService
      .uploadImportTemplateFiles(
        this.selectedEntity.idTransactionImportPlatform,
        formData
      )
      .subscribe((sitt: SuccessFailedImportTransactionTemplate) => {
        this.messageToastService.showMessageI18nEnableHtml(
          InfoLevelType.INFO,
          'UPLOAD_TEMPLATES_SUCCESS',
          {
            successNew: sitt.successNew,
            successUpdated: sitt.successUpdated,
            notOwner: sitt.notOwner,
            fileNameError: sitt.fileNameError,
            contentError: sitt.contentError,
          }
        );
