...
import {
  ...
  TemplateCategory,
} from '../../entities/import.transaction.template';
...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'IMPORTTRANSACTIONTEMPLATE' | translate }}"
  ...
  implements OnInit
{
  ...
  protected override initialize(): void {
    this.importTransactionTemplateService
      .getPossibleLanguagesForTemplate()
      .subscribe((data) => {
        this.configObject.templateFormatType.valueKeyHtmlOptions =
          SelectOptionsHelper.createHtmlOptionsFromEnum(
            this.translateService,
            TemplateFormatType
          );
        this.configObject.templateCategory.valueKeyHtmlOptions =
          SelectOptionsHelper.createHtmlOptionsFromEnum(
            this.translateService,
            TemplateCategory
          );
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    if (this.callParam.thisObject) {
      ...
      >this.callParam.parentObject).idTransactionImportPlatform;
    }
    ...
    this.form.cleanMaskAndTransferValuesToBusinessObject(
      importTransactionTemplate
    );
