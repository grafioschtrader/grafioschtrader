...
@Directive()
export abstract class AlgoAssetclassSecurityBaseEdit<
  T
> extends SimpleEntityEditBase<T> {
  ...
  onHide(event): void {
    this.securityaccount1ChangedSub &&
      this.securityaccount1ChangedSub.unsubscribe();
    super.onHide(event);
  }
  ...
  protected getFieldDefinition(): FieldConfig[] {
    return [
      DynamicFieldHelper.createFieldSelectString(
        'idSecurityaccount1',
        'ALGO_SECURITYACCOUNT_1',
        false
      ),
      DynamicFieldHelper.createFieldSelectString(
        'idSecurityaccount2',
        'ALGO_SECURITYACCOUNT_2',
        false
      ),
  ...
  protected setSecurityaccounts(): void {
    this.configObject.idSecurityaccount1.valueKeyHtmlOptions =
      this.createPorfolioSecurityaccountHtmlSelectOptions(
        this.portfolios,
        '-1'
      );
    if (this.algoCallParam.thisObject != null) {
      this.form.transferBusinessObjectToForm(this.algoCallParam.thisObject);
      const securityaccount1: number = (<AlgoAssetclassSecurity>(
        this.algoCallParam.thisObject
      )).idSecurityaccount1;
      this.configObject.idSecurityaccount2.valueKeyHtmlOptions =
        this.createPorfolioSecurityaccountHtmlSelectOptions(
          this.portfolios,
  ...
  protected valueChangedOnSecurityaccount1(): void {
    this.securityaccount1ChangedSub =
      this.configObject.idSecurityaccount1.formControl.valueChanges.subscribe(
        (idSecurityaccount: string) => {
          this.configObject.idSecurityaccount2.valueKeyHtmlOptions =
            this.createPorfolioSecurityaccountHtmlSelectOptions(
              this.portfolios,
