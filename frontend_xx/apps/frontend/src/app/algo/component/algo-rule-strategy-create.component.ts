...
export class AlgoRuleStrategyCreateComponent
  ...
  implements OnInit
{
  ...
  protected assetclassChangeSubList: Subscription[] = new Array(
    AlgoRuleStrategyCreateComponent.maxAssetclassRows
  );
  private assetsclasses: Assetclass[] = [];

  constructor(
    public algoTopService: AlgoTopService,
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFunctionButton('ALGO_ADD_ASSETCLASS', () =>
        this.addAssetclassPercentageRow(false)
      ),
      DynamicFieldHelper.createFunctionButton('ALGO_REMOVE_ASSETCLASS', () =>
        this.removeAssetclassPercentageRow()
      ),
  ...
  initialize() {
    ...
    if (
      this.algoTopCreate.ruleStrategy === RuleStrategy[RuleStrategy.RS_RULE]
    ) {
    ...
    this.watchlistService.getWatchlistsByIdTenant().subscribe((watchlists) => {
      ...
      this.configObject.idWatchlist.valueKeyHtmlOptions =
  ...
  removeAssetclassPercentageRow(): void {
    this.assetclassChangeSubList[this.assetclassCounter] &&
      this.assetclassChangeSubList[this.assetclassCounter].unsubscribe();
    this.assetclassChangeSubList[this.assetclassCounter] = null;
    this.assetclassCounter--;
    this.config = [
      ...this.config.slice(
        0,
        this.config.length - AlgoRuleStrategyCreateComponent.NO_OF_BUTTONS - 2
      ),
      ...this.config.slice(
        this.config.length - AlgoRuleStrategyCreateComponent.NO_OF_BUTTONS,
        this.config.length
      ),
    ];
  }
  ...
  addAssetclassPercentageRow(addFirstElement: boolean): void {
    ...
    const fieldConfig: FieldConfig[] = [
      DynamicFieldHelper.createFieldSelectString(
        AlgoRuleStrategyCreateComponent.ASSETCLASS_FIELD +
          this.assetclassCounter,
        AppSettings.ASSETCLASS.toUpperCase(),
        true,
        { labelSuffix: '' + this.assetclassCounter, usedLayoutColumns: 8 }
      ),
      DynamicFieldHelper.createFieldMinMaxNumber(
        DataType.Numeric,
        AlgoRuleStrategyCreateComponent.PERCENTAGE_FIELD +
          this.assetclassCounter,
        'ALGO_F_WEIGHTING_PERCENTAGE',
        true,
        0.5,
        100,
        { fieldSuffix: '%', usedLayoutColumns: 4 }
      ),
    ];
    this.config = [
      ...this.config.slice(
  ...
  valueChangedOnAssetclass(): void {
    const fieldName =
      AlgoRuleStrategyCreateComponent.ASSETCLASS_FIELD + this.assetclassCounter;
    this.assetclassChangeSubList[this.assetclassCounter] = this.configObject[
      fieldName
    ].formControl.valueChanges.subscribe((idAssetclass) => {
      this.disableEnableAssetclasses(this.getUsedAssetclasses());
    });
  }

  valueChangedOnWatchlist(): void {
    this.watchlistChangedSub =
      this.configObject.idWatchlist.formControl.valueChanges.subscribe(
        (idWatchlist) => {
          if (idWatchlist) {
            this.assetclassService
              .getInvestableAssetclassesByWatchlist(idWatchlist)
              .subscribe((assetsclasses: Assetclass[]) => {
                this.assetsclasses = assetsclasses;
                this.valueKeyHtmlOptionsAssetclasses =
  ...
  onHide(event): void {
    ...
    this.assetclassChangeSubList.forEach(
      (assetclassChangeSub) =>
        assetclassChangeSub && assetclassChangeSub.unsubscribe()
    );
    super.onHide(event);
  }
    ...
    for (let i = 1; i <= this.assetclassCounter; i++) {
      this.algoTopCreate.assetclassPercentageList.push(
        new AssetclassPercentage(
          parseInt(
            value[AlgoRuleStrategyCreateComponent.ASSETCLASS_FIELD + i],
            10
          ),
          parseInt(
            value[AlgoRuleStrategyCreateComponent.PERCENTAGE_FIELD + i],
            10
          )
        )
      );
    ...
    this.algoTopService.create(this.algoTopCreate).subscribe(
      (returnEntity) => {
        this.messageToastService.showMessageI18n(
          ...
          'MSG_RECORD_SAVED',
          { i18nRecord: this.algoTitleKey }
        );
        this.closeDialog.emit(
          new ProcessedActionData(ProcessedAction.CREATED, returnEntity)
        );
      },
  ...
  private getUsedAssetclasses(): string[] {
    ...
    for (let i = 1; i <= this.assetclassCounter; i++) {
      ...
      if (
        usedAssetclassIds.length > 0 &&
        this.configObject[fieldName].formControl
      ) {
        usedAssetclassIds[i - 1] =
          this.configObject[fieldName].formControl.value;
      }
    }
  ...
  private fillAndfilterAssetclasses(): void {
    for (let i = 1; i <= this.assetclassCounter; i++) {
      ...
      this.configObject[fieldName].valueKeyHtmlOptions =
        this.valueKeyHtmlOptionsAssetclasses;
    }
    this.disableEnableAssetclasses(this.getUsedAssetclasses());
  }
