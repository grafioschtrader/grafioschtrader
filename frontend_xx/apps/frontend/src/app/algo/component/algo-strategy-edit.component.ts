...
export class AlgoStrategyEditComponent
  extends SimpleEntityEditBase<AlgoStrategy>
  implements OnInit
{
  ...
  protected override initialize(): void {
    ...
    if (this.algoCallParam.thisObject) {
      ...
      this.configObject[
        AlgoStrategyHelper.FIELD_STRATEGY_IMPL
      ].valueKeyHtmlOptions = SelectOptionsHelper.createHtmlOptionsFromEnum(
        this.translateService,
        AlgoStrategyImplementations,
        [
          AlgoStrategyImplementations[
            (<AlgoStrategy>this.algoCallParam.thisObject)
              .algoStrategyImplementations
          ],
        ],
        false
      );
      this.createViewFromSelectedEnum(
        (<AlgoStrategy>this.algoCallParam.thisObject)
          .algoStrategyImplementations
      );
      this.configObject[
  ...
  private valueChangedOnAlgoStrategyImplementation(): void {
    this.algoStrategyImplementationsChangedSub = this.configObject[
      AlgoStrategyHelper.FIELD_STRATEGY_IMPL
    ].formControl.valueChanges.subscribe((asi: AlgoStrategyImplementations) => {
  ...
  private createViewFromSelectedEnum(
    asi: string | AlgoStrategyImplementations
  ): void {
    const asiNo: number = AlgoStrategyImplementations[asi];
    const inputAndShowDefinition =
      this.algoCallParam.algoStrategyDefinitionForm.inputAndShowDefinitionMap.get(
        asiNo
      );
    if (!inputAndShowDefinition) {
      this.algoStrategyService
        ...
        .subscribe((iasd) => {
          this.algoCallParam.algoStrategyDefinitionForm.inputAndShowDefinitionMap.set(
            asiNo,
            iasd
          );
  ...
  private setExistingModel(): void {
    ...
    const dynamicModel =
      DynamicFieldModelHelper.createAndSetValuesInDynamicModel(
        (<AlgoStrategy>this.algoCallParam.thisObject)[
          AlgoStrategyHelper.FIELD_STRATEGY_IMPL
        ],
        AlgoStrategyHelper.FIELD_STRATEGY_IMPL,
  ...
  protected getNewOrExistingInstanceBeforeSave(value: {
    ...
    algoStrategy.idAlgoAssetclassSecurity =
      this.algoCallParam.parentObject.idAlgoAssetclassSecurity;
    algoStrategy.algoRuleStrategyParamMap = {};
    this.fieldDescriptorInputAndShows.forEach(
      (fDIAS) =>
        (algoStrategy.algoRuleStrategyParamMap[fDIAS.fieldName] = new BaseParam(
          value[fDIAS.fieldName]
        ))
    );
  ...
  override onHide(event): void {
    this.algoStrategyImplementationsChangedSub &&
      this.algoStrategyImplementationsChangedSub.unsubscribe();
    super.onHide(event);
  }
}
