...
export class AlgoSecurityEditComponent
  extends AlgoAssetclassSecurityBaseEdit<AlgoSecurity>
  implements OnInit
{
  constructor(
  ...
  protected initialize(): void {
    const securitiesObservable: Observable<Security[]> =
      this.securityService.getUnusedSecurityForAlgo(
        (<AlgoAssetclass>this.algoCallParam.parentObject)
    ...
    ).subscribe((data: [Security[], Portfolio[]]) => {
      this.configObject.security.referencedDataObject = data[0];
      this.algoCallParam.thisObject &&
        this.configObject.security.referencedDataObject.push(
          (<AlgoSecurity>this.algoCallParam.thisObject).security
        );
  ...
  protected getNewOrExistingInstanceBeforeSave(value: {
    ...
    algoSecurity.idAlgoSecurityParent = (<AlgoAssetclass>(
