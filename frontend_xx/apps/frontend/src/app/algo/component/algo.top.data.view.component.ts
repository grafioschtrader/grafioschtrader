...
@Component({
  template: `
    ...
    >
      ...
      >
        <ng-template pTemplate="caption">
          <h4>
            {{ 'ALGO_OVERVIEW' | translate }} ({{
              (algoTop.ruleStrategy === RuleStrategy[RuleStrategy.RS_RULE]
        ...
        >
          <tr
            ...
            [ngClass]="{
              'kb-row': rowData.constructor.name === 'AlgoAssetclass'
            }"
          >
            <td
      ...
      ></p-contextMenu>
      <strategy-detail
        *ngIf="algoStrategyShowParamCall.algoStrategy"
        [algoStrategyParamCall]="algoStrategyShowParamCall"
      >
      </strategy-detail>
    </div>
    <algo-assetclass-edit
      *ngIf="visibleDialogs[AlgoDialogVisible.ALGO_ASSETCLASS]"
    ...
    <algo-security-edit
      *ngIf="visibleDialogs[AlgoDialogVisible.ALGO_SECURITY]"
    ...
    <algo-strategy-edit
      *ngIf="visibleDialogs[AlgoDialogVisible.ALGO_STRATEGY]"
...
export class AlgoTopDataViewComponent
  ...
  implements IGlobalMenuAttach, OnInit, OnDestroy
{
  ...
  // Detail Show param
  algoStrategyShowParamCall: AlgoStrategyParamCall =
    new AlgoStrategyParamCall();
  ...
  readDataWithTopLevel(): void {
    this.algoTopService
      .getAlgoTopByIdAlgoAssetclassSecurity(
        this.algoTop.idAlgoAssetclassSecurity
      )
      .subscribe((algoTop) => {
        this.dataChangedService.dataHasChanged(
          new ProcessedActionData(ProcessedAction.UPDATED, new AlgoTop())
        );
  ...
  readDataWithoutTopLevel(): void {
    this.algoAssetclassService
      .getAlgoAssetclassByIdTenantAndIdAlgoAssetclassParent(
        this.algoTop.idAlgoAssetclassSecurity
      )
  ...
  getReadableUniqueName(
    dataobject: AlgoTreeName,
    field: ColumnConfig,
    valueField: any
  ): string {
  ...
  extendMenuWithAlgoStrategy(
    ...
    if (
      this.algoStrategyDefinitionForm.unusedAlgoStrategyMap.has(
        parent.idAlgoAssetclassSecurity
      )
    ) {
      algoStrategyMenuItem.disabled =
        this.algoStrategyDefinitionForm.unusedAlgoStrategyMap.get(
          parent.idAlgoAssetclassSecurity
        ).length === 0;
    } else {
      this.algoStrategyService
        .getUnusedStrategiesForManualAdding(parent.idAlgoAssetclassSecurity)
  ...
  handleDeleteEntity<T extends BaseID>(
    entity: T,
    deleteService: DeleteService
  ): void {
    const entityMsg = AppHelper.convertPropertyNameToUppercase(
  ...
  handleCloseAlgoAssetclassDialog(processedActionData: ProcessedActionData) {
    ...
    if (processedActionData.action !== ProcessedAction.NO_CHANGE) {
      if (processedActionData.data instanceof AlgoTopAssetSecurity) {
        this.algoStrategyDefinitionForm.unusedAlgoStrategyMap.delete(
          processedActionData.data.idAlgoAssetclassSecurity
        );
      }
      if (
        this.algoCallParam.parentObject.getId() ===
        this.algoTop.idAlgoAssetclassSecurity
      ) {
  ...
  private translateDataForAssetclass(): void {
    ...
    TranslateHelper.createTranslatedValueStore(
      this.translateService,
      fieldsAssetclass,
      this.algoTop.algoAssetclassList
    );
  }
  ...
  private traverseObjectTreeForAlgoStrategy(
    ...
    algoTopAssetSecurity.algoStrategyList &&
      algoStrategyList.push(...algoTopAssetSecurity.algoStrategyList);
  ...
  private getEditMenu(selectedNode: TreeNode): MenuItem[] {
    ...
    if (selectedNode instanceof TreeAlgoTop) {
      menuItems.push({
        ...
        command: (e) =>
          this.addEdit(AlgoDialogVisible.ALGO_ASSETCLASS, this.algoTop, null),
      });
      ...
      menuItems.push({
        ...
        command: (e) =>
          this.addEdit(
            AlgoDialogVisible.ALGO_ASSETCLASS,
            this.algoTop,
            selectedNode.data
          ),
      });
      ...
      menuItems.push({
        ...
        command: (e) =>
          this.addEdit(
            ...
            <AlgoAssetclass>selectedNode.data,
            null
          ),
      });
      this.extendMenuWithAlgoStrategy(menuItems, selectedNode.data, null);
      ...
      menuItems.push({
        ...
        command: (e) =>
          this.handleDeleteEntity(selectedNode.data, this.algoSecurityService),
      });
      this.extendMenuWithAlgoStrategy(menuItems, selectedNode.data, null);
      ...
      menuItems.push({
        ...
        command: (e) =>
          this.handleDeleteEntity(selectedNode.data, this.algoStrategyService),
      });
    }
  ...
  private refreshSelectedEntity(): void {
    if (this.selectedNode) {
      this.selectedNode = this.searchTree(
        this.treeNodes[0],
        this.selectedNode.data.idTree
      );
  ...
  private setFieldDescriptorInputAndShow<T extends AlgoTopAssetSecurity>(
    ...
    const asiNo: number =
      AlgoStrategyImplementations[algoStrategy.algoStrategyImplementations];
    const inputAndShowDefinition =
      this.algoStrategyDefinitionForm.inputAndShowDefinitionMap.get(asiNo);
    if (!inputAndShowDefinition) {
      this.algoStrategyService
        .getFormDefinitionsByAlgoStrategy(asiNo)
        .subscribe((iasd) => {
          this.algoStrategyDefinitionForm.inputAndShowDefinitionMap.set(
            asiNo,
            iasd
          );
          algoStrategyParamCall.fieldDescriptorShow =
            AlgoStrategyHelper.getFieldDescriptorInputAndShowByLevel(
