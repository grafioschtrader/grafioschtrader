...
export class CashaccountEditComponent
  extends SimpleEntityEditBase<Cashaccount>
  implements OnInit
{
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_PORTFOLIO_ACCOUNT,
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldSelectStringHeqF('currency', true, {
        inputWidth: 5,
        disabled:
          this.callParam.optParam && this.callParam.optParam.hasTransaction,
      }),
      DynamicFieldHelper.createFieldSelectNumber(
        'connectIdSecurityaccount',
        'SECURITYACCOUNT_ASSIGNMENT',
        false
      ),
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    const cashaccount: Cashaccount = this.copyFormToPrivateBusinessObject(
      new Cashaccount(),
      <Cashaccount>this.callParam.thisObject
    );
  ...
  private prepareSecurityaccountOption() {
    if (this.portfolio.securityaccountList.length >= 2) {
      AppHelper.enableAndVisibleInput(
        this.configObject.connectIdSecurityaccoun
      );
      this.configObject.connectIdSecurityaccount.valueKeyHtmlOptions =
        SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromArray(
          ...
          'name',
          this.portfolio.securityaccountList,
          true
        );
    } else {
