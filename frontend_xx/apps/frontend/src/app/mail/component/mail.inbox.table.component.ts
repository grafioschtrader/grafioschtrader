...
export class MailInboxTableComponent
  extends MailInOutTable<MailInbox>
  implements OnDestroy
{
  replyMenuItem: MenuItem = {
    label: 'REPLY',
    command: (event) => this.replyMessage(),
  };
  sendToAdminMenuItem: MenuItem = {
    label: 'SEND_TO_GROUP',
    command: (event) => this.sendToGroup(),
  };
  ...
  constructor(
    ...
    this.addColumnFeqH(DataType.DateTimeString, 'receivedTime', true, false, {
      width: 80,
    });
    ...
    TranslateHelper.translateMenuItems(
      [this.replyMenuItem, this.sendToAdminMenuItem],
      translateService
    );
  ...
  onRowSelect(event): void {
    ...
    if (!mailInbox.hasBeenRead) {
      this.mailInboxService
        .markForRead(mailInbox.idMailInOut)
        .subscribe((mailInboxRc) => (mailInbox.hasBeenRead = true));
    }
  }
  ...
  protected resetMenu(mailInbox: MailInbox): void {
    ...
    this.contextMenuItems = [
      ...(this.selectedEntity ? this.prepareEditMenu(this.selectedEntity) : []),
    ];
