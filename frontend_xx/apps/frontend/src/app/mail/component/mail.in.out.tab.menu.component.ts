...
export class MailInOutTabMenuComponent implements OnInit {
  ...
  constructor(
    ...
    this.items = [
      {
        label: AppSettings.MAIL_INBOX.toUpperCase(),
        command: (event) => this.navigateToChangeRequest(true),
      },
      {
        label: AppSettings.MAIL_SENDBOX.toUpperCase(),
        command: (event) =>
          this.router.navigate([AppSettings.MAIL_SENDBOX_KEY], {
  ...
  private navigateToChangeRequest(activatePanel: boolean) {
    this.router.navigate([AppSettings.MAIL_INBOX_KEY], {
      relativeTo: this.activatedRoute,
    });
  }
}
