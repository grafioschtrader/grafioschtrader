...
export class MailInboxService
  extends AuthServiceWithLogout<MailInbox>
  implements DeleteService, ServiceEntityUpdate<MailInbox>
{
  constructor(
  ...
  public getAllInboxByUser(): Observable<MailInbox[]> {
    return <Observable<MailInbox[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.MAIL_INBOX_KEY}`,
          this.getHeaders()
        )
  ...
  public update(mailInbox: MailInbox): Observable<MailInbox> {
    return this.updateEntity(
      mailInbox,
      mailInbox.idMailInOut,
      AppSettings.MAIL_INBOX_KEY
    );
  }
  ...
  public markForRead(idMailInbox: number): Observable<MailInbox> {
    return this.httpClient
      .post(
        `${AppSettings.API_ENDPOINT}${AppSettings.MAIL_INBOX_KEY}/${idMailInbox}/markforread`,
        null,
  ...
  public deleteEntity(idMailInbox: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.MAIL_INBOX_KEY}/${idMailInbox}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
