...
@Injectable()
export class MailSendboxService
  extends AuthServiceWithLogout<MailSendbox>
  implements DeleteService
{
  ...
  public getAllSendboxByUser(): Observable<MailSendbox[]> {
    return <Observable<MailSendbox[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.MAIL_SENDBOX_KEY}`,
          this.getHeaders()
        )
  ...
  public deleteEntity(idMailInbox: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.MAIL_SENDBOX_KEY}/${idMailInbox}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
