...
export class TradingCalendarStockexchangeComponent
  extends TradingCalendarBase
  implements OnInit
{
  ...
  readData(yearChange: boolean): void {
    const plusObservable: Observable<TradingDaysWithDateBoundaries> =
      this.tradingDaysPlusService.getTradingDaysByYear(
  ...
  markGlobalTradingDay(): void {
    ...
    this.tradingDaysPlusList.forEach((tradingDaysPlus, i) => {
      ...
      this.yearCalendarData.dates.push({
        ...
        select: (range: RangeSelectDays, ranges: RangeSelectDays[]) =>
          this.onDayPlusSelect(range, ranges),
      });
    });
    this.yearCalendarData.disabledDays = [];
    this.globalTradingMinusDaysSet.forEach((time) =>
      this.yearCalendarData.disabledDays.push({ date: new Date(time) })
    );
  }
  ...
  copyCalendarFromOtherExchange(fullCopy: boolean): void {
    this.translateService
      .get(fullCopy ? this.COPY_FULL_TITLE_KEY : this.COPY_YEAR_TITLE_KEY)
      .subscribe((msg) => {
        const ref = this.dialogService.open(
          TradingCalendarOtherExchangeDynamicComponent,
          {
            data: {
              copyTradingDaysFromSourceToTarget:
                new CopyTradingDaysFromSourceToTarget(
                  this.stockexchange.idStockexchange,
                  this.yearCalendarData.year,
                  fullCopy
                ),
  ...
  public hasRightsToModify(): boolean {
    return AuditHelper.hasRightsForEditingOrDeleteEntity(
      this.gps,
      this.stockexchange
    );
  }
