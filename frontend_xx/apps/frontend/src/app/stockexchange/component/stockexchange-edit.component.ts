...
export class StockexchangeEditComponent
  extends SimpleEntityEditBase<Stockexchange>
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldDAInputStringHeqF(
        DataType.TimeString,
        'timeOpen',
        8,
        true
  ...
  private canAssignMic(): boolean {
    return (
      (this.callParam.stockexchange &&
        (!this.callParam.stockexchange.mic || this.callParam.proposeChange)) ||
      !this.callParam.stockexchange
    );
  }
  ...
  protected override initialize(): void {
    ...
    combineLatest(obserables).subscribe((data) => {
      this.countriesAsKeyValue = StockexchangeHelper.transform(
        this.callParam.countriesAsHtmlOptions
      );
      this.configObject.mic.groupItem = this.createMicOptions(true);
      this.configObject.timeZone.valueKeyHtmlOptions = data[0];
      this.configObject.countryCode.valueKeyHtmlOptions =
        this.callParam.countriesAsHtmlOptions;
      ...
      if (this.canAssignMic()) {
        ...
        (<any>(
          this.configObject.mic.elementRef
        )).accessibleViewChild.nativeElement.focus();
      } else {
  ...
  private valueChangedOnOnlyMainStockexchange(): void {
    this.onlyMainStockexchangeSubscribe =
      this.configObject.onlyMainStockexchange.formControl.valueChanges.subscribe(
        (oms: boolean) => {
  ...
  private valueChangedOnMic(): void {
    this.micSubscribe =
      this.configObject.mic.formControl.valueChanges.subscribe((mic) => {
        const sm: StockexchangeMic = this.callParam.stockexchangeMics.find(
          (smf) => smf.mic === mic
        );
        this.configObject.name.formControl.setValue(
          sm.name.toLowerCase().substring(0, this.nameMaxLength)
        );
  ...
  private disableEnableCountry(): void {
    FormHelper.disableEnableFieldConfigs(
      this.configObject.mic.formControl.value.length > 0,
  ...
  private createMicOptions(onlyMainStockexchange: boolean): GroupItem[] {
    ...
    this.callParam.stockexchangeMics
      .filter(
        (sm) =>
          (this.canAssignMic() &&
            ((onlyMainStockexchange && sm.timeZone) ||
              !onlyMainStockexchange) &&
            !this.callParam.existingMic.has(sm.mic)) ||
          ((!this.canAssignMic() || this.callParam.proposeChange) &&
            this.callParam.stockexchange.mic === sm.mic)
      )
      .forEach((sm) => {
    ...
    return Object.values(countryMap).sort((a, b) =>
      a.optionsText.localeCompare(b.optionsText)
    );
  }

  private createGreopAndFristEntry(
    countryMap: { [cc: string]: GroupItem },
    sm: StockexchangeMic,
    country: string
  ): void {
    const gp = new GroupItem(
      null,
      null,
      country,
      'fi fi-' + sm.countryCode.toLowerCase()
    );
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    stockexchange.mic =
      stockexchange.mic.length === 0 ? null : stockexchange.mic;
    return stockexchange;
  }

  private getSecurityObservable(): Observable<Security[]> {
    const securitycurrencySearch = new SecuritycurrencySearch();
    securitycurrencySearch.assetclassType =
      AssetclassType[AssetclassType.EQUITIES];
    ...
    securitycurrencySearch.activeDate = moment().format(
      AppSettings.FORMAT_DATE_SHORT_US
    );
    securitycurrencySearch.stockexchangeCounrtyCode =
      this.callParam.stockexchange.countryCode;
    return this.securityService.searchByCriteria(securitycurrencySearch);
  }

  override onHide(event): void {
    this.onlyMainStockexchangeSubscribe &&
      this.onlyMainStockexchangeSubscribe.unsubscribe();
