...
import {
  StockexchangeBaseData,
  StockexchangeMic,
} from '../model/stockexchange.base.data';
...
@Component({
  template: `
    ...
    >
            ...
            <td *ngFor="let field of fields" [style.min-width.px]="field.width">
              <ng-container [ngSwitch]="field.templateName">
                <ng-container *ngSwitchCase="'owner'">
                  <span
                    [style]="
                      isNotSingleModeAndOwner(field, el)
                        ? 'font-weight:500'
                        : null
                    "
                  >
          ...
          <tr>
            ...
            >
              <trading-calendar-stockexchange
                ...
                [sourceCopyStockexchanges]="
                  getCopySourceStockexchanges(stockexchange.idStockexchange)
                "
              >
              </trading-calendar-stockexchange>
            </td>
      ...
      <p-contextMenu
        *ngIf="contextMenuItems && isActivated()"
        [target]="cmDiv"
...
export class StockexchangeTableComponent
  extends TableCrudSupportMenuSecurity<Stockexchange>
  implements OnDestroy
{
  ...
  protected override readData(): void {
    if (this.callParam.countriesAsHtmlOptions) {
      ...
      this.stockexchangeService
        .getAllStockexchangesBaseData()
        .subscribe((sbd: StockexchangeBaseData) => {
  ...
  private prepareStockexchanges(
    stockexchanges: Stockexchange[],
    shs: StockexchangeHasSecurity[]
  ): void {
    this.entityList = plainToClass(Stockexchange, stockexchanges);
    shs.forEach(
      (keyvalue) => (this.hasSecurityObject[keyvalue.id] = keyvalue.s)
    );
  ...
  protected override prepareCallParam(entity: Stockexchange) {
    this.callParam.hasSecurity =
      entity && this.hasSecurityObject[this.getId(entity)] !== 0;
