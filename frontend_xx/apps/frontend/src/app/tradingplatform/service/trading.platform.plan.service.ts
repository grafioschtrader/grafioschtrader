  ...
  implements ServiceEntityUpdate<TradingPlatformPlan>, DeleteService
{
  ...
  public getAllTradingPlatform(): Observable<TradingPlatformPlan[]> {
    return <Observable<TradingPlatformPlan[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.TRADING_PLATFORM_PLAN_KEY}`,
          this.getHeaders()
        )
  ...
  public deleteEntity(idTradingPlatform: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.TRADING_PLATFORM_PLAN_KEY}/${idTradingPlatform}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  ): Observable<TradingPlatformPlan> {
