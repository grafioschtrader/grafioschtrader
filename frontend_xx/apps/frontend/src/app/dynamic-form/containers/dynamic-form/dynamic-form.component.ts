...
import {
  FieldFormGroup,
  FormGroupDefinition,
} from '../../models/form.group.definition';
  ...
  template: `
    <div class="nopadding container-fluid #actualTarget">
      <form [formGroup]="form" (ngSubmit)="handleSubmit($event)">
        <div
          *ngIf="showWithFieldset; then withFieldset; else withoutFieldset"
        ></div>
        <ng-template #withFieldset>
          <fieldset
            [ngClass]="fieldsetConfig.fieldsetName ? 'out-border' : ''"
            *ngFor="let fieldsetConfig of fieldsetConfigs"
          >
            <legend
              [ngClass]="fieldsetConfig.fieldsetName ? 'out-border-legend' : ''"
            >
            ...
            <ng-container *ngFor="let field of controlsWithGroups">
              ...
              <ng-template #groupTemplate>
                <div [formGroupName]="field['formGroupName']">
                  ...
                  <error-message
                    [baseFieldFieldgroupConfig]="field"
                  ></error-message>
                </div>
              </ng-template>
            </ng-container>
        ...
        <div [class.ui-widget-content]="!formConfig.nonModal">
          <button
            *ngIf="formConfig.helpLinkFN"
            pButton
            pRipple
            type="button"
            icon="pi pi-question"
            (click)="helpLink($event)"
...
export class DynamicFormComponent implements OnChanges, OnInit {
  ...
  get formGroups(): FormGroupDefinition[] {
    return this.config.filter(
      (fieldFormGroup: any) => !!fieldFormGroup.formGroupName
    ) as FormGroupDefinition[];
  }
  ...
  get controlsWithGroups(): FieldFormGroup[] {
    return this.config.filter(
      (fieldFormGroup: any) =>
        ...
        !(
          fieldFormGroup.inputType === InputType.Button ||
          fieldFormGroup.inputType === InputType.Pbutton
        )
    );
  }
  ...
  get buttons(): FieldConfig[] {
    return FormHelper.getFieldConfigs(this.config).filter(
      (config) =>
        !config.buttonInForm &&
        (config.inputType === InputType.Button ||
          config.inputType === InputType.Pbutton)
    );
  }
  ...
  createGroupAndControl(): UntypedFormGroup {
    ...
    this.formGroups.forEach((formGroupDefinition) => {
      ...
      childGroups[formGroupDefinition.formGroupName] =
        formGroupDefinition.formControl;
      formGroupDefinition.fieldConfig.forEach((control) =>
        childGroups[formGroupDefinition.formGroupName].addControl(
          control.field,
          this.createControl(control)
        )
      );
      formGroupDefinition.formControl.setValidators(
        formGroupDefinition.validation
      );
    });
      ...
      if (config.inputType === InputType.Input && config.valueKeyHtmlOptions) {
        ...
        if (!value) {
          valueKeyHtmlOption = config.valueKeyHtmlOptions.find(
            (valueKeyHtmlOptions) => '' === valueKeyHtmlOptions.key
          );
        } else {
          valueKeyHtmlOption = config.valueKeyHtmlOptions.find(
  ...
  cleanMaskAndTransferValuesToBusinessObject(
    targetObject: any,
    createProperty: boolean = false
  ): void {
  ...
  setDisableAll(disable: boolean) {
    ...
    FormHelper.flattenConfigMap(this.config).forEach((fieldConfig) => {
      if (
        fieldConfig.inputType !== InputType.Button &&
        fieldConfig.inputType !== InputType.Pbutton
      ) {
