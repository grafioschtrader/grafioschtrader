...
export abstract class SimpleEntityEditBase<T> extends SimpleEditBase {
  ...
  submit(value: { [name: string]: any }): void {
    ...
    this.serviceEntityUpdate.update(entityNew).subscribe({
      ...
      error: (transformedError: TransformedError) => {
        if (
          transformedError.errorClass &&
          transformedError.errorClass instanceof LimitEntityTransactionError
        ) {
          this.closeDialog.emit(
            new ProcessedActionData(
              ProcessedAction.NO_CHANGE,
              null,
              transformedError
            )
          );
  ...
  public copyFormToPrivateBusinessObject(
    targetEntity: T,
    existingEntity: T
  ): T {
    if (existingEntity) {
  ...
  protected abstract getNewOrExistingInstanceBeforeSave(value: {
    [name: string]: any;
  }): T;
}
