  ...
  implements ServiceEntityUpdate<Globalparameters>
{
      ...
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}` +
            `${AppSettings.GLOBALPARAMETERS_P_KEY}`,
          this.getHeaders()
        )
  ...
  public isUiShowMyProperty(): boolean {
    return (
      sessionStorage.getItem(GlobalSessionNames.UI_SHOW_MY_PROPERTY) === 'true'
    );
  }
  ...
  public isEntityCreatedByUser(entity: Auditable): boolean {
    return (
      entity.createdBy === +sessionStorage.getItem(GlobalSessionNames.ID_USER)
    );
  }
  ...
  public getTimeDateFormatForTable(): string {
    if (!this.timeDateFormat) {
      ...
      this.timeDateFormat =
        formatTime + ' ' + formatYear.replace(/YYYY/g, 'YY');
    }
  ...
  public getDateFormatWithoutYear(): string {
    if (!this.dateFormatWithoutYear) {
      ...
      if (this.dateFormatWithoutYear.match(/.YYYY/g)) {
        this.dateFormatWithoutYear = this.dateFormatWithoutYear.replace(
          /.YYYY/,
          ''
        );
      }
  ...
  public getDateFormat(): string {
    return (
      this.dateFormat ||
      (this.dateFormat = this.getDateFormatYearCalendar('YY'))
    );
  }

  public getDateFormatForCalendar(): string {
    return (
      this.dateFormatCalendar ||
      (this.dateFormatCalendar = this.getDateFormatYearCalendar('Y'))
    );
  }
  ...
  public getKeyNameByEntityName(entityName: string): string {
    if (!this.entityKeyMapping) {
      this.entityKeyMapping = JSON.parse(
        sessionStorage.getItem(GlobalSessionNames.ENTITY_KEY_MAPPING)
      );
    }
  ...
  public getCurrencyPrecision(currency: string): number {
    if (!this.currencyPrecisionMap) {
      this.currencyPrecisionMap = JSON.parse(
        sessionStorage.getItem(GlobalSessionNames.CURRENCY_PRECISION)
      );
    }
    return this.currencyPrecisionMap[currency]
  ...
  public getThousandsSeparatorSymbol(): string {
    if (!this.thousandsSeparatorSymbol) {
      this.thousandsSeparatorSymbol = sessionStorage.getItem(
  ...
  public getDecimalSymbol(): string {
    if (!this.decimalSymbol) {
      this.decimalSymbol = sessionStorage.getItem(
  ...
  public getNumberFormatRaw(): NumberFormat {
    if (!this._numberFormatRaw) {
      this._numberFormatRaw = new Intl.NumberFormat(this.getLocale(), {
  ...
  public useWebsocket(): boolean {
    if (this._useWebsocket === null) {
      this._useWebsocket =
        sessionStorage.getItem(GlobalSessionNames.USE_WEBSOCKET) === 'true';
    }
  ...
  public getCountriesForSelectBox(): Observable<ValueKeyHtmlSelectOptions[]> {
    return <Observable<ValueKeyHtmlSelectOptions[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}` +
            `${AppSettings.GLOBALPARAMETERS_P_KEY}/countries`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  public getMaxTenantLimitsByMsgKey(
    msgKeys: TenantLimitTypes[]
  ...
  public getIntraUpdateTimeout(): Observable<number> {
    return this.getGlobalparameterNumber(
      GlobalSessionNames.UPDATE_TIME,
      'updatetimeout'
    );
  }
  ...
  public getStartFeedDateAsTime(): Observable<number> {
    return this.getGlobalparameterNumber(
      GlobalSessionNames.START_FEED_DATE,
      'startfeeddate'
    );
  }
  ...
  private getGlobalparameterNumber(
    ...
    if (sessionStorage.getItem(globalSessionNames)) {
      ...
      return <Observable<number>>(
        this.httpClient
          ...
          .pipe(
            tap((value) =>
              sessionStorage.setItem(globalSessionNames, '' + value)
            ),
