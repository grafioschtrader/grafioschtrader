...
@Injectable()
export class ProposeUserTaskService
  extends AuthServiceWithLogout<ProposeUserTask>
  implements DeleteService, ServiceEntityUpdate<ProposeUserTask>
{
  ...
  public rejectUserTask(
    idProposeRequest: number,
    rejectNote: string
  ): Observable<StringResponse> {
    return <Observable<StringResponse>>(
      this.httpClient
        .post(
  ...
  public getFormDefinitionsByUserTaskType(
    userTaskType: UserTaskType
  ): Observable<FieldDescriptorInputAndShow[]> {
    return <Observable<FieldDescriptorInputAndShow[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}` +
            `${AppSettings.PROPOSE_USER_TASK_KEY}/form/${userTaskType}`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  public update(userTask: ProposeUserTask): Observable<ProposeUserTask> {
    return this.updateEntity(
      userTask,
      userTask.idProposeRequest,
      AppSettings.PROPOSE_USER_TASK_KEY
    );
  }
  ...
  public deleteEntity(idProposeRequest: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.PROPOSE_USER_TASK_KEY}/${idProposeRequest}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
