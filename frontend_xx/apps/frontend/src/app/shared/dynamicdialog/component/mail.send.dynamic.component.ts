...
export class MailSendDynamicComponent
  extends FormBase
  implements OnInit, AfterViewInit
{
  ...
  submit(value: { [name: string]: any }): void {
    ...
    this.mailSendboxService
      .replyMessage(mailSendbox)
      .subscribe((sendMailSendbox: MailSendbox) => {
  ...
  private getGroupOrUserField(): FieldConfig {
    if (this.mailSendParam.idUserTo) {
      return DynamicFieldHelper.createFieldInputStringHeqF(
        this.ID_USER_TO,
        10,
        true
      );
    } else {
      return DynamicFieldHelper.createFieldSelectStringHeqF(
        this.ROLE_NAME_TO,
        true
      );
    }
  ...
  private updateView(): void {
    if (this.mailSendParam.idUserTo) {
      this.configObject[this.ID_USER_TO].formControl.setValue(
        this.mailSendParam.idUserTo
      );
      this.configObject[this.ID_USER_TO].formControl.disable();
    } else {
      this.configObject[this.ROLE_NAME_TO].valueKeyHtmlOptions =
        SelectOptionsHelper.translateArrayKeyEqualValue(this.translateService, [
          AppSettings.ROLE_ALL_EDIT,
          AppSettings.ROLE_ADMIN,
        ]);
    }
    const mailReplayLine = 'MAIL_REPLY_LINE';
    const idUserFrom = AppHelper.convertPropertyForLabelOrHeaderKey(
      this.ID_USER_FROM
    );
    ...
    if (this.mailSendParam.mailInbox) {
      this.translateService
        .get([mailReplayLine, idUserFrom, subject])
        .subscribe((replyLines) => {
          this.configObject[this.SUBJECT].formControl.setValue(
            this.mailSendParam.mailInbox.subject
          );
          const replayText =
            this.NEW_LINE +
