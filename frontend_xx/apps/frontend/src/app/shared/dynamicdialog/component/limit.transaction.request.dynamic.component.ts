 ...
 */
@Component({
  template: ` {{ 'APPLY_LIMIT_TEXT' | translate }}
    <dynamic-form
      [config]="config"
...
export class LimitTransactionRequestDynamicComponent
  extends FormBase
  implements OnInit
{
  ...
  ngOnInit(): void {
    ...
    this.proposeUserTaskService
      ...
      .subscribe((fDIaSs: FieldDescriptorInputAndShow[]) => {
        ...
        this.configObject[this.ENTITY_NAME].defaultValue =
          this.dynamicDialogConfig.data.entityName;
      });
  }
  ...
  submit(value: { [name: string]: any }): void {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      proposeUserTask.addProposeChangeField(
        new ProposeChangeField(
          this.config[i].field,
          fieldObject[this.config[i].field]
        )
      );
    }
