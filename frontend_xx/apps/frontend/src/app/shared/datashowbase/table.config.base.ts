...
export abstract class TableConfigBase extends TableTreetableTotalBase {
  ...
  get groupFields(): ColumnConfig[] {
    ...
    for (let i = 0; i < this.fields.length; i++) {
      ...
      if (
        this.fields[i].columnGroupConfigs &&
        this.fields[i].columnGroupConfigs[0].colspan
      ) {
        const nextUsedGroupColumn = this.getNextUsedGroupColumnIndex(i + 1);
        const newI = Math.min(
          i + this.fields[i].columnGroupConfigs[0].colspan,
          nextUsedGroupColumn
        );
  ...
  public prepareFilter(data: any[]) {
    this.fields.forEach((field) => {
      if (field.filterType && field.filterType === FilterType.withOptions) {
        ...
        valueLabelHtmlSelectOptions.push(
          new ValueLabelHtmlSelectOptions('', ' ')
        );
        if (field.translateValues && field.translatedValueMap) {
          Object.keys(field.translatedValueMap)
            ...
            .forEach((key) =>
              valueLabelHtmlSelectOptions.push(
                new ValueLabelHtmlSelectOptions(
                  key,
                  field.translatedValueMap[key]
                )
              )
            );
        } else {
          const uniqueValuesSet = new Set(
            data.map((item) => this.getValueByPath(item, field))
          );
          ...
          Array.from(uniqueValuesSet)
            ...
            .forEach((value) => {
              valueLabelHtmlSelectOptions.push(
                new ValueLabelHtmlSelectOptions(value, value)
              );
            });
        }
  ...
  createFilterField(data: any[]): void {
    const columnConfigs = this.fields.filter(
      (columnConfig) =>
        ...
        columnConfig.dataType === DataType.DateNumeric
    );
  ...
  public filterDate(event, columnConfig: ColumnConfig, table: Table): void {
    if (event) {
      if (columnConfig.dataType === DataType.DateNumeric) {
        ...
        const dateStringUS = moment(event).format(
  ...
  changeToUserSetting() {
    let i = 0;
    this.fields.forEach(
      (columnConfig) => (columnConfig.visible = this.visibleRestore[i++])
    );
  }
  ...
  onColResize(event) {
    const columnConfig = this.getColumnConfigByHeaderTranslated(
      event.element.innerText.trim()
    );
  ...
  customSort(event: SortEvent): void {
    if (event.mode === 'single') {
      this.customSortMultiple(event.data, [
    ...
    data.sort((data1, data2) => {
      ...
      do {
        ...
        const isDirectAccess =
          columnConfig.translateValues || columnConfig.fieldValueFN;
  ...
  hideShowColumnByFileHeader(fileHeader: string, visible: boolean) {
    this.fields
      .filter((field) => field.headerKey === fileHeader)
      .forEach((field) => (field.visible = visible));
  }

  handleHideShowColumn(event, field: ColumnConfig) {
    field.visible = !field.visible;
    event.item.icon = field.visible
      ? AppSettings.ICONNAME_SQUARE_CHECK
      : AppSettings.ICONNAME_SQUARE_EMTPY;
