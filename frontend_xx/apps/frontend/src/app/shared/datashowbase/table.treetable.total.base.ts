...
export abstract class TableTreetableTotalBase extends ShowRecordConfigBase {
  ...
  setSameFieldNameForGroupField(startIndex: number): void {
    for (let i = startIndex; i < this.fields.length; i++) {
      this.fields[i].columnGroupConfigs = [
        new ColumnGroupConfig(this.fields[i].field),
      ];
    }
  }

  isValueColumnTotalMinus(
    columnConfig: ColumnConfig,
    arrIndex: number,
    data: any,
    mapKey: any
  ): boolean {
    return this.getValueColumnTotal(
      columnConfig,
      arrIndex,
      data,
      mapKey
  ...
  getValueColumnTotal(
    ...
    let value = '';
    if (
      columnConfig.columnGroupConfigs &&
      arrIndex < columnConfig.columnGroupConfigs.length
    ) {
      if (columnConfig.columnGroupConfigs[arrIndex].fieldTextFN) {
        ...
        if (columnConfig.columnGroupConfigs[arrIndex].fieldValue) {
          const dataValue = this.getFieldValueColumnTotal(
            columnConfig,
            arrIndex,
            data,
            mapKey
  ...
  isColumnTotalByRowIndexMinus(
    ...
    return this.getValueColumnTotalByRowIndex(
      columnConfig,
      arrIndex,
      groupChangeIndexMap,
      rowIndex
  ...
  public isNotSingleModeAndOwner(
    columnConfig: ColumnConfig,
    entity: Auditable
  ): boolean {
    return (
      this.gps.isUiShowMyProperty() &&
      columnConfig.templateName === 'owner' &&
      this.gps.isEntityCreatedByUser(entity)
    );
  }

  getTextValueColumnTotal(
    columnConfig: ColumnConfig,
    arrIndex: number
  ): string {
