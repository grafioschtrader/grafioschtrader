  ...
  implements OnInit, IGlobalMenuAttach
{
  ...
  handleDeleteEntity(entity: T) {
    AppHelper.confirmationDialog(
      ...
      () => {
        ...
        this.deleteService
          .deleteEntity(entity[this.entityKeyName])
          .subscribe((response) => {
  ...
  protected getEditMenuItems(entity: T): MenuItem[] {
    ...
    if (this.crudMenuOptions.indexOf(CrudMenuOptions.Allow_Create) >= 0) {
      menuItems.push({
        label:
          'CREATE|' + this.entityNameUpper + AppSettings.DIALOG_MENU_SUFFIX,
  ...
  protected refreshSelectedEntity(): void {
    if (this.selectedEntity) {
      this.resetMenu(
        this.entityList.find(
          (entity) =>
            entity[this.entityKeyName] ===
            this.selectedEntity[this.entityKeyName]
        )
      );
    }
  }
