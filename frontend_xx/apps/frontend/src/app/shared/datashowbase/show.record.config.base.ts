...
export abstract class ShowRecordConfigBase {
  ...
  addColumnFeqH(
    ...
    return this.addColumnToFields(
      ...
      dataType,
      field,
      AppHelper.convertPropertyForLabelOrHeaderKey(field),
      visible,
      changeVisibility,
      optionalParams
    );
  ...
  getColumnConfigByHeader(header: string): ColumnConfig {
    return this.fields.find(
      (columnConfig) => columnConfig.headerKey === header
    );
  }
  ...
  getColumnConfigByFieldAndHeader(field: string, header: string): ColumnConfig {
    return this.fields.find(
      (columnConfig) =>
        columnConfig.field === field && columnConfig.headerKey === header
    );
  }

  getColumnConfigByHeaderTranslated(headerTranslated: string): ColumnConfig {
    return this.fields.find(
      (columnConfig) => columnConfig.headerTranslated === headerTranslated
    );
  }
  ...
  getValueByPath(dataobject: any, field: ColumnConfig) {
    return this.getValueByPathWithField(
      dataobject,
      field,
      field.fieldTranslated || field.field
    );
  }

  getValueByPathWithField(
    dataobject: any,
    field: ColumnConfig,
    valueField: string
  ) {
    let value = AppHelper.getValueByPathWithField(
      this.gps,
      this.translateService,
      dataobject,
      field,
  ...
  protected translateHeaders(
    translateHeaderKeys: string[],
    columConfig: ColumnConfig[]
  ): void {
    this.translateService
      .get(translateHeaderKeys)
      .subscribe((allTranslatedTexts: any) =>
        columConfig.map(
          (field) =>
  ...
  protected translateHeadersTooltips(
    translateHeaderKeys: string[],
    columConfig: ColumnConfig[]
  ): void {
    const tSuffix = '_TOOLTIP';
    this.translateService
      .get(translateHeaderKeys.map((thtk) => thtk + tSuffix))
      .subscribe((allTranslatedTexts: any) =>
        columConfig.map((field) => {
          const key = field.headerKey + tSuffix;
          field.headerTooltipTranslated =
            allTranslatedTexts[key] !== key ? allTranslatedTexts[key] : null;
        })
      );
  }
        ...
        columConfig.columnGroupConfigs.forEach((cc) => {
          if (cc.textValueKey) {
            this.translateService
              .get(cc.textValueKey)
              .subscribe(
                (translatedText) => (cc.textValueTranslated = translatedText)
              );
          }
        });
      }
