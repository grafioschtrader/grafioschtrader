...
export abstract class PasswordBaseComponent {
  ...
  afterViewInit(): void {
    ...
    TranslateHelper.translateMessageError(
      ...
      this.configObject[this.passwordConfirm]
    );
    this.configObject[this.passwordConfirm].validation.push(
      equalTo(this.configObject.password.formControl)
    );
    this.configObject[this.passwordConfirm].formControl.setValidators(
      this.configObject.passwordConfirm.validation
    );
    this.configObject[
  ...
  protected init(
    ...
    this.configPassword = [
      ...
      DynamicFieldModelHelper.ccWithFieldsFromDescriptor(
        'password',
        newPassword ? 'PASSWORD_NEW' : 'PASSWORD',
        fdias
      ),
      DynamicFieldModelHelper.ccWithFieldsFromDescriptor(
        'password',
