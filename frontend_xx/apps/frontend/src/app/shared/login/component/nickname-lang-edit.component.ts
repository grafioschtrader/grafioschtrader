...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'NICKNAME_LOCALE_CHANGE' | translate }}"
...
export class NicknameLangEditComponent
  ...
  implements OnInit
{
  ...
  submit(value: { [name: string]: any }): void {
    ...
    this.loginService.updateNicknameLocale(userOwnProjection).subscribe({
      next: (successfullyChanged: SuccessfullyChanged) => {
        this.messageToastService.showMessage(
          InfoLevelType.INFO,
          successfullyChanged.message
        );
  ...
  protected initialize(): void {
    combineLatest([
      this.gps.getSupportedLocales(),
      this.loginService.getOwnUser(),
    ]).subscribe((data) => {
