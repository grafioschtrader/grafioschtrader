...
export class MenubarComponent implements OnInit, OnDestroy {
  ...
  public async downloadPersonalDataAsZip(): Promise<void> {
    ...
    if (blob) {
      ...
      this.messageToastService.showMessageI18n(
        ...
        'DOWNLOAD_PERSONAL_DATA_SUCCESS'
      );
    }
  }
  ...
  private toggleMainTree(setOnlyIcon: boolean) {
    ...
    this.menuItems[TopMenuTypes.COLLAPSE_TREE].icon =
      'pi ' +
      (this.viewSizeChangedService.isMainTreeVisible()
        ? 'pi-chevron-left'
        : 'pi-chevron-right');
  }
  ...
  private contextHelp() {
    if (this.activePanelService.activatedPanel) {
      const helpIds: HelpIds =
        this.activePanelService.activatedPanel.getHelpContextId();
      if (helpIds) {
