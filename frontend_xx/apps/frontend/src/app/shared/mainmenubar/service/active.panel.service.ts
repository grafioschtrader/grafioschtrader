...
export class ActivePanelService {
  ...
  activatePanel(
    ...
    if (menus.menuGroup != null) {
      ...
      menus.showMenu.push(
        ...this.addMenuItemsGroup(menus.menuGroup, menus.menuGroupParam)
      );
    }
    ...
    this.registeredPanel
      .filter((panel) => panel !== componentClass)
      .forEach((panel) => panel.callMeDeactivate());
  }
  ...
  private navigateToRoute(
    ...
    this.router.navigate([
      ...
      { outlets: { mainbottom: [route, param] } },
    ]);
  }
}
