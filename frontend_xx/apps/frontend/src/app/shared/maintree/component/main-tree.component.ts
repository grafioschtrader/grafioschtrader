...
import {
  TenantLimit,
  TenantLimitTypes,
} from '../../../entities/backend/tenant.limit';
...
export class MainTreeComponent implements OnInit, OnDestroy, IGlobalMenuAttach {
  ...
  addAndRefreshWatchlistToTree(): void {
    combineLatest([
      ...
      data[1].forEach(
        (keyvalue) => (this.hasSecurityObject[keyvalue[0]] = keyvalue[1])
      );
      ...
      for (const watchlist of watchlists) {
        const treeNode = {
          ...
          icon:
            'pi ' +
            (watchlist.idWatchlist === this.tenant.idWatchlistPerformance
              ? 'pi-chart-line'
              : null),
          data: new TypeNodeData(
  ...
  addAdminData(): void {
    ...
    this.portfolioTrees[this.ADMINDATA_INDEX].children.push({
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.GLOBAL_SETTINGS_KEY),
        null,
        null,
        null
      ),
    ...
    this.portfolioTrees[this.ADMINDATA_INDEX].children.push({
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.TASK_DATA_CHANGE_MONITOR_KEY),
        null,
        null,
        null
      ),
    ...
    if (AuditHelper.hasAdminRole(this.globalParamService)) {
      this.portfolioTrees[this.ADMINDATA_INDEX].children.push({
        ...
        data: new TypeNodeData(
          TreeNodeType.NO_MENU,
          this.addMainRoute(AppSettings.CONNECTOR_API_KEY_KEY),
          null,
          null,
          null
        ),
      ...
      this.portfolioTrees[this.ADMINDATA_INDEX].children.push({
        ...
        data: new TypeNodeData(
          TreeNodeType.NO_MENU,
          this.addMainRoute(AppSettings.USER_ENTITY_LIMIT_KEY),
          null,
          null,
          null
        ),
  ...
  private addGTNetToTree(): void {
    const gtNetNode: TreeNode = {
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.GTNET_CONSUME_MONITOR_KEY),
        null,
        null,
        null
      ),
    ...
    gtNetNode.children.push({
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.GTNET_KEY),
        null,
        null,
        null
      ),
    ...
    gtNetNode.children.push({
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.GTNET_AUTO_ANWSER_KEY),
        null,
        null,
        null
      ),
    ...
    gtNetNode.children.push({
      ...
      data: new TypeNodeData(
        TreeNodeType.NO_MENU,
        this.addMainRoute(AppSettings.GTNET_PROVIDER_MONITOR_KEY),
        null,
        null,
        null
      ),
  ...
  addSecurityaccountToTree(portfolio: Portfolio): TreeNode {
    ...
    const securityAccountNode: TreeNode = {
      ...
      data: new TypeNodeData(
        ...
        this.addMainRoute(
          portfolio.securityaccountList.length > 0
            ? AppSettings.SECURITYACCOUNT_SUMMERIES_ROUTE_KEY
      ...
      children: this.addSecurityaccountsToTree(
        portfolioJson,
  ...
  ////////////////////////////////////////////////////////////////////////////////
  ngOnInit(): void {
    this.portfolioTrees = new Array(
      this.ADMINDATA_INDEX +
        (AuditHelper.hasAdminRole(this.globalParamService) ? 1 : 0)
    );
  ...
  handleNew(
    ...
    parentObject: any,
    data: Tenant | Portfolio | Cashaccount | Securityaccount | AlgoTopCreate,
    tenantLimitType: TenantLimitTypes
  ): void {
    if (tenantLimitType) {
      if (
        BusinessHelper.isLimitCheckOk(
          this.tenantLimits[tenantLimitType],
  ...
  */
    ...
    AppHelper.confirmationDialog(
      ...
      () => {
        this.securityaccountService
          .deleteSecurityaccount(idSecuritycashaccount)
          .subscribe((response) => {
            this.messageToastService.showMessageI18n(
              InfoLevelType.SUCCESS,
              'MSG_DELETE_RECORD',
            ...
            if (portfolio.securityaccountList.length === 1) {
              treeNode.parent.data.route = this.addMainRoute(
                AppSettings.SECURITYACCOUNT_EMPTY_ROUTE_KEY
              );
  ...
  handleDeleteStrategy(treeNode: TreeNode, idAlgoAssetclassSecurity: number) {
    AppHelper.confirmationDialog(
      ...
      () => {
        this.algoTopService
          .deleteEntity(idAlgoAssetclassSecurity)
          .subscribe((response) => {
            this.messageToastService.showMessageI18n(
              InfoLevelType.SUCCESS,
              'MSG_DELETE_RECORD',
  ...
  getEditMenuItemsByTypeNode(treeNode: TreeNode): MenuItem[] {
    ...
    const parentNodeData = typeNodeData.parentObject
      ? JSON.parse(typeNodeData.parentObject)
      : null;
    const selectedNodeData = typeNodeData.entityObject
      ? JSON.parse(typeNodeData.entityObject)
      : null;
    switch (typeNodeData.treeNodeType) {
        ...
        menuItems.push(
          {
            ...
            command: (event) => {
              ...
              this.handleEdit(DialogVisible.DvTenant, null, selectedNodeData);
            },
          },
          {
      ...
      case TreeNodeType.Portfolio:
        menuItems.push({
          label: 'EDIT_RECORD|PORTFOLIO' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
            this.handleEdit(
              DialogVisible.DvPortfolio,
              parentNodeData,
        ...
        menuItems.push({
          ...
          disabled: !(
            selectedNodeData.cashaccountList.length === 0 &&
            selectedNodeData.securityaccountList.length === 0
          ),
        });
        break;
      case TreeNodeType.SecurityaccountRoot:
        menuItems.push({
          ...
          command: (event) =>
            this.handleNew(
              DialogVisible.DvSecurityaccount,
              selectedNodeData,
              null,
      ...
      case TreeNodeType.SecurityAccount:
        ...
        menuItems.push({
          ...
          command: (event) =>
            this.handleDeleteSecurityaccount(
              treeNode,
              parentNodeData,
              selectedNodeData.idSecuritycashAccount
            ),
      ...
      case TreeNodeType.AlgoRoot:
        menuItems.push({
          label:
            'CREATE|ALGO_PORTFOLIO_STRATEGY' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
            this.handleNew(
              DialogVisible.DvAlgoRuleStrategy,
              parentNodeData,
              new AlgoTopCreate(RuleStrategy.RS_STRATEGY),
              null
            ),
        });
        menuItems.push({
          label: 'CREATE|ALGO_RULE_BASED' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
            this.handleNew(
              DialogVisible.DvAlgoRuleStrategy,
              parentNodeData,
              new AlgoTopCreate(RuleStrategy.RS_RULE),
              null
            ),
        });
        break;
      case TreeNodeType.Strategy:
        menuItems.push({
          ...
          command: (event) =>
            this.handleDeleteStrategy(
              treeNode,
              selectedNodeData.idAlgoAssetclassSecurity
            ),
        });
        break;
      case TreeNodeType.WatchlistRoot:
        menuItems.push({
          label: 'CREATE|WATCHLIST' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
            this.handleNew(
              DialogVisible.DvWatchlist,
              parentNodeData,
              selectedNodeData,
      ...
      case TreeNodeType.Watchlist:
        menuItems.push({
          label: 'EDIT_RECORD|WATCHLIST' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
            this.handleEdit(
              DialogVisible.DvWatchlist,
              parentNodeData,
        ...
        menuItems.push({ separator: true });
        menuItems.push({
          label: 'WATCHLIST_AS_PERFORMANCE' + AppSettings.DIALOG_MENU_SUFFIX,
          command: (event) =>
  ...
  onComponentClick(event) {
    this.activePanelService.activatePanel(this, {
      editMenu: this.selectedNode
        ? this.getEditMenuItemsByTypeNode(this.selectedNode)
        : null,
    });
  }
  ...
  handleWatchlistForPerformance(idWatchlist: number): void {
    this.tenantService
      .setWatchlistForPerformance(idWatchlist)
      .subscribe((tenant) => {
        this.tenant = tenant;
        this.addAndRefreshWatchlistToTree();
      });
  ...
  public drop(event: DragEvent, treeNode: TreeNode) {
    event.preventDefault();
    const wse: WatchlistSecurityExists = JSON.parse(
      event.dataTransfer.getData('text/plain')
    );
    this.watchlistService
      .getAllWatchlistsWithSecurityByIdSecuritycurrency(wse.idSecuritycurrency)
      .subscribe((existWatchlistsIds) => {
        if (existWatchlistsIds.includes(treeNode.data.id)) {
          ...
          // Move is possible
          this.watchlistService
            .moveSecuritycurrency(
              wse.idWatchlistSource,
              treeNode.data.id,
              wse.idSecuritycurrency
            )
            .subscribe((success) => {
              this.messageToastService.showMessageI18n(
                InfoLevelType.SUCCESS,
  ...
  private addAndRefreshPortfolioToTree() {
    const portfolioObservable =
      this.portfolioService.getPortfoliosForTenantOrderByName();
    const teantLimitsObservalte =
      this.globalParamService.getMaxTenantLimitsByMsgKey([
        TenantLimitTypes.MAX_SECURITY_ACCOUNT,
        TenantLimitTypes.MAX_PORTFOLIO,
        TenantLimitTypes.MAX_WATCHLIST,
      ]);

    combineLatest([portfolioObservable, teantLimitsObservalte]).subscribe(
      (results) => {
        ...
        this.setLangTrans(
          'PORTFOLIOS',
          this.portfolioTrees[this.PORTFOLIO_INDEX],
          '-' + this.tenant.tenantName + ' / ' + this.tenant.currency
        );
        const portfolios = results[0];
        this.portfolioTrees[this.PORTFOLIO_INDEX].data.entityObject =
          tenantStringify;
        this.tenantLimits = results[1].reduce(
          (ac, tl) => ({ ...ac, [tl.msgKey]: tl }),
          {}
        );
  ...
  private addRefreshAlgoToTree(): void {
    if (this.useAlgo()) {
      this.algoTopService
        .getAlgoTopByIdTenantOrderByName()
        .subscribe((algoTopList) => {
          ...
          for (const algoTop of algoTopList) {
            const treeNode = {
              ...
              icon:
                'pi ' +
                (algoTop.activatable ? 'pi-check-circle' : 'pi-question'),
              data: new TypeNodeData(
  ...
  private refreshTreeBecauseOfParentAction(): void {
    this.subscription = this.dataChangedService.dateChanged$.subscribe(
      (processedActionData) => {
  ...
  private selectPreviousSelection(): void {
    if (this.selectedNode) {
      this.selectedNode = this.getTreeNodeByTypeAndId(
        this.portfolioTrees,
        this.selectedNode.data.treeNodeType,
  ...
  private setLangTrans(key: string, target: TreeNode, sufix: string = '') {
    this.translateService
      .get(key)
      .subscribe((translated) => (target.label = translated + sufix));
  }
  ...
  private getPreviousNode(treeNode: TreeNode): TreeNode {
    let i = 0;
    while (
      i < treeNode.parent.children.length &&
      treeNode.parent.children[i] !== treeNode
    ) {
      i++;
    }
    this.selectedNode =
      i === 0 ? treeNode.parent : treeNode.parent.children[i - 1];
        ...
        if (
          ...
          if (data.useQueryParams) {
            this.router.navigate([data.route, data.id], {
              queryParams: { object: data.entityObject },
            });
          } else {
            this.router.navigate([
              data.route,
              data.id,
              { object: data.entityObject },
            ]);
          }
        }
      } else {
  ...
  private getTreeNodeByTypeAndId(
    ...
    for (const treeNode of treeNodeParent) {
      if (treeNode.data.treeNodeType === type && treeNode.data.id === id) {
        ...
        foundTreeNode = this.getTreeNodeByTypeAndId(
          treeNode.children,
          type,
          id
        );
