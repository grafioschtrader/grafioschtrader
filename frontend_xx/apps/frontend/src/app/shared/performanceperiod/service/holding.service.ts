...
export class HoldingService extends AuthServiceWithLogout<Tenant> {
  ...
  getFirstAndMissingTradingDays(
    idPortfolio: number
  ): Observable<FirstAndMissingTradingDays> {
    return <Observable<FirstAndMissingTradingDays>>(
      this.httpClient
        ...
        .pipe(
          map((response) => plainToClass(FirstAndMissingTradingDays, response)),
  ...
  getPeriodPerformance(
    performanceWindowDef: PerformanceWindowDef
  ...
  getMissingQuotesWithSecurities(
    year: number
  ): Observable<MissingQuotesWithSecurities> {
