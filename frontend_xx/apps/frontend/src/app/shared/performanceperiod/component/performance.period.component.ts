...
import {
  ...
  HoldingService,
  PerformanceWindowDef,
  WeekYear,
} from '../service/holding.service';
  ...
  template: `
    ...
    >
      <p>
        ...
        {{
          firstAndMissingTradingDays?.firstEverTradingDay
            | date : dateFormatPipe
        }}
      </p>
      <dynamic-form
      ...
      <performance-period-from-to-diff
        [periodHoldingsAndDiff]="periodHoldingsAndDiff"
      >
      </performance-period-from-to-diff>
...
export class PerformancePeriodComponent
  extends FormBase
  implements OnInit, OnDestroy, IGlobalMenuAttach
{
      ...
      .subscribe((famtd: FirstAndMissingTradingDays) => {
        ...
        if (famtd.firstEverTradingDay) {
          ...
          this.configObject.dateFrom.formControl.setValue(
            AppHelper.getDateFromSessionStorage(
              GlobalSessionNames.PERFORMANCE_DATE_FROM,
              new Date(fromDate)
            )
          );
          this.configObject.dateTo.formControl.setValue(
            new Date(famtd.latestTradingDay)
          );
  ...
  valueChangedOnDateFrom(): void {
    this.dateFromSubscribe =
      this.configObject.dateFrom.formControl.valueChanges.subscribe(
        (dateFrom: Date) => {
  ...
  valueChangedOnDateTo(): void {
    this.dateToSubscribe =
      this.configObject.dateTo.formControl.valueChanges.subscribe(
        (dateTo: Date) => {
          this.setMonthWeekPeriod();
        }
      );
  ...
  helpLink(): void {
    ...
    BusinessHelper.toExternalHelpWebpage(
      ...
      HelpIds.HELP_PORTFOLIOS_PERIODPERFORMANCE
    );
  }
  ...
  submit(value: { [name: string]: any }): void {
    const pWD: PerformanceWindowDef = new PerformanceWindowDef(
      this.idPortfolio
    );
  ...
  private setMinDateForToDate(dateFrom: Date): void {
    ...
    while (
      ...
      (dateTo.day() === Weekday.Saturday ||
        ...
        this.firstAndMissingTradingDays.holidayAndMissingQuoteDays.indexOf(
          dateTo.toDate()
        ) >= 0)
    ) {
    ...
    if (
      this.configObject.dateTo.formControl.value.getTime() <
      this.configObject.dateTo.calendarConfig.minDate.getTime()
    ) {
  ...
  private setMonthWeekPeriod(): void {
    ...
    if (pWD.dateFrom && pWD.dateTo) {
      if (
        moment(pWD.dateTo).diff(pWD.dateFrom, 'months', true) <
        this.firstAndMissingTradingDays.minIncludeMonthLimit
      ) {
        disabled.push(WeekYear.WM_YEAR);
      }
      if (
        moment(pWD.dateTo).diff(pWD.dateFrom, 'weeks', true) >
        this.firstAndMissingTradingDays.maxWeekLimit
      ) {
  ...
  private changeToOpenChart(): void {
    if (this.subscriptionRequestFromChart) {
      ...
      const urlPattern = new RegExp(
        `.*\/\/${AppSettings.MAIN_BOTTOM}.*\/${AppSettings.PERFORMANCE_KEY}\\\)`
      );
      ...
      Object.keys(pcdd)
        ...
        .forEach((key, tIndex) => {
          ...
          if (i === 0) {
            ...
            this.translateService
              .get(AppHelper.convertPropertyForLabelOrHeaderKey(key))
