...
import {
  ColumnConfig,
  ColumnGroupConfig,
} from '../../datashowbase/column.config';
          ...
          <td
            *ngFor="let field of fields; let i = index"
            [ngClass]="{
              ...
              'cell-holiday':
                getHolidayMissing(rowData, field) ===
                HolidayMissing[HolidayMissing.HM_HOLIDAY],
              'cell-data-missing':
                getHolidayMissing(rowData, field) ===
                HolidayMissing[HolidayMissing.HM_HISTORY_DATA_MISSING]
            }"
          ...
          >
            ...
            <ng-container [ngSwitch]="field.templateName">
              <ng-container *ngSwitchCase="'greenRed'">
                <span
                  [style.color]="
                    isValueByPathMinusWithEmptyColor(rowData, field)
                      ? 'red'
                      : 'green'
                  "
                >
              ...
              <ng-container [ngSwitch]="field.templateName">
                <ng-container *ngSwitchCase="'greenRed'">
                  <span
                    [style.color]="
                      isValueByPathMinus(
                        performancePeriod?.sumPeriodColSteps,
                        field
                      )
                        ? 'red'
                        : 'green'
...
export class TenantPerformanceTreetableComponent
  ...
  implements OnInit, OnChanges
{
  ...
  getFirstColumnLabel(
    dataobject: PeriodWindowWithField,
    field: ColumnConfig,
    valueField: any
  ): string | number {
    ...
    switch (dataobject.showField) {
      ...
      default:
        if (this.performancePeriod.periodSplit === WeekYear[WeekYear.WM_WEEK]) {
          // Date range for week
          colVal =
            moment(dataobject.periodWindow.startDate).format(
              this.gps.getDateFormatWithoutYear()
            ) +
            ' - ' +
            moment(dataobject.periodWindow.endDate).format(
              this.gps.getDateFormatWithoutYear()
            );
        } else {
          // Only year
  ...
  getDataValue(
    ...
    const psmh: PeriodStepMissingHoliday =
      dataobject.periodWindow.periodStepList[+field.field];
  ...
  isValueByPathMinusWithEmptyColor(
    dataobject: PeriodWindowWithField,
    field: ColumnConfig
  ): boolean {
        ...
        if (this.performancePeriod.periodSplit === WeekYear[WeekYear.WM_WEEK]) {
          ...
          for (let i = 1; i < calendarLang.dayNamesShort.length - 1; i++) {
            ...
            this.fields[i].columnGroupConfigs = [
              new ColumnGroupConfig(fieldName),
            ];
          }
        } else {
          // for year-month
          for (let i = 0; i < calendarLang.monthNamesShort.length; i++) {
            ...
            this.insertColumn(
              ...
              DataType.Numeric,
              fieldName,
              calendarLang.monthNamesShort[i],
              true,
              false,
              {
                fieldValueFN: this.getDataValue.bind(this),
            ...
            this.fields[i + 1].columnGroupConfigs = [
              new ColumnGroupConfig(fieldName),
            ];
          }
        }
    ...
    if (this.performancePeriod) {
      this.performancePeriod.periodWindows.forEach((periodWindow) => {
        ...
        pwTreeNode.children = [
          ...
          {
            data: new PeriodWindowWithField(
              this.TOTAL_SECURITIES_MC,
              periodWindow
            ),
            children: [],
            expanded: false,
