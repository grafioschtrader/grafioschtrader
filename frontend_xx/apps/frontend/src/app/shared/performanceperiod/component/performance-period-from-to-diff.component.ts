...
@Component({
  ...
  template: `
    <div class="fcontainer">
      <fieldset
        *ngFor="let phad of periodHoldingsAndDiff; let i = index"
      ...
      >
        ...
        <ng-container *ngFor="let field of fields">
          ...
          <div
            class="col-lg-3 col-md-3 col-sm-3 col-xs-3 nopadding wrap"
            align="right"
          >
            <span
              [style.color]="
                isValueByPathMinus(phad, field) ? 'red' : 'inherit'
              "
            >
...
export class TenantPerformanceFromToDiffComponent
  ...
  implements OnInit, OnChanges
{
  ...
  ngOnInit(): void {
    ...
    this.translateService
      .get('DIFFERENCE')
      .subscribe((trans) => (this.titles[2] = trans));
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (this.periodHoldingsAndDiff && this.periodHoldingsAndDiff.length > 0) {
      this.titles[0] = AppHelper.getDateByFormat(
        this.gps,
        this.periodHoldingsAndDiff[0].date
      );
      this.titles[1] = AppHelper.getDateByFormat(
        this.gps,
        this.periodHoldingsAndDiff[1].date
      );
    }
  }
}
