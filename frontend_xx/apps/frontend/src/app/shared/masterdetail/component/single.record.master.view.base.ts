...
@Directive()
export abstract class SingleRecordMasterViewBase<T extends BaseID, S>
  implements IGlobalMenuAttach
{
  ...
  valueChangedMainField(): void {
    this.changeOnMainFieldSub = this.configObject[
      this.mainFieldId
    ].formControl.valueChanges.subscribe((key: number) => {
      if (!this.ignoreChangeOnMonitorField) {
        this.selectedEntity = this.entityList.find(
          (entity) => entity[this.mainFieldId] === +key
        );
        this.setFieldValues();
      }
    });
  ...
  getBaseEditMenu(entityName: string): MenuItem[] {
    ...
    menuItems.push({
      ...
      disabled:
        ...
        !AuditHelper.hasRightsForEditingOrDeleteEntity(
          this.gps,
          this.selectedEntity
        ),
  ...
  handleDeleteEntity(entity: T) {
    AppHelper.confirmationDialog(
      ...
      () => {
        ...
        this.deleteService
          .deleteEntity(entity[this.mainFieldId])
          .subscribe((response) => {
