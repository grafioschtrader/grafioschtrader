...
import {
  FileUploadParam,
  SupportedCSVFormat,
  UploadHistoryquotesSuccess,
} from './model/file.upload.param';
  ...
  implements OnInit
{
  ...
  submit(value: { [name: string]: any }): void {
    const files: FileList =
      this.configObject.fileToUpload.elementRef.nativeElement.files;
    ...
    if (this.fileUploadParam.supportedCSVFormats) {
      ...
      Object.keys(supportedCSVFormat).forEach((e) =>
        formData.append(e, supportedCSVFormat[e])
      );
    }
    ...
    this.fileUploadParam.uploadService
      .uploadFiles(this.fileUploadParam.entityId, formData)
      .subscribe(
        (response) => {
          if (response.hasOwnProperty('duplicatedInImport')) {
            ...
            this.messageToastService.showMessageI18nEnableHtml(
              ...
              'UPLOAD_HISTORYQUOTE_SUCCESS',
              uhs
            );
          }
          this.closeDialog.emit(
            new ProcessedActionData(ProcessedAction.UPDATED)
          );
        },
  ...
  valueChangedOnDecimalSeparator(): void {
    this.decimalSeparatorSub =
      this.configObject.decimalSeparator.formControl.valueChanges.subscribe(
        (data: string) => {
          const thousandSeparators: string[] =
            this.fileUploadParam.supportedCSVFormats.thousandSeparators.filter(
              (separator) =>
                separator !==
  ...
  protected initialize(): void {
    if (this.fileUploadParam.supportedCSVFormats) {
      ...
      const supportedCSVFormat: SupportedCSVFormat =
