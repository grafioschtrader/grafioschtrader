...
import {
  CalendarConfig,
  FieldConfig,
} from '../../dynamic-form/models/field.config';
...
import {
  ErrorMessageRules,
  RuleEvent,
} from '../../dynamic-form/error/error.message.rules';
...
export class DynamicFieldHelper {
  public static readonly RULE_REQUIRED_TOUCHED = {
    name: 'required',
    keyi18n: 'required',
    rules: [RuleEvent.TOUCHED],
  };
  public static readonly RULE_REQUIRED_DIRTY = {
    name: 'required',
    keyi18n: 'required',
  ...
  public static readonly validationsErrorMap: { [key: string]: ValidError } = {
    ...
    [VALIDATION_SPECIAL.EMail]: {
      ...
      msgR: {
        name: 'email',
        keyi18n: 'patternEmail',
        rules: [RuleEvent.FOCUSOUT],
      },
    },
  ...
  public static createFunctionButton(
    ...
    return this.createFunctionButtonFieldName(
      null,
      labelKey,
      buttonFN,
      fieldOptions
  ...
  public static createSubmitButtonFieldName(
    fieldName: string,
    labelKey = 'SAVE'
  ): FieldConfig {
    return {
  ...
  public static createFieldDropdownStringHeqF(
    ...
    const fieldConfig = DynamicFieldHelper.createFieldDropdownNumberString(
      DataType.String,
      fieldName,
      AppHelper.convertPropertyForLabelOrHeaderKey(fieldName),
      required,
      fieldOptions
    );
  ...
  public static createFieldInputNumber(
    ...
    fieldConfig.max = Number(
      '9'.repeat(integerLimit) + '.' + '9'.repeat(maxFractionDigits)
    );
  ...
  public static createFieldCurrencyNumber(
    fieldName: string,
    labelKey: string,
    required: boolean,
    integerLimit: number,
    maxFractionDigits: number,
    allowNegative: boolean,
    defaultCurrencyMaskConfig: CurrencyMaskConfig,
    isCurrency: boolean,
    fieldOptions?: FieldOptions
  ): FieldConfig {
    return this.createFieldCurrencyNumberVSParam(
      fieldName,
      labelKey,
      required,
      integerLimit,
  ...
  public static createFieldCurrencyNumberHeqF(
    fieldName: string,
    required: boolean,
    integerLimit: number,
    maxFractionDigits: number,
    ...
    return this.createFieldCurrencyNumberVSParamHeqF(
      fieldName,
      required,
      integerLimit,
      maxFractionDigits,
  ...
  public static createFieldCurrencyNumberVSParamHeqF(
    fieldName: string,
    required: boolean,
    integerLimit: number,
    maxFractionDigits: number,
  ...
  public static createFieldCurrencyNumberVSParam(
    fieldName: string,
    labelKey: string,
    required: boolean,
    ...
    const width =
      fieldOptions?.inputWidth ||
      integerDigits + maxFractionDigits + (isCurrency ? 4 : 0);
    const fieldConfig: FieldConfig = this.setFieldBaseAndOptions(
      {
        dataType: DataType.Numeric,
    ...
    return validationSpecials
      ? this.addValidationParam(fieldConfig, validationSpecials, param)
      : fieldConfig;
  }

  public static adjustNumberFraction(
    ...
    DynamicFieldHelper.setCurrencyMaskMaxMin(
      fieldConfig,
      Math.max(
        integerDigits - precision + 2,
  ...
  public static setCurrencyMaskMaxMin(
    fieldConfig: FieldConfig,
    integerDigits: number,
    maxFractionDigits: number
  ): void {
    this.setMinMaxValues(
      ...
      integerDigits,
      maxFractionDigits,
      fieldConfig.currencyMaskConfig.allowNegative
    );
  ...
  public static isRequired(fieldConfig: FieldConfig): boolean {
    return (
      ...
      fieldConfig.validation.includes(Validators.required)
    );
  }

  public static createFieldCheckboxHeqF(
    fieldName: string,
    fieldOptions?: FieldOptions
  ): FieldConfig {
    return this.setFieldBaseAndOptions(
  ...
  public static createFieldCheckbox(
    fieldName: string,
    labelKey: string,
    fieldOptions?: FieldOptions
  ): FieldConfig {
  ...
  public static createFieldTriStateCheckboxHeqF(
    fieldName: string,
    fieldOptions?: FieldOptions
  ): FieldConfig {
    return this.setFieldBaseAndOptions(
  ...
  public static createFieldMinMaxNumber(
    ...
    const errorMessageRules: ErrorMessageRules[] = required
      ? [this.RULE_REQUIRED_TOUCHED]
      : [];

    if (min !== null && max) {
      validations.push(range([min, max]));
      errorMessageRules.push({
        name: 'range',
        keyi18n: 'range',
        param1: min,
        param2: max,
  ...
  private static setMinMaxValues(
    ...
    fieldConfig.max = Number(
      '9'.repeat(integerDigits) + '.' + '9'.repeat(maxFractionDigits)
    );
    fieldConfig.min = allowNegative
      ? fieldConfig.max * -1
  ...
  private static addValidations(
    ...
    for (const validationSpecial of validationSpecials) {
      const validError: ValidError =
        DynamicFieldHelper.validationsErrorMap[validationSpecial];
      this.addValidationParam(fieldConfig, validationSpecial, null);
    }
    return fieldConfig;
  }

  private static addValidationParam(
    fieldConfig: FieldConfig,
    validationSpecial: VALIDATION_SPECIAL,
    param1: any,
    param2?: any
  ): FieldConfig {
    const validError: ValidError =
