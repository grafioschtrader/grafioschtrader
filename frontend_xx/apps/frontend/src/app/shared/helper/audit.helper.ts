...
export class AuditHelper {
  ...
  public static convertToProposeChangeEntityWithEntity(
    ...
    if (proposeUserTask) {
      ...
      proposeUserTask.proposeChangeFieldList.forEach(
        (put) => (pewe.proposedEntity[put.field] = put.valueDesarialized)
      );
      pewe.proposeChangeEntity = proposeUserTask;
      (<Auditable>pewe.entity).idProposeRequest =
        proposeUserTask.idProposeRequest;
    }
    return pewe;
  }
  ...
  public static configureFormFromAuditableRights(
    ...
    if (
      !AuditHelper.hasRightsForEditingOrDeleteAuditable(gps, entityAuditable)
    ) {
      // User can not change entity directly but propose one
      AuditHelper.setHeaderChangeRequest(
        translateService,
        form,
        'YOUR_CHANGE_REQUEST'
      );
      ...
      FormHelper.disableEnableFieldConfigs(false, [
        configObject[AuditHelper.NOTE_REQUEST_INPUT],
      ]);
      configObject[AuditHelper.SUBMIT_FIELD_BUTTON].labelKey =
        AuditHelper.SAVE_CHANGE_REQUEST;
    } else {
      ...
      if (proposeChangeEntityWithEntity) {
        ...
        AuditHelper.createDifferenceAsLabelTitle(
          configObject,
          proposeChangeEntityWithEntity
        );
      } else {
  ...
  public static getFullNoteRequestInputDefinition(
    ...
    return [
      ...
      DynamicFieldHelper.createSubmitButtonFieldName(
        AuditHelper.SUBMIT_FIELD_BUTTON
      ),
      DynamicFieldHelper.createFunctionButtonFieldName(
        AuditHelper.REJECT_FIELD_BUTTON,
        'REJECT_DATA_CHANGE',
        (e) =>
          closed.emit(
            new ProcessedActionData(
              ProcessedAction.REJECT_DATA_CHANGE,
              formBase.configObject[
                AuditHelper.NOTE_ACCEPT_REJECT_INPUT
              ].formControl.value
            )
          )
  ...
  public static copyNoteRequestToEntity(
    formBase: FormBase,
    targetEntity: ProposeTransientTransfer
  ) {
    targetEntity.noteRequestOrReject =
      formBase.configObject[AuditHelper.NOTE_REQUEST_INPUT].formControl.value;
  }
  ...
  public static copyProposeChangeEntityToEntityAfterEdit(
    ...
    if (proposeChangeEntityWithEntity) {
      targetEntity.idProposeRequest =
        proposeChangeEntityWithEntity.proposeChangeEntity.idProposeRequest;
      targetEntity.noteRequestOrReject =
        formBase.configObject[
          AuditHelper.NOTE_ACCEPT_REJECT_INPUT
        ].formControl.value;
    ...
    if (
      ...
      !formBase.configObject[AuditHelper.NOTE_REQUEST_INPUT].formControl
        .disabled
    ) {
   ...
   */
  public static hasRightsForEditingOrDeleteEntity<T>(
    gps: GlobalparameterService,
    entity: any
  ): boolean {
    return (
      !(entity instanceof Auditable) ||
      AuditHelper.hasRightsForEditingOrDeleteAuditable(gps, <Auditable>entity)
    );
  }

  public static hasRightsForEditingOrDeleteAuditable(
    gps: GlobalparameterService,
    entityAuditable: Auditable
  ): boolean {
    return (
      ...
      (entityAuditable instanceof Security &&
        (<Security>entityAuditable).idTenantPrivate &&
  ...
  public static hasHigherPrivileges(gps: GlobalparameterService): boolean {
    return (
      gps.hasRole(AppSettings.ROLE_ADMIN) ||
  ...
  private static createDifferenceAsLabelTitle(
    ...
    Object.keys(configObject).forEach((key) => {
      ...
      let oldValue = Helper.getValueByPath(
        proposeChangeEntityWithEntity.entity,
        field
      );
      if (
        oldValue !==
        ...
        if (
          fieldConfig.inputType === InputType.Select &&
          fieldConfig.valueKeyHtmlOptions
        ) {
          oldValue = fieldConfig.valueKeyHtmlOptions.find(
            (v) => v.key === oldValue
          ).value;
        }
  ...
  private static setHeaderChangeRequest(
    ...
    if (textKey) {
      ...
      translateService
        ...
        .subscribe(
          (text) =>
            (form.formConfig.fieldHeaders[AuditHelper.NOTE_REQUEST_INPUT] =
              text)
        );
    } else {
      if (
        form.formConfig.fieldHeaders &&
        form.formConfig.fieldHeaders[AuditHelper.NOTE_REQUEST_INPUT]
      ) {
