...
export class BusinessHelper {
  public static getTransactionTypeAsName(
    transactionType: TransactionType
  ): string {
    for (const n in TransactionType) {
      if (
        typeof TransactionType[n] === 'number' &&
   ...
   */
  public static getCurrencypairWithSetOfFromAndTo(
    ...
    currencypairService
      ...
      .subscribe((cWh: CurrencypairWithHistoryquote) => {
        if (cWh.currencypair) {
          ...
          if (
            transactionTime > cWh.currencypair.sTimestamp ||
  ...
  public static setSecurityTransactionSummary(
    ...
    if (
      ...
      idSecuritycashAccounts.length > 0 &&
      idSecuritycashAccounts[0] !== null
    ) {
      ...
      return securityService.getTransactionsByIdTenantAndIdSecurity(
        idSecuritycurrency,
        forChart
      );
    }
  ...
  static getUrlLinkMenus(securitycurrency: Securitycurrency): MenuItem[] {
    ...
    menuItems.push({
      ...
      command: (e) =>
        this.toExternalWebpage(securitycurrency.stockexchangeLink, 'exchange'),
    ...
    if (securitycurrency.hasOwnProperty('productLink')) {
      menuItems.push({
        label: 'PRODUCT_LINK',
        command: (e) =>
          this.toExternalWebpage(
            (<Security>securitycurrency).productLink,
            'product'
          ),
  ...
  public static toExternalHelpWebpage(language: string, helpIds: HelpIds) {
    this.toExternalWebpage(
      AppSettings.HELP_DOMAIN + '/' + language + '/' + helpIds,
      'help'
    );
  }

  public static hasSecurityDenomination(
    ...
    if (hasMarkedPrice) {
      return (
        ...
        (assetclass.categoryType ===
          ...
          assetclass.categoryType ===
            AssetclassType[AssetclassType.MONEY_MARKET]) &&
  ...
  public static getDisplayLeverageFactor(
    dataobject: any,
    field: ColumnConfig,
    valueField: any
  ): string {
