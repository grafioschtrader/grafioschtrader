...
import {
  FieldFormGroup,
  FormGroupDefinition,
} from '../../dynamic-form/models/form.group.definition';
...
export class TranslateHelper {
  ...
  public static prepareFieldsAndErrors(
    ...
    const flattenFieldConfigObject: { [name: string]: FieldConfig } =
      Object.assign(
        {},
        ...fieldConfigs.map((d) => ({ [d.field]: d })),
        ...FormHelper.getFormGroupDefinition(fieldFormGroup).map((d) => ({
          [d.formGroupName]: d,
        }))
      );
  ...
  public static translateMessageErrors(
    translateService: TranslateService,
    ...
    fieldConfigs.forEach((fieldConfig: FieldFormGroup) => {
      if (!FormHelper.isFieldConfig(fieldConfig)) {
        ...
        this.translateMessageErrors(
          translateService,
          (<FormGroupDefinition>fieldConfig).fieldConfig
        );
      } else {
        fieldConfigs
          .filter(
            (fc) =>
              (<FieldConfig>fc).labelHelpText &&
              /^[A-Z,_]*$/.test((<FieldConfig>fc).labelHelpText)
          )
          .forEach((fc) =>
  ...
  public static translateLabelHelpText(
    translateService: TranslateService,
    fieldConfig: FieldConfig
  ) {
    translateService
      .get(fieldConfig.labelHelpText)
      .subscribe((transText) => (fieldConfig.labelHelpText = transText));
  }

  public static translateMessageError(
    translateService: TranslateService,
    fieldConfig: FieldConfig
  ) {
    fieldConfig.errors.forEach((error) =>
      translateService
        .get(error.keyi18n, {
          param1: error.param1,
          param2: error.param2,
  ...
  public static translateMenuItems(
    menuItems: MenuItem[],
    translateService: TranslateService,
    translateParam = true
  ) {
    menuItems.forEach((menuItem) => {
      if (menuItem.label) {
        if (menuItem.label.startsWith('_')) {
          ...
          menuItem.title =
            TranslateHelper.cutOffDialogDots(menuItem.label) + '_TITLE';
        }
        TranslateHelper.translateMenuItem(
          menuItem,
          'label',
          translateService,
          translateParam
  ...
  public static createTranslatedValueStore(
    ...
    const columnConfigs = fields.filter(
      (columnConfig) => !!columnConfig.translateValues
    );
    if (columnConfigs.length > 0) {
      data.forEach((datavalue) =>
        TranslateHelper.createTranslatedValueStoreForTranslation(
          translateService,
          columnConfigs,
          datavalue
        )
      ...
      columnConfigs.forEach(
        (columnConfig) =>
          (columnConfig.fieldTranslated =
            columnConfig.field + AppSettings.FIELD_SUFFIX)
      );
    }
  }
    ...
    fields.forEach((columnConfig) => {
      ...
      if (!columnConfig.translatedValueMap.hasOwnProperty(value)) {
        if (value) {
          ...
          value =
            columnConfig.translateValues === TranslateValue.UPPER_CASE
              ? value.toUpperCase()
              : value;
  ...
  private static translateMenuItem(
    ...
    if (
      menuItem[targetProperty] &&
      menuItem[targetProperty].toUpperCase() === menuItem[targetProperty]
    ) {
      // Translate only once
      const dialogMenuItem = menuItem[targetProperty].endsWith(
        AppSettings.DIALOG_MENU_SUFFIX
      );
      if (dialogMenuItem) {
        menuItem[targetProperty] = TranslateHelper.cutOffDialogDots(
          menuItem[targetProperty]
        );
      }
              ...
              translateService
                ...
                .subscribe(
                  (message) =>
                    (menuItem[targetProperty] =
                      message +
                      (dialogMenuItem ? AppSettings.DIALOG_MENU_SUFFIX : ''))
                )
            );
        } else {
          translateService
  ...
  private static cutOffDialogDots(label: string): string {
    return label.endsWith(AppSettings.DIALOG_MENU_SUFFIX)
      ? label.slice(0, -AppSettings.DIALOG_MENU_SUFFIX.length)
      : label;
  }
}
