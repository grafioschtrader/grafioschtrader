...
export class SelectOptionsHelper {
   ...
   */
  public static securityCreateValueKeyHtmlSelectOptions(
    securities: Security[],
  ...
  public static securitiesEnableDisableOptionsByActiveDate(
    ...
    fieldConfig.valueKeyHtmlOptions.forEach((vkho) => {
      const security = securities.find(
        (s) => s.idSecuritycurrency === vkho.key
      );
      vkho.disabled = !(
        activeDate >= security.activeFromDate &&
        activeDate <= security.activeToDate
      );
    });
  }
  ...
  public static translateAssetclass(
    ...
    const observableTranslateCategoryType = translateService.get(
      <string>assetclass.categoryType
    );
    const observableTranslateSpecialInvestment = translateService.get(
      <string>assetclass.specialInvestmentInstrument
    );
    const valueKeyHtmlSelectOption = new ValueKeyHtmlSelectOptions(
      assetclass.idAssetClass,
      null
    );
    combineLatest([
      observableTranslateCategoryType,
      observableTranslateSpecialInvestment,
    ]).subscribe((translated) => {
      let subCategory: string = Helper.getValueByPath(
        assetclass,
        'subCategoryNLS.map.' + language
      );
  ...
  public static createHtmlOptionsFromStringArray(
    ...
    keysAndValues.forEach((keyAndValue) =>
      valueKeyHtmlSelectOptions.push(
        new ValueKeyHtmlSelectOptions(
          keyAndValue,
          uppecaseValue ? keyAndValue.toUpperCase() : keyAndValue
        )
      )
    );
    return valueKeyHtmlSelectOptions;
  ...
  public static createValueKeyHtmlSelectOptionsFromArray(
    ...
    values.forEach((element) => {
      valueKeyHtmlSelectOptions.push(
        new ValueKeyHtmlSelectOptions(element[key], element[propertyName])
      );
    });
    return valueKeyHtmlSelectOptions;
  }
  ...
  public static createValueKeyHtmlSelectOptionsFromObject(
    propertyName: string,
    values: { [keySN: string | number]: any },
    addEmpty: boolean,
    excludeKeys: string[] = []
  ): ValueKeyHtmlSelectOptions[] {
    const valueKeyHtmlSelectOptions: ValueKeyHtmlSelectOptions[] = [];
    ...
    Object.keys(values)
      ...
      .forEach((k) =>
        valueKeyHtmlSelectOptions.push(
          new ValueKeyHtmlSelectOptions(k, values[k][propertyName])
        )
      );
    return valueKeyHtmlSelectOptions;
  }
  ...
  public static translateExistingValueKeyHtmlSelectOptions(
    ...
    const newValueKeyHtmlSelectOptions: ValueKeyHtmlSelectOptions[] = addEmpty
      ...
      : [];
    hSelOpt.forEach((h) =>
      newValueKeyHtmlSelectOptions.push(
        this.translateValueKeyHtmlSelectOptions(
          translateService,
          h.key,
          h.value
        )
  ...
  public static createSelectItemForEnum(
    translateService: TranslateService,
    e: EnumI,
    items: SelectItem[]
  ): void {
    for (const n in e) {
      if (typeof e[n] === 'number') {
        ...
        translateService
          .get(stringType)
          .subscribe((result) =>
            items.push({ value: stringType, label: result })
          );
      }
    }
  }
    ...
    for (const n in e) {
      if (typeof e[n] === 'number') {
        ...
        if (
          ...
          const valueKeyHtmlSelectOption =
            this.translateValueKeyHtmlSelectOptions(
              translateService,
              stringType,
              stringType
  ...
  private static translateValueKeyHtmlSelectOptions(
    ...
    value !== '' &&
      translateService
        .get(value)
        .subscribe(
          (translated) => (valueKeyHtmlSelectOptions.value = translated)
        );
