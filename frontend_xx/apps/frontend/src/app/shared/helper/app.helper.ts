...
export class AppHelper {
  static readonly fieldToLabelRegex = new RegExp(
    '^(rand|year|group)|(DiffMC|MC)$',
    'g'
  );

  static getUntilDateBySessionStorage(): Date {
    return AppHelper.getDateFromSessionStorage(
      GlobalSessionNames.REPORT_UNTIL_DATE,
      new Date()
    );
  }

  static getDateFromSessionStorage(
    property: string,
  ...
  static saveUntilDateInSessionStorage(untilDate: Date): void {
    AppHelper.saveDateToSessionStore(
      GlobalSessionNames.REPORT_UNTIL_DATE,
      untilDate
    );
  }
  ...
  static saveDateToSessionStore(property: string, date: Date) {
    sessionStorage.setItem(
      property,
      moment(date).format(AppSettings.FORMAT_DATE_SHORT_NATIVE)
    );
  }
  ...
  static getOptionsWithIncludeClosedPositionAndUntilDate(
    ...
    const headerParam = AppHelper.getOptionsWithUntilDate(
      untilDate,
      httpHeaders
    );
    headerParam.params = headerParam.params.append(
      'includeClosedPosition',
      includeClosedPosition.toString()
    );
    return headerParam;
  }
  ...
  static getOptionsWithUntilDate(untilDate: Date, httpHeaders: HttpHeaders) {
    const httpParams = new HttpParams().set(
      'untilDate',
      moment(untilDate).format(AppSettings.FORMAT_DATE_SHORT_NATIVE)
    );
  ...
  public static createParamObjectFromParamMap(paramMap: ParamMap): any {
    ...
    paramMap.keys.forEach(
      (key) => (paramObject[key] = JSON.parse(paramMap.get(key)))
    );
    return paramObject;
  }
  ...
  public static convertPropertyForLabelOrHeaderKey(upperLower: string): string {
    return this.convertPropertyNameToUppercase(
      upperLower.replace(this.fieldToLabelRegex, '')
    );
  }

  public static invisibleAndHide(
  ...
  public static getValueByPathWithField(
    ...
    if (
      dataobject ||
      (field.dataType === DataType.NumericShowZero && dataobject === 0)
    ) {
      switch (field.dataType) {
        ...
        case DataType.DateTimeSecondString:
          return moment
            .utc(dataobject)
            .local()
            .format(gps.getTimeSecondDateFormatForTable());
        default:
          return dataobject;
      }
    }
  ...
  public static numberIntegerFormat(
    gps: GlobalparameterService,
    value: number
  ) {
   ...
   */
  public static binarySearch<T, S>(
    array: T[],
    item: S,
    compare: CompareFunc<T, S>
  ): number {
    ...
    if (
      array.length > middle &&
      compare(array[middle], item) === Comparison.LT
    ) {
      middle += 1;
    }
    return middle * -1;
  ...
  public static processDroppedFiles(
    ...
    for (let i = 0; i < files.length; i++) {
      if (
        ...
        files[i].fileEntry.name
          .toLocaleLowerCase()
          .endsWith('.' + allowedFileExtension)
      ) {
        // Is it a PDF-file
        ...
        messageToastService.showMessageI18n(
          InfoLevelType.ERROR,
          allowedFileExtension.toUpperCase() + '_ONLY_ALLOWED'
        );
        break;
      }
    }
  ...
  private static confirmationDialogParam(
    ...
    const observableMsg = param
      ? translateService.get(msgKey, { i18nRecord: param })
      : translateService.get(msgKey);
    const observableHeaderKey = translateService.get(headerKey);

    combineLatest([observableMsg, observableHeaderKey]).subscribe(
      (translated: string[]) => {
...
export class TranslateParam {
  ...
  getInterpolateParam(): { [key: string]: any } {
    ...
    interpolateParam[this.paramName] = this.translate
