...
export class DynamicFieldModelHelper {
      ...
      for (const [key, value] of Object.entries(cdias.constraintValidatorMap)) {
        ...
        switch (ConstraintValidatorType[key]) {
          case ConstraintValidatorType.DateRange:
            const fdDate1 = cdias.fieldDescriptorInputAndShows.find(
              (f) => f.fieldName === value.startField
            );
            const fdDate2 = cdias.fieldDescriptorInputAndShows.find(
              (f) => f.fieldName === value.endField
            );
            ...
            const fieldFormGroup: FieldFormGroup = {
              formGroupName: 'dateRange' + counter,
              fieldConfig: fieldConfigs,
            };
            fieldFormGroup.validation = [
              dateRange(
                fdDate1.fieldName,
                fdDate2.fieldName,
                fdDate2.fieldName
            ...
            const rfwg = new ReplaceFieldWithGroup(
              fdDate1.fieldName,
              fieldFormGroup,
              fdDate2.fieldName
            );
            config = this.ccFieldsFromDescriptorWithGroup(
  ...
  public static ccWithFieldsFromDescriptor(
    ...
    const fd = fieldDescriptorInputAndShows.filter(
      (fdias) => fdias.fieldName === fieldName
    )[0];
    return this.createConfigFieldFromDescriptor(
      fd,
      null,
      labelKey,
      fieldOptionsCc
  ...
  public static ccFieldsFromDescriptorWithGroup(
    fieldDescriptorInputAndShows: FieldDescriptorInputAndShow[],
    labelPreffix: string,
    addSubmitButton = false,
    ...
    fieldDescriptorInputAndShows.forEach((fd) => {
      if (
        rpg &&
        (fd.fieldName === rpg.replaceFieldName ||
          fd.fieldName === rpg.removeFieldName)
      ) {
        if (fd.fieldName === rpg.replaceFieldName) {
          fieldConfigs.push(rpg.fieldFormGroup);
        }
      } else {
        const fieldConfig: FieldConfig = this.createConfigFieldFromDescriptor(
          fd,
          labelPreffix,
          null
        );
  ...
  private static createConfigFieldFromDescriptor(
    ...
    const targetField =
      fieldOptionsCc && fieldOptionsCc.targetField
        ? fieldOptionsCc.targetField
        : fd.fieldName;
    labelKey = labelKey
      ? labelKey
      : labelPreffix + AppHelper.convertPropertyNameToUppercase(fd.fieldName);

    switch (DataType[fd.dataType]) {
      case DataType.String:
        fieldConfig = this.createStringInputFromDescriptor(
          fd,
          labelKey,
          targetField,
          fieldOptionsCc
      ...
      case DataType.NumericInteger:
        fieldConfig = DynamicFieldHelper.createFieldMinMaxNumber(
      ...
      case DataType.DateStringShortUS:
        ...
        if (
          ...
          DynamicFormPropertyHelps[fd.dynamicFormPropertyHelps[0]] ===
            DynamicFormPropertyHelps.DATE_FUTURE
        ) {
          fieldConfig.calendarConfig = { minDate: fieldConfig.defaultValue };
          fieldConfig.validation = fd.required
            ? [...fieldConfig.validation, gteDate(fieldConfig.defaultValue)]
          ...
          fieldConfig.errors = fd.required
            ? [...fieldConfig.errors, emr]
            : [emr];
        }
        break;
    }
    ...
    if (fd.dynamicFormPropertyHelps) {
      switch (DynamicFormPropertyHelps[fd.dynamicFormPropertyHelps[0]]) {
        ...
        case DynamicFormPropertyHelps.SELECT_OPTIONS:
          ...
          fieldConfig = DynamicFieldHelper.createFieldSelectStringHeqF(
            targetField,
            fd.required,
            fieldOptions
          );
  ...
  public static setValuesOfMapModelToDynamicModel(
    ...
    fieldDescriptorInputAndShows.forEach((fieldDescriptorInputAndShow) => {
      console.log(
        'param',
        paramMap[fieldDescriptorInputAndShow.fieldName].paramValue
      );
  ...
  public static getFieldPercentageSuffix(
    fDIAS: FieldDescriptorInputAndShow
  ): string {
    if (
  ...
  public static isDataModelEqual(
    model1: any,
    model2: any,
