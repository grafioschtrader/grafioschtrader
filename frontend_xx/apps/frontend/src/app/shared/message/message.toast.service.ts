...
export class MessageToastService {
  ...
  showMessage(
    ...
    this.showMessageSource.next(
      new MessageContainer(false, infoLevelType, message, interpolateParams)
    );
  }
  ...
  showMessageI18n(
    ...
    this.showMessageSource.next(
      new MessageContainer(true, infoLevelType, key, interpolateParams)
    );
  }

  showMessageI18nEnableHtml(
    infoLevelType: InfoLevelType,
    key: string | Array<string>,
    interpolateParams?: any
  ) {
    const messageContainer: MessageContainer = new MessageContainer(
      true,
      infoLevelType,
      key,
      interpolateParams
