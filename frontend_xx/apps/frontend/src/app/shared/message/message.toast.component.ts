...
export class MessageToastComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.subscription = messageToastService.showMessageSource$.subscribe(
      (messageConainer) => {
        if (messageConainer.i8n) {
          ...
          this.showMessage(
            <string>messageConainer.key,
            messageConainer.title,
            messageConainer
          );
        }
      }
  ...
  private translateTitle(messageConainer: MessageContainer) {
    this.translateService
      ...
      .subscribe((title: string) =>
        this.showMessage(messageConainer.titleKey, title, messageConainer)
      );
  }
  ...
  private translateInterpolateParams(messageConainer: MessageContainer) {
    if (messageConainer.interpolateParams) {
      ...
      this.translateMessageAndTitle(
        messageConainer,
        messageConainer.interpolateParams
      );
    }
  }

  private translateMessageAndTitle(
    messageConainer: MessageContainer,
    interpolateParams
  ) {
    combineLatest([
  ...
  private showMessage(
    ...
    switch (messageConainer.infoLevelType) {
      case InfoLevelType.SUCCESS:
        this.toastr.success(
          message,
          messageConainer.titleKey ? messageConainer.titleKey : title,
          {
            timeOut: 10000,
            enableHtml: messageConainer.enableHtml,
            closeButton: true,
          }
        );
        break;
