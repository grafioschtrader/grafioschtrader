...
export class TaskDataChangeEditComponent
  extends SimpleEntityEditBase<TaskDataChange>
  implements OnInit
{
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_TASK_DATA_CHANGE_MONITOR,
      AppSettings.TASK_DATE_CHANGE.toUpperCase(),
      translateService,
      gps,
      messageToastService,
        ...
        true,
        {
          calendarConfig: {
            ...
            maxDate: moment(new Date())
              .add(this.tdcFormConstraints.maxDaysInFuture, 'd')
              .toDate(),
          },
        }
      ),
  ...
  valueChangedOnIdTask(): void {
    this.idTaskSubscribe =
      this.configObject.idTask.formControl.valueChanges.subscribe((idTask) => {
        this.configObject.entity.valueKeyHtmlOptions = this.tdcFormConstraints
          .taskTypeConfig[idTask]
          ? SelectOptionsHelper.translateExistingValueKeyHtmlSelectOptions(
              this.translateService,
              SelectOptionsHelper.createHtmlOptionsFromStringArray(
                this.tdcFormConstraints.taskTypeConfig[idTask],
                true
              ),
              false
            )
        ...
        FormHelper.disableEnableFieldConfigs(
          ...
          [this.configObject.entity, this.configObject.idEntity]
        );
        const allowEmpty =
          this.tdcFormConstraints.taskTypeConfig[idTask] &&
          this.tdcFormConstraints.taskTypeConfig[idTask].includes;
        DynamicFieldHelper.resetValidator(
          this.configObject.entity,
          allowEmpty ? null : [Validators.required],
  ...
  valueChangedOnEntity(): void {
    this.entitySubscribe =
      this.configObject.entity.formControl.valueChanges.subscribe((entity) => {
        FormHelper.disableEnableFieldConfigs(!entity || entity.length === 0, [
  ...
  protected initialize(): void {
    ...
    this.configObject.idTask.valueKeyHtmlOptions =
      SelectOptionsHelper.createHtmlOptionsFromEnum(
        this.translateService,
        TaskType,
        Object.keys(TaskType)
          .filter(
            (key) => TaskType[key] <= this.tdcFormConstraints.maxUserCreateTask
          )
          .map((key) => TaskType[key])
      );
    if (this.callParam) {
      this.form.transferBusinessObjectToForm(this.callParam);
    }
    this.configObject.earliestStartTime.formControl.setValue(
      moment().add(1, 'm').toDate()
    );
    this.configObject.idTask.elementRef.nativeElement.focus();
  }

  protected getNewOrExistingInstanceBeforeSave(value: {
    [name: string]: any;
  }): TaskDataChange {
    const taskDataChange = this.copyFormToPrivateBusinessObject(
      new TaskDataChange(),
      null
    );
