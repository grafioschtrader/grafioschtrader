...
} from '../../datashowbase/table.crud.support.menu';
            ...
            >
              <ng-container *ngSwitchCase="FilterType.likeDataType">
                <ng-container [ngSwitch]="field.dataType">
                  ...
                  <p-columnFilter
                    *ngSwitchCase="DataType.NumericShowZero"
                    type="numeric"
                    [field]="field.field"
                    [locale]="formLocale"
                    minFractionDigits="0"
              ...
              <ng-container *ngSwitchCase="FilterType.withOptions">
                <p-dropdown
                  ...
                  [style]="{ width: '100%' }"
                  (onChange)="table.filter($event.value, field.field, 'equals')"
                ></p-dropdown>
              </ng-container>
            </th>
          </tr>
        ...
        >
          <tr [pSelectableRow]="el">
            <td>
              <a
                *ngIf="
                  ProgressStateType[el.progressStateType] ===
                  ProgressStateType.PROG_FAILED
                "
                href="#"
                [pRowToggler]="el"
              >
            ...
            >
              <ng-container [ngSwitch]="field.templateName">
                ...
                <ng-container *ngSwitchDefault>
                  <span
                    [pTooltip]="getValueByPath(el, field)"
                    tooltipPosition="top"
        ...
        <ng-template pTemplate="rowexpansion" let-tdc let-columns="fields">
          <tr>
            ...
            >
              ...
              <textarea
                *ngIf="tdc.failedStackTrace"
                [rows]="getShowLines(tdc.failedStackTrace)"
              >
...
export class TaskDataChangeTableComponent extends TableCrudSupportMenu<TaskDataChange> {
  ...
  constructor(
    ...
    super(
      ...
      gps.hasRole(AppSettings.ROLE_ADMIN)
        ? [CrudMenuOptions.Allow_Create, CrudMenuOptions.Allow_Delete]
        : []
    );
    ...
    this.fields
      .filter((f) => f.dataType === DataType.DateTimeSecondString)
      .map((f) => (f.width = 100));
  ...
  protected override readData(): void {
    combineLatest([
      this.taskDataChangeService.getAllTaskDataChange(),
      this.taskDataChangeService.getFormConstraints(),
    ]).subscribe((data) => {
  ...
  protected addCustomMenusToSelectedEntity(
    ...
    if (this.gps.hasRole(AppSettings.ROLE_ADMIN)) {
      menuItems.push({
        ...
        disabled:
          taskDataChange.idTask > this.tdcFormConstraints.maxUserCreateTask,
      });

      menuItems.push({
        label: 'TASK_INTERRUPT|' + this.entityNameUpper,
        command: (event) =>
          this.interruptingRunningJob(taskDataChange.idTaskDataChange),
        disabled: !(
          typeof taskDataChange.idTask === 'string' &&
          this.tdcFormConstraints.canBeInterruptedList.includes(
            taskDataChange.idTask
          ) &&
  ...
  private interruptingRunningJob(idTaskDataChange: number): void {
    this.taskDataChangeService
      .interruptingRunningJob(idTaskDataChange)
