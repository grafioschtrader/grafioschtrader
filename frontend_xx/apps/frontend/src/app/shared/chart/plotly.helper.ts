...
export class PlotlyHelper {
  public static initializeChartTrace(
    name: string,
    type: string,
    mode?: string
  ): Partial<ChartTrace> {
  ...
  public static registerPlotlyClick(
    ...
    nativeElement.on('plotly_click', (data) => {
      ...
      for (let i = 0; i < 1; i++) {
        point = `x=  + ${data.points[i].x}; y= ${data.points[i].y.toPrecision(
          4
        )}, pn=${data.points[i].curveNumber}`;
  ...
  public static translateLayout(
    ...
    founds.forEach((elements) =>
      elements
        .filter((e) => e.hasOwnProperty(LABEL))
  ...
  private static translateLayoutTitle(
    translateService: TranslateService,
    layout: any
  ): void {
    ...
    if (labelParts.length > 1) {
      ...
      translateService.get(wordKey).subscribe((paramTrans) => {
        ...
        translateService
          ...
          .subscribe((trans) => (layout.title = trans));
      });
    } else {
      translateService
        .get(labelParts[0])
