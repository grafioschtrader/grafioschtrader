...
export abstract class CalendarNavigation implements IGlobalMenuAttach {
  ...
  constructor(
    ...
    this.yearCalendarData = {
      ...
      disableWeekDays: [DayOfWeek.SATURDAY, DayOfWeek.SUNDAY],
    };
  }
  ...
  abstract onRangeSelect(
    range: RangeSelectDays,
    ranges: RangeSelectDays[]
  ): void;
  ...
  containsYear(year: number): boolean {
    return (
      this.possibleYears.find((selectItem) => selectItem.value === year) != null
    );
  }
  ...
  addRemoveOnOffDay(date: Date): void {
    ...
    for (; i < this.yearCalendarData.dates.length; i++) {
      if (
        ...
        (this.markExistingColors.includes(
          this.yearCalendarData.dates[i].color
        ) ||
    ...
    if (i < this.yearCalendarData.dates.length) {
      ...
      this.yearCalendarData.dates = this.yearCalendarData.dates
        .slice(0, i)
        .concat(
          this.yearCalendarData.dates.slice(
            i + 1,
      ...
      this.yearCalendarData.dates = [
        ...this.yearCalendarData.dates,
        {
          ...
          color: this.originalDaysMark.has(date.getTime())
            ? this.getExistingColor(date)
            : this.NEW_DATE_COLOR,
          select: (range: RangeSelectDays, ranges: RangeSelectDays[]) =>
            this.onRangeSelect(range, ranges),
        },
      ];
    }
  }
  ...
  protected getMenuShowOptions(): MenuItem[] {
    const menuItems: MenuItem[] = [
      {
        label: 'TRADING_CALENDAR_UNDERLINE',
        command: (e) => (this.underline = !this.underline),
      },
    ];
