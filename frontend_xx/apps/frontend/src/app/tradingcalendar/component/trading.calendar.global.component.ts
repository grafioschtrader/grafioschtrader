...
} from '../service/trading.days.plus.service';
...
export class TradingCalendarGlobalComponent
    ...
    this.tradingDaysPlusService
      ...
      .subscribe(
        (tradingDaysWithDateBoundaries: TradingDaysWithDateBoundaries) => {
          this.clearDaysAndTransformReadData(
            tradingDaysWithDateBoundaries.dates
          );
          this.setYearsBoundariesAfterRead(
  ...
  private markWorkingDaysOfFullYear(): void {
    ...
    while (startDate <= toDate) {
      ...
      if (weekDay !== 0 && weekDay !== 6) {
        if (
          !this.originalDaysMark.has(startDate.getTime()) &&
          !this.addRemoveDaysMap.has(startDate.getTime())
        ) {
