...
} from '../component/indicator.definitions';
...
export class HistoryquoteService
  ...
  implements
    ServiceEntityUpdate<Historyquote>,
    DeleteService,
    UploadServiceFunction
{
  constructor(
  ...
  getHistoryqoutesByIdSecuritycurrencyWithMissing(
    ...
    return <Observable<IHistoryquoteQuality>>(
      this.httpClient
        .get(
  ...
  getDateCloseByIdSecuritycurrency(
    idSecuritycurrency: number
  ): Observable<HistoryquoteDateClose[]> {
  ...
  getCertainOrOlderDayInHistoryquoteByIdSecuritycurrency(
    ...
    return <Observable<ISecuritycurrencyIdDateClose>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.HISTORYQUOTE_KEY}/` +
      ...
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}` +
            `${AppSettings.HISTORYQUOTE_KEY}/alltaforms`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  update(historyquote: Historyquote): Observable<Historyquote> {
    return this.updateEntity(
      historyquote,
      historyquote.idHistoryQuote,
      AppSettings.HISTORYQUOTE_KEY
    );
  }
  ...
  uploadFiles(idSecuritycurrency: number, formData: FormData): Observable<any> {
    return this.httpClient
      .post(
        `${AppSettings.API_ENDPOINT}${AppSettings.HISTORYQUOTE_KEY}/${idSecuritycurrency}/uploadhistoryquotes`,
        formData,
  ...
  public deleteEntity(idHistoryQuote: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.HISTORYQUOTE_KEY}/${idHistoryQuote}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  public deleteHistoryquotesByCreateTypes(
    ...
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.HISTORYQUOTE_KEY}/delete/${idSecuritycurrency}`,
        this.getOptionsCreateType(hct)
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  private getOptionsCreateType(hct: HistoryquoteCreateType[]) {
    ...
    hct.forEach(
      (id: number) =>
        (httpParams = httpParams.append(`createTypes`, id.toString()))
    );
