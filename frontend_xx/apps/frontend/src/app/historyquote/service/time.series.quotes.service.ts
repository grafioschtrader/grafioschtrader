...
export class TimeSeriesQuotesService {
  ...
  getMenuItems(
    ...
    menuItems.push({
      label: 'LINE_CHART',
      command: (e) =>
        this.showEodChartTable(AppSettings.TIME_SERIE_QUOTES, true),
    });
    menuItems.push({
      label: 'ADD_TO LINE_CHART',
      command: (e) =>
        this.showEodChartTable(AppSettings.TIME_SERIE_QUOTES, false),
    ...
    if (!this.optionalParameters.noMarketValue) {
      menuItems.push({
        ...
        command: (e) =>
          this.showEodChartTable(AppSettings.HISTORYQUOTE_P_KEY, true),
      });
    }
    return menuItems;
  }
  ...
  protected isTimeSeriesShown(): boolean {
    const routeUrlComponents: string[] = /.*mainbottom:(\w*)/.exec(
      this.router.url
    );
    return (
      routeUrlComponents &&
      routeUrlComponents[1] === AppSettings.TIME_SERIE_QUOTES
    );
  }

  private showEodChartTable(
    routeKey: string,
    initializeTimeSeriesParam: boolean
  ): void {
