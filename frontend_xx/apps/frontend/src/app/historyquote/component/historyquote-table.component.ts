...
import {
  Historyquote,
  HistoryquoteCreateType,
} from '../../entities/historyquote';
...
@Component({
  template: `
      ...
      <p-panel>
        <p-header>
          ...
          <ng-container *ngIf="security">
            {{ 'TRADING_FROM_TO' | translate }}:
            {{ getDateByFormat(security.activeFromDate) }} -
              ...
              >
                ...
                <ng-container *ngSwitchCase="FilterType.withOptions">
                  <p-dropdown
                    ...
                    [style]="{ width: '100%' }"
                    (onChange)="
                      table.filter($event.value, field.field, 'equals')
                    "
                  ></p-dropdown>
                </ng-container>
              </th>
    ...
    <historyquote-delete-dialog
      *ngIf="visibleDeleteHistoryquotes"
      [visibleDialog]="visibleDeleteHistoryquotes"
      [idSecuritycurrency]="
        historyquotesWithMissings.securitycurrency.idSecuritycurrency
      "
      [historyquoteQuality]="historyquotesWithMissings.historyquoteQuality"
      (closeDialog)="handleCloseDialogAndRead($event)"
    >
...
export class HistoryquoteTableComponent
  extends TableCrudSupportMenu<Historyquote>
  implements OnDestroy
{
  ...
  constructor(
    ...
    this.addColumnFeqH(DataType.Numeric, 'volume', true, false, {
      export: true,
    });
  ...
  private static registerIcons(iconReg: SvgIconRegistryService): void {
    if (!HistoryquoteTableComponent.iconLoadDone) {
      for (const [key, iconName] of Object.entries(
        HistoryquoteTableComponent.createTypeIconMap
      )) {
        iconReg.loadSvg(
          AppSettings.PATH_ASSET_ICONS +
            iconName +
            HistoryquoteTableComponent.SVG,
          iconName
        );
      }
  ...
  readData(): void {
    ...
    this.dataChangedService.dataHasChanged(
      new ProcessedActionData(ProcessedAction.UPDATED, new Historyquote())
    );
  }
  ...
  ngOnDestroy(): void {
    ...
    this.usersettingsService.saveSingleValue(
      AppSettings.HISTORYQUOTE_TABLE_SETTINGS_STORE,
      this.rowsPerPage
    );
  ...
  getCreateTypeIcon(historyquote: Historyquote, field: ColumnConfig): string {
    return HistoryquoteTableComponent.createTypeIconMap[
      HistoryquoteCreateType[historyquote.createType]
    ];
  }
  ...
  prepareMenu(): void {
    this.historyquoteSpecMenuItems = [
      this.importQuotesMenu,
      {
        label: 'EXPORT_CSV',
        command: (event) =>
          this.downloadCSvFile(this.historyquotesWithMissings.historyquoteList),
      },
  ...
  resetMenu(historyquote: Historyquote): void {
    if (!this.security || !this.security.idLinkSecuritycurrency) {
      ...
      this.fillGapsMenu.disabled =
        ...
        (this.historyquotesWithMissings.historyquoteQuality.totalMissing ===
          0 &&
          this.historyquotesWithMissings.historyquoteQuality
            .toManyAsCalendar === 0);
  ...
  protected initialize(): void {
    this.rowsPerPage =
      this.usersettingsService.readSingleValue(
        AppSettings.HISTORYQUOTE_TABLE_SETTINGS_STORE
      ) || 30;
  ...
  protected prepareCallParam(entity: Historyquote) {
    ...
    this.callParam = new HistoryquoteSecurityCurrency(
      entity,
  ...
  protected hasRightsForCreateEntity(histroyquote: Historyquote): boolean {
    return AuditHelper.hasRightsForEditingOrDeleteAuditable(
      this.gps,
      this.nameSecuritycurrency.getSecuritycurrency()
    );
  }
  ...
  private readAndShowData(timeSeriesParam: TimeSeriesParam): void {
    ...
    const historyquoteObservable =
      this.historyquoteService.getHistoryqoutesByIdSecuritycurrencyWithMissing(
        timeSeriesParam.idSecuritycurrency,
        !timeSeriesParam.currencySecurity
      );

    this.firstRow = 0;
    combineLatest([stsObservable, historyquoteObservable]).subscribe(
      (data: any[]) => {
        ...
        this.security = <Security>(
          (timeSeriesParam.currencySecurity
            ? this.historyquotesWithMissings.securitycurrency
            : null)
        );
        this.prepareMenu();
  ...
  private readDataAndProposeDataChangedEvent(
...
export class HistoryquoteSecurityCurrency {
  ...
  get showName(): string {
    return this.securitycurrency.hasOwnProperty('name')
      ...
      : (<Currencypair>this.securitycurrency).fromCurrency +
          '/' +
          (<Currencypair>this.securitycurrency).toCurrency;
  }
}
