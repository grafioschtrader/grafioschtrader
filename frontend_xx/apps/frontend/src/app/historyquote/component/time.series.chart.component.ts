  ...
  template: `
    ...
    >
      <div class="input-w">
        ...
        <i
          class="fa fa-undo fa-border fa-lg"
          (click)="onResetOldestDate($event)"
        ></i>
        <button type="button" (click)="fiveDays($event)">
          5{{ 'D' | translate }}
        </button>
        ...
        <button type="button" (click)="oneYear($event)">
          1{{ 'Y' | translate }}
        </button>
        <!--
        ...
        <input
          type="checkbox"
          [(ngModel)]="usePercentage"
          (change)="togglePercentage($event)"
        />
        <!--
        </ng-container>
        -->
        ...
        <input
          type="checkbox"
          [(ngModel)]="connectGaps"
          (change)="toggleConnectGaps($event)"
        />
  ...
  implements OnInit, OnDestroy, IGlobalMenuAttach
{
  ...
  readonly compareHistoricalFN = (h, o) =>
    h.date === o ? Comparison.EQ : h.date > o ? Comparison.GT : Comparison.LT;
  readonly compareXaxisFN = (x, b) =>
    x === b ? Comparison.EQ : x > b ? Comparison.GT : Comparison.LT;
  ...
  ngOnInit(): void {
    ...
    this.historyquoteService
      .getAllTaForms()
      .subscribe((formDefinition) => (this.taFormDefinitions = formDefinition));
  }

  ngOnDestroy(): void {
    this.subscriptionLatest && this.subscriptionLatest.unsubscribe();
    this.subscriptionViewSizeChanged &&
      this.subscriptionViewSizeChanged.unsubscribe();
    this.activePanelService.destroyPanel(this);
  }

  prepareChart(timeSeriesParams: TimeSeriesParam[]): void {
    if (
      timeSeriesParams.length === 1 ||
      timeSeriesParams.length - 1 !== this.loadedData.length
    ) {
      this.loadedData = [];
    ...
    for (let i = this.loadedData.length; i < timeSeriesParams.length; i++) {
      const stsObservable = timeSeriesParams[i].currencySecurity
        ? BusinessHelper.setSecurityTransactionSummary(
            this.securityService,
            timeSeriesParams[i].idSecuritycurrency,
            timeSeriesParams[i].idSecurityaccount
              ? [timeSeriesParams[i].idSecurityaccount]
              : null,
            timeSeriesParams[i].idPortfolio,
            true
          )
        : this.currencypairService.getTransactionForCurrencyPair(
            timeSeriesParams[i].idSecuritycurrency,
            true
          );

      const historyquoteObservable =
        this.historyquoteService.getDateCloseByIdSecuritycurrency(
          timeSeriesParams[i].idSecuritycurrency
        );
      const observable: Observable<any>[] = [
        stsObservable,
        historyquoteObservable,
      ];
      this.addCurrencyCrossRateObservable(
        timeSeriesParams,
        timeSeriesParams[i].currencySecurity,
        observable
      );

      this.subscriptionLatest = combineLatest(observable).subscribe(
        (data: any[]) => {
          const nameSecuritycurrency = timeSeriesParams[i].currencySecurity
            ? new SecurityTransactionSummary(
                data[0].transactionPositionList,
                data[0].securityPositionSummary
              )
          ...
          if ((<HistoryquoteDateClose[]>data[1]).length > 0) {
            ...
            if (observable.length === 3) {
              ...
              this.normalizeSecurityPrice(
                this.loadedData[this.loadedData.length - 1]
              );
            }
  ...
  private goNextWorkDay(momentDate: Moment, event): void {
    momentDate =
      momentDate.day() % 6 === 0 ? momentDate.add(1, 'day').day(1) : momentDate;
  ...
  getHistoricalLineTrace(loadedData: LoadedData): any {
    let foundStartIndex = Math.abs(
      AppHelper.binarySearch(
        ...
        moment(this.fromDate).format(AppSettings.FORMAT_DATE_SHORT_NATIVE),
    ...
    loadedData.factor =
      100 / loadedData.historyquotesNorm[foundStartIndex].close;
  ...
  getBuySellDivMarkForCurrency(
    ...
    this.addBuySellDivMarkForCurrency(
      traces,
      historicalLine,
      currencypairWithTransaction.transactionList,
      cptv,
      false
    );
    ...
    if (this.loadedData.length === 1 && !this.usePercentage) {
      ...
      const step =
        maxAmountSqrt - minAmountSqrt
          ? 18 / (maxAmountSqrt - minAmountSqrt)
          : 0;

      Object.entries(traces).forEach(([key, value]) => {
        (<any>value).marker = {
          size: cptv.sizes[key].size.map(
            (amount) => (Math.sqrt(amount) - minAmountSqrt) * step + 6
          ),
        };
      });
    }
  ...
  private addBuySellDivMarkForCurrency(
    ...
    transactions
      .filter(
        (transaction) => transaction.transactionTime >= this.fromDate.getTime()
      )
      .forEach((transaction) => {
        const transactionType: TransactionType =
          TransactionType[transaction.transactionType];
        const transactionTypeStr = this.getTransactionTypeStr(
          transaction,
          transactionType,
          reverse
        );
        ...
        if (!traces[transactionTypeStr]) {
          traces[transactionTypeStr] = this.initializeBuySellTrace(
            transactionTypeStr,
            transactionType
          );
          cptv.sizes[transactionTypeStr] = { size: [] };
        }
        const transactionDateStr = moment(transaction.transactionTime).format(
          AppSettings.FORMAT_DATE_SHORT_NATIVE
        );

        traces[transactionTypeStr].x.push(transactionDateStr);
        traces[transactionTypeStr].text.push(
          transaction.cashaccount.name + ':' + amount
        );
        ...
        if (this.loadedData.length > 1 || this.usePercentage) {
          ...
          traces[transactionTypeStr].y.push(
            historicalLine.y[Math.abs(foundIndex)]
          );
        } else {
          traces[transactionTypeStr].y.push(
            reverse
              ? 1 / transaction.currencyExRate
              : transaction.currencyExRate
          );
  ...
  getBuySellDivMarkForSecurity(
    ...
    securityTransactionSummary.transactionPositionList
      .filter(
        (stp) => stp.transaction.transactionTime >= this.fromDate.getTime()
      )
      .forEach((securityTransactionPosition) => {
        ...
        if (
          ...
          if (
            ...
            traces[transactionTypeStr].y.push(
              loadedData.historicalLine.y[Math.abs(foundIndex)]
            );
          } else {
            ...
            if (!this.normalizeNotNeeded(loadedData)) {
              const foundStartIndex = AppHelper.binarySearch(
                ...
                moment(
                  securityTransactionPosition.transaction.transactionTime
                ).format(AppSettings.FORMAT_DATE_SHORT_NATIVE),
                this.compareHistoricalFN
              );
              normalizeFactor =
                loadedData.historyquotesNorm[foundStartIndex].close /
                loadedData.historyquotes[foundStartIndex].close;
            }
            traces[transactionTypeStr].y.push(
  ...
  translateTrace(trace) {
    this.translateService
      .get(trace.name)
      .subscribe((transText) => (trace.name = transText));
  }
  ...
  handleCloseTaDialog(processedActionData: ProcessedActionData): void {
    ...
    if (processedActionData.action !== ProcessedAction.NO_CHANGE) {
      ...
      const taFormDefinition: TaFormDefinition =
        this.taFormDefinitions[taEditReturn.taIndicators];
      const iDef: IndicatorDefinition = this.indicatorDefinitions.defMap.get(
        TaIndicators[taEditReturn.taIndicators]
      );
  ...
  private addCurrencyCrossRateObservable(
    ...
    if (
      timeSeriesParams[timeSeriesParams.length - 1].currencySecurity != null
    ) {
      const currencypairList: string[] = [];
      this.crossRateMap
        .getValues()
        .forEach((crossRateResponse) =>
          currencypairList.push(
            crossRateResponse.currencypair.fromCurrency +
              '|' +
              crossRateResponse.currencypair.toCurrency
          )
        );
      const uniqueSecuritycurrency = new Set(
        this.loadedData
          .filter((ld) => ld.currencySecurity)
          .map((ld) => ld.currencySecurity)
      );
  ...
  private addCrossRateResponse(crossRateResponse: CrossRateResponse): void {
    ...
    crossRateResponse.currenciesAndClosePrice.forEach((crr) => {
      this.createTodayAsHistoryquote(
        crr.currencypair.sTimestamp,
        crr.currencypair.sLast,
        crr.closeAndDateList
      );
      this.crossRateMap.set(
        crr.currencypair.fromCurrency,
        crr.currencypair.toCurrency,
        crr
      );
    });
    const co = new Set(
    ...
    co.forEach((currency) =>
      this.currenciesOptions.push({ value: currency, label: currency })
    );
  }
  ...
  private getShowMenu() {
    ...
    this.indicatorDefinitions.defMap.forEach(
      (iDef: IndicatorDefinition, taIndicators: TaIndicators) => {
        iDef.menuItem = {
          label: TaIndicators[taIndicators],
          icon: iDef.shown
            ? AppSettings.ICONNAME_SQUARE_CHECK
            : AppSettings.ICONNAME_SQUARE_EMTPY,
          command: (event) =>
            this.onOffIndicatorEvent(event, TaIndicators[taIndicators], iDef),
          disabled: !this.possibleShowingTaIndicator(),
          items: [
            {
              label: `EDIT_RECORD|${TaIndicators[taIndicators]}`,
              command: () =>
                this.editIndicatorParam(iDef, TaIndicators[taIndicators]),
            },
          ],
        };
  ...
  private possibleShowingTaIndicator(): boolean {
    return (
      this.loadedData.length === 1 &&
      (this.loadedData[0].currencySecurity === this.requestedCurrency ||
        this.requestedCurrency === '')
    );
  }

  private onOffIndicatorEvent(
    event,
    taIndicators: string,
  ...
  private editIndicatorParam(
    iDef: IndicatorDefinition,
    taIndicators: string
  ): void {
    const taFormDefinition: TaFormDefinition =
      this.taFormDefinitions[taIndicators];
    const dataModel = this.usersettingsService.retrieveObject(
      AppSettings.TA_INDICATORS_STORE + taIndicators
    );
  ...
  private loadAndShowIndicatorDataWithMenu(
    taIndicators: string,
    iDef: IndicatorDefinition
  ): void {
  ...
  private loadAndShowIndicatorData(
    taIndicators: string,
    iDef: IndicatorDefinition
  ): void {
    const dataModel: string =
      this.usersettingsService.retrieveObject(
        AppSettings.TA_INDICATORS_STORE + taIndicators
      ) || this.taFormDefinitions[taIndicators].defaultDataModel;
    this.indicatorDefinitions.idSecuritycurrency =
      this.loadedData[0].idSecuritycurrency;
    this.historyquoteService
  ...
  private deleteShownTaTraceWhenInputDataHasChanged(
    ...
    if (iDef.shown) {
      const existingDataModel = this.usersettingsService.retrieveObject(
        AppSettings.TA_INDICATORS_STORE + taIndicator
      );
      if (
        !DynamicFieldModelHelper.isDataModelEqual(
          existingDataModel,
          newDataModel,
          taFormDefinition.taFormList
        )
  ...
  private deleteTaIndicator(iDef: IndicatorDefinition) {
    if (iDef.shown) {
      ...
      iDef.shown = false;
      const traceIndices: number[] = iDef.taTraceIndicatorDataList
        .map((taT) => taT.traceIndex)
        .reverse();
      Plotly.deleteTraces(this.chartElement.nativeElement, traceIndices);
      iDef.taTraceIndicatorDataList.forEach(
        (taT) => (taT.traceIndex = undefined)
      );
  ...
  private adjustTATraceIndices(traceIndices: number[]): void {
    for (const index of traceIndices) {
      this.indicatorDefinitions.defMap.forEach(
        (iDefI: IndicatorDefinition, taIndicators: TaIndicators) => {
          if (iDefI.shown) {
            iDefI.taTraceIndicatorDataList.forEach(
              (ttid) =>
                (ttid.traceIndex =
                  ttid.traceIndex > index
                    ? ttid.traceIndex - 1
                    : ttid.traceIndex)
            );
          }
  ...
  private createTaTrace(iDef: IndicatorDefinition): void {
    iDef.taTraceIndicatorDataList.forEach(
      (taTraceIndicatorData: TaTraceIndicatorData) => {
        const foundStartIndex = AppHelper.binarySearch(
        ...
        Plotly.addTraces(this.chartElement.nativeElement, {
          ...
          y: taTraceIndicatorData.taIndicatorData
            .slice(foundStartIndex, taTraceIndicatorData.taIndicatorData.length)
            .map((taIndicatorData) =>
              this.usePercentage
                ? taIndicatorData.value * this.loadedData[0].factor - 100
                : taIndicatorData.value
            ),
          line: { width: 1 },
        });
        taTraceIndicatorData.traceIndex =
          this.chartElement.nativeElement.data.length - 1;
      }
    );
  }
  ...
  private evaluateOldestYoungestMatchDate(): void {
    ...
    if (
      !this.fromDate ||
      this.loadedData.length === 1 ||
      this.oldestDate.getTime() > this.fromDate.getTime()
    ) {
      this.fromDate = this.oldestDate;
    }
  ...
  private createAllMarkerTraces(): any[] {
    ...
    for (const ld of this.loadedData) {
      if (ld.currencySecurity) {
        this.getBuySellDivMarkForSecurity(
          traces,
          ld,
          <SecurityTransactionSummary>ld.nameSecuritycurrency
        );
      } else {
        this.getBuySellDivMarkForCurrency(
          traces,
          ld.historicalLine,
          <CurrencypairWithTransaction>ld.nameSecuritycurrency
        );
      }
    }
  ...
  private plot(element: any, traces: any, layout: any): void {
    ...
    PlotlyHelper.registerPlotlyClick(
      element,
      this.chartDataPointClicked.bind(this)
    );
    if (!this.subscriptionViewSizeChanged) {
      this.subscriptionViewSizeChanged =
        this.viewSizeChangedService.viewSizeChanged$.subscribe(
          (changedViewSizeType) => Plotly.Plots.resize(element)
        );
    }
  }

  private createTodayAsHistoryquote(
    sTimestamp: number,
    sLast: number,
    historyquotes: HistoryquoteDateClose[]
  ) {
    if (sLast) {
      const dateLastStr = moment(sTimestamp).format(
        AppSettings.FORMAT_DATE_SHORT_NATIVE
      );
      if (
        historyquotes.length === 0 ||
        dateLastStr > historyquotes[historyquotes.length - 1].date
      ) {
  ...
  private attachTooltip(): void {
    PlotlyHelper.attachTooltip(
      Plotly,
      this.legendTooltipMap,
      this.chartElement
    );
  }

  private getLayout(): any {
    const dateNative = moment(this.fromDate).format(
      AppSettings.FORMAT_DATE_SHORT_NATIVE
    );
    const layout = {
      title: 'LINE_CHART',
  ...
  private normalizeAllSecurityPrices(): void {
    this.loadedData
      .filter((ld) => ld.currencySecurity != null)
      .forEach((ld) => this.normalizeSecurityPrice(ld));
  }
  ...
  private normalizeSecurityPrice(loadedData: LoadedData): void {
    if (this.normalizeNotNeeded(loadedData)) {
      ...
      const mainToSecurityCurrency: CurrenciesAndClosePrice =
        this.getCurrencypairOrReverse(
          this.mainCurrency,
          loadedData.currencySecurity
        );
      const requestToSecurityCurrency: CurrenciesAndClosePrice =
        this.getCurrencypairOrReverse(
          this.requestedCurrency,
          loadedData.currencySecurity
        );
      if (this.requestedCurrency === this.mainCurrency) {
        this.calculateHistoryquotes(
          loadedData,
          [mainToSecurityCurrency],
          [
            mainToSecurityCurrency.currencypair.fromCurrency !==
              this.mainCurrency,
          ]
        );
      } else if (
        requestToSecurityCurrency != null &&
        (requestToSecurityCurrency.currencypair.fromCurrency ===
          this.mainCurrency ||
          requestToSecurityCurrency.currencypair.toCurrency ===
            this.mainCurrency)
      ) {
        this.calculateHistoryquotes(
          loadedData,
          [requestToSecurityCurrency],
          [
            requestToSecurityCurrency.currencypair.fromCurrency ===
              this.mainCurrency,
          ]
        ...
        const mainToRequestCurrency: CurrenciesAndClosePrice =
          this.getCurrencypairOrReverse(
            this.mainCurrency,
            this.requestedCurrency
          );
        this.calculateHistoryquotes(
          loadedData,
          [mainToSecurityCurrency, mainToRequestCurrency],
          [
            mainToSecurityCurrency.currencypair.fromCurrency !==
              this.mainCurrency,
            mainToRequestCurrency.currencypair.fromCurrency ===
              this.mainCurrency,
          ]
        );
      }
  ...
  private normalizeNotNeeded(loadedData: LoadedData): boolean {
    return (
      this.requestedCurrency.length === 0 ||
      loadedData.currencySecurity == null ||
      loadedData.currencySecurity === this.requestedCurrency
    );
  }

  private getCurrencypairOrReverse(
    reqesteOrMainCurrency: string,
    currencySecurity: string
  ): CurrenciesAndClosePrice {
    let mainToSecurityCurrency: CurrenciesAndClosePrice = this.crossRateMap.get(
      reqesteOrMainCurrency,
      currencySecurity
    );
    if (mainToSecurityCurrency == null) {
      mainToSecurityCurrency = this.crossRateMap.get(
        currencySecurity,
        reqesteOrMainCurrency
      );
    }
  ...
  private calculateHistoryquotes(
    loadedData: LoadedData,
    ...
    for (let i = 0; i < currenciesAndClosePrice.length; i++) {
      let cIndex = Math.abs(
        AppHelper.binarySearch(
          currenciesAndClosePrice[i].closeAndDateList,
          loadedData.historyquotes[0].date,
          this.compareHistoricalFN
        )
      );
      for (let hIndex = 0; hIndex < loadedData.historyquotes.length; hIndex++) {
        do {
          if (
            loadedData.historyquotes[hIndex].date >
            currenciesAndClosePrice[i].closeAndDateList[cIndex].date
          ) {
            cIndex++;
          } else if (
            loadedData.historyquotes[hIndex].date <
          ...
          hIndex < loadedData.historyquotes.length &&
          cIndex < currenciesAndClosePrice[i].closeAndDateList.length &&
          loadedData.historyquotes[hIndex].date !==
            currenciesAndClosePrice[i].closeAndDateList[cIndex].date
        );
        if (
          ...
          if (loadedData.historyquotes[hIndex].close != null) {
            if (i === 0) {
              const historyquoteDateClose = {
                ...
                close:
                  loadedData.historyquotes[hIndex].close *
                  (multiple[i]
                    ? currenciesAndClosePrice[i].closeAndDateList[cIndex].close
                    : 1 /
                      currenciesAndClosePrice[i].closeAndDateList[cIndex]
                        .close),
              };
              loadedData.historyquotesNorm.push(historyquoteDateClose);
              currenciesAndClosePrice.length > 1 &&
                (map2Loop[historyquoteDateClose.date] = historyquoteDateClose);
            } else {
              const historyquoteDateClose =
                map2Loop[loadedData.historyquotes[hIndex].date];
              if (historyquoteDateClose) {
                historyquoteDateClose.close *= multiple[i]
                  ? currenciesAndClosePrice[i].closeAndDateList[cIndex].close
                  : 1 /
                    currenciesAndClosePrice[i].closeAndDateList[cIndex].close;
              }
...
class LoadedData {
  ...
  constructor(
    public idSecuritycurrency: number,
    public nameSecuritycurrency:
      | CurrencypairWithTransaction
