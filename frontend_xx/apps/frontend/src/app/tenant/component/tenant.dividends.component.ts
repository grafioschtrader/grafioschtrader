...
export class TenantDividendsComponent
  ...
  implements IGlobalMenuAttach, OnInit, OnDestroy
{
  ...
  get selectedCashAccounts(): number {
    return this.idsAccounts.idsCashaccount.length === 1 &&
      this.idsAccounts.idsCashaccount[0] === -1
  ...
  get totalSecurityAccounts(): number {
    return this.securityDividendsGrandTotal
      ? this.securityDividendsGrandTotal.numberOfSecurityAccounts
      : 0;
  }
  ...
  ngOnInit(): void {
    ...
    this.columnConfigs.push(
      this.addColumn(
        DataType.Numeric,
        'securityCostGroup.groupTotalTaxCostMc',
    ...
    this.columnConfigs.push(
      this.addColumnFeqH(DataType.Numeric, 'yearAutoPaidTaxMC', true, false)
    );
    this.columnConfigs.push(
    ...
    this.columnConfigs.push(
      this.addColumnFeqH(
        DataType.Numeric,
        'yearRealReceivedDivInterestMC',
        true,
        false,
        {
          columnGroupConfigs: [
            new ColumnGroupConfig('grandRealReceivedDivInterestMC'),
          ],
        }
      )
    );
    ...
    const idsCashaccount = this.usersettingsService.readArray(
      AppSettings.DIV_CASHACCOUNTS
    );
    this.idsAccounts = new IdsAccounts(
      this.usersettingsService.readArray(AppSettings.DIV_SECURITYACCOUNTS),
  ...
  handleOnProcessedDialog(processedActionData: ProcessedActionData) {
    ...
    if (processedActionData.action === ProcessedAction.CREATED) {
      ...
      this.usersettingsService.saveArray(
        AppSettings.DIV_SECURITYACCOUNTS,
        this.idsAccounts.idsSecurityaccount
      );
      this.usersettingsService.saveArray(
        AppSettings.DIV_CASHACCOUNTS,
        this.idsAccounts.idsCashaccount
      );
      this.readData();
    }
  }
  ...
  private readData(): void {
    this.portfolioService
      ...
      .subscribe((data: SecurityDividendsGrandTotal) => {
        ...
        this.securityDividendsYearGroup =
          this.securityDividendsGrandTotal.securityDividendsYearGroup;
        this.columnConfigs.forEach(
          (columnConfig) =>
            (columnConfig.headerSuffix =
              this.securityDividendsGrandTotal.mainCurrency)
        );
