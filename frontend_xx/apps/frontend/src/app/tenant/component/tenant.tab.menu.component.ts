...
export class TenantTabMenuComponent implements OnInit {
  ...
  ngOnInit(): void {
    const activeItem = sessionStorage.getItem(
      GlobalSessionNames.TAB_MENU_TENANT
    );
  ...
  navigateTo(toIdActiveItem: number): void {
    ...
    sessionStorage.setItem(
      GlobalSessionNames.TAB_MENU_TENANT,
      this.idActiveItem.toString()
    );
    this.router.navigate([this.nameRouteMapping[this.idActiveItem][1]], {
