...
@Component({
  ...
  template: `
    <p-dialog
      header="{{
        (onlyCurrency ? 'CLIENT_CHANGE_CURRENCY' : 'CLIENT') | translate
      }}"
      [(visible)]="visibleTenantDialog"
      [responsive]="true"
      [style]="{ width: '450px' }"
      (onShow)="onShow($event)"
      (onHide)="onHide($event)"
  ...
  implements OnInit
{
  ...
  submit(value: { [name: string]: any }) {
    if (this.onlyCurrency) {
      this.tenantService
        .changeCurrencyTenantAndPortfolios(value.currency)
        .subscribe(
          (tenant) => {
  ...
  protected afterSaved(tenant: Tenant): void {
    this.closeDialog.emit(
      new ProcessedActionData(ProcessedAction.CREATED, tenant)
    );
  }
}
