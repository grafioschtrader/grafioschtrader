  ...
  template: `
    ...
    >
      <p-panel>
        ...
        <label class="small-padding control-label" for="idYearSelect">{{
          'YEAR' | translate
        }}</label>
        <p-dropdown
          id="idYearSelect"
        ...
        <p-footer>
          <div class="ui-dialog-buttonpane ui-widget-content flexRight">
            <button
              pButton
              class="btn"
              (click)="addMinusYear(-1)"
              label="{{ yearCalendarData.year - 1 }}"
              icon="pi pi-angle-left"
              *ngIf="containsYear(yearCalendarData.year - 1)"
            ></button>
            <button
              pButton
              class="btn"
      ...
      <tenant-performance-eod-missing-table
        ...
        [countIdSecurityMissingsMap]="
          missingQuotesWithSecurities?.countIdSecurityMissingsMap
        "
  ...
  implements IGlobalMenuAttach, OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    this.subscriptionHistoryquoteChanged =
      this.dataChangedService.dateChanged$.subscribe((processedActionData) => {
        if (
          processedActionData.data instanceof Historyquote &&
  ...
  readData(yearChange: boolean): void {
    this.holdingService
      ...
      .subscribe((mqws: MissingQuotesWithSecurities) => {
        ...
        if (!yearChange) {
          this.setYearsBoundaries(
            moment(mqws.firstEverTradingDay).year(),
            moment().year()
          );
        }
      });
  ...
  onDayPlusSelect(range: RangeSelectDays, ranges: RangeSelectDays[]) {
    this.foundDayMarkedRange =
      ranges.length === 2 &&
      this.securityMissingDays.find((date) => date === range.start) !== null;
  ...
  ngOnDestroy(): void {
    this.subscriptionHistoryquoteChanged &&
      this.subscriptionHistoryquoteChanged.unsubscribe();
  }
