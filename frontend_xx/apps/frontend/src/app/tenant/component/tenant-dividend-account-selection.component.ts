    ...
    >
      <ng-template pTemplate="caption">
        <div style="text-align:left">
          ...
          <span style="margin-left: .25em; vertical-align: middle">{{
            'TOGGLE_ALL' | translate
          }}</span>
        </div>
      </ng-template>
...
export class TenantDividendAccountSelectionComponent
  ...
  implements OnInit
{
  ...
  prepareData(): void {
    const markAll =
      (this.idsAccount.length === 1 && this.idsAccount[0] === -1) ||
      this.idsAccount.length === 0;

    const psa: TreeNode[] = [];
    this.portfolios.forEach((portfolio) => {
      if (portfolio[this.listAttributeName].length > 0) {
        const portfolioTreeNode: TreeNode = {
          data: portfolio,
          children: [],
          expanded: true,
          leaf: false,
        ...
        portfolio[this.listAttributeName].forEach((account) => {
          ...
          if (
            markAll ||
            this.idsAccount.indexOf(account.idSecuritycashAccount) >= 0
          ) {
        ...
        if (
          childSelectonCount > 0 &&
          portfolio[this.listAttributeName].length === childSelectonCount
        ) {
  ...
  getSelectedAccountIds(): number[] {
    return this.selectedNodes
      .filter((treeNode) => treeNode.leaf)
      .map((treeNode) => treeNode.data.idSecuritycashAccount);
  }
}
