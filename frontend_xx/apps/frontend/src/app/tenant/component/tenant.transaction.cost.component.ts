    ...
    >
      <div class="datatable">
        <p-table
          #dataTable
          [columns]="fields"
          [value]="transactionCostGroupSummaries"
...
export class TenantTransactionCostComponent
  ...
  implements IGlobalMenuAttach, OnInit, OnDestroy
{
  ...
  ngOnInit(): void {
    ...
    this.addColumn(
      ...
      'groupCountPaidTransaction',
      'PAID_TRANSACTIONS',
      true,
      false,
      {
        columnGroupConfigs: [
          new ColumnGroupConfig('grandCountPaidTransaction'),
        ],
      }
    );
    this.columnConfigs.push(
      this.addColumn(
        DataType.Numeric,
        'groupTotalAverageTransactionCostMC',
        'TRANSACTION_AVERAGE_PAID',
        true,
        false,
        {
          columnGroupConfigs: [
            new ColumnGroupConfig('grandTotalAverageTransactionCostMC'),
          ],
        }
      )
    );
    this.columnConfigs.push(
      this.addColumn(
        DataType.Numeric,
        'groupTotalTransactionCostMC',
        'TRANSACTION_COST',
        true,
        false,
        {
          columnGroupConfigs: [
            new ColumnGroupConfig('grandTotalTransactionCostMC'),
          ],
        }
      )
    );
  ...
  readData() {
    this.portfolioService
      .getTransactionCostGrandSummaryByTenant()
      .subscribe((data: TransactionCostGrandSummary) => {
        this.transactionCostGrandSummary = data;
        this.transactionCostGroupSummaries =
          this.transactionCostGrandSummary.transactionCostGroupSummaries;
        this.columnConfigs.forEach(
          (columnConfig) =>
            (columnConfig.headerSuffix =
              this.transactionCostGrandSummary.mainCurrency)
        );
        this.prepareTableAndTranslate();

        const urlPattern = new RegExp(
          `.*\/\/${AppSettings.MAIN_BOTTOM}.*\/${AppSettings.TRANSACTION_COST_KEY}\\\)`
        );
        urlPattern.test(this.router.url) &&
  ...
  chartDataPointClicked(traceIndex: number, dataPointIndex: number) {
    const tcgs =
      this.transactionCostGrandSummary.transactionCostGroupSummaries.find(
        (tcgsTemp) =>
          tcgsTemp.securityaccount.idSecuritycashAccount ===
          this.chartData[this.refId][traceIndex]
      );
    this.expandedTCGSid[tcgs.securityaccount.idSecuritycashAccount] = true;
    setTimeout(() =>
      this.selectRowAndGoToPage(traceIndex, dataPointIndex, tcgs)
    );
  }
      ...
      AppSettings.MAINVIEW_KEY + '/',
      {
        outlets: {
          mainbottom: [
            ...
            AppSettings.TRANSACTION_COST_KEY,
          ],
        },
      },
    ]);
  ...
  private prepareCharDataAndSentToChart(): void {
    ...
    this.transactionCostGrandSummary.transactionCostGroupSummaries.forEach(
      (tcgs) => {
        const traceSecurityaccount: Partial<ChartTrace> =
          PlotlyHelper.initializeChartTrace(
        ...
        this.chartData[this.refId].push(
          tcgs.securityaccount.idSecuritycashAccount
        );
  ...
  private selectRowAndGoToPage(
    traceIndex: number,
    dataPointIndex: number,
    tcgs: TransactionCostGroupSummary
  ): void {
    const idTransaction =
      this.chartData[traceIndex][this.refId][dataPointIndex];
    let rowIndex = 0;
    for (let i = 0; i < tcgs.transactionCostPositions.length; i++) {
      if (
        tcgs.transactionCostPositions[i].transaction.idTransaction ===
        idTransaction
      ) {
        rowIndex = i;
