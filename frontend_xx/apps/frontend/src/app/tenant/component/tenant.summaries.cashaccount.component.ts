...
{
  TenantPortfolioSummary: typeof TenantPortfolioSummary =
    TenantPortfolioSummary;

  readonly CASHBALANCE_MC = 'cashBalanceMC';
  readonly VALUE_SECURITIES_MAIN_CURRENCY = 'valueSecuritiesMC';
  public groupChangeIndexMap: Map<number, AccountPositionGroupSummary> =
    new Map();
  untilDate: Date;
  ...
  selectedGroup: string =
    TenantPortfolioSummary[TenantPortfolioSummary.GROUP_BY_CURRENCY];
  ...
  constructor(
    ...
    this.addColumn(DataType.String, 'cashaccount.name', 'NAME', true, false, {
      width: 100,
      columnGroupConfigs: [
        new ColumnGroupConfig('groupName', 'TOTAL'),
    ...
    this.columnConfigs.push(
      this.addColumnFeqH(DataType.Numeric, 'cashTransferMC', true, false, {
        templateName: 'greenRed',
        columnGroupConfigs: [
          new ColumnGroupConfig('groupCashTransferMC'),
    ...
    this.columnConfigs.push(
      this.addColumn(DataType.Numeric, 'accountFeesMC', 'FEE', true, false, {
        templateName: 'greenRed',
        columnGroupConfigs: [
          new ColumnGroupConfig('groupAccountFeesMC'),
    ...
    this.columnConfigs.push(
      this.addColumn(
        ...
        false,
        {
          templateName: 'greenRed',
          columnGroupConfigs: [
            new ColumnGroupConfig('groupCashBalanceMC'),
    ...
    this.columnConfigs.push(
      this.addColumnFeqH(DataType.Numeric, 'valueMC', true, false, {
        templateName: 'greenRed',
        columnGroupConfigs: [
          new ColumnGroupConfig('groupValueMC'),
          new ColumnGroupConfig('grandValueMC'),
        ],
      })
    );
    ...
    SelectOptionsHelper.createSelectItemForEnum(
      translateService,
      TenantPortfolioSummary,
      this.groupOptions
    );
  ...
  ngOnInit(): void {
    this.translateService
      .get(this.CHART_TITLE)
      .subscribe((translated) => (this.CHART_TITLE = translated));
  ...
  private readData(): void {
    this.portfolioService
      .getGroupedAccountsSecuritiesTenantSummary(
        this.untilDate,
        TenantPortfolioSummary[this.selectedGroup]
      )
      .subscribe((result) => {
        ...
        this.columnConfigs.forEach(
          (columnConfig) =>
            (columnConfig.headerSuffix =
              this.accountPositionGrandSummary.mainCurrency)
        );
  ...
  private getChartDefinition(): any {
    const securityBar = PlotlyHelper.initializeChartTrace(
      this.getColumnConfigByField(this.VALUE_SECURITIES_MAIN_CURRENCY)
        .headerTranslated,
      'bar'
    );
    ...
    for (const accountPositionGroupSummary of this.accountPositionGrandSummary
      .accountPositionGroupSummaryList) {
      if (
