...
export class TenantService extends AuthServiceWithLogout<Tenant> {
  ...
  getTenantAndPortfolio(): Observable<Tenant> {
    return <Observable<Tenant>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.TENANT_KEY}`,
          this.getHeaders()
        )
  ...
  public setWatchlistForPerformance(idWatchlist: number): Observable<Tenant> {
    return this.httpClient
      .patch(
        `${AppSettings.API_ENDPOINT}${AppSettings.TENANT_KEY}/watchlistforperformance/${idWatchlist}`,
        null,
  ...
  public changeCurrencyTenantAndPortfolios(
    currency: string
  ): Observable<Tenant> {
    return this.httpClient
      .patch(
        `${AppSettings.API_ENDPOINT}${AppSettings.TENANT_KEY}/${currency}`,
        null,
        { headers: this.prepareHeaders() }
      )
      .pipe(catchError(this.handleError.bind(this)));
