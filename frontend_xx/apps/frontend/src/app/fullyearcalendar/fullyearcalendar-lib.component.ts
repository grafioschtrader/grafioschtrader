  ...
  @Input() set yearCalendarData(val: YearCalendarData) {
    if (val.disableWeekDays) {
      ...
      val.disableWeekDays.forEach((dayOfWeek: DayOfWeek) =>
        this.disabledDaysOfWeek.push(
          this.locale.dayNamesMin[Year.getDayNumberByDayOfWeek(dayOfWeek)]
        )
      );
    }
    ...
    for (const m of this.year.months) {
      for (const w of m.weeks) {
        for (const day of w.daysOfWeek) {
          if (
            this._yearCalendarData.dates &&
            this._yearCalendarData.dates.length > 0
          ) {
            for (const d of this._yearCalendarData.dates) {
              if (day.day >= d.start && day.day <= d.end) {
                ...
                range.foregroundColor =
                  this.getForegroundColorByBackgroundColor(
          ...
          day.isDisabled =
            ...
            this.isWeekdayDisabled(
              day.dayOfWeek,
              this._yearCalendarData.disableWeekDays
            );
        }
      }
    }
