      ...
      <table style="margin: 0px auto;">
        <thead>
          <tr>
            <th *ngFor="let wd of locale.dayNamesMin">
              <div
                [className]="
                  disabledDaysOfWeek && disabledDaysOfWeek.indexOf(wd) >= 0
                    ? 'disabledHeaderDay'
                    : ''
                "
              >
        ...
        <tbody>
          <tr *ngFor="let week of month.weeks">
            <td
              *ngFor="let day of week.daysOfWeek"
              style="text-align: center;padding: 0px;"
            >
              <div
                [className]="day && day.isDisabled ? 'daydisabled' : ''"
                (click)="
                  day && day.isDisabled
                    ...
                    day.ranges
                      ? underline
                        ? {
                            'border-bottom':
                              '5px solid ' +
                              day.ranges[day.ranges.length - 1].backgroundColor,
                            'margin-bottom': '-5px'
                          }
                        : {
                            background:
                              day.ranges[day.ranges.length - 1].backgroundColor,
                            color:
                              day.ranges[day.ranges.length - 1].foregroundColor
                          }
