...
export class SecurityService extends SecurityCurrencyService<Security> {
  ...
  getTradableSecuritiesByTenantAndIdWatschlist(
    ...
    return <Observable<Security[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/` +
            `${AppSettings.WATCHLIST_KEY}/${idWatchlist}`,
          this.getHeaders()
        )
  ...
  getSecurityByIdSecuritycurrency(
    idSecuritycurrency: number
  ): Observable<Security> {
    return <Observable<Security>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/${idSecuritycurrency}`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  getOpenPositionByIdSecuritycurrencyAndIdTenant(
    ...
    return <Observable<SecurityOpenPositionPerSecurityaccount>>this.httpClient
      .get(
        `${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/${idSecuritycurrency}/date/` +
          `${dateString}/${before}`,
        {
          headers: this.prepareHeaders(),
          params: this.getHttpParamsTransactions(
            idTransaction,
  ...
  searchByCriteria(
    ...
    return <Observable<Security[]>>this.httpClient
      .get(`${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/search`, {
        headers: this.prepareHeaders(),
  ...
  getHistoryquoteQualityHead(
    groupedBy: string
  ): Observable<HistoryquoteQualityHead> {
    return <Observable<HistoryquoteQualityHead>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/historyquotequality`,
  ...
  private getDateFromAndTo(
    ...
    dateFrom &&
      (httpParams = httpParams.append(
        'dateFrom',
        moment(dateFrom).format(AppSettings.FORMAT_DATE_SHORT_NATIVE)
      ));
    dateTo &&
      (httpParams = httpParams.append(
        'dateTo',
        moment(dateTo).format(AppSettings.FORMAT_DATE_SHORT_NATIVE)
      ));
    return { headers: httpHeaders, params: httpParams };
  }

  getDerivedInstrumentsLinksForSecurity(
    idSecuritycurrency: number
  ): Observable<SecurityCurrencypairDerivedLinks> {
    return <Observable<SecurityCurrencypairDerivedLinks>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.SECURITY_KEY}/` +
            `${idSecuritycurrency}/derivedlinks`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  private getHttpParamsTransactions(
    idTransaction: number,
    idOpenMarginTransaction
  ): HttpParams {
