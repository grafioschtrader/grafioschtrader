...
export class CurrencypairService extends SecurityCurrencyService<Currencypair> {
  ...
  getCurrencypairByIdSecuritycurrency(
    idSecuritycurrency: number
  ): Observable<Currencypair> {
    return <Observable<Currencypair>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.CURRENCYPAIR_KEY}/` +
            `${idSecuritycurrency}`,
          this.getHeaders()
        )
  ...
  getTransactionForCurrencyPair(
    idCurrencypair: number,
    forChart: boolean
  ): Observable<CurrencypairWithTransaction> {
  ...
  getFeedConnectors(): Observable<IFeedConnector[]> {
    return <Observable<IFeedConnector[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.CURRENCYPAIR_KEY}/feedConnectors`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  searchByCriteria(
    ...
    return <Observable<Currencypair[]>>this.httpClient
      .get(
        `${AppSettings.API_ENDPOINT}${AppSettings.CURRENCYPAIR_KEY}/search`,
        {
  ...
  getCurrencypairForCrossRate(
    crossRateRequest: CrossRateRequest
  ): Observable<CrossRateResponse[]> {
    return <Observable<CrossRateResponse[]>>this.httpClient
      .get(
        `${AppSettings.API_ENDPOINT}${AppSettings.CURRENCYPAIR_KEY}/crossrate`,
        {
          headers: this.prepareHeaders(),
