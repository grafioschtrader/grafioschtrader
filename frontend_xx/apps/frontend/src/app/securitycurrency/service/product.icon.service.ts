...
export class ProductIconService {
  ...
  constructor(private iconReg: SvgIconRegistryService) {
    this.icons.forEach((icon) =>
      this.iconReg.loadSvg(AppSettings.PATH_ASSET_ICONS + icon + '.svg', icon)
    );
  }
  ...
  getIconForAssetclass(assetclass: Assetclass, icon: string) {
    switch (
      ...
      case SpecialInvestmentInstruments.PENSION_FUNDS:
        icon =
          assetclass.categoryType === AssetclassType[AssetclassType.REAL_ESTATE]
            ? 'fr'
            : 'f';
        break;
  ...
  private getDirectInvestmentIcon(
    assetclassType: AssetclassType | string
  ): string {
    let icon: string;
