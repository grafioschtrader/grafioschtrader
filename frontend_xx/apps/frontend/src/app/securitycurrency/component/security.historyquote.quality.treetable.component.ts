  ...
  template: `
    ...
    >
      <p-panel>
        <p-header>
          <h4>
            {{ 'SECURITY_HISTORY_QUALITY' | translate }} ({{
              'LAST_UPDATE' | translate
            }}: {{ lastUpdate }})
          </h4>
        </p-header>
        <label class="small-padding control-label" for="groupSelect">{{
          'MAIN_QUALITY_GROUP' | translate
        }}</label>
        <p-dropdown
          id="groupSelect"
          [options]="groups"
...
export class SecurityHistoryquoteQualityTreetableComponent
  ...
  implements OnInit, IGlobalMenuAttach
{
  ...
  ngOnInit(): void {
    this.selectedGroup =
      HistoryquoteQualityGrouped[
        HistoryquoteQualityGrouped.STOCKEXCHANGE_GROUPED
      ];
  ...
  readData(historyquoteQualityGrouped: string): void {
    this.securityService
      .getHistoryquoteQualityHead(historyquoteQualityGrouped)
      .subscribe((hqg: HistoryquoteQualityHead) => {
        const tn: TreeNode[] = [];
        this.historyquoteQualityHead = hqg;
        this.lastUpdate = AppHelper.getDateByFormat(
          this.gps,
          this.historyquoteQualityHead.lastUpdate
        );
  ...
  nodeSelect(event) {
    ...
    if (hqg.categoryType !== null) {
      this.groupTitle =
        event.node.parent.parent.data.name +
        ' / ' +
        event.node.parent.data.name +
        ' / ' +
        hqg.name;
      this.historyquoteQualityIds = plainToClass(
        HistoryquoteQualityIds,
        event.node.data,
  ...
  protected getMenuShowOptions(): MenuItem[] {
    if (this.securityIdWithCurrency) {
      const menuItems: MenuItem[] = this.timeSeriesQuotesService.getMenuItems(
        this.securityIdWithCurrency.idSecurity,
        this.securityIdWithCurrency.currency,
        false
      );
  ...
  private addTreeNode(
    ...
    tn.push(treeNode);
    hqg.childrendHqg.forEach(
      (historyquoteQualityGroup: HistoryquoteQualityGroup) =>
        this.addTreeNode(treeNode.children, historyquoteQualityGroup, treeNode)
    );
  }
}
