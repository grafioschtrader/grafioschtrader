...
import {
  DynamicFieldHelper,
  VALIDATION_SPECIAL,
} from '../../shared/helper/dynamic.field.helper';
...
export class SecurityEditSupport {
  ...
  static getSecurityBaseFieldDefinition(
    securityDerived: SecurityDerived
  ): FieldConfig[] {
    const fc: FieldConfig[] = [];

    fc.push(
      DynamicFieldHelper.createFieldInputString(
        'name',
        'NAME_SECURITY',
        80,
        true,
    ...
    fc.push(
      DynamicFieldHelper.createFieldCheckbox(
        'isTenantPrivate',
        'PRIVATE_SECURITY',
    ...
    fc.push(
      DynamicFieldHelper.createFieldSelectNumberHeqF(
    ...
    if (securityDerived === SecurityDerived.Security) {
      fc.push(
        DynamicFieldHelper.createFieldInputStringVSHeqF(
          'isin',
          12,
          false,
      ...
      fc.push(
        DynamicFieldHelper.createFieldInputStringHeqF(
          'tickerSymbol',
          6,
          false,
    ...
    fc.push(
      DynamicFieldHelper.createFieldPcalendarHeqF(
        ...
        'activeFromDate',
        true,
        { fieldsetName: 'BASE_DATA', defaultValue: new Date(2000, 1, 3) }
      )
    );
    ...
    fc.push(
      DynamicFieldHelper.createFieldPcalendarHeqF(
        ...
        'activeToDate',
        true,
        { fieldsetName: 'BASE_DATA', defaultValue: new Date(2025, 1, 1) }
      )
    );

    fc.push(
      DynamicFieldHelper.createFieldSelectStringHeqF(
    ...
    fc.push(
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(
        ...
        -9.99,
        9.99,
        { fieldsetName: 'BASE_DATA', defaultValue: 1 }
      ),
  ...
  static getIntraHistoryFieldDefinition(
    securityDerived: SecurityDerived
  ): FieldConfig[] {
    const fc: FieldConfig[] = [];
    if (
      securityDerived === SecurityDerived.Security ||
      securityDerived === SecurityDerived.Currencypair
    ) {
      fc.push(
    ...
    fc.push(
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(
        ...
        0,
        3,
        { defaultValue: 0, fieldsetName: this.HISTORY_SETTINGS }
      )
    );
    if (
      securityDerived === SecurityDerived.Security ||
    ...
    fc.push(
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(
        ...
        0,
        3,
        { defaultValue: 0, fieldsetName: this.INTRA_SETTINGS }
      )
    );
    return fc;
  }
  ...
  public getDividendFieldDefinition(): FieldConfig[] {
    ...
    fc.push(
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(
        ...
        0,
        3,
        { defaultValue: 0, fieldsetName: AppSettings.DIVIDEND_SETTINGS }
      )
    );
    return fc;
  }
  ...
  public getSplitDefinition(): FieldConfig[] {
    ...
    fc.push(
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(
        ...
        0,
        3,
        { defaultValue: 0, fieldsetName: AppSettings.SPLIT_SETTINGS }
      )
    );
    return fc;
  }
  ...
  assignLoadedValues(
    ...
    configObject.stockexchange.valueKeyHtmlOptions =
    ...
    configObject.currency.valueKeyHtmlOptions = [
      new ValueKeyHtmlSelectOptions('', ''),
    ].concat(vksoCurrency);
    configObject.dividendCurrency &&
      (configObject.dividendCurrency.valueKeyHtmlOptions = [
        new ValueKeyHtmlSelectOptions('', ''),
      ].concat(vksoCurrency));
    configObject.assetClass.referencedDataObject = assetclasses;
    configObject.assetClass.valueKeyHtmlOptions =
      SelectOptionsHelper.assetclassCreateValueKeyHtmlSelectOptions(
        this.gps,
  ...
  valueChangedOnAssetClass(
    securityDerived: SecurityDerived,
    configObject: { [name: string]: FieldConfig }
  ): void {
    this.assetClassSubscribe =
      configObject.assetClass.formControl.valueChanges.subscribe(
        (idAssetclass: number) => {
          const assetClass: Assetclass = this.disableEnableFieldsOnAssetclass(
            securityDerived,
            configObject,
            idAssetclass
          );
          this.callbackValueChanged &&
            this.callbackValueChanged.valueChangedOnAssetClassExtend(
              assetClass
            );
        }
      );
  ...
  disableEnableFieldsOnAssetclass(
    ...
    if (idAssetclass) {
      assetClass = Helper.getReferencedDataObject(
        configObject.assetClass,
        null
      );
      this.enableDisableDenomination(
        configObject,
        assetClass,
        this.hasMarketValue
      );
  ...
  public enableDisableDenominationStockexchangeAssetclass(configObject: {
    [name: string]: FieldConfig;
  }): void {
    const assetClass = Helper.getReferencedDataObject(
      configObject.assetClass,
      null
    );
    this.enableDisableDenomination(
  ...
  valueChangedOnaActiveFromDate(configObject: {
    [name: string]: FieldConfig;
  }): void {
    this.activeFromDateSubscribe =
      configObject.activeFromDate.formControl.valueChanges.subscribe(
        (activeFromDate: Date) => {
          if (activeFromDate) {
            configObject.activeToDate.calendarConfig.minDate = new Date(
              activeFromDate.getTime() + 86400000
            );
            if (
   ...
   */
  valueChangedOnPrivateSecurity(
    securityDerived: SecurityDerived,
    configObject: { [name: string]: FieldConfig }
  ): void {
    this.securitySubscribe =
      configObject.isTenantPrivate.formControl.valueChanges.subscribe(
        (isPrivate) =>
  ...
  prepareForSave(
    ...
    security.idTenantPrivate = value.isTenantPrivate
      ? this.gps.getIdTenant()
      : null;
    return security;
  }
  ...
  private enableDisableDenomination(
    ...
    if (configObject.denomination) {
      const hsd = BusinessHelper.hasSecurityDenomination(
        assetClass,
        hasMarkedPrice
      );
  ...
  private hideShowSomeFields(
    ...
    FormHelper.hideVisibleFieldConfigs(
      assetClass.specialInvestmentInstrument !==
        SpecialInvestmentInstruments[SpecialInvestmentInstruments.ETF] &&
        assetClass.specialInvestmentInstrument !==
  ...
  private isHideIsinAndTicker(configObject: {
    [name: string]: FieldConfig;
  }): boolean {
    const assetClass: Assetclass = Helper.getReferencedDataObject(
      configObject.assetClass,
      null
    );
    return (
