...
export abstract class SecuritycurrencySearchBase implements OnInit {
  ...
  constructor(
    protected multiplyAddClose: boolean,
  ...
  ngOnInit(): void {
    ...
    this.config = [
      DynamicFieldHelper.createFieldInputStringVSHeqF(
        ...
        { userDefinedValue: this.firstGroup }
      ),
      ...
      DynamicFieldHelper.createFieldPcalendarHeqF(
        DataType.DateStringShortUS,
        'activeDate',
        false,
        { userDefinedValue: <string>this.secondGroup }
      ),
      DynamicFieldHelper.createFieldTriStateCheckbox(
        'onlyTenantPrivate',
        'PRIVATE_SECURITY',
        { userDefinedValue: this.secondGroup }
      ),
    ...
    if (this.multiplyAddClose) {
      this.config.push(
        DynamicFieldHelper.createFunctionButton('EXIT', (e) =>
          this.closeSearchDialog(e)
        )
      );
    }
  ...
  submit(value: { [name: string]: any }): void {
    const securitycurrencySearch: SecuritycurrencySearch =
      new SecuritycurrencySearch();
    this.dynamicFormComponent.cleanMaskAndTransferValuesToBusinessObject(
      securitycurrencySearch
    );
  ...
  helpLink(): void {
    BusinessHelper.toExternalHelpWebpage(
      ...
      HelpIds.HELP_WATCHLIST_SEARCHDIALOG
    );
  }

  protected initialize(): void {
    this.dynamicFormComponent.form.setValidators(atLeastOneFieldValidator);
    const denyAssetClass = [
      AssetclassType.CURRENCY_CASH,
    ...
    this.configObject.assetclassType.valueKeyHtmlOptions =
      SelectOptionsHelper.createHtmlOptionsFromEnumAddEmpty(
        this.translateService,
        AssetclassType,
    ...
    this.configObject.specialInvestmentInstruments.valueKeyHtmlOptions =
      SelectOptionsHelper.createHtmlOptionsFromEnumAddEmpty(
        this.translateService,
        SpecialInvestmentInstruments
      );
    ...
    this.multipleRequestToOneService
      .getDataForCurrencySecuritySearch()
      .subscribe((dfcss: DataForCurrencySecuritySearch) => {
        this.setValueKeyHtmlOptions(
          this.configObject.currency,
          dfcss.currencies
        );
        this.setValueKeyHtmlOptions(
          this.configObject.subCategoryNLS,
          dfcss.assetclasses
        );
        this.setValueKeyHtmlOptions(
          this.configObject.idConnectorHistory,
          dfcss.feedConnectorsHistory
        );
        this.setValueKeyHtmlOptions(
          this.configObject.idConnectorIntra,
          dfcss.feedConnectorsIntra
        );
        this.configObject.idStockexchange.valueKeyHtmlOptions =
          SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromArray(
            'idStockexchange',
            'name',
            dfcss.stockexchanges,
            true
          );
  ...
  private setValueKeyHtmlOptions(
    fieldConfig: FieldConfig,
    values: ValueKeyHtmlSelectOptions[]
  ): void {
    fieldConfig.valueKeyHtmlOptions = values;
    fieldConfig.valueKeyHtmlOptions.splice(
      0,
      0,
  ...
  private valueChangedOnForm(): void {
    this.dynamicFormComponent.form.valueChanges.subscribe(
      (security: Security) => {
        if (!this.monitor) {
          this.monitor = true;
          this.dynamicFormComponent.controls
            .filter(
              (fieldConfig) => fieldConfig.userDefinedValue === this.secondGroup
            )
            .forEach((fieldConfig) =>
          ...
          if (!security.isin || security.isin === '') {
            ...
            this.dynamicFormComponent.controls
              .filter(
                (fieldConfig) =>
                  fieldConfig.userDefinedValue === this.firstGroup
              )
              .forEach((fieldConfig) =>
                AppHelper.invisibleAndHide(fieldConfig, !secondGroupIsEmpty)
              );
            AppHelper.invisibleAndHide(
              this.configObject.name,
              this.configObject.name.invisible || this.isExactCurrecny()
            );
            AppHelper.invisibleAndHide(
              this.configObject.tickerSymbol,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.onlyTenantPrivate,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.leverageFactor,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.activeDate,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.idStockexchange,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.specialInvestmentInstruments,
              this.isCurrency()
            );
            AppHelper.invisibleAndHide(
              this.configObject.subCategoryNLS,
  ...
  private isSecondGroupEmpty(): boolean {
    const foundFieldConfig = this.dynamicFormComponent.controls
      .filter(
        (fieldConfig) => fieldConfig.userDefinedValue === this.secondGroup
      )
      .find(
  ...
  private isCurrency(): boolean {
    return (
      this.configObject.assetclassType.formControl.value ===
