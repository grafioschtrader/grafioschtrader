...
@Component({
  ...
  template: ` <p-dialog
    ...
    [(visible)]="visibleEditSecurityDialog"
    ...
    <p-tabView>
      <p-tabPanel header="{{ 'SECURITY' | translate }}">
        <dynamic-form
          ...
          #dynamicFieldsetForm="dynamicForm"
          (submitBt)="submit($event)"
        >
        </dynamic-form>
      </p-tabPanel>
      <p-tabPanel
        header="{{ 'SECURITY_SPLITS' | translate }}"
        *ngIf="canHaveSplits || !dataLoaded"
      >
        <dynamic-form
          [config]="configSplit"
          [formConfig]="formConfig"
          [translateService]="translateService"
          #splitForm="dynamicForm"
          (submitBt)="addSplit($event)"
        >
        </dynamic-form>
        <securitysplit-edit-table
          (editData)="onSelectedSecuritysplit($event)"
      ...
      <p-tabPanel
        header="{{ 'HISTORYQUOTE_FOR_PERIOD' | translate }}"
        *ngIf="!this.securityEditSupport?.hasMarketValue || !dataLoaded"
      >
        <p>{{ 'HISTORYQUOTE_FOR_PERIOD_COMMENT' | translate }}</p>
        <dynamic-form
          [config]="periodPrices"
          [formConfig]="formConfigPeriod"
          [translateService]="translateService"
          #periodPriceForm="dynamicForm"
          (submitBt)="addHistoryquotePeriod($event)"
        >
        </dynamic-form>
        <security-historyquote-period-edit-table
          (editData)="onSelectedHistoryquote($event)"
          (savedData)="onDependingDialogSave($event)"
...
export class SecurityEditComponent
  extends SecuritycurrencyEdit
  implements OnInit, CallbackValueChanged
{
  // Access child components
  @ViewChild('splitForm') dynamicSplitForm: DynamicFormComponent;
  @ViewChild(SecuritysplitEditTableComponent)
  seetc: SecuritysplitEditTableComponent;
  @ViewChild('periodPriceForm') dynamicPeriodPriceForm: DynamicFormComponent;
  @ViewChild(SecurityHistoryquotePeriodEditTableComponent)
  shpetc: SecurityHistoryquotePeriodEditTableComponent;
  ...
  ngOnInit(): void {
    this.securityEditSupport = new SecurityEditSupport(
    ...
    this.formConfigPeriod = AppHelper.getDefaultFormConfig(
      this.gps,
      2,
      this.helpLinkPeriod.bind(this)
    );

    this.config = SecurityEditSupport.getSecurityBaseFieldDefinition(
      SecurityDerived.Security
    );
    this.connectorPriceFieldConfig =
      SecurityEditSupport.getIntraHistoryFieldDefinition(
        SecurityDerived.Security
      );
    this.securityEditSupport.connectorDividendConfig =
      this.securityEditSupport.getDividendFieldDefinition();
    this.securityEditSupport.connectorSplitConfig =
      this.securityEditSupport.getSplitDefinition();
    this.config.push(
      ...this.connectorPriceFieldConfig,
      ...this.securityEditSupport.connectorDividendConfig,
      ...this.securityEditSupport.connectorSplitConfig
    ...
    this.configPeriodPrices = TranslateHelper.prepareFieldsAndErrors(
      this.translateService,
      this.periodPrices
    );
    this.configSplitObject = TranslateHelper.prepareFieldsAndErrors(
      this.translateService,
      this.configSplit
    );
    this.configObject.distributionFrequency.valueKeyHtmlOptions =
  ...
  valueChangedOnStockexchange(): void {
    this.stockexchangeSubscribe =
      this.configObject.stockexchange.formControl.valueChanges.subscribe(
        (idStockexchange: number) => {
          this.setHasMarkedValue(idStockexchange);
          this.hideVisibleFeedConnectorsFields(
            [...this.connectorPriceFieldConfig],
            !this.securityEditSupport.hasMarketValue,
            FeedIdentifier.SECURITY
          );
          this.enableDisableDividendSplitConnector(
            Helper.getReferencedDataObject(this.configObject.assetClass, null)
          );
          this.securityEditSupport.enableDisableDenominationStockexchangeAssetclass(
            this.configObject
          );
        }
      );
  ...
  valueChangedOnDistributionFrequency(): void {
    this.distributionFrequencySubscribe =
      this.configObject.distributionFrequency.formControl.valueChanges.subscribe(
        (distributionFrequency) => {
          this.enableDisableDividendConnector(
            Helper.getReferencedDataObject(this.configObject.assetClass, null),
            distributionFrequency
          );
        }
  ...
  addSplit(value: { [name: string]: any }): void {
    ...
    this.dynamicSplitForm.cleanMaskAndTransferValuesToBusinessObject(
      securitysplit
    );
  ...
  addHistoryquotePeriod(value: { [name: string]: any }): void {
    ...
    this.dynamicPeriodPriceForm.cleanMaskAndTransferValuesToBusinessObject(
      historyquotePeriod
    );
  ...
  submit(value: { [name: string]: any }): void {
    const security = this.securityEditSupport.prepareForSave(
      this,
      this.proposeChangeEntityWithEntity,
      <Security>this.securityCurrencypairCallParam,
      this.dynamicForm,
      value
    );
    this.securityService.update(security).subscribe(
      (newSecurity) => {
        this.messageToastService.showMessageI18n(
          ...
          'MSG_RECORD_SAVED',
          { i18nRecord: AppSettings.SECURITY.toUpperCase() }
        );
        ...
        if (this.securityEditSupport.hasMarketValue) {
          if (this.canHaveSplits && this.seetc) {
            ...
            this.seetc.save(
              newSecurity,
              this.configObject[AuditHelper.NOTE_REQUEST_INPUT].formControl
                .value
            );
          }
        } else {
        ...
        if (!savedDepending) {
          this.onDependingDialogSave(
            new SaveSecuritySuccess(newSecurity, true)
          );
        }
      },
  ...
  onSelectedHistoryquote(historyquotePeriod: HistoryquotePeriod) {
    this.dynamicPeriodPriceForm.transferBusinessObjectToForm(
      historyquotePeriod
    );
  }
  ...
  override onHide(event) {
    ...
    this.distributionFrequencySubscribe &&
      this.distributionFrequencySubscribe.unsubscribe();
    super.onHide(event);
  }
  ...
  helpLink(): void {
    BusinessHelper.toExternalHelpWebpage(
      ...
      HelpIds.HELP_WATCHLIST_SECURITY
    );
  }
  ...
  helpLinkPeriod(): void {
    BusinessHelper.toExternalHelpWebpage(
      this.gps.getUserLang(),
      HelpIds.HELP_WATCHLIST_WITHOUT_PRICE_DATA
    );
  }

  protected override loadHelperData(): void {
    this.securityEditSupport.registerValueOnChanged(
      SecurityDerived.Security,
      this.configObject
    );
    ...
    if (this.securityCurrencypairCallParam) {
      this.securityEditSupport.hasMarketValue = !(<Security>(
        this.securityCurrencypairCallParam
      )).stockexchange.noMarketValue;
    }
    ...
    if (this.securityCurrencypairCallParam) {
      if (this.securityEditSupport.hasMarketValue) {
        ...
        if ((<Security>this.securityCurrencypairCallParam).splitPropose) {
          // Propose change
          this.seetc.setDataList(
            (<Security>this.securityCurrencypairCallParam).splitPropose,
            true
          );
        } else {
          observables.push(
        ...
        if ((<Security>this.securityCurrencypairCallParam).hpPropose) {
          this.shpetc.setDataList(
            (<Security>this.securityCurrencypairCallParam).hpPropose,
            true
          );
        } else {
          observables.push(
    ...
    combineLatest(observables).subscribe(
      (
        ...
        const isPrivatePaper =
          this.securityCurrencypairCallParam &&
          (<Security>this.securityCurrencypairCallParam).idTenantPrivate !==
            null;
        ...
        this.securityEditSupport.setPrivatePaper(
          SecurityDerived.Security,
          isPrivatePaper,
          this.configObject
        );
  ...
  protected prepareSplitDividendConnector(
    feedConnectors: IFeedConnector[]
  ): void {
    this.splitDividendCreateValueKeyHtmlSelectOptions(
      this.configObject.idConnectorSplit,
      FeedSupport.SPLIT
    );
    this.splitDividendCreateValueKeyHtmlSelectOptions(
      this.configObject[this.securityEditSupport.ID_CONNECTOR_DIVIDEND],
      FeedSupport.DIVIDEND
    );
  }

  private enableDisableDividendSplitConnector(assetClass: Assetclass): void {
    this.canHaveSplits = Security.canHaveSplitConnector(
      assetClass,
      this.securityEditSupport.hasMarketValue
    );
    this.hideVisibleFeedConnectorsFields(
    ...
    this.enableDisableDividendConnector(
      assetClass,
      this.configObject.distributionFrequency.formControl.value
    );
  }

  private enableDisableDividendConnector(
  ...
  private setHasMarkedValue(idStockexchange: number): void {
    this.securityEditSupport.hasMarketValue =
      !!idStockexchange &&
      !(<Stockexchange[]>(
        this.configObject.stockexchange.referencedDataObject
      )).find(
  ...
  private disableEnableInputForExisting(): void {
    if (this.securityCurrencypairCallParam !== null) {
      FormHelper.disableEnableFieldConfigsWhenAlreadySet(true, [
        this.configObject.isTenantPrivate,
        this.configObject.isin,
        this.configObject.currency,
      ]);
    }
  }

  private splitDividendCreateValueKeyHtmlSelectOptions(
    fieldConfig: FieldConfig,
    filterType: FeedSupport
  ): void {
    const provider: IFeedConnector[] = this.feedPriceConnectors.filter(
      (feedConnector) =>
