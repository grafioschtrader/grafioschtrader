...
export abstract class SplitPeriodTableBase<T> extends TableConfigBase {
  ...
  @Output() savedData: EventEmitter<SaveSecuritySuccess> =
    ...
    if (replacePos === -1) {
      if (this._dataList.length < this.maxRows) {
        ...
        this.messageToastService.showMessageI18n(
          InfoLevelType.ERROR,
          'SECURITY_SPLITS_MAX_REACHED',
          { maxSplits: this.maxRows }
        );
        return;
      }
    } else {
      this._dataList = Object.assign([], this._dataList, {
        [replacePos]: rowData,
      });
    }
  ...
  onClickDelete() {
    ...
    this._dataList = [
      ...this.dataList.slice(0, i),
      ...this._dataList.slice(i + 1, this._dataList.length),
    ];
  ...
  save(security: Security, noteRequest: string): void {
    if (this.dataChanged) {
      const securitySplitsCleaned = plainToClass(this.classz, this._dataList, <
        any
      >{ excludeExtraneousValues: true });
      this.deleteCreateMultipleService
        .deleteAndCreateMultiple(
          security.idSecuritycurrency,
          securitySplitsCleaned,
          noteRequest
        )
