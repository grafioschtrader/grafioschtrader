...
} from './securitycurrency-search-and-set.component';
...
export class SecurityDerivedEditComponent
  extends SimpleEditBase
  implements OnInit, CallBackSetSecurityWithAfter
{
  ...
  private additionalInstruments: {
    [fieldName: string]: Security | CurrencypairWatchlist;
  } = {};
  ...
  ngOnInit(): void {
    ...
    this.config = [
      DynamicFieldHelper.createFieldInputButtonHeqF(
        DataType.String,
        this.BASE_PRODUCT_NAME,
        this.handleSecurityClick.bind(this),
        true,
        { fieldsetName: this.derivedData }
      ),
    ...
    this.config.push(
      ...SecurityEditSupport.getSecurityBaseFieldDefinition(
        SecurityDerived.Derived
      )
    );
    this.config.push(
  ...
  disableBaseInstrumentFields(): void {
    if (
      !AuditHelper.hasRightsForEditingOrDeleteAuditable(
        this.gps,
        this.securityCallParam
      )
    ) {
      this.config
        .filter((fieldConfig) => fieldConfig.fieldsetName === this.derivedData)
        .forEach((fieldConfig) => fieldConfig.formControl.disable());
    }
  }

  valueChangedOnFormula(): void {
    this.formulaSubscribe = this.configObject[
      this.FORMULA_PRICES
    ].formControl.valueChanges.subscribe((formulaPrices: string) => {
      if (!this.configObject[this.FORMULA_PRICES].formControl.disabled) {
        if (formulaPrices) {
          let match =
            SecurityCurrencypairDerivedLinks.VAR_NAME_REGEX.exec(formulaPrices);
          this.usedFormulaVars = [];
          while (match != null) {
            this.usedFormulaVars.push(match[1]);
            match =
              SecurityCurrencypairDerivedLinks.VAR_NAME_REGEX.exec(
                formulaPrices
              );
          }
  ...
  setSecurity(
    security: Security | CurrencypairWatchlist,
    ...
    this.configObject[this.dialogSecurityTargetFieldname].formControl.setValue(
      security.name
    );
  }
  ...
  protected initialize(): void {
    ...
    if (this.securityCallParam) {
      observables.push(
        this.securityService.getDerivedInstrumentsLinksForSecurity(
          this.securityCallParam.idSecuritycurrency
        )
      );
    }
    ...
    combineLatest(observables).subscribe(
      (
        ...
        this.securityCallParam &&
          this.setInstrumentsForExistingSecurity(
            <SecurityCurrencypairDerivedLinks>data[data.length - 1]
          );
  ...
  private addInvestmentInstrumentFieldRow(): void {
    ...
    this.usedFormulaVars.forEach((ufv: string) => {
      if (
        SecurityCurrencypairDerivedLinks.ALLOWED_VAR_NAMES.indexOf(ufv) >= 0 &&
        ufv !== SecurityCurrencypairDerivedLinks.ALLOWED_VAR_NAMES[0]
      ) {
        const fieldName =
          SecurityCurrencypairDerivedLinks.ADDITIONAL_INSTRUMENT_NAME +
          '_' +
          ufv;
        ...
        if (!this.configObject[fieldName]) {
          fieldConfig.push(
            DynamicFieldHelper.createFieldInputButton(
              DataType.String,
              fieldName,
              'ADDITIONAL_INSTRUMENT_NAME',
              this.handleSecurityClick.bind(this),
              true,
              { fieldsetName: this.derivedData, labelSuffix: `(${ufv})` }
            )
          );
        }
      }
    ...
    const unusedInstruments = this.config.filter(
      (fc) =>
        fc.field.startsWith(
    ...
    unusedInstruments.forEach((ffc) =>
      this.config.splice(
        this.config.findIndex((fc) => fc.field === ffc.field),
        1
      )
    );
    if (fieldConfig.length > 0 || unusedInstruments.length > 0) {
      this.config = [
        ...this.config.slice(0, 2),
        ...fieldConfig,
        ...this.config.slice(2, this.config.length),
      ];
      this.configObject = TranslateHelper.prepareFieldsAndErrors(
        this.translateService,
        this.config
  ...
  private setInstrumentsForExistingSecurity(
  ...
  private setBaseInstrumentForExistingSecurity(
    scdl: SecurityCurrencypairDerivedLinks
  ): void {
    ...
    this.baseInstrument = SecurityCurrencypairDerivedLinks.getBaseInstrument(
      scdl,
      this.securityCallParam.idLinkSecuritycurrency
    );
    this.reduceExpandChoosableAssetclass();
    this.configObject[this.BASE_PRODUCT_NAME].formControl.setValue(
      this.baseInstrument.name
    );
  }

  private setAdditionalInstrumentsForExistingSecurity(
    scdl: SecurityCurrencypairDerivedLinks
  ): void {
    this.additionalInstruments =
      SecurityCurrencypairDerivedLinks.getAdditionalInstrumentsForExistingSecurity(
        scdl
      );
    for (const [fieldName, securitycurrency] of Object.entries(
      this.additionalInstruments
    )) {
  ...
  private reduceExpandChoosableAssetclass(): void {
    const formula: string =
      this.configObject[this.FORMULA_PRICES].formControl.value;
    if (
      this.baseInstrument &&
      this.baseInstrument.assetClass.categoryType ===
        AssetclassType[AssetclassType.CURRENCY_PAIR] &&
      (!formula || formula.length === 0)
    ) {
      this.securityEditSupport.filterAssetclasses(
        this.configObject,
        AssetclassType.CURRENCY_PAIR
      );
    } else {
  ...
  private setSecurityBaseInstrument(
    security: Security | CurrencypairWatchlist
  ): void {
    this.baseInstrument = security;
    if (security instanceof CurrencypairWatchlist) {
      this.configObject.currency.formControl.setValue(
        (<CurrencypairWatchlist>security).fromCurrency
      );
      this.reduceExpandChoosableAssetclass();
    } else {
  ...
  private disableEnableInputForExisting(disable: boolean): void {
    FormHelper.disableEnableFieldConfigsWhenAlreadySet(disable, [
  ...
  private translateFormulaFromUserLanguage(security: Security): void {
    if (this.gps.getDecimalSymbol() !== '.' && security.formulaPrices) {
      security.formulaPrices = security.formulaPrices
        .split(this.gps.getDecimalSymbol())
        .join('.');
    }
  }
  ...
  private setSecurityDerivedLinks(security: Security): void {
    ...
    if (this.usedFormulaVars) {
      this.usedFormulaVars.forEach((ufv: string) => {
        if (
          SecurityCurrencypairDerivedLinks.ALLOWED_VAR_NAMES.indexOf(ufv) >=
            0 &&
          ufv !== SecurityCurrencypairDerivedLinks.ALLOWED_VAR_NAMES[0]
        ) {
          const fieldName =
            ...
            '_' +
            ufv;
          security.securityDerivedLinks.push(
            new SecurityDerivedLink(
              this.securityCallParam
                ? this.securityCallParam.idSecuritycurrency
                : null,
              ufv,
              this.additionalInstruments[fieldName].idSecuritycurrency
            )
          );
        }
      });
