...
export abstract class SecuritycurrencyEdit extends FormBase {
  ...
  public hideVisibleFeedConnectorsFields(
    ...
    const idFieldConfigurations = connectorFieldConfig.filter((field) =>
      field.field.startsWith('id')
    );
    const urlFieldConfigurations = connectorFieldConfig.filter((field) =>
      field.field.startsWith('url')
    );
    if (hideConnector) {
      ...
      this.valueChangedOnFeedConnectors(
        idFieldConfigurations,
        urlFieldConfigurations,
        feedIdentifier
      );
    }
  ...
  onHide(event): void {
    Object.values(this.connectorSubscribe).forEach(
      (cs) => cs && cs.unsubscribe
    );
  ...
  protected prepareFeedConnectors(
  ...
  protected disableEnableFeedUrlExtended(
    ...
    AppHelper.invisibleAndHide(
      urlExtended,
      feedIdentifiers.indexOf(FeedIdentifier[feedIdentifier]) >= 0
    );
  ...
  private valueChangedOnFeedConnectors(
    ...
    for (let i = 0; i < connectorIdConfigs.length; i++) {
      this.connectorSubscribe[connectorIdConfigs[i].field] = connectorIdConfigs[
        i
      ].formControl.valueChanges.subscribe((connector) => {
        const foundConnector = this.feedPriceConnectors.find(
          (fc) => fc.id === connector
        );
        if (foundConnector) {
          urlExtends[i].labelHelpText =
            this.configObject[this.ID_CONNECTOR_HISTORY] ===
            connectorIdConfigs[i]
              ? foundConnector.description.historicalDescription
              : foundConnector.description.intraDescription;
          if (
            this.ID_CONNECTOR_INTRA === connectorIdConfigs[i].field &&
            foundConnector.securitycurrencyFeedSupport[
              FeedSupport[FeedSupport.INTRA]
            ]
          ) {
            ...
            this.ID_CONNECTOR_HISTORY === connectorIdConfigs[i].field &&
            foundConnector.securitycurrencyFeedSupport[
              FeedSupport[FeedSupport.HISTORY]
            ]
          ) {
            this.disableEnableFeedUrlExtended(
              urlExtends[i],
            ...
            foundConnector.securitycurrencyFeedSupport[
              FeedSupport[FeedSupport.DIVIDEND]
            ]
          ) {
            this.disableEnableFeedUrlExtended(
              urlExtends[i],
  ...
  private feedConnectorsCreateValueKeyHtmlSelectOptions(
    ...
    const historyProvider: IFeedConnector[] = this.feedPriceConnectors.filter(
      (feedConnector) =>
        ...
        this.checkCurrencySecurityProvider(
          feedConnector,
          filterType,
          isCurrency
        )
  ...
  private checkCurrencySecurityProvider(
    ...
    if (isCurrency) {
      return (
        feedConnector.securitycurrencyFeedSupport[
          FeedSupport[filterType]
        ].indexOf(FeedIdentifier[FeedIdentifier.CURRENCY]) >= 0 ||
        feedConnector.securitycurrencyFeedSupport[
          FeedSupport[filterType]
        ].indexOf(FeedIdentifier[FeedIdentifier.CURRENCY_URL]) >= 0
      );
    } else {
      return (
        feedConnector.securitycurrencyFeedSupport[
          FeedSupport[filterType]
        ].indexOf(FeedIdentifier[FeedIdentifier.SECURITY]) >= 0 ||
        feedConnector.securitycurrencyFeedSupport[
          FeedSupport[filterType]
        ].indexOf(FeedIdentifier[FeedIdentifier.SECURITY_URL]) >= 0
      );
    }
  }
}
