...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'CURRENCYPAIR' | translate }}"
    [(visible)]="visibleEditCurrencypairDialog"
    [responsive]="true"
    [style]="{ width: '600px' }"
    (onShow)="onShow($event)"
    (onHide)="onHide($event)"
  ...
  implements OnInit
{
  ...
  valueChangedOnFromCurrency(): void {
    this.fromCurrencyChangedSub =
      this.configObject.fromCurrency.formControl.valueChanges.subscribe(
        (data: string) => {
          const existingFromCurrencies: Currencypair[] =
            this.existingCurrencypairs.filter(
              (currencypair) => currencypair.fromCurrency === data
            );
          this.configObject.toCurrency.valueKeyHtmlOptions =
            this.configObject.fromCurrency.valueKeyHtmlOptions.filter(
              (vKHO) =>
                existingFromCurrencies.filter(
                  (currencypair) => currencypair.toCurrency === vKHO.key
                ).length === 0
            );
        }
      );
  ...
  helpLink(): void {
    BusinessHelper.toExternalHelpWebpage(
      ...
      HelpIds.HELP_WATCHLIST_CURRENCYPAIR
    );
  }
  ...
  protected loadHelperData(): void {
    ...
    const observableCurrencies: Observable<ValueKeyHtmlSelectOptions[]> =
      this.gps.getCurrencies();
    const observalbeFeedConnectors: Observable<IFeedConnector[]> =
      this.currencypairService.getFeedConnectors();
    const observalbeAllCurrencypairs: Observable<Currencypair[]> =
      this.currencypairService.getAllCurrencypairs();
    combineLatest([
      observableCurrencies,
      observalbeFeedConnectors,
      observalbeAllCurrencypairs,
    ]).subscribe((data) => {
  ...
  private removeByUpdateFormExstingCurrency(): void {
    if (this.securityCurrencypairCallParam) {
      this.existingCurrencypairs = this.existingCurrencypairs.filter(
        (currencypair) =>
          currencypair.idSecuritycurrency !==
  ...
  private disableEnableInputForExisting(disable: boolean): void {
    FormHelper.disableEnableFieldConfigs(disable, [
      this.configObject.fromCurrency,
      this.configObject.toCurrency,
    ]);
    this.prepareExistingSecuritycurrency(
      disable ? this.configObject.note : this.configObject.fromCurrency
    );
  }
}
