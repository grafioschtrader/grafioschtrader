...
export class SecurityaccountTabMenuComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.platTransImportMenuItem = {
      ...
      command: (event) =>
        this.navigateToRoute(AppSettings.SECURITYACCOUNT_IMPORT_KEY, 1),
    };
  ...
  ngOnInit(): void {
    this.routeParamSubscribe = this.activatedRoute.params.subscribe(
      (params: Params) => {
        if (params['object']) {
          ...
          if (params[AppSettings.SUCCESS_FAILED_IMP_TRANS]) {
            ...
            this.navigateToRoute(
              AppSettings.SECURITYACCOUNT_IMPORT_KEY,
              1,
