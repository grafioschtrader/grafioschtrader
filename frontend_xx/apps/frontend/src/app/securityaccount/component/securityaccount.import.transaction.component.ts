...
import {
  ...
  FileUploadParam,
} from '../../shared/generaldialog/model/file.upload.param';
...
export class SecurityaccountImportTransactionComponent
  ...
  implements
    OnInit,
    OnDestroy,
    ParentChildRowSelection<CombineTemplateAndImpTransPos>
{
  ...
  ngOnInit(): void {
    this.routeSubscribe = this.activatedRoute.params.subscribe(
      (params: Params) => {
        this.securityAccount = JSON.parse(
          params[AppSettings.SECURITYACCOUNT.toLowerCase()]
        );
          ...
          .subscribe(
            (importTransactionTemplates: ImportTransactionTemplate[]) => {
              ...
              if (params[AppSettings.SUCCESS_FAILED_IMP_TRANS]) {
                this.successFailedDirectImportTransaction = JSON.parse(
                  params[AppSettings.SUCCESS_FAILED_IMP_TRANS]
                );
                this.messageToastService.showMessageI18n(
                  InfoLevelType.ERROR,
                  'FAILED_TRANS_FROM_IMPORT'
                );
              }
              setTimeout(() => {
  ...
  readData(): void {
    this.importTransactionHeadService
      .getImportTransactionHeadBySecurityaccount(
        ...
        if (!this.selectedEntity && this.successFailedDirectImportTransaction) {
          this.selectedEntity = this.entityList.find(
            (imporTtransactionHead) =>
              ...
              this.successFailedDirectImportTransaction.idTransactionHead
          );
        }
        this.setFieldValues();
      });
  ...
  setChildData(selectedEntity: ImportTransactionHead): void {
    this.sitdc.parentSelectionChanged(
      this.selectedEntity,
      this,
      this.importTransactionTemplates
    );
  }
  ...
  handleUploadCSVFile(): void {
    this.importTransactionTemplateService
      .getCSVTemplateIdsAsValueKeyHtmlSelectOptions(
      ...
      .subscribe((vkhso) => {
        ...
        this.handleUploadFiles(
          new AdditionalFieldConfig(
            fieldConfig,
            this.submitPrepareFN.bind(this)
          ),
          this.selectedEntity,
          'UPLOAD_CSV',
          'csv',
