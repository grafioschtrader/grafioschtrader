 ...
 */
export class SecurityaccountCurrencyGroup extends SecurityaccountGroupBase<
...
> {
  ...
  public getGroupValueFromGroupRow(
    securityPositionGroupSummary: SecurityPositionCurrenyGroupSummary
  ) {
    return securityPositionGroupSummary.currency;
  }

  override getGroupRowFieldText(
    columnConfig: ColumnConfig,
    arrIndex: number,
    data: any,
    mapKey: any
  ): string {
    const text = this.datatableConfigBase.getTextValueColumnTotal(
      columnConfig,
      arrIndex
    );
    const exchangeRate =
      this.datatableConfigBase.getFieldValueForFieldColumnTotal(
        columnConfig,
        arrIndex,
        data,
        mapKey,
  ...
  public override getChartDefinition(
    ...
    securityPositionGrandSummary.securityPositionGroupSummaryList.forEach(
      (spgs: SecurityPositionCurrenyGroupSummary) => {
        values.push(
          (spgs.groupAccountValueSecurityMC /
            securityPositionGrandSummary.grandAccountValueSecurityMC) *
            100
        );
