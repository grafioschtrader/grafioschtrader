...
export class SecurityaccountEditComponent
  extends SimpleEntityEditBase<Securityaccount>
  implements OnInit
{
  ...
  constructor(
    ...
    this.untilFields = [
      new UntilField('shareUseUntil', true),
  ...
  ngOnInit(): void {
    ...
    this.config = [
      DynamicFieldHelper.createFieldInputString(
        'name',
        'SECURITYACCOUNT_NAME',
        25,
        true
      ),
      DynamicFieldHelper.createFieldSelectString(
        'tradingPlatformPlan',
        AppSettings.TRADING_PLATFORM_PLAN.toUpperCase(),
        true,
      ...
      DynamicFieldHelper.createFieldCurrencyNumberHeqF(
        ...
        false,
        {
          ...
          prefix: AppHelper.addSpaceToCurrency(
  ...
  tradingPlatformPlanCreateValueKeyHtmlSelectOptions(
    tradingPlatformPlans: TradingPlatformPlan[]
  ): ValueKeyHtmlSelectOptions[] {
    const valueKeyHtmlSelectOptions: ValueKeyHtmlSelectOptions[] = [
      new ValueKeyHtmlSelectOptions('', ''),
    ];
    this.configObject.tradingPlatformPlan.referencedDataObject =
      tradingPlatformPlans;

    tradingPlatformPlans.forEach((tradingPlatformPlan) => {
      this.translateService
        .get(tradingPlatformPlan.transactionFeePlan)
        .subscribe((tp) => {
          const valueKeyHtmlSelectOption = new ValueKeyHtmlSelectOptions(
            tradingPlatformPlan.idTradingPlatformPlan,
            null
          );
          valueKeyHtmlSelectOption.value =
            Helper.getValueByPath(
              tradingPlatformPlan,
              'platformPlanNameNLS.map.' + this.gps.getUserLang()
            ) +
            ' / ' +
            tp;
          ...
          valueKeyHtmlSelectOptions.splice(
            Math.abs(indexPos),
            0,
            valueKeyHtmlSelectOption
          );
        });
    });
  ...
  protected override initialize(): void {
    this.tradingPlatformPlanService
      .getAllTradingPlatform()
      .subscribe((tradingPlatformPlans: TradingPlatformPlan[]) => {
        this.configObject.tradingPlatformPlan.valueKeyHtmlOptions =
          this.tradingPlatformPlanCreateValueKeyHtmlSelectOptions(
            tradingPlatformPlans
          );
        ...
        if (this.callParam.thisObject != null) {
          ...
          this.untilFields.forEach(
            (uf) =>
              uf.active &&
              this.configObject[uf.fieldName].formControl.setValue(
                new Date('2099-12-31')
              )
          );
        }
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    const securityaccount: Securityaccount =
      this.copyFormToPrivateBusinessObject(
        new Securityaccount(),
