...
import {
  ColumnConfig,
  TranslateValue,
} from '../../shared/datashowbase/column.config';
    ...
    <div class="datatable">
      ...
      >
              ...
              >
                <ng-container [ngSwitch]="field.templateName">
                  <ng-container *ngSwitchCase="'check'">
                    <span
                      ><i
                        [ngClass]="{ 'fa fa-check': getValueByPath(el, field) }"
                        aria-hidden="true"
                      ></i
                    ></span>
                  </ng-container>
          ...
          <tr>
            <td [attr.colspan]="numberOfVisibleColumns + 2">
              <template-form-check-dialog-result-failed
                *ngIf="failedParsedTemplateStateList.length > 0"
        ...
        <ng-template pTemplate="paginatorleft" let-state>
          {{ selectedEntities.length }} {{ 'SELECTED_FROM' | translate }}
          {{ entityList.length }}
        </ng-template>
      </p-table>
    </div>
...
export class SecurityaccountImportTransactionTableComponent
  ...
  implements OnDestroy, CallBackSetSecurityWithAfter
{
  ...
  constructor(
    ...
    this.addColumnFeqH(
      ...
      ImportSettings.IMPORT_TRANSACTION_POS + 'idFilePart',
      true,
      false
    );
    ...
    this.addColumnFeqH(
      DataType.String,
      ImportSettings.IMPORT_TRANSACTION_POS + 'isin',
      true,
      true,
      { width: 100 }
    );
    this.addColumnFeqH(
      DataType.String,
      ImportSettings.IMPORT_TRANSACTION_POS + 'symbolImp',
      true,
      true
    );
    this.addColumn(
    ...
    this.addColumn(
      DataType.Numeric,
      ImportSettings.IMPORT_TRANSACTION_POS + 'units',
      'QUANTITY',
      true,
      false
    );
    ...
    this.addColumnFeqH(
      DataType.Numeric,
      ImportSettings.IMPORT_TRANSACTION_POS + 'diffCashaccountAmount',
      true,
      false
    );
    this.addColumn(
    ...
    this.addColumn(
      ...
      {
        fieldValueFN:
          SecurityaccountImportTransactionTableComponent.hasTransaction,
        templateName: 'icon',
      }
    );
    this.addColumn(
    ...
    this.readTableDefinition(
      AppSettings.IMPORT_TRANSACTION_POS_TABLE_SETTINGS_STORE
    );
  }

  public static hasTransaction(
    entity: CombineTemplateAndImpTransPos,
    field: ColumnConfig,
    valueField: any
  ): string {
  ...
  private static registerIcons(iconReg: SvgIconRegistryService): void {
    if (!SecurityaccountImportTransactionTableComponent.iconLoadDone) {
      for (const [key, iconName] of Object.entries(
        SecurityaccountImportTransactionTableComponent.createTypeIconMap
      )) {
        iconReg.loadSvg(
          AppSettings.PATH_ASSET_ICONS +
            iconName +
            SecurityaccountImportTransactionTableComponent.SVG,
          iconName
        );
      }
  ...
  readData(): void {
    if (this.selectImportTransactionHead) {
      this.importTransactionPosService
        .getCombineTemplateAndImpTransPosListByTransactionHead(
          this.selectImportTransactionHead.idTransactionHead
        )
        .subscribe(
          (combineTemplateAndImpTransPos: CombineTemplateAndImpTransPos[]) => {
            this.createTranslatedValueStoreAndFilterField(
              combineTemplateAndImpTransPos
            );
            this.entityList = combineTemplateAndImpTransPos;
            this.parentChildRowSelection &&
              this.parentChildRowSelection.rowSelectionChanged(
                this.entityList,
                null
              );
          }
  ...
  onRowExpand(event): void {
    ...
    if (ctaitp.importTransactionPos.importTransactionPosFailedList) {
      ctaitp.importTransactionPos.importTransactionPosFailedList.forEach(
        (importTransactionPosFailed) => {
          const importTransactionTemplate =
            this.importTransactionTemplates.find(
              (itt) =>
                itt.idTransactionImportTemplate ===
                importTransactionPosFailed.idTransactionImportTemplate
            );
          this.failedParsedTemplateStateList.push(
  ...
  public prepareEditMenu(): MenuItem[] {
    ...
    menuItems.push({
      ...
      command: (event) =>
        toClipboard(
          this.selectedEntities[0].importTransactionPos.fileNameOriginal
        ),
    });
    ...
    menuItems.push({
      ...
      disabled:
        ...
        this.selectedEntities.some((ctaitp) =>
          this.hasTransactionOrMaybe(ctaitp.importTransactionPos)
        ),
    ...
    menuItems.push({
      ...
      disabled:
        ...
        !this.selectedEntities.every(
          (ctaitp) => !!ctaitp.importTransactionPos.calcCashaccountAmount
        ) ||
        this.selectedEntities.some((ctaitp) =>
          this.hasTransactionOrMaybe(ctaitp.importTransactionPos)
        ),
    ...
    menuItems.push({
      ...
      disabled:
        ...
        !this.selectedEntities.every((ctaitp) =>
          Transaction.isSecurityTransaction(
            ctaitp.importTransactionPos.transactionType
          )
        ) ||
        this.selectedEntities.some((ctaitp) =>
          this.hasTransactionOrMaybe(ctaitp.importTransactionPos)
    ...
    menuItems.push({
      ...
      disabled:
        ...
        this.selectedEntities.length === 0 ||
        this.selectedEntities.some(
          (ctaitp) => ctaitp.importTransactionPos.idTransactionMaybe !== 0
        ),
    ...
    menuItems.push({
      ...
      disabled:
        ...
        this.selectedEntities.length === 0 ||
        !this.selectedEntities.every(
          (ctaitp) => ctaitp.importTransactionPos.readyForTransaction
        ) ||
        this.selectedEntities.some((ctaitp) =>
          this.hasTransactionOrMaybe(ctaitp.importTransactionPos)
        ),
    ...
    this.importTransactionPosService
      .adjustCurrencyExRateOrQuotation(
        this.selectedEntities.map(
          (ctaitp: CombineTemplateAndImpTransPos) =>
            ctaitp.importTransactionPos.idTransactionPos
        )
      )
      .subscribe((importTransactionPosList) =>
        this.showMessageAndReadData(
          'MSG_ACCEPTED_ADJUST_MULTIPLICATION',
          importTransactionPosList.length
        )
      );
  }
  ...
  setSecurity(security: Security, afterSetSecurity: AfterSetSecurity): void {
    this.importTransactionPosService
      .setSecurity(
        ...
        this.selectedEntities.map(
          (combineTemplateAndImpTransPos) =>
            combineTemplateAndImpTransPos.importTransactionPos.idTransactionPos
        )
      )
    ...
    AppHelper.confirmationDialog(
      ...
      () => {
        this.importTransactionPosService
          .deleteMultiple(
            this.selectedEntities.map(
              (ctaitp) => ctaitp.importTransactionPos.idTransactionPos
            )
          )
          .subscribe((response) => {
            this.messageToastService.showMessageI18n(
  ...
  getFileTypeIcon(
    ...
    return SecurityaccountImportTransactionTableComponent.createTypeIconMap[
      entity.importTransactionPos.fileType
    ];
  }

  getMayBeHasTransactionIcon(
    entity: CombineTemplateAndImpTransPos,
    field: ColumnConfig,
    valueField: any
  ): string {
  ...
  private hasTransactionOrMaybe(itp: ImportTransactionPos): boolean {
    return (
      itp.idTransaction != null ||
      (itp.idTransactionMaybe != null && itp.idTransactionMaybe > 0)
    );
  }
}
