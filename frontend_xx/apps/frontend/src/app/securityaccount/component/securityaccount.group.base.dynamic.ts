 ...
 */
export abstract class SecurityaccountGroupBaseDynamic<
  S
> extends SecurityaccountGroupBase<
...
> {
  ...
  public extendColumns(internalColumnConfigs: ColumnConfig[]) {
    ...
    this.datatableConfigBase.insertColumnFeqH(
      8,
      DataType.NumericRaw,
      'security.leverageFactor',
      true,
      true,
      {
  ...
  public getGroupValueFromGroupRow(
    securityPositionGroupSummary: SecurityPositionDynamicGroupSummary<S>
  ) {
    return securityPositionGroupSummary.groupField;
  }

  public override getGroupRowFieldText(
  ...
  public translateGroupValues(
    securityPositionGroupSummaries: SecurityPositionDynamicGroupSummary<S>[]
  ) {
    securityPositionGroupSummaries.forEach((spgs) => {
      const groupFieldValue: string = this.getGroupFieldAsString(
        spgs.groupField
      );
      if (!this.translatedGroupValues[groupFieldValue]) {
        this.translateService
          .get(groupFieldValue)
          .subscribe(
            (groupValueTranslated) =>
              (this.translatedGroupValues[groupFieldValue] =
                groupValueTranslated)
          );
      }
    });
  ...
  public override getChartDefinition(
    title: string,
    spdgs: SecurityPositionDynamicGrandSummary<
      SecurityPositionDynamicGroupSummary<AssetclassType>
    >
  ): any {
    ...
    spdgs.securityPositionGroupSummaryList.forEach(
      (spgs: SecurityPositionDynamicGroupSummary<AssetclassType>) => {
        labels.push(this.translatedGroupValues[spgs.groupField]);
        valuesGross.push(
          (spgs.groupAccountValueSecurityMC /
            spdgs.grandAccountValueSecurityMC) *
            100
        );
        const enumGroup = (<any>AssetclassType)[spgs.groupField];
        if (
          enumGroup === AssetclassType.CURRENCY_FOREIGN ||
          enumGroup === AssetclassType.CURRENCY_CASH
        ) {
