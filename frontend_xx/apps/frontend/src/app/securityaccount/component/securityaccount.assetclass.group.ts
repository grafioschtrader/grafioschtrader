...
export class SecurityaccountAssetclassGroup extends SecurityaccountGroupBaseDynamic<number> {
  ...
  public translateGroupValues(
    securityPositionGroupSummaries: SecurityPositionDynamicGroupSummary<number>[]
  ) {
    securityPositionGroupSummaries.forEach((spgs) => {
      const groupFieldValue: string = this.getGroupFieldAsString(
        spgs.groupField
      );
      const valueKeyHtmlSelectOptions = SelectOptionsHelper.translateAssetclass(
        this.translateService,
        this.language,
        spgs.securityPositionSummaryList[0].security.assetClass,
        null
      );
      this.translatedGroupValues[groupFieldValue] =
        valueKeyHtmlSelectOptions.value;
    });
  }
