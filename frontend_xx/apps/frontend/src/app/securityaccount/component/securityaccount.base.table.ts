...
} from '../../historyquote/service/time.series.quotes.service';
...
@Directive()
export abstract class SecurityaccountBaseTable
  extends TableConfigBase
  implements IGlobalMenuAttach
{
  ...
  securityaccountGroupBase: SecurityaccountGroupBase<
    | SecurityPositionDynamicGroupSummary<any>
  ...
  getGroupValueByRowIndex(
    columnConfig: ColumnConfig,
    securityPositionSummary: SecurityPositionSummary
  ): string {
    const groupValue = this.securityaccountGroupBase.getGroupValue(
      securityPositionSummary.security
    );
  ...
  transformToFlatArray(
    securityPositionGrandSummary: SecurityPositionGrandSummary
  ): void {
    ...
    for (const securityPositionGroupSummary of securityPositionGrandSummary.securityPositionGroupSummaryList) {
      for (const securityPositionSummary of securityPositionGroupSummary.securityPositionSummaryList) {
        ...
        if (
          this.expandedIdSecuritycurrency.indexOf(
            securityPositionSummary.security.idSecuritycurrency
          ) >= 0
        ) {
      ...
      this.groupMap.set(
        this.securityaccountGroupBase.getGroupValueFromGroupRow(
  ...
  getMenuShowOptionsParam(
    securitycurrencySelected: Securitycurrency
  ): MenuItem[] {
    ...
    if (
      securitycurrencySelected &&
      securitycurrencySelected.idSecuritycurrency >= 0
    ) {
      menuItems = this.addTimeSeriesMenus(
        <Security>securitycurrencySelected,
        menuItems,
        false
      );
    }
    ...
    menuItems.push({
      ...
      icon: this.includeClosedPosition
    ...
    if (
      ...
      menuItems.push(
        ...BusinessHelper.getUrlLinkMenus(securitycurrencySelected)
      );
    }
  ...
  ///////////////////////////////////////////////
  onRowExpand(event) {
    this.expandedIdSecuritycurrency.push(
      event.data.security.idSecuritycurrency
    );
  }

  onRowCollapse(event) {
    const index = this.expandedIdSecuritycurrency.indexOf(
      event.data.security.idSecuritycurrency
    );
  ...
  destroy(): void {
    ...
    this.subscriptionRequestFromChart &&
      this.subscriptionRequestFromChart.unsubscribe();
  }
  ...
  protected abstract extendTransactionParamData(
    transactionCallParam: TransactionCallParam
  ): void;
  ...
  protected createColumns() {
    ...
    this.addColumn(DataType.String, 'security.name', 'NAME', true, false, {
      columnGroupConfigs: [
        new ColumnGroupConfig(
          'security.currency',
          'EXCHANGE_RATE',
          this.securityaccountGroupBase.getGroupRowFieldTextByRowIndex.bind(
            this.securityaccountGroupBase
          ),
          { colspan: 99 }
        ),
    ...
    this.addColumn(
      DataType.String,
      AppSettings.SECURITY.toLowerCase(),
    ...
    this.addColumnFeqH(DataType.String, 'security.currency', true, false, {
      width: 35,
    });
...
*/
    ...
    this.addColumn(
      DataType.Numeric,
      'accountValueSecurity',
      AppSettings.VALUE_SECURITY_ACCOUNT_HEADER,
      true,
      false,
      {
        columnGroupConfigs: [
          new ColumnGroupConfig('groupAccountValueSecurity'),
        ],
      }
    );
    ...
    this.fields
      .filter((cc) => cc.dataType === DataType.Numeric)
      .map((cc) => (cc.templateName = 'greenRed'));
  }
  ...
  protected initTableTextTranslation() {
    this.internalColumnConfigs.forEach(
      (columnConfig) =>
  ...
  protected changeToOpenChart() {
    this.subscriptionRequestFromChart &&
      this.chartDataService.sentToChart(
        this.securityaccountGroupBase.getChartDefinition(
          this.getTitleChart(),
  ...
  private resetMenu() {
    this.contextMenuItems = this.getEditMenu(
      this.selectedSecurityPositionSummary
    );
  ...
  private getEditMenu(
    ...
    if (
      ...
      (AssetclassType[
        securityPositionSummary.security.assetClass.categoryType
      ] < AssetclassType.CURRENCY_PAIR ||
        (AssetclassType[
          securityPositionSummary.security.assetClass.categoryType
        ] === AssetclassType.CURRENCY_PAIR &&
          SpecialInvestmentInstruments[
            securityPositionSummary.security.assetClass
      ...
      menuItems.push({
        ...
        disabled:
          !securityPositionSummary || securityPositionSummary.units === 0,
      });
