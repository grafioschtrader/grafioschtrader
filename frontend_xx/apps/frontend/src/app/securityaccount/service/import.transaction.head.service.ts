...
@Injectable()
export class ImportTransactionHeadService
  extends AuthServiceWithLogout<ImportTransactionHead>
  implements
    DeleteService,
    ServiceEntityUpdate<ImportTransactionHead>,
    UploadServiceFunction
{
  ...
  getPlatformTransactionImport(): Observable<IPlatformTransactionImport[]> {
    return <Observable<IPlatformTransactionImport[]>>(
      this.httpClient
        .get(
  ...
  ): Observable<ImportTransactionHead[]> {
    return <Observable<ImportTransactionHead[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}` +
  ...
  uploadPdfFileSecurityAccountTransactions(
    idSecuritycashaccount: number,
    formData: FormData
  ): Observable<SuccessFailedDirectImportTransaction> {
    return this.httpClient
      .post(
        `${AppSettings.API_ENDPOINT}${AppSettings.IMPORT_TRANSACTION_HEAD_KEY}/${idSecuritycashaccount}/uploadpdftransactions`,
        formData,
  ...
  uploadFiles(
    ...
    return this.httpClient
      .post(
        `${AppSettings.API_ENDPOINT}${AppSettings.IMPORT_TRANSACTION_HEAD_KEY}/${idTransactionHead}/uploadtransaction`,
        formData,
        this.getMultipartHeaders()
      )
      .pipe(
  ...
  update(
    ...
    return this.updateEntity(
      ...
      AppSettings.IMPORT_TRANSACTION_HEAD_KEY
    );
  }
  ...
  public deleteEntity(idTransactionHead: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.IMPORT_TRANSACTION_HEAD_KEY}/` +
          `${idTransactionHead}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
