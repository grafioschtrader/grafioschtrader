...
export class AssetclassEditComponent
  extends SimpleEntityEditBase<Assetclass>
  implements OnInit
{
  @Input() callParam: AssetclassCallParam;
  @Input() proposeChangeEntityWithEntity: ProposeChangeEntityWithEntity;

  private assetclassSpezInstMap: {
    [key in AssetclassType]: SpecialInvestmentInstruments[];
  };
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_BASEDATA_ASSETCLASS,
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldSuggestionInputString(
        'en',
        'SUB_ASSETCLASS',
        64,
        true,
        {
          dataproperty: 'subCategoryNLS.map.en',
          labelSuffix: 'EN',
          suggestionsFN: this.filterSuggestionsEN.bind(this),
        }
      ),
      DynamicFieldHelper.createFieldSuggestionInputString(
        'de',
        ...
        true,
        {
          dataproperty: 'subCategoryNLS.map.de',
          labelSuffix: 'DE',
          suggestionsFN: this.filterSuggestionsDE.bind(this),
        }
      ),
      DynamicFieldHelper.createFieldSelectString(
  ...
  public filterSuggestionsEN(event: any) {
    ...
    this.configObject.en.suggestions =
      this.callParam.subCategorySuggestionsEN.filter((suggestion) =>
  ...
  public filterSuggestionsDE(event: any) {
    const query: string = event.query.toLocaleLowerCase();
    this.configObject.de.suggestions =
      this.callParam.subCategorySuggestionsDE.filter((suggestion) =>
        suggestion.toLocaleLowerCase().startsWith(query)
      );
  }

  valueChangedOnCategoryType(): void {
    this.categoryTypeSubscribe =
      this.configObject.categoryType.formControl.valueChanges.subscribe(
        (categoryType) => {
          if (
            !this.callParam.assetclass &&
            categoryType &&
            categoryType.length > 0
          ) {
            this.configObject.specialInvestmentInstrument.valueKeyHtmlOptions =
  ...
  protected override initialize(): void {
    (<AssetclassService>this.serviceEntityUpdate)
      .getPossibleAssetclassInstrumentMap()
      .subscribe((assetclassSpezInstMap) => {
        ...
        this.configObject.specialInvestmentInstrument.valueKeyHtmlOptions =
          this.valueKeyHtmlOptionsSpecInvest;
        this.configObject.en.suggestions =
          this.callParam.subCategorySuggestionsEN;
        ...
        FormHelper.disableEnableFieldConfigs(this.callParam.hasSecurity, [
          this.configObject.categoryType,
          this.configObject.specialInvestmentInstrument,
        ]);

        if (!this.callParam.assetclass) {
          setTimeout(() =>
            this.configObject.categoryType.elementRef.nativeElement.focus()
          );
        }
      });
  }
