...
export abstract class Helper {
  ...
  public static copyFormSingleFormConfigToBusinessObject(
    ...
    if (createProperty || config.field in targetObject) {
      if (config.referencedDataObject) {
        targetObject[config.field] = this.getReferencedDataObject(
          config,
          config.field
        );
      } else if (
  ...
  public static getReferencedDataObject(
    fieldConfig: FieldConfig,
    keyProperty: string
  ): any {
    if (fieldConfig.dataproperty) {
      keyProperty = fieldConfig.dataproperty.substring(
        fieldConfig.dataproperty.indexOf('.') + 1
      );
    }
  ...
  public static formatDateString(
    ...
    if (
      ...
      fieldConfig.formControl.value.toString().trim().length > 0
    ) {
      targetObject[fieldConfig.field] = moment(
        fieldConfig.formControl.value
      ).format(dateFormat);
    }
  }
