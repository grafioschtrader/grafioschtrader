...
@Component({
  ...
  template: `
    <div class="fcontainer">
      <fieldset
        *ngFor="let fieldSetName of Object.keys(fieldSetGroups)"
        class="out-border fbox"
      >
        <legend class="out-border-legend">
          {{ fieldSetName | translate }}
        </legend>
        <div *ngFor="let field of fieldSetGroups[fieldSetName]" class="row">
          <div
            class="col-lg-6 col-md-6 col-sm-6 col-xs-6 showlabel"
            align="right"
          >
          ...
          <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6 nopadding wrap">
            <ng-container [ngSwitch]="field.templateName">
              <ng-container *ngSwitchCase="'greenRed'">
                <span
                  [style.color]="
                    isValueByPathMinus(content, field) ? 'red' : 'green'
                  "
                >
              ...
              <ng-container *ngSwitchCase="'check'">
                <span
                  ><i
                    [ngClass]="{
                      'fa fa-check': getValueByPath(content, field)
                    }"
                    aria-hidden="true"
                  ></i
                ></span>
...
export class SecuritycurrencyExtendedInfoComponent
  extends SingleRecordConfigBase
  implements OnInit
{
  ...
  getDerivedValues(dataobject: any, field: ColumnConfig, valueField: any): any {
    if (field.field === this.BASE_PRODUCT_NAME) {
      ...
      return this.additionalInstruments[field.field]
        ? this.additionalInstruments[field.field].name
        : '';
    }
  }
  ...
  private addSecurityFields(security: Security): void {
    ...
    this.addFieldProperty(
      DataType.Boolean,
      this.SECURITYCURRENCY + 'isTenantPrivate',
      'PRIVATE_SECURITY',
      {
        fieldsetName: 'BASE_DATA',
        templateName: 'check',
    ...
    if (!security.idLinkSecuritycurrency) {
      this.addFieldPropertyFeqH(
        DataType.String,
        this.SECURITYCURRENCY + 'isin',
        { fieldsetName: 'BASE_DATA' }
      );
      this.addFieldPropertyFeqH(
        DataType.String,
        this.SECURITYCURRENCY + 'tickerSymbol',
        { fieldsetName: 'BASE_DATA' }
      );
    }

    if (
      BusinessHelper.hasSecurityDenomination(
        security.assetClass,
        !security.stockexchange.noMarketValue
      )
    ) {
      this.addFieldPropertyFeqH(
        DataType.NumericInteger,
        this.SECURITYCURRENCY + 'denomination',
        { fieldsetName: 'BASE_DATA' }
      );
    }
    this.addFieldPropertyFeqH(
      DataType.DateString,
      this.SECURITYCURRENCY + 'activeFromDate',
      { fieldsetName: 'BASE_DATA' }
    );
    this.addFieldPropertyFeqH(
      DataType.DateString,
      this.SECURITYCURRENCY + 'activeToDate',
      { fieldsetName: 'BASE_DATA' }
    );
    this.addFieldPropertyFeqH(
      DataType.String,
      this.SECURITYCURRENCY + 'distributionFrequency',
      { translateValues: TranslateValue.NORMAL, fieldsetName: 'BASE_DATA' }
    );
    this.addFieldPropertyFeqH(
      DataType.NumericRaw,
      this.SECURITYCURRENCY + 'leverageFactor',
      {
        fieldsetName: 'BASE_DATA',
  ...
  private addPerformanceFields(): void {
    this.addFieldProperty(
      ...
      this.SECURITYCURRENCY + 'sTimestamp',
      'TIMEDATE',
      { fieldsetName: this.PERFORMANCE }
    );
    this.addFieldProperty(
      DataType.Numeric,
      this.SECURITYCURRENCY + 'sLast',
      'LAST',
      {
        fieldsetName: this.PERFORMANCE,
    ...
    this.addFieldProperty(
      DataType.Numeric,
      this.SECURITYCURRENCY + 'sChangePercentage',
      'DAILY_CHANGE',
      {
        fieldsetName: this.PERFORMANCE,
        headerSuffix: '%',
    ...
    this.addFieldProperty(
      DataType.Numeric,
      this.SECURITYCURRENCY + 'sPrevClose',
      'DAY_BEFORE_CLOSE',
      {
        fieldsetName: this.PERFORMANCE,
        maxFractionDigits: AppSettings.FID_MAX_FRACTION_DIGITS,
  ...
  private addDerivedFields(security: Security): void {
    if (security.idLinkSecuritycurrency) {
      ...
      this.addFieldPropertyFeqH(
        DataType.String,
        this.SECURITYCURRENCY + 'formulaPrices',
        { fieldsetName: this.DERIVED_DATA }
      );

      let match = SecurityCurrencypairDerivedLinks.VAR_NAME_REGEX.exec(
        security.formulaPrices
      );
      while (match != null) {
        if (
          match[1] !==
          SecurityCurrencypairDerivedLinks.ALLOWED_VAR_NAMES.charAt(0)
        ) {
        ...
        match = SecurityCurrencypairDerivedLinks.VAR_NAME_REGEX.exec(
          security.formulaPrices
        );
      }
      ...
      this.securityService
        ...
        .subscribe((scdl: SecurityCurrencypairDerivedLinks) => {
          this.baseInstrument =
            SecurityCurrencypairDerivedLinks.getBaseInstrument(
              scdl,
  ...
  private addCurrencypairFields(): void {
    this.addFieldProperty(
      DataType.String,
      this.SECURITYCURRENCY + 'fromCurrency',
      'CURRENCY_FROM',
      { fieldsetName: 'BASE_DATA' }
    );
    this.addFieldProperty(
      DataType.String,
      this.SECURITYCURRENCY + 'toCurrency',
      'CURRENCY_TO',
      { fieldsetName: 'BASE_DATA' }
    );
    this.addNoteField();
  ...
  private addHistoricalIntraday(): void {
    this.addFieldPropertyFeqH(
      DataType.String,
      this.SECURITYCURRENCY + 'urlHistoryExtend',
      { fieldsetName: 'HISTORY_SETTINGS' }
    );
    this.addFieldPropertyFeqH(DataType.String, 'historicalUrl', {
      fieldsetName: 'HISTORY_SETTINGS',
    });
    this.addFieldPropertyFeqH(
      DataType.String,
      this.SECURITYCURRENCY + 'urlIntraExtend',
      { fieldsetName: 'INTRA_SETTINGS' }
    );
    this.addFieldPropertyFeqH(DataType.String, 'intradayUrl', {
      fieldsetName: 'INTRA_SETTINGS',
    });
    this.addFieldPropertyFeqH(
      DataType.String,
      this.SECURITYCURRENCY + 'idConnectorDividend',
      {
    ...
    this.addFieldPropertyFeqH(
      DataType.String,
      this.SECURITYCURRENCY + 'urlDividendExtend',
      {
        fieldsetName: AppSettings.DIVIDEND_SETTINGS,
      }
    );
