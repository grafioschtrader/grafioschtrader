...
@Directive()
export abstract class WatchlistTable
  extends TableConfigBase
  implements OnDestroy, IGlobalMenuAttach
{
  ...
  SpecialInvestmentInstruments: typeof SpecialInvestmentInstruments =
  ...
  singleMultiSelection:
    ...
    | SecuritycurrencyPosition<Security | Currencypair>[];
  selectedSecuritycurrencyPosition: SecuritycurrencyPosition<
    Security | Currencypair
  >;
  ...
  createSecurityPositionList(data: SecuritycurrencyGroup) {
    ...
    this.securitycurrencyGroup.currencypairPositionList.forEach(
      (sp: SecuritycurrencyPosition<Currencypair>) => {
        const currencypairWatchlist: CurrencypairWatchlist =
          new CurrencypairWatchlist(
    ...
    this.watchlistService
      ...
      .subscribe((watchlist) => {
        this.messageToastService.showMessageI18n(
          ...
          'REMOVED_SECURITY_FROM_WATCHLIST',
          { count: 1 }
        );
        this.dataChangedService.dataHasChanged(
          new ProcessedActionData(ProcessedAction.DELETED, new Watchlist())
        );
      });
  }

  removeSecuritiesAndCurrencypairs(
    selectedSecurityCurrencies: SecuritycurrencyPosition<
      Security | Currencypair
    >[]
  ): void {
      ...
      () => {
        this.watchlistService
          .removeMultipleFromWatchlist(
            this.idWatchlist,
            selectedSecurityCurrencies.map(
              (sc) => sc.securitycurrency.idSecuritycurrency
            )
          )
          .subscribe((count) => {
            this.messageToastService.showMessageI18n(
  ...
  removeAndDeleteSecuritycurrency(
    ...
    AppHelper.confirmationDialog(
      ...
      () => {
        this.watchlistService
          .removeSecuritycurrencyFromWatchlistAndDelete(
            this.idWatchlist,
            securityCurrency
          )
          .subscribe((response) => {
  ...
  handleTransaction(transactionType: TransactionType, security: Security) {
    this.transactionCallParam = Object.assign(new TransactionCallParam(), {
      ...
      security:
        transactionType !== TransactionType.ACCUMULATE ? security : null,
    });
  ...
  handleCloseEditSecuritycurrencyDialog(
  ...
  isMarginProduct(
    ...
    return BusinessHelper.isMarginProduct(
  ...
  dragStart(
    event: DragEvent,
    data: SecuritycurrencyPosition<Security | Currencypair>
  ) {
    this.changeDetectionStrategy.detach();
    event.dataTransfer.setData(
      'text/plain',
      JSON.stringify(
        new WatchlistSecurityExists(
          this.watchlist.idWatchlist,
          data.securitycurrency.idSecuritycurrency
        )
      )
    );
  }
  ...
  protected addBaseColumns(): void {
    ...
    this.addColumnFeqH(DataType.String, 'securitycurrency.isin', true, true, {
      width: 90,
    });
    this.addColumnFeqH(
      DataType.String,
      'securitycurrency.tickerSymbol',
      true,
      true
    );
    this.addColumnFeqH(
  ...
  protected watchlistHasModifiedFromOutside(): void {
    this.subscriptionWatchlistAdded =
      this.dataChangedService.dateChanged$.subscribe((processedActionData) => {
        if (
    ...
    this.routeSubscribe = this.activatedRoute.params.subscribe(
      (params: Params) => {
          ...
          this.gps
            ...
            .subscribe(
              (updateTimeout: number) =>
                (this.intraUpdateTimoutSeconds = updateTimeout)
            );
        ...
        this.watchlist = JSON.parse(
          params[AppSettings.WATCHLIST.toLowerCase()]
        );
      }
    );
  }

  protected getShowMenu(
    securitycurrencyPosition: SecuritycurrencyPosition<Security | Currencypair>
  ): MenuItem[] {
    const menuItems = [
  ...
  protected getShowContextMenuItems(
    ...
    if (securitycurrencyPosition) {
      const isCurrencypair =
        this.selectedSecuritycurrencyPosition.securitycurrency instanceof
        Currencypair;
      const optionalParameters = {
        noMarketValue:
          !isCurrencypair &&
          (<Security>this.selectedSecuritycurrencyPosition.securitycurrency)
            .stockexchange.noMarketValue,
      };
      menuItems = this.timeSeriesQuotesService.getMenuItems(
        this.selectedSecuritycurrencyPosition.securitycurrency
          .idSecuritycurrency,
        isCurrencypair
          ? null
          : (<Security>this.selectedSecuritycurrencyPosition.securitycurrency)
      ...
      menuItems.push(
        ...BusinessHelper.getUrlLinkMenus(
          securitycurrencyPosition.securitycurrency
        )
      );
      menuItems.push({
        label: '_INTRADAY_URL',
        command: (e) =>
          BusinessHelper.toExternalWebpage(
      ...
      menuItems.push({
        ...
        command: (e) =>
          this.mailToCreator(securitycurrencyPosition.securitycurrency),
      });
    }
    translate &&
      TranslateHelper.translateMenuItems(menuItems, this.translateService);
  ...
  protected getEditMenuItems(
    securitycurrencyPosition: SecuritycurrencyPosition<Security | Currencypair>
  ): MenuItem[] {
    const menuItems: MenuItem[] = [];

    menuItems.push({
      label: 'ADD_EXISTING_SECURITY' + AppSettings.DIALOG_MENU_SUFFIX,
      command: (e) => this.addExistingSecurity(e),
      disabled: this.reachedWatchlistLimits(),
    });

    if (
      Array.isArray(this.singleMultiSelection) &&
      this.singleMultiSelection.length > 1
    ) {
      menuItems.push({ separator: true });
      menuItems.push({
        ...
        command: (e) =>
          this.removeSecuritiesAndCurrencypairs(
            this.singleMultiSelection as SecuritycurrencyPosition<
              Security | Currencypair
            >[]
          ),
      });
    ...
    if (securitycurrencyPosition) {
      menuItems.push({
        ...
        command: (e) =>
          this.removeInstrument(securitycurrencyPosition.securitycurrency),
      });
      menuItems.push({
    ...
    if (
      ...
      !(
        securitycurrencyPosition.securitycurrency instanceof
        CurrencypairWatchlist
      )
    ) {
      menuItems.push({
        ...
        disabled:
          ...
          (!AuditHelper.hasHigherPrivileges(this.gps) &&
            !!(<Security>securitycurrencyPosition.securitycurrency)
              .idTenantPrivate &&
            (<Security>securitycurrencyPosition.securitycurrency)
              .idTenantPrivate !== this.gps.getIdTenant()),
      });

      menuItems.push({
        label: 'EDIT_RECORD|INSTRUMENT' + AppSettings.DIALOG_MENU_SUFFIX,
        command: (e) =>
          this.modifySecurityOrSecurityDerived(
            <Security>securitycurrencyPosition.securitycurrency
          ),
      });

      if (
        (<Security>securitycurrencyPosition.securitycurrency).assetClass
        ...
        menuItems.push({
          ...
          command: (e) =>
            securitycurrencyPosition
              ...
              : null,
          disabled:
            (securitycurrencyPosition.units === null ||
              securitycurrencyPosition.units === 0) &&
        ...
        if (!this.isMarginProduct(securitycurrencyPosition)) {
          menuItems.push({
            label:
              AppSettings.DIVIDEND.toUpperCase() +
              AppSettings.DIALOG_MENU_SUFFIX,
            command: (e) =>
              this.handleTransaction(
    ...
    if (securitycurrencyPosition) {
      if (
        ...
        menuItems.push({
          ...
          command: (e) =>
            this.modifyOrCreateAndAddCurrencypair(
              securitycurrencyPosition.securitycurrency
            ),
        });
        menuItems.push({
  ...
  private translateFormulaToUserLanguage(): void {
    if (this.gps.getDecimalSymbol() !== '.') {
      this.securitycurrencyGroup.securityPositionList
        ...
        .map(
          (sp) =>
            (sp.securitycurrency.formulaPrices =
              sp.securitycurrency.formulaPrices
                .split('.')
  ...
  private getSSP(
    ...
    if (Array.isArray(singleMultiSelection)) {
      ...
      return singleMultiSelection as SecuritycurrencyPosition<
        Security | Currencypair
      >;
    }
  }
  ...
  private mailToCreator(securitycurrency: Securitycurrency): void {
    ...
    DynamicDialogHelper.getOpenedMailSendComponent(
      this.translateService,
      this.dialogService,
  ...
  private handleUpdateAllPrice() {
    if (
      Date.now() <
      +this.securitycurrencyGroup.lastTimestamp +
        this.intraUpdateTimoutSeconds * 1000
    ) {
      const minutes = this.millisToMinutesAndSeconds(
        +this.securitycurrencyGroup.lastTimestamp +
          this.intraUpdateTimoutSeconds * 1000 -
          Date.now()
      );
  ...
  private getEditMenu(
    ...
    const menuItems: MenuItem[] = this.getEditMenuItems(
      securitycurrencyPosition
    );
    TranslateHelper.translateMenuItems(menuItems, this.translateService);
    return menuItems;
