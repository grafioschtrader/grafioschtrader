...
export class WatchlistPerformanceComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    this.timeFrames.push(
      new TimeFrame(
        'YEAR_1',
        moment(date).diff(moment(date).subtract(1, 'years'), 'days')
      )
    );
    this.timeFrames.push(
      new TimeFrame(
        'YEAR_2',
        moment(date).diff(moment(date).subtract(2, 'years'), 'days')
      )
    );
    this.timeFrames.push(
      new TimeFrame(
        'YEAR_3',
        moment(date).diff(moment(date).subtract(3, 'years'), 'days')
      )
    );
    ...
    this.addColumn(
      DataType.String,
      'securitycurrency.assetClass.categoryType',
    ...
    this.addColumn(
      DataType.String,
      'securitycurrency.assetClass.specialInvestmentInstrument',
    ...
    this.addColumn(
      DataType.String,
      'securitycurrency.assetClass.subCategoryNLS.map.' +
    ...
    this.addColumnFeqH(
      DataType.NumericRaw,
      'securitycurrency.leverageFactor',
      true,
      true,
      {
    ...
    this.addColumn(
      DataType.Numeric,
      'securitycurrency.sChangePercentage',
      'DAILY_CHANGE',
      true,
      true,
      {
    ...
    this.addColumn(
      DataType.Numeric,
      'timeFrameAnnualChangePercentage',
      'TIME_FRAME_ANNUAL',
      true,
      true,
      {
  ...
  protected getWatchlistWithoutUpdate() {
    const watchListObservable: Observable<SecuritycurrencyGroup> =
      this.watchlistService.getWatchlistWithoutUpdate(this.idWatchlist);
    const tenantLimitObservable: Observable<TenantLimit[]> =
      this.watchlistService.getSecuritiesCurrenciesWatchlistLimits(
        this.idWatchlist
      );
      ...
      childMenuItems.push({
        ...
        icon:
          this.choosenTimeFrame === timeFrame
            ? AppSettings.ICONNAME_CIRCLE_CHECK
            : AppSettings.ICONNAME_CIRCLE_EMTPY,
        command: (event) =>
          this.handleTimeFrame(event, timeFrame, childMenuItems),
      });
    });
  ...
  private handleTimeFrame(
    ...
    this.choosenTimeFrame = timeFrame;
    childMenuItems.forEach(
      (menuItem) => (menuItem.icon = AppSettings.ICONNAME_CIRCLE_EMTPY)
    );
