...
export class WatchlistPriceFeedComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  constructor(
    ...
    super(
      WatchListType.PRICE_FEED,
      AppSettings.WATCHLIST_PRICE_FEED_TABLE_SETTINGS_STORE,
      dialogService,
    ...
    this.addColumn(
      DataType.String,
      'securitycurrency.idConnectorIntra',
      'INTRA_DATA_PROVIDER',
      true,
      true,
    ...
    this.addColumn(
      ...
      'securitycurrency.' + this.f_retryIntraLoad,
      'RETRY_INTRA_LOAD',
      true,
      true
    );
    this.addColumn(
      DataType.String,
      'securitycurrency.idConnectorHistory',
    ...
    this.addColumn(
      DataType.NumericInteger,
      'securitycurrency.' + this.f_retryHistoryLoad,
      'RETRY_HISTORY_LOAD',
      true,
      true
    );
    this.addColumn(
      ...
      'securitycurrency.fullLoadTimestamp',
      'FULL_LOAD_DATE',
      true,
      true
    );
  ...
  getWatchlistWithoutUpdate() {
    const watchListObservable: Observable<SecuritycurrencyGroup> =
      this.watchlistService.getWatchlistWithoutUpdateAndMaxHistoryquote(
        this.idWatchlist
      );
      ...
      const menuItems: MenuItem[] = [
        {
          ...
          disabled: this.disableUpToDateFeedDataMenu(
            this.f_retryHistoryLoad,
            null
          ),
        },
        {
  ...
  private disableUpToDateFeedDataMenu(
    propName: string,
    untilDate: Date
  ): boolean {
    return this.securityPositionList.every(
      (sp) =>
        sp.securitycurrency[propName] === 0 ||
        (!(sp.securitycurrency instanceof CurrencypairWatchlist) &&
          new Date((<Security>sp.securitycurrency).activeToDate) <=
            (untilDate === null ? sp.youngestHistoryDate : untilDate))
    );
  }
