...
export class CorrelationEditingSupport {
  ...
  private valueChangedOnSamplingPeriod(
    configObject: { [name: string]: FieldConfig },
    correlationLimit: CorrelationLimit
  ): void {
    this.changeOnSamplingPeriodSub = configObject[
      this.samplingPeriod
      ...
      if (this.actualSamplingPeriod !== SamplingPeriodType[key]) {
        ...
        switch (this.actualSamplingPeriod) {
          case SamplingPeriodType.DAILY_RETURNS:
            this.periodMoment = 'd';
            this.createOptionsForRolling(
              configObject,
              correlationLimit.dailyConfiguration
            );
            break;
          case SamplingPeriodType.MONTHLY_RETURNS:
            ...
            this.createOptionsForRolling(
              configObject,
              correlationLimit.monthlyConfiguration
            );
            break;
          default:
            this.periodMoment = 'y';
            this.createOptionsForRolling(
              configObject,
              correlationLimit.annualConfiguration
            );
        }
      }
  ...
  public getPeriodAndRollingWithParamPrefix(configObject: {
    [name: string]: FieldConfig;
  }): string[] {
    return [
      'p0@' + configObject[this.samplingPeriod].formControl.value,
      'p1@' + configObject[this.rolling].formControl.value,
    ];
  }

  private valueChangedOnDateFrom(configObject: {
    [name: string]: FieldConfig;
  }): void {
    this.changeOnDateFromSub =
      configObject.dateFrom.formControl.valueChanges.subscribe((dateFrom) =>
        this.setDateToMin(configObject)
      );
  }

  private setDateToMin(configObject: { [name: string]: FieldConfig }): void {
    configObject.dateTo.calendarConfig.minDate = moment(
      configObject.dateFrom.formControl.value
    )
      .add(this.requiredMinPeriods, this.periodMoment)
      .toDate();
  }

  private createOptionsForRolling(
    configObject: { [name: string]: FieldConfig },
    ...
    if (configuration) {
      ...
      for (let i = stepMinMax[1]; i <= stepMinMax[2]; i += stepMinMax[0]) {
        valueKeyHtmlSelectOptions.push(
          new ValueKeyHtmlSelectOptions(i, '' + i)
        );
      }
      configObject[this.rolling].valueKeyHtmlOptions =
        valueKeyHtmlSelectOptions;
      configObject[this.rolling].formControl.setValue(
        valueKeyHtmlSelectOptions[0].value
      );
    } else {
      configObject[this.rolling].formControl.setValue('');
  ...
  destroy(): void {
    this.changeOnSamplingPeriodSub &&
      this.changeOnSamplingPeriodSub.unsubscribe;
