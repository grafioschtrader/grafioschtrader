...
} from './correlation-table.component';
...
@Component({
  template: `
    ...
    >
      ...
      <p *ngIf="nonOverlappingDates" style="color:tomato;">
        {{ 'NON_OVERLAPPING_DATES' | translate }}
      </p>
...
export class CorrelationComponent
  extends SingleRecordMasterViewBase<CorrelationSet, Securitycurrency>
  implements AfterViewInit, OnDestroy, ChildToParent
{
  ...
  private correlationEditingSupport: CorrelationEditingSupport =
    new CorrelationEditingSupport();
  ...
  constructor(
    ...
    this.formConfig = this.formConfig = AppHelper.getDefaultFormConfig(
      this.gps,
      2,
      null,
      true
  ...
  ngAfterViewInit(): void {
    this.correlationSetService
      .getCorrelationSetLimit()
      .subscribe((correlationLimit: CorrelationLimit) => {
        this.correlationLimit = correlationLimit;
        this.readData();
        this.valueChangedMainField();
        this.correlationEditingSupport.setUpValueChange(
          this.configObject,
  ...
  refreshData(correlationSet: CorrelationSet): void {
    if (correlationSet) {
      this.selectedEntity.securitycurrencyList =
        correlationSet.securitycurrencyList;
      this.setChildDataAndCalculateWhenPossible(
        correlationSet,
        this.nonOverlappingDates
      );
    } else {
      this.readData();
    }
  ...
  getPeriodAndRollingWithParamPrefix(): string[] {
    return this.correlationEditingSupport.getPeriodAndRollingWithParamPrefix(
      this.configObject
    );
  }
  ...
  override readData(): void {
    this.correlationSetService
      .getCorrelationSetByTenant()
      .subscribe((correlationSets: CorrelationSet[]) => {
        ...
        this.selectedEntity &&
          (this.selectedEntity = this.entityList.find(
            (entity) =>
              entity[CorrelationComponent.MAIN_FIELD] ===
  ...
  protected override canCreate(): boolean {
    return (
      this.correlationLimit.tenantLimit.actual <
      this.correlationLimit.tenantLimit.limit
    );
  }

  setChildDataAndCalculateWhenPossible(
    selectedEntity: CorrelationSet,
    calculate: boolean
  ): void {
      ...
      if (selectedEntity.securitycurrencyList.length >= 2 && calculate) {
        this.correlationSetService
          ...
          .subscribe((correlationResult: CorrelationResult) => {
            ...
            this.correlationTableComponent.parentSelectionChanged(
              selectedEntity,
              correlationResult
            );
            if (correlationResult.firstAvailableDate) {
              this.timePeriod =
                AppHelper.getDateByFormat(
                  this.gps,
                  correlationResult.firstAvailableDate
                ) +
                ' - ' +
                AppHelper.getDateByFormat(
                  this.gps,
                  correlationResult.lastAvailableDate
                );
