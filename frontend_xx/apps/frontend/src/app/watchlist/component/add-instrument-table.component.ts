...
@Directive()
export abstract class AddInstrumentTable<
  T
> extends SecuritycurrencySearchTableBase {
    ...
    this.addSearchToListService
      ...
      .subscribe((securitycurrencyLists: SecuritycurrencyLists) => {
        this.createTranslatedValueStoreAndFilterField(
          securitycurrencyLists.securityList
        );
        this.securitycurrencyList = securitycurrencyLists.securityList;
        this.transformCurrencypairToCurrencypairWatchlist(
  ...
  onClickAdd(): void {
    ...
    this.selectedSecuritycurrencies.forEach((securitycurrency) => {
      if (securitycurrency instanceof CurrencypairWatchlist) {
        securitycurrencyLists.currencypairList.push(
          Object.assign(new Currencypair(null, null), securitycurrency)
        );
      } else {
        securitycurrencyLists.securityList.push(
          Object.assign(new Security(), securitycurrency)
        );
      }
    });
    this.addSearchToListService
      .addSecuritycurrenciesToList(this.id, securitycurrencyLists)
      .subscribe((data: T) => {
        this.clearList();
  ...
  reachedListLimits(): boolean {
    for (const tenantLimit of this.tenantLimits) {
      if (
        tenantLimit.actual + this.selectedSecuritycurrencies.length >
        tenantLimit.limit
      ) {
        return true;
      }
