  ...
  template: `
    ...
    >
        ...
        <tr>
          ...
          <th
            *ngFor="let field of fields"
            ...
            [ngStyle]="
              field.width ? { 'flex-basis': '0 0 ' + field.width + 'px' } : {}
            "
          >
        ...
        <tr [pSelectableRow]="el">
          ...
          <td
            *ngFor="let field of fields"
            ...
            [ngClass]="
              field.dataType === DataType.NumericShowZero ? 'text-right' : ''
            "
      ...
      >
        <tr>
          <td
            [attr.colspan]="numberOfVisibleColumns + 1"
    ...
    <correlation-add-instrument
      *ngIf="visibleAddInstrumentDialog"
  ...
  implements OnDestroy
{
  ...
  cellClick(field: ColumnConfig): void {
    const columnHeader =
      field.headerKey === this.nameHeader ||
      field.headerKey === this.tickerSymbol
        ? null
        : field.headerKey;

    this.securityCurrencyColumn = this.correlationSet.securitycurrencyList.find(
      (sc) =>
        (sc.hasOwnProperty(this.tickerSymbol)
          ? (<Security>sc)[this.tickerSymbol]
          : sc.name) === columnHeader
    );
  }
  ...
  public prepareEditMenu(): MenuItem[] {
    ...
    menuItems.push({
      ...
      disabled:
        !this.tenantLimits ||
        !this.correlationSet ||
        this.correlationSet.securitycurrencyList.length >=
          this.tenantLimits[0].limit,
    });
    ...
    if (this.selectedEntity) {
      menuItems.push({
        ...
        command: (event) =>
          this.removeInstrumentFromCorrelationSet(
  ...
  public prepareShowMenu(): MenuItem[] {
    ...
    if (
      this.correlationSet &&
      SamplingPeriodType[this.correlationSet.samplingPeriod] !==
        SamplingPeriodType.ANNUAL_RETURNS
    ) {
      if (
        this.selectedEntity &&
        ...
        menuItems.push({
          label:
            'CORRELATION_SET_GRAPH|' +
            this.getMenuName(this.selectedEntity, this.securityCurrencyColumn),
          command: (event) => this.navigateToChartRoute(),
        });
        if (Object.keys(this.traceShow).length > 0) {
          menuItems.push({
            label:
              'CORRELATION_ADD_GRAPH|' +
              this.getMenuName(
                this.selectedEntity,
                this.securityCurrencyColumn
              ),
  ...
  private getMenuName(
    securityCurrencyRow: Securitycurrency,
    securityCurrencyColumn: Securitycurrency
  ): string {
    return (
      this.getTickerOrName(securityCurrencyRow) +
      '<->' +
      this.getTickerOrName(securityCurrencyColumn)
    );
  }
  ...
  removeInstrumentFromCorrelationSet(idSecuritycurrency: number) {
    this.correlationSetService
      .removeInstrumentFromCorrelationSet(
        this.correlationSet.idCorrelationSet,
        idSecuritycurrency
      )
      .subscribe((correlationSet) => {
        this.messageToastService.showMessageI18n(
          InfoLevelType.SUCCESS,
          'REMOVED_INSTRUMENT_FROM_CORRELATIONSET'
        );
  ...
  getCorrelation(
    dataobject: Securitycurrency,
    field: ColumnConfig,
    valueField: any
  ): string | number {
    if (this.correlationResult) {
      const ci: CorrelationInstrument =
        this.correlationResult.correlationInstruments.find(
  ...
  getBackgroundColor(
    ...
    if (field.userValue != null) {
      const value: number = this.getCorrelation(
        dataobject,
        field,
        null
      ) as number;
  ...
  ngOnDestroy(): void {
    this.subscriptionInstrumentAdded &&
      this.subscriptionInstrumentAdded.unsubscribe();
  }
  ...
  private createDynamicTableDefinition(
    correlationSet: CorrelationSet,
    correlationResult: CorrelationResult
  ): void {
    ...
    if (
      ...
      Object.keys(this.traceShow)
        .filter((key) => {
          ...
          return (
            this.correlationSet.securitycurrencyList.find(
              (s) => s.idSecuritycurrency === +ids[0]
            ) &&
            this.correlationSet.securitycurrencyList.find(
              (s) => s.idSecuritycurrency === +ids[1]
            )
          );
        })
        .forEach((key) => {
      ...
      if (idsPairs.length > 0) {
        this.correlationSetService
          ...
          .subscribe((crs: CorrelationRollingResult[]) =>
            this.prepareCharDataAndSentToChart(crs)
          );
      }
    }
  }
  ...
  private navigateToChartRoute(): void {
    if (this.isChartShow()) {
      ...
      this.router.navigate([
        AppSettings.MAINVIEW_KEY + '/',
        {
          outlets: {
            mainbottom: [
              AppSettings.CHART_GENERAL_PURPOSE,
              AppSettings.CORRELATION_CHART,
            ],
          },
        },
      ]);
  ...
  private isChartShow(): boolean {
    return (
      ...
      this.chartDataService.isChartOfIdShown(AppSettings.CORRELATION_CHART)
    );
  }
  ...
  private prepareChartDataWithRequest(): void {
    this.subscriptionRequestFromChart =
      this.chartDataService.requestFromChart$.subscribe((id) => {
  ...
  private prepareCharDataAndSentToChart(
    crsArray: CorrelationRollingResult[]
  ): void {
    let minDate = moment();
    ...
    crsArray.forEach((crs) => {
      const traceName =
        this.getTickerOrName(crs.securitycurrencyList[0]) +
        '<->' +
        this.getTickerOrName(crs.securitycurrencyList[1]);
      const fullName =
        crs.securitycurrencyList[0].name +
        '<->' +
        crs.securitycurrencyList[1].name;
      ...
      this.traceShow[
        crs.securitycurrencyList[0].idSecuritycurrency +
          ',' +
          crs.securitycurrencyList[1].idSecuritycurrency
      ] = trace;
    });
    ...
    this.chartDataService.sentToChart({
      ...
      layout: this.getChartLayout(
        minDate.format(AppSettings.FORMAT_DATE_SHORT_NATIVE),
  ...
  private getChartLayout(minDate: string, maxDate: string): any {
    const layout = {
      title:
        'CORRELATION_ROLLING|' +
        this.childToParent.getPeriodAndRollingWithParamPrefix().join('|'),
      showlegend: true,
