...
export class AssetclassPrepareEdit
  ...
  implements PrepareCallParam
{
  ...
  prepareForEditEntity(entity: Assetclass, entityMapping: EntityMapping): void {
    ...
    const observableAssetclassHasSecurity =
      this.assetclassService.assetclassHasSecurity(entity.idAssetClass);

    combineLatest([
      observableAllAssetclasses,
      observableAssetclassHasSecurity,
    ]).subscribe((data) => {
