import {
  EntityMapping,
  PrepareCallParam,
} from './request.for.you.table.component';
...
export class HistoryquotePrepareEdit
  extends BasePrepareEdit
  implements PrepareCallParam
{
  constructor(
  ...
  prepareForEditEntity(
    ...
    const cpObservable =
      this.currencypairService.getCurrencypairByIdSecuritycurrency(
        entity.idSecuritycurrency
      );
    const securityObservable =
      this.securityService.getSecurityByIdSecuritycurrency(
        entity.idSecuritycurrency
      );
    ...
    combineLatest(cpObservable, securityObservable).subscribe((data: any[]) => {
      ...
      entityMapping.callParam = data[0]
        ? new HistoryquoteSecurityCurrency(
            historyquote,
