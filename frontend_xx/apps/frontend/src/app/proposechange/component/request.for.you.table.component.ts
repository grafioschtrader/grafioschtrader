...
@Component({
  template: `
    ...
    >
      <p-table
        [columns]="fields"
        [value]="proposeChangeEntityWithEntityList"
      ...
      >
        <ng-template pTemplate="caption">
          <h4>
            {{ 'PROPOSE_CHANGE_ENTITY_FOR_USER' | translate }}
            {{ gps.getIdUser() }}
          </h4>
        </ng-template>
    ...
    <assetclass-edit
      *ngIf="entityMappingArr[ASSETCLASS].visibleDialog"
    ...
    <stockexchange-edit
      *ngIf="entityMappingArr[STOCKEXCHANGE].visibleDialog"
    ...
    <import-transaction-edit-platform
      *ngIf="entityMappingArr[IMPORT_TRANSACTION_PLATFORM].visibleDialog"
      [visibleDialog]="
        entityMappingArr[IMPORT_TRANSACTION_PLATFORM].visibleDialog
      "
      [callParam]="entityMappingArr[IMPORT_TRANSACTION_PLATFORM].callParam"
      [platformTransactionImportHtmlOptions]="
        entityMappingArr[IMPORT_TRANSACTION_PLATFORM].option
      "
      [proposeChangeEntityWithEntity]="selectedEntity"
      (closeDialog)="handleCloseDialog(selectedEntity, $event)"
    >
    ...
    <import-transaction-edit-template
      *ngIf="entityMappingArr[IMPORT_TRANSACTION_TEMPLATE].visibleDialog"
      [visibleDialog]="
        entityMappingArr[IMPORT_TRANSACTION_TEMPLATE].visibleDialog
      "
    ...
    <currencypair-edit
      *ngIf="entityMappingArr[CURRENCYPAIR].visibleDialog"
      [visibleEditCurrencypairDialog]="
        entityMappingArr[CURRENCYPAIR].visibleDialog
      "
    ...
    <security-edit
      *ngIf="entityMappingArr[SECURITY].visibleDialog"
    ...
    <security-derived-edit
      *ngIf="entityMappingArr[SECURITY_DERIVED].visibleDialog"
    ...
    <trading-platform-plan-edit
      *ngIf="entityMappingArr[TRADING_PLATFORM_PLAN].visibleDialog"
    ...
    <historyquote-edit
      *ngIf="entityMappingArr[HISTORYQUOTE].visibleDialog"
...
export class RequestForYouTableComponent
  ...
  implements OnInit, IGlobalMenuAttach
{
  ...
  readonly IMPORT_TRANSACTION_PLATFORM =
    AppSettings.IMPORT_TRANSACTION_PLATFORM;
  readonly IMPORT_TRANSACTION_TEMPLATE =
  ...
  constructor(
    ...
    this.entityMappingArr[this.ASSETCLASS] = new EntityMapping(
      new AssetclassPrepareEdit(assetclassService)
    );
    this.entityMappingArr[this.STOCKEXCHANGE] = new EntityMapping(
      new StockexchangePrepareEdit(stockexchangeService, this.gps)
    );
    this.entityMappingArr[this.IMPORT_TRANSACTION_PLATFORM] = new EntityMapping(
      new ImportTransactionPlatformPrepareEdit(importTransactionPlatformService)
    );
    this.entityMappingArr[this.IMPORT_TRANSACTION_TEMPLATE] = new EntityMapping(
      new ImportTransactionPlatformPrepareEdit(importTransactionPlatformService)
    );
    this.entityMappingArr[this.CURRENCYPAIR] = new EntityMapping(
      new GeneralEntityPrepareEdit(Currencypair)
    );
    this.entityMappingArr[AppSettings.SECURITY] = new EntityMapping(
      new SecurityPrepareEdit(this.SECURITY_DERIVED)
    );
    this.entityMappingArr[this.SECURITY_DERIVED] = new EntityMapping(
      new GeneralEntityPrepareEdit(Security)
    );
    this.entityMappingArr[this.TRADING_PLATFORM_PLAN] = new EntityMapping(
      new GeneralEntityPrepareEdit(TradingPlatformPlan)
    );
    this.entityMappingArr[this.HISTORYQUOTE] = new EntityMapping(
      new HistoryquotePrepareEdit(
    ...
    this.addColumnFeqH(
      DataType.String,
      'proposeChangeEntity.noteRequest',
      true,
      false
    );
    this.addColumnFeqH(
      DataType.NumericInteger,
      'proposeChangeEntity.createdBy',
      true,
      false
    );
    this.addColumn(
      DataType.NumericInteger,
      'proposeChangeEntity.idOwnerEntity',
      'OWNER_ENTITY',
      true,
      false
    );
    this.addColumnFeqH(
      DataType.DateString,
      'proposeChangeEntity.creationTime',
      true,
      false
    );
  ...
  readData(): void {
    this.proposeChangeEntityService
      ...
      .subscribe((proposeChangeEntityWithEntityList) => {
        this.proposeChangeEntityWithEntityList =
  ...
  handleEditEntity(
    ...
    entityMapping.prepareCallParam.prepareForEditEntity(
  ...
  handleCloseDialog(
    ...
    if (processedActionData.action !== ProcessedAction.NO_CHANGE) {
      if (processedActionData.action === ProcessedAction.REJECT_DATA_CHANGE) {
        ...
        proposeChangeEntityWithEntity.proposeChangeEntity.noteAcceptReject =
          processedActionData.data;
        this.proposeChangeEntityService
          .update(
            <ProposeChangeEntity>(
              proposeChangeEntityWithEntity.proposeChangeEntity
            )
          )
          .subscribe((returnEntity) => {
            this.messageToastService.showMessageI18n(
              InfoLevelType.SUCCESS,
              'MSG_PROPOSECHANGE_REJECT'
            );
            this.readData();
          });
      } else {
  ...
  onComponentClick(event): void {
    if (this.selectedEntity) {
      ...
      TranslateHelper.translateMenuItems(
        this.contextMenuItems,
  ...
  private getEntityMapping(
    proposeChangeEntityWithEntity: ProposeChangeEntityWithEntity
  ): EntityMapping {
    const entityMapping =
      this.entityMappingArr[
        proposeChangeEntityWithEntity.proposeChangeEntity.entity
      ];
    const entityMappingRedirect =
      this.entityMappingArr[
        entityMapping.prepareCallParam.redirectEntityMapping(
          proposeChangeEntityWithEntity.proposedEntity
        )
      ];
...
export interface PrepareCallParam {
  ...
  prepareForEditEntity(
    entity: ProposeTransientTransfer,
