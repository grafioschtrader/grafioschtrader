...
import {
  ConnectorApiKey,
  SubscriptionType,
  SubscriptionTypeReadableName,
} from '../../entities/connector.api.key';
...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'CONNECTORAPIKEY' | translate }}"
...
export class ConnectorApiKeyEditComponent
  extends SimpleEntityEditBase<ConnectorApiKey>
  implements OnInit
{
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_CONNECTOR_API_KEY,
      AppSettings.CONNECTOR_API_KEY.toUpperCase(),
      translateService,
      gps,
      messageToastService,
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldInputStringHeqF('apiKey', 64, true, {
        minLength: 10,
      }),
  ...
  protected override initialize(): void {
    this.configObject.idProvider.valueKeyHtmlOptions =
      SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromObject(
        'readableName',
        this.strn,
        false,
        this.callParam.thisObject ? [] : this.existingProviders
      );
  ...
  valueChangedOnIdProvider(): void {
    this.idProviderChangedSub =
      this.configObject.idProvider.formControl.valueChanges.subscribe(
        (selection) => {
          const subscriptionTypes = this.strn[selection].subscriptionTypes.map(
            (st) => SubscriptionType[st]
          );
          this.configObject.subscriptionType.valueKeyHtmlOptions =
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: {
    ...
    return this.copyFormToPrivateBusinessObject(
      new ConnectorApiKey(),
      <ConnectorApiKey>this.callParam.thisObject
    );
  }
