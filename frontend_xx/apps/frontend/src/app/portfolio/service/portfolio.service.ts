...
export class PortfolioService
  extends AuthServiceWithLogout<Portfolio>
  implements ServiceEntityUpdate<Portfolio>
{
  constructor(
  ...
  getPortfolioByIdSecuritycashaccount(
    idSecuritycashaccount: number
  ): Observable<Portfolio> {
    return <Observable<Portfolio>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.PORTFOLIO_KEY}/` +
  ...
  getPortfolioByIdPortfolio(idPortfolio: number): Observable<Portfolio> {
    return <Observable<Portfolio>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.PORTFOLIO_KEY}/${idPortfolio}`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  getSecurityDividendsGrandTotalByTenant(
    ...
    return <Observable<SecurityDividendsGrandTotal>>(
      this.httpClient
        .get(
  ...
  getTransactionCostGrandSummaryByTenant(): Observable<TransactionCostGrandSummary> {
    return <Observable<TransactionCostGrandSummary>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.PORTFOLIO_KEY}/transactioncost`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
