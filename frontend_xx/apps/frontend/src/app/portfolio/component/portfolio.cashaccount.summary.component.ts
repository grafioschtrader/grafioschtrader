...
export class PortfolioCashaccountSummaryComponent
  ...
  implements OnInit, OnDestroy, IGlobalMenuAttach
{
  ...
  constructor(
    ...
    this.addColumn(
      ...
      true,
      false,
      { width: 60, maxFractionDigits: AppSettings.FID_MAX_FRACTION_DIGITS }
    );
    ...
    this.columnConfigs.push(
      this.addColumn(DataType.Numeric, 'valueMC', 'TOTAL', true, false, {
        templateName: 'greenRed',
        width: 80,
        columnGroupConfigs: [new ColumnGroupConfig('groupValueMC')],
      })
    );
    ...
    this.routeSubscribe = this.activatedRoute.params.subscribe(
      (params: Params) => {
        this.portfolio = JSON.parse(
          params[AppSettings.PORTFOLIO.toLowerCase()]
        );
  ...
  readData(): void {
    this.cashaccountService
      .getCashaccountPositionGroupSummary(
        this.portfolio.idPortfolio,
        this.untilDate
      )
      .subscribe((data: AccountPositionGroupSummary) => {
        this.accountPositionGroupSummary = data;
        this.expandedAccountPositionSummary =
          this.accountPositionGroupSummary.accountPositionSummaryList.filter(
            (accountPositionSummary) =>
              this.expandedIdCashaccount.indexOf(
                accountPositionSummary.cashaccount.idSecuritycashAccount
              ) >= 0
          );
        this.columnConfigs.forEach(
          (columnConfig) =>
            (columnConfig.headerSuffix =
              this.accountPositionGroupSummary.currency)
        );
  ...
  handleDeleteCashaccount(idSecuritycashAccount: number) {
    AppHelper.confirmationDialog(
      ...
      () => {
        this.cashaccountService
          .deleteCashaccount(idSecuritycashAccount)
          .subscribe((response) => {
            this.messageToastService.showMessageI18n(
              InfoLevelType.SUCCESS,
              'MSG_DELETE_RECORD',
              { i18nRecord: AppSettings.CASHACCOUNT.toUpperCase() }
            );
  ...
  prepareEditMenu(accountPositionSummary: AccountPositionSummary): MenuItem[] {
    ...
    if (accountPositionSummary) {
        ...
        command: (e) =>
          accountPositionSummary
            ? this.handleEditAccount(
                this.portfolio,
                accountPositionSummary.cashaccount,
                { hasTransaction: accountPositionSummary.hasTransaction }
              )
            : null,
      });
      ...
      menuItems.push({
        ...
        command: (e) =>
          accountPositionSummary
            ? this.handleSingleAccountTransaction(
      ...
      menuItems.push({
        ...
        command: (e) =>
          accountPositionSummary
            ? this.handleDoubleAccountTransaction(
                accountPositionSummary.cashaccount
              )
            : null,
      });
  ...
  onRowExpand(event): void {
    this.expandedIdCashaccount.push(
      event.data.cashaccount.idSecuritycashAccount
    );
  }
  ...
  onRowCollapse(event): void {
    const index = this.expandedIdCashaccount.indexOf(
      event.data.cashaccount.idSecuritycashAccount
    );
    this.expandedIdCashaccount.splice(index, 1);
  }

  handleSingleAccountTransaction(
  ...
  transactionDataChanged(processedActionData: ProcessedActionData) {
    ...
    this.parentChildRegisterService.callChildrenToPreserve(
  ...
  private handleNewAccount(portfolio: Portfolio) {
    if (this.tenantLimit) {
      ...
      this.gps
        .getMaxTenantLimitsByMsgKey([TenantLimitTypes.MAX_CASH_ACCOUNT])
        .subscribe((tenantLimits) => {
  ...
  private resetMenu(
    ...
    this.contextMenuItems = this.prepareEditMenu(
      this.selectedAccountPositionSummary
    );
