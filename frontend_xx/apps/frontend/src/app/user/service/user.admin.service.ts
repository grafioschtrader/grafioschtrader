...
export class UserAdminService
  extends AuthServiceWithLogout<User>
  implements DeleteService, ServiceEntityUpdate<User>
{
  constructor(
  ...
  public getAllUsers(): Observable<User[]> {
    return <Observable<User[]>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.USER_ADMIN_KEY}`,
          this.getHeaders()
        )
  ...
  public getUserByIdUser(idUser: number): Observable<User> {
    return <Observable<User>>(
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.USER_ADMIN_KEY}/${idUser}`,
          this.getHeaders()
        )
        .pipe(catchError(this.handleError.bind(this)))
    );
  ...
  public moveCreatedByUserToOtherUser(
    fromIdUser: number,
    toIdUser: number
  ): Observable<number> {
    return this.httpClient
