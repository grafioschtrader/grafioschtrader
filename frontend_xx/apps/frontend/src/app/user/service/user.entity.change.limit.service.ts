...
@Injectable()
export class UserEntityChangeLimitService
  extends AuthServiceWithLogout<UserEntityChangeLimit>
  implements DeleteService, ServiceEntityUpdate<UserEntityChangeLimit>
{
  constructor(
      ...
      this.httpClient
        .get(
          `${AppSettings.API_ENDPOINT}${AppSettings.USER_ENTITY_LIMIT_KEY}/${idUser}/entities`,
          {
            ...
            params: this.getHttpParams(idUserEntityChangeLimit),
          }
        )
  ...
  public deleteEntity(userEntityChangeLimit: number): Observable<any> {
    return this.httpClient
      .delete(
        `${AppSettings.API_ENDPOINT}${AppSettings.USER_ENTITY_LIMIT_KEY}/` +
          `${userEntityChangeLimit}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  ...
  private getHttpParams(idUserEntityChangeLimit: number): HttpParams {
    return idUserEntityChangeLimit
      ? new HttpParams().set(
          'idUserEntityChangeLimit',
          idUserEntityChangeLimit.toString()
        )
      : null;
  }
}
