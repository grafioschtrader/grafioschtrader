...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'USER_ENTITY_CHANGE_LIMIT' | translate }}"
...
export class UserEntityChangeLimitEditComponent
  extends SimpleEntityEditBase<UserEntityChangeLimit>
  implements OnInit
{
  @Input() user: User;
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      ...AuditHelper.getFullNoteRequestInputDefinition(
        this.closeDialog,
        this,
        true
      ),
  ...
  protected initialize(): void {
    (<UserEntityChangeLimitService>this.serviceEntityUpdate)
      .getPublicEntitiesAsHtmlSelectOptions(
        this.user.idUser,
        this.existingUserEntityChangeLimit
          ? this.existingUserEntityChangeLimit.idUserEntityChangeLimit
          : undefined
      )
      .subscribe((valueKeyHtmlSelectOptions: ValueKeyHtmlSelectOptions[]) => {
        this.configObject.entityName.valueKeyHtmlOptions =
