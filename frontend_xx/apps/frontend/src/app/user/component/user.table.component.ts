import {
  CrudMenuOptions,
  TableCrudSupportMenu,
} from '../../shared/datashowbase/table.crud.support.menu';
...
@Component({
  template: `
    ...
    >
      ...
      >
        ...
        >
          <tr [pSelectableRow]="el">
            <td>
              <a
                *ngIf="
                  el.userEntityChangeLimitList.length +
                    el.userChangeLimitProposeList.length >
                  0
                "
                href="#"
                [pRowToggler]="el"
        ...
        <ng-template pTemplate="rowexpansion" let-user let-columns="fields">
          <tr
            *ngIf="
              user.userEntityChangeLimitList.length +
                user.userChangeLimitProposeList.length >
              0
            "
          >
    ...
    <user-change-owner-entities
      *ngIf="visibleChangeEntitiesOwnerDialog"
...
export class UserTableComponent
  extends TableCrudSupportMenu<User>
  implements OnDestroy
{
  ...
  private changeEntitiesOwnerMenuItem: MenuItem = {
    label: 'USER_CHANGE_OWNER_ENTITIES' + AppSettings.DIALOG_MENU_SUFFIX,
  ...
  constructor(
    ...
    this.addColumn(DataType.None, 'userChangePropose', 'U', true, true, {
      fieldValueFN: this.getReleaseLogoutProposeIcon.bind(this),
      templateName: 'icon',
      width: 20,
    });
    ...
    TranslateHelper.translateMenuItems(
      [this.limitChangeMenuItem, this.changeEntitiesOwnerMenuItem],
      translateService
    );
  ...
  private static registerIcons(iconReg: SvgIconRegistryService): void {
    if (!UserTableComponent.iconLoadDone) {
      for (const [key, iconName] of Object.entries(
        UserTableComponent.createTypeIconMap
      )) {
        iconReg.loadSvg(
          AppSettings.PATH_ASSET_ICONS + iconName + UserTableComponent.SVG,
          iconName
        );
      }
  ...
  getLimitProposeIcon(user: User, field: ColumnConfig): string {
    if (user.userChangeLimitProposeList.length > 0) {
      return UserTableComponent.createTypeIconMap[
        UserTaskType.LIMIT_CUD_CHANGE
      ];
    }
  }
    ...
    if (processedActionData.action !== ProcessedAction.NO_CHANGE) {
      if (processedActionData.action === ProcessedAction.REJECT_DATA_CHANGE) {
        ...
        this.proposeUserTaskService
          .rejectUserTask(
            this.selectedEntity.userChangePropose.idProposeRequest,
      ...
      processedActionData.transformedError.errorClass instanceof
        LimitEntityTransactionError
    ) {
      const dynamicDialogHelper =
  ...
  resetMenu(user: User): void {
    ...
    if (this.selectedEntity) {
      ...
      if (
        this.selectedEntity &&
        AuditHelper.isLimitedEditUser(this.selectedEntity)
      ) {
