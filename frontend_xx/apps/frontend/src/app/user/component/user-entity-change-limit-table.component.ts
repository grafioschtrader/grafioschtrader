  ...
  template: `
    ...
    >
      <div class="datatable nestedtable">
        <p-table
          [columns]="fields"
          [value]="user.userEntityChangeLimitList"
...
export class UserEntityChangeLimitTableComponent
  ...
  implements OnInit, IGlobalMenuAttach
{
  ...
  private deleteMenu: MenuItem = {
    label:
      'DELETE_RECORD|' +
      UserEntityChangeLimitTableComponent.USER_ENTITY_CHANGE_LIMIT,
  ...
  private menuItems: MenuItem[] = [
    {
      ...
      command: (event) =>
        this.handleEditEntity(this.selectedUserEntityChangeLimit),
    },
    this.deleteMenu,
  ];
  ...
  constructor(
    ...
    this.entityKeyName = this.gps.getKeyNameByEntityName(
      UserEntityChangeLimit.name
    );
  ...
  getLimitProposeIcon(
    userEntityChangeLimit: UserEntityChangeLimit,
    field: ColumnConfig
  ): string {
    if (
      !userEntityChangeLimit.idUserEntityChangeLimit ||
      this.proposeMap.has(userEntityChangeLimit.idUserEntityChangeLimit)
    ) {
  ...
  setMenuItemsToActivePanel(): void {
    this.activePanelService.activatePanel(this, {
      editMenu: this.getMenuItemsOnUserEntityChangeLimit(
        this.selectedUserEntityChangeLimit
      ),
    });
    this.contextMenuItems = this.getMenuItemsOnUserEntityChangeLimit(
      this.selectedUserEntityChangeLimit
    );
  }
  ...
  handleEditEntity(selectedUserEntityChangeLimit: UserEntityChangeLimit): void {
    ...
    this.proposeChangeEntityWithEntity = this.proposeMap.get(
      selectedUserEntityChangeLimit.idUserEntityChangeLimit
    );
  ...
  handleDelete(selectedUserEntityChangeLimit: UserEntityChangeLimit) {
    AppHelper.confirmationDialog(
      ...
      'MSG_CONFIRM_DELETE_RECORD|' +
        UserEntityChangeLimitTableComponent.USER_ENTITY_CHANGE_LIMIT,
      () => {
        this.userEntityChangeLimitService
          .deleteEntity(selectedUserEntityChangeLimit[this.entityKeyName])
          .subscribe((response) => {
            ...
            this.dateChanged.emit(
              new ProcessedActionData(ProcessedAction.DELETED, null)
            );
          });
      }
    );
  ...
  ngOnInit(): void {
    ...
    this.user.userEntityChangeLimitList.forEach(
      (userEntityChangeLimit: UserEntityChangeLimit) =>
        (userEntityChangeLimit[this.UPPER_CASE_ENTITY_NAME] =
          userEntityChangeLimit.entityName.toUpperCase())
    );
    this.createTranslatedValueStoreAndFilterField(
      this.user.userEntityChangeLimitList
    );
  }
    ...
    if (processedActionData.action !== ProcessedAction.NO_CHANGE) {
      if (processedActionData.action === ProcessedAction.REJECT_DATA_CHANGE) {
        this.proposeUserTaskService
          .rejectUserTask(
            this.proposeChangeEntityWithEntity.proposeChangeEntity
              .idProposeRequest,
            processedActionData.data
          )
          .subscribe((stringResponse) =>
            this.messageToastService.showMessage(
              InfoLevelType.SUCCESS,
  ...
  protected getMenuItemsOnUserEntityChangeLimit(
    userEntityChangeLimit: UserEntityChangeLimit
  ): MenuItem[] {
  ...
  private createProposeLimitForView(): void {
    if (this.user.userChangeLimitProposeList) {
      for (const proposeUserTask of this.user.userChangeLimitProposeList) {
        const entityField = proposeUserTask.proposeChangeFieldList.find(
          (p) => p.field === 'entity'
        );
        let existingLimit = this.user.userEntityChangeLimitList.find(
          (uec) => uec.entityName === entityField.valueDesarialized
        );
        ...
        if (!existingLimit) {
          ...
          uecl.dayLimit = proposeUserTask.proposeChangeFieldList.find(
            (p) => p.field === 'dayLimit'
          ).valueDesarialized;
          uecl.untilDate = proposeUserTask.proposeChangeFieldList.find(
            (p) => p.field === 'untilDate'
          ).valueDesarialized;
          this.user.userEntityChangeLimitList.push(uecl);
        }
        this.proposeMap.set(
          existingLimit.idUserEntityChangeLimit,
          AuditHelper.convertToProposeChangeEntityWithEntity(
            existingLimit,
            proposeUserTask
          )
        );
