...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'USER_CHANGE_OWNER_ENTITIES' | translate }}"
...
export class UserChangeOwnerEntitiesComponent
  ...
  implements OnInit
{
  ...
  onHide(event) {
    this.mainDialogService.visibleDialog(
      false,
      UserSettingsDialogs.NicknameLocale
    );
  }

  submit(values: { [name: string]: any }): void {
    this.userAdminService
      .moveCreatedByUserToOtherUser(this.fromUser.idUser, values.toIdUser)
      .subscribe((numberOfChangedEntities) => {
        this.messageToastService.showMessageI18nEnableHtml(
          InfoLevelType.SUCCESS,
          'CHANGED_OWNER_OF_ENTITIES',
          { numberOfChangedEntities }
        );
        this.closeDialog.emit(
          new ProcessedActionData(ProcessedAction.NO_CHANGE, null)
        );
      });
  }
  ...
  protected initialize(): void {
    ...
    this.configObject.toIdUser.valueKeyHtmlOptions =
      SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromArray(
        'idUser',
        'nickname',
        this.allUsers.filter(
          (u) => u.nickname !== this.fromUser.nickname && u.enabled
        ),
        false
      );
  }
