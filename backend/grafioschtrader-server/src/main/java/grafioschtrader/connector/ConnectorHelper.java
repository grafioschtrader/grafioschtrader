...
/**
 * Utility class providing static helper methods for feed connector operations and management. This class serves as a
 * central location for common connector tasks including connector lookup, access control validation, and HTTP content
 * retrieval functionality.
 */
public class ConnectorHelper {

  /**
   * Locates a feed connector by its unique identifier that supports the specified feed type. This method searches
   * through a list of available connectors to find one that matches both the connector ID and supports the requested
   * feed support type.
   *
   * <p>
   ...
   *
   * <p>
   * This method is commonly used when the system needs to route data requests to a specific connector based on user
   * configuration or automatic selection.
   * </p>
   *
  ...
  /**
   * Determines whether the current authenticated user has permission to access the specified feed connector's
   * configuration and API key information.
   *
   * <p>
   ...
   *
   * <p>
   * This method is essential for protecting sensitive API key information while allowing general users to configure and
   * use public data connectors.
   * </p>
   *
   *
  ...
  /**
   * Retrieves the content of a web page as a string via HTTP GET request. This utility method provides a simple way to
   * fetch content from web URLs for display purposes, particularly in frontend interfaces.
   *
   * <p>
   ...
   *
   * <p>
   * <strong>Security Note:</strong> This method makes direct HTTP requests to external URLs. Ensure that URLs are
   * validated and trusted before calling this method to prevent SSRF (Server-Side Request Forgery) attacks.
   * </p>
   *
   * @param urlString       the URL to retrieve content from (must be a valid HTTP/HTTPS URL)
   * @param addHttpNewlines if true, replaces line breaks with HTML &lt;br /&gt; tags; if false, preserves original line
   *                        breaks
