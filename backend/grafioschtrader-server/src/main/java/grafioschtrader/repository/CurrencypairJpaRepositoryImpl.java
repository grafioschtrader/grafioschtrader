...
public class CurrencypairJpaRepositoryImpl extends SecuritycurrencyService<Currencypair, CashaccountPositionSummary>
  ...
  private void postConstruct() {
    ...
    historyquoteThruConnector = new HistoryquoteThruGTNet<>(connectorThru, gtNetHistoryquoteService,
        globalparametersJpaRepository, globalparametersService, currencypairJpaRepository);
  ...
  @Override
  public List<SecurityCurrencyMaxHistoryquoteData<Currencypair>> findGTNetFallbackBandInstruments(short connectorCap,
      short absoluteCap) {
    return currencypairJpaRepository.findRetryExhaustedGTNetEnabled(connectorCap, absoluteCap);
  }
  ...
  public void createTaskDataChangeOfEmptyHistoryqoute() {
    ...
    for (int i = 0; i < ids.size(); i++) {
      taskDataChangeJpaRepository.save(
          new TaskDataChange(TaskTypeExtended.LOAD_EMPTY_CURRENCYPAIR_HISTORYQUOTES, TaskDataExecPriority.PRIO_VERY_LOW,
  ...
  public void updateAllLastPrices() {
    updateLastPriceByList(
        currencypairJpaRepository.findByRetryIntraLoadLessThan(globalparametersService.getMaxIntraRetry()), true);
