 ...
 *
 * Provides access to security instruments in the GTNet instrument pool, identified by ISIN and currency. For price data
 * operations, use this repository to find instruments and then query GTNetLastpriceJpaRepository for the associated
 * price data.
 *
...
public interface GTNetInstrumentSecurityJpaRepository
  ...
  /**
   * Finds all local security instruments (those that exist in the local database). Uses a JOIN to determine locality
   * dynamically based on matching ISIN and currency.
   *
  ...
  /**
   * Finds all foreign security instruments (those that do NOT exist in the local database). Uses a LEFT JOIN to find
   * instruments with no matching local security.
   *
  ...
  /**
   * Finds security instruments by a list of composite keys (ISIN|currency format). Used for batch lookups when matching
   * instruments from GTNet requests.
   *
  ...
  /**
   * Determines which GTNet security instruments have matching local securities via JOIN. Returns mapping from GTNet
   * instrument ID to local security ID.
   *
   * Uses a native JOIN query to find local securities that match the ISIN and currency of the given GTNet instruments.
   * This allows dynamic determination of instrument locality without storing the reference in the GTNetInstrument
   * entity.
   *
