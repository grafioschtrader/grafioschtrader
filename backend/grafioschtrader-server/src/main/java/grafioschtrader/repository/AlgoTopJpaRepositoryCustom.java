...
public interface AlgoTopJpaRepositoryCustom extends BaseRepositoryCustom<AlgoTop> {
  ...
  /**
   * Normalizes the percentages of all direct children of the given parent node so they sum to exactly 100.00. The
   * parent can be an AlgoTop (children are AlgoAssetclass) or an AlgoAssetclass (children are AlgoSecurity).
   *
  ...
  /**
   * Normalizes the entire AlgoTop tree: first the AlgoAssetclass children, then the AlgoSecurity children of each
   * AlgoAssetclass. All levels are normalized to sum to exactly 100.00.
   *
