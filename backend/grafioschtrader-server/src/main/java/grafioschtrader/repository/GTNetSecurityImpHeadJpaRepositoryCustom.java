...
public interface GTNetSecurityImpHeadJpaRepositoryCustom extends BaseRepositoryCustom<GTNetSecurityImpHead> {
  ...
  /**
   * Queues a background job to import securities from GTNet, preventing duplicates. Checks if a pending job already
   * exists for this header before creating a new one.
   *
   * <p>
   * If idTransactionHead is provided (non-null), the task will also auto-assign linked securities to matching
   ...
   * @param idTransactionHead      optional import transaction head ID to indicate context from transaction import; if
   *                               non-null, matching ImportTransactionPos entries will be auto-updated
