...
public interface GTNetLastpriceCurrencypairJpaRepository
  ...
  @Query(value = "SELECT c.*, p.* FROM gt_net_lastprice_currencypair c JOIN gt_net_lastprice p ON c.id_gt_net_lastprice = p.id_gt_net_lastprice WHERE (c.from_currency, c.to_currency) IN ((?1, ?2))", 
      nativeQuery = true)
