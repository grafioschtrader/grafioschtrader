 ...
 *
 * Handles batch queries for instrument matching. Price pool updates are delegated to {@link GTNetLastpricePoolService}
...
public class GTNetInstrumentSecurityJpaRepositoryImpl implements GTNetInstrumentSecurityJpaRepositoryCustom {
  ...
  public List<GTNetInstrumentSecurity> findByIsinCurrencyTuples(List<String[]> isinCurrencyPairs) {
    ...
    List<String> keys = isinCurrencyPairs.stream().filter(pair -> pair[0] != null && pair[1] != null)
        .map(pair -> pair[0] + "|" + pair[1]).collect(Collectors.toList());
  ...
  public GTNetInstrumentSecurity findOrCreateInstrument(String isin, String currency) {
    Optional<GTNetInstrumentSecurity> existing = gtNetInstrumentSecurityJpaRepository.findByIsinAndCurrency(isin,
        currency);
