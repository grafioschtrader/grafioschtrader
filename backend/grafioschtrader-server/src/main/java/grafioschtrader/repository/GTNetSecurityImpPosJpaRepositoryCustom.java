...
public interface GTNetSecurityImpPosJpaRepositoryCustom {
  ...
  /**
   * Uploads and processes a CSV file containing GTNet security import positions. Skips duplicates (same ISIN + currency
   ...
   */
  UploadHistoryquotesSuccess uploadCSV(Integer idGtNetSecurityImpHead, MultipartFile[] uploadFiles, Integer idTenant)
      throws Exception;

  /**
   * Deletes the linked security from a position and removes it from the system. The position itself remains and can be
   * queried again via GTNet.
   *
   * <p>
   * This operation:
   * <ol>
  ...
  /**
   * Creates GTNet security import positions from an import transaction head. Reads ImportTransactionPos entries where
   * security is null and (isin or symbolImp exists), then creates corresponding GTNetSecurityImpPos entries. Can create
   * a new header or add to an existing one. Skips duplicates (same ISIN + currency) and entries with invalid data.
   *
