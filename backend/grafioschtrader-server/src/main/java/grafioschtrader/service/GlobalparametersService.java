...
public class GlobalparametersService {
  ...
  public QuoteToleranceRange getStandingOrderQuoteToleranceRange() {
    String range = globalparametersJpaRepository.findById(GlobalParamKeyDefault.GLOB_KEY_STANDING_ORDER_QUOTE_TOLERANCE)
        .map(Globalparameters::getPropertyString).orElse(GlobalParamKeyDefault.DEFAULT_STANDING_ORDER_QUOTE_TOLERANCE);
    return parseQuoteToleranceRange(range).orElseGet(
        () -> parseQuoteToleranceRange(GlobalParamKeyDefault.DEFAULT_STANDING_ORDER_QUOTE_TOLERANCE).orElseThrow());
  }
  ...
  /**
   * Gets the connector / asset class compatibility enforcement mode. 0 = off, 1 = server-side enforcement, 2 =
   * server-side + frontend dropdown filtering.
   *
   ...
   *
   * @return the import platform id, or null when no platform is configured for this instance
   * @see GlobalParamKeyDefault#GLOB_KEY_GT_IMPORT_PLATFORM_ID
   */
  public Integer getGtImportPlatformId() {
    return globalparametersJpaRepository.findById(GlobalParamKeyDefault.GLOB_KEY_GT_IMPORT_PLATFORM_ID)
        .map(Globalparameters::getPropertyInt).orElse(null);
  }

  /**
   * Sets or clears the import platform holding the Grafioschtrader authored import templates. Callers are responsible
   * for the administrator check and for verifying that the platform exists.
   *
   * @param idTransactionImportPlatform the platform to use, or null to clear the setting
   */
  @Transactional
  public void setGtImportPlatformId(Integer idTransactionImportPlatform) {
    if (idTransactionImportPlatform == null) {
      globalparametersJpaRepository.findById(GlobalParamKeyDefault.GLOB_KEY_GT_IMPORT_PLATFORM_ID)
          .ifPresent(globalparametersJpaRepository::delete);
    } else {
      Globalparameters globalparameters = globalparametersJpaRepository
          .findById(GlobalParamKeyDefault.GLOB_KEY_GT_IMPORT_PLATFORM_ID)
          .orElseGet(() -> new Globalparameters(GlobalParamKeyDefault.GLOB_KEY_GT_IMPORT_PLATFORM_ID));
      globalparametersJpaRepository.save(globalparameters.setPropertyInt(idTransactionImportPlatform));
    }
  }
   ...
   *
   * <p>
   * Resolved for the acting user, so a per-role or per-user override applies. The user comes from the security context
   * rather than a parameter, which keeps the signature and the {@code MaxInstrumentLimits} REST contract unchanged;
   * without an authenticated user the default row of the key applies.
   * </p>
   *
   ...
   *
   * The timestamp is stored as a Date in the global parameters. If the parameter is not configured or has never been
   * set, returns the epoch (1970-01-01) indicating no sync has occurred.
   *
  ...
  public LocalDateTime getGTNetExchangeSyncTimestamp() {
    return globalparametersJpaRepository.findById(GlobalParamKeyDefault.GLOB_KEY_GTNET_EXCHANGE_SYNC_TIMESTAMP)
        .map(Globalparameters::getPropertyDateTime).orElse(GlobalParamKeyDefault.DEFAULT_GTNET_EXCHANGE_SYNC_TIMESTAMP);
  }

  /**
   * Updates the GTNet exchange sync timestamp to the current time.
   *
   * Called after a successful GTNet exchange synchronization job to record when the last sync occurred. This timestamp
   * is used to determine which GTNetExchange entries have changed since the last sync.
   *
   * @return the saved Globalparameters entity
   * @see GlobalParamKeyDefault#GLOB_KEY_GTNET_EXCHANGE_SYNC_TIMESTAMP
   */
  public Globalparameters updateGTNetExchangeSyncTimestamp() {
    Globalparameters gp = globalparametersJpaRepository
        .findById(GlobalParamKeyDefault.GLOB_KEY_GTNET_EXCHANGE_SYNC_TIMESTAMP)
  ...
  public LocalDate getStartFeedDate() {
    return globalparametersJpaRepository.findById(GlobalParamKeyDefault.GLOB_KEY_START_FEED_DATE)
        .map(Globalparameters::getPropertyDate).orElse(GlobalParamKeyDefault.DEFAULT_START_FEED_DATE);
  }
