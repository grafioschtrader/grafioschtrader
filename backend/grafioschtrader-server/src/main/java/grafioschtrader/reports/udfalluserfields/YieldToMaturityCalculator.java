...
public class YieldToMaturityCalculator {
  ...
  // Constructor
  public YieldToMaturityCalculator(double Price, double FaceValue, double CouponRate, LocalDate SettlementDate,
      LocalDate MaturityDate, double frequency, double daysInYear) {
    this.Price = Price;
  ...
  public double calculateYTM() {
    ...
    UnivariateFunction function = new UnivariateFunction() {
      ...
      public double value(double r) {
        ...
        double dsc = ChronoUnit.DAYS.between(SettlementDate, MaturityDate); // Number of days from settlement to
                                                                            // maturity
        double e = Math.floor(dsc / (daysInYear / frequency)); // Number of full coupon periods from settlement to
                                                               // maturity
        double d1 = ChronoUnit.DAYS.between(SettlementDate,
            MaturityDate.minusDays((long) (e * daysInYear / frequency))); // Number of days from beginning of coupon
        double d2 = ChronoUnit.DAYS.between(SettlementDate.minusDays((long) d1), SettlementDate); // Number of days from
                                                                                                  // of coupon period
...
//          LocalDate.of(2029, 12, 21), 1, 360);
    ...
    System.out.println("0.10 FLUGH 20-27, CH0570576568:" +
        YTMCalculator.yield(LocalDate.of(2024, 6, 30), LocalDate.of(2027, 12, 30), 0.001, 96.35, 100, 1, 4) * 100);
