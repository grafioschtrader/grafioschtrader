...
/**
 * Service class for generating Yahoo Finance earnings calendar links as user-defined fields for securities. This class
 * extends YahooUDFFields to provide automatic creation of direct links to Yahoo Finance's earnings calendar pages for
 * securities across all users in the system.
 *
 * The service creates clickable URLs that direct users to Yahoo Finance's earnings calendar, showing historical and
 * upcoming earnings announcements, earnings dates, estimated vs. actual earnings per share (EPS), revenue figures, and
 * surprise percentages. This provides valuable insights for fundamental analysis and investment timing decisions.
 *
 ...
 *
 * The generated links follow the format: https://finance.yahoo.com/calendar/earnings?day={current_date}&symbol={symbol}
 * where {symbol} is the resolved Yahoo Finance symbol and {current_date} ensures the calendar displays current and
 * upcoming earnings events.
 *
 * This service is particularly valuable for investors tracking earnings seasons, analysts monitoring company
 * performance, and portfolio managers making timing decisions around earnings announcements. The links provide
 * immediate access to comprehensive earnings data without requiring manual navigation or symbol lookup.
 */
@Service
