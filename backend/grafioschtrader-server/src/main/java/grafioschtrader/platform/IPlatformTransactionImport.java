 ...
 *
 * <p>
 * This interface standardizes how different trading platforms handle document import across various file formats. Each
 * trading platform (Swissquote, DEGIRO, Interactive Brokers, etc.) provides its own implementation while maintaining
 * consistent behavior for CSV, PDF, and batch-processed document imports. The interface ensures uniform integration
 * with the GT import system regardless of platform-specific parsing requirements.
 * </p>
 *
 ...
 * <h4>CSV Import</h4>
 * <p>
 * Handles tabular transaction data exported from trading platforms. CSV imports can target specific templates when
 * multiple templates are available for the same platform.
 * </p>
 *
 * <h4>PDF Import</h4>
 * <p>
 * Processes trading platform statements and confirmations in PDF format:
 * </p>
 * <ul>
 ...
 * <p>
 * Handles text files produced by GT-PDF-Transform containing multiple converted and anonymized PDF documents in a
 * single batch file. Ideal for initial imports of historical transaction data.
 * </p>
 *
 * <h3>Template-Based Parsing</h3>
 * <p>
 * All import methods use template-based parsing where each platform can have multiple templates to handle different
 * document formats or format changes over time. Templates define field positions, data types, and validation rules
 * specific to each trading platform's document structure.
 * </p>
 *
 * <h3>Error Handling and Persistence</h3>
 * <p>
 * Import methods are responsible for:
 * </p>
 * <ul>
...
public interface IPlatformTransactionImport {
   ...
   * This method handles batch-processed files where multiple PDF documents have been converted to text format and
   * potentially anonymized for privacy protection.
   *
   * @param importTransactionHead                   Import group container for this import session
   * @param uploadFile                              Text file containing multiple converted PDF documents from
   *                                                GT-PDF-Transform
