 ...
 *
 * <p>
 * This specialized importer extends the generic PDF import functionality to handle specific formatting issues found in
 * older Migros Bank PDF transaction documents. Some legacy PDFs from Migros Bank contain corrupted numeric values where
 * digits are surrounded by extraneous "E" characters, likely due to OCR processing artifacts or PDF generation issues
 * in older systems.
 * </p>
 */
public class MigrosTransactionImportPDF extends GenericTransactionImportPDF {
  ...
  /**
   * Applies Migros Bank-specific text cleaning to handle legacy PDF formatting issues. Automatically detects documents
   * containing corrupted numeric patterns and applies appropriate cleaning transformations while preserving modern
   * documents unchanged.
   *
   * <p>
   * The method identifies legacy documents through content analysis and applies regex-based pattern replacement to
   * restore correct numeric formatting. Documents that don't contain formatting artifacts are passed through unchanged.
   * </p>
   *
  ...
  /**
   * Removes corrupted digit formatting from legacy Migros Bank PDF text. Applies regex pattern matching to replace
   * {@code E[digit]EE} sequences with the actual digit, correcting OCR or PDF generation artifacts in older documents.
   *
   * <p>
   * This method specifically handles the pattern where digits in numeric values are corrupted with surrounding "E"
   * characters, such as converting "34.0E5EE" back to "34.05" for proper numeric parsing.
   * </p>
   *
