 ...
 *
 * <p>
 * This class serves as the base implementation for transaction import operations, providing support for multiple file
 * formats and import scenarios. It implements a delegation pattern where the actual import work is performed by
 * specialized importer classes (GenericTransactionImportPDF and GenericTransactionImportCSV) while this class handles
 * the orchestration and provides a unified interface for all import operations.
 * </p>
 *
 * <h3>Supported Import Formats</h3>
 * <p>
 * The class supports importing transaction data from PDF documents (both single and multiple files) and CSV files. Each
 * format uses specialized processing logic appropriate for the file type, with PDF imports supporting form-based
 * extraction and CSV imports handling structured data parsing.
 * </p>
 */
public class GenericTransactionImport extends BaseTransactionImport implements IPlatformTransactionImport {
   ...
   *
   * <p>
   * This constructor allows for custom platform identification, enabling the creation of specialized instances while
   * maintaining the generic import functionality. The ID and readable name are used for platform identification and
   ...
   *
   * <p>
   * This default constructor creates an instance with "generic" as the platform ID and "Generic" as the readable name,
   * suitable for general-purpose transaction import operations.
   * </p>
   */
