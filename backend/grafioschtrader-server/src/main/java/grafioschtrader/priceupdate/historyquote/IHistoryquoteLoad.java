...
/**
 * Current and historical price data of instruments can arise differently. They can be loaded from an external data
 * source or derived from existing instruments. Therefore this interface exists.
 *
 * @param <S>
 */
public interface IHistoryquoteLoad<S extends Securitycurrency<S>> {
   ...
   *
   * @param jpaRepository    JPA Repository is used for saving the currency pair or security
  ...
  /**
   * Update history quotes from the list of currency pairs and securities. There is no check for retry, that means it is
   * ignored.
   */
