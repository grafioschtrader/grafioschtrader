 ...
 *
 * <p>
 * This class handles intraday price updates for securities whose prices are calculated based on other securities or
 * currency pairs rather than retrieved from external data feeds. It supports two calculation modes:
 * <ul>
 * <li><strong>Formula-based calculation</strong>: Uses mathematical expressions to calculate prices from linked
 * instruments</li>
 ...
 *
 * <p>
 * <strong>Important:</strong> This calculation depends on other securities and currency pairs having up-to-date prices.
 * Therefore, this class should be executed <em>after</em> the intraday update of base securities and currencies to
 * ensure accurate calculated results. The calculation uses the EvalEx expression engine for mathematical formula
 * evaluation.
 * </p>
 *
 * @param <S> the type of security currency extending Securitycurrency (note: implementation works specifically with
 *            Security entities)
 */
public class IntradayThruCalculation<S extends Securitycurrency<S>> extends BaseIntradayThru<Security> {
   ...
   *
   * <p>
   * This method performs a comprehensive calculation process:
   * <ul>
   ...
   * <li><strong>Link Resolution</strong>: Retrieves derived instrument links and current prices for linked
   * securities</li>
   * <li><strong>Price Calculation</strong>:
   * <ul>
   * <li>For calculated securities: evaluates the formula expression using EvalEx with linked instrument prices as
   * variables</li>
   * <li>For non-calculated securities: directly copies the price from the first linked instrument</li>
   * </ul>
   * </li>
   * <li><strong>Timestamp Management</strong>: Sets the timestamp to the newest intraday timestamp from linked
   * instruments</li>
   ...
   * @return the security with updated calculated price and retry counter, or original security if calculation was
   *         skipped
   */
  @Override
   ...
   *
   * <p>
   * Unlike connector-based updates, calculated price updates don't use feed connector delay settings. Instead, this
   * method uses a simple timeout-based approach to prevent excessive calculation frequency.
   * </p>
   *
   ...
   * @return true if the security should be updated (no previous timestamp or timeout period has elapsed), false
   *         otherwise
   */
   ...
   *
   * <p>
   * Calculated securities derive their prices from other instruments rather than external data sources, so download
   * links are not applicable for this calculation-based approach.
   * </p>
   *
   ...
   *
   * <p>
   * This method is not applicable for calculated securities since they compute their prices based on linked instruments
   * rather than retrieving data from external feed connectors.
   * </p>
   *
   * @param securitycurrency the security (parameter ignored for calculated securities)
