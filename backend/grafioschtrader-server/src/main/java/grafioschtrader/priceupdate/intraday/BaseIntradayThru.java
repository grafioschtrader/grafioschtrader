 ...
 *
 * <p>
 * This class serves as the foundation for all intraday price update strategies by providing standardized batch
 * processing capabilities with configurable threading options. It implements the common orchestration logic for
 * processing multiple securities while delegating the actual update implementation to concrete subclasses.
 * </p>
 *
 * <p>
 * Key features provided by this base class:
 * <ul>
 * <li><strong>Batch Processing</strong>: Efficient handling of multiple securities with single-threaded and parallel
 * execution modes</li>
 * <li><strong>Configuration Integration</strong>: Automatic retrieval of global parameters for retry limits and
 * timeouts</li>
 * <li><strong>Threading Control</strong>: Flexible execution using either sequential processing or ForkJoinPool for
 * parallelism</li>
 ...
 *
 * <p>
 * <strong>Transactions:</strong> concrete subclasses are instantiated with {@code new} from the {@code @PostConstruct}
 ...
 * would hold the {@code securitycurrency} row locks of all processed instruments until the last provider call returned.
 * </p>
 *
 * <p>
 * Concrete implementations must provide the entity-specific update logic by implementing the abstract
 * {@code updateLastPriceSecurityCurrency} method, while this base class handles the orchestration of batch operations
 * and threading coordination.
 * </p>
 *
...
public abstract class BaseIntradayThru<S extends Securitycurrency<S>> extends BaseQuoteThru
   ...
   *
   * <p>
   * This convenience method retrieves the maximum retry limit from global parameters and delegates to the full
   * parameter version. It provides a simplified interface for batch updates when custom retry configuration is not
   * required.
   * </p>
   *
   * <p>
   * Each entity is saved in its own transaction; see the class comment on why no transaction spans the batch.
   * </p>
   *
   ...
   *
   * <p>
   * This method provides the core batch processing logic with intelligent execution strategy selection:
   * <ul>
   * <li><strong>Single Entity</strong>: Direct method call without threading overhead for optimal performance</li>
   * <li><strong>Multiple Entities + Single Thread</strong>: Sequential forEach processing for debugging or when
   * parallelism is undesired</li>
   * <li><strong>Multiple Entities + Parallel</strong>: ForkJoinPool-based parallel execution using ThreadHelper for
   * optimal throughput</li>
   * </ul>
   * </p>
   *
   * <p>
   * The parallel execution leverages {@code FORK_JOIN_POOL_CORE_MULTIPLIER} to optimize thread pool sizing based on
   * system capabilities, ensuring efficient resource utilization without overwhelming the system.
   * </p>
   *
   * <p>
   * Global timeout configuration is automatically retrieved and applied consistently across all entities in the batch,
   * ensuring uniform timing behavior regardless of the processing strategy selected.
   * </p>
   *
   * @param securtycurrencies list of securities or currency pairs to update
   * @param maxIntraRetry     maximum number of retry attempts for failed updates, -1 for unlimited retries
   * @param singleThread      true to force single-threaded sequential execution, false for parallel ForkJoinPool
   *                          processing
   * @return list of updated securities with new intraday prices and retry state, maintaining input order for
    ...
    if (securtycurrencies.size() > 1) {
      if (singleThread) {
        ...
        ThreadHelper.executeForkJoinPool(
            () -> securtycurrenciesUpd.addAll(securtycurrencies.parallelStream()
