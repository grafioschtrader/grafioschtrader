 ...
 * ownership rule of {@link grafiosch.common.UserAccessHelper#hasRightsOrPrivilegesForEditingOrDelete} — i.e. row owner
 * OR holder of ROLE_ADMIN / ROLE_ALL_EDIT. ROLE_LIMIT_EDIT users are capped at 2 CUD operations per day on this entity
 * (registered via {@code GLOB_KEY_LIMIT_DAY_RISKFREERATEMAPPING} in {@code GlobalParamKeyDefault}).
 *
 * <p>
 * Cross-field validation (the posted {@code currency} must match the ISO currency of the selected security) is enforced
 * in {@link grafioschtrader.repository.RiskFreeRateMappingJpaRepositoryImpl#saveOnlyAttributes}, not here, so the rule
 * cannot be bypassed by non-REST entry points.
 */
@RestController
...
public class RiskFreeRateMappingResource extends UpdateCreateDeleteAuditResource<RiskFreeRateMapping> {
  ...
  @Operation(summary = "List all risk-free rate mappings", description = "Returns every (currency, security) mapping row. Visible to any authenticated user.", tags = {
      RequestGTMappings.RISKFREERATEMAPPING })
  ...
  @Operation(summary = "List all risk-free instrument candidates", description = """
      ...
      filtering is applied client-side so the row being edited still sees its own current selection.""", tags = {
      RequestGTMappings.RISKFREERATEMAPPING })
