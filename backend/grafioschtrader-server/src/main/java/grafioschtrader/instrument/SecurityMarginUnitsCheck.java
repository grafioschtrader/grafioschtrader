 ...
 * <dd>Calculates accumulated daily finance costs for leveraged positions, accounting for partial position closures and
 * varying position sizes over time.</dd>
 *
 * </dl>
 *
 * <h3>Margin Trading Business Rules:</h3>
 * <ul>
 * <li><strong>Opening Position Constraints:</strong> Opening positions must have sufficient units to cover all current
 * and future closing transactions</li>
 * <li><strong>Closing Position Constraints:</strong> Closing transactions cannot exceed the available units from
 * associated opening positions</li>
 * <li><strong>Deletion Restrictions:</strong> Opening positions with associated closing transactions cannot be deleted
 * to maintain audit integrity</li>
 * <li><strong>Finance Cost Attribution:</strong> Daily holding costs are attributed to specific opening positions and
 * adjusted for partial closures</li>
 * </ul>
 *
 * <h3>Split Factor Integration:</h3>
 * <p>
 * All unit calculations automatically incorporate security splits to ensure that validation remains accurate after
 * corporate actions. Split factors are applied consistently across opening positions, closing transactions, and finance
 * cost calculations to maintain mathematical integrity.
 * </p>
 *
...
public class SecurityMarginUnitsCheck {
   ...
   *
   * <p>
   * Performs comprehensive validation of margin transaction units, ensuring business rules for leveraged trading are
   * enforced. Validation logic adapts based on operation type.
   * </p>
   *
   * <h4>Validation Rules:</h4>
   * <ul>
   * <li><strong>DELETE:</strong> Opening positions cannot be deleted if they have closing transactions</li>
   * <li><strong>ADD/UPDATE:</strong> Opening positions must have sufficient units, closing transactions cannot exceed
   * available units</li>
   * </ul>
   *
   ...
   *
   * <p>
   * Calculates net position requirements by filtering relevant transactions, applying split factors, and ensuring
   * opening positions can cover all closing requirements.
   * </p>
   *
  ...
  private static void addUpdateTransaction(final SecuritysplitJpaRepository securitysplitJpaRepository,
    ...
    Double unitsRequired = Math.abs(transactions.stream().filter(
    ...
    if (targetTransaction.isMarginOpenPosition()) {
      ...
      if (targetTransaction.getTransactionType() != TransactionType.FINANCE_COST) {
        ...
        targetTransaction.setSplitFactorFromBaseTransaction(Securitysplit.calcSplitFatorForFromDateAndToDate(
            ...
            targetTransaction.getTransactionTime().toLocalDate().plusDays(1), securitySplitMap).fromToDateFactor);
      }
    }
  }
   ...
   *
   * <p>
   * Provides finance cost calculation accounting for position lifecycle including partial closures, finance cost
   * payments, and varying position sizes over time.
   * </p>
   *
   ...
   * @return unpaid calendar days and end date, with zero estimated cost when no daily rate is specified
   */
  public static ProposedMarginFinanceCost getEstimatedFinanceCost(TransactionJpaRepository transactionJpaRepository,
    ...
    if (!transactions.isEmpty()) {
      ...
      if (openUnits > 0 && today.isAfter(dayUnitsMap.lastKey())) {
        dayUnitsMap.put(today, openUnits);
      }
      return calcFinanceCost(openTransaction.getUnits(), dailyFinanceCost, dayUnitsMap, lastPaidDay);
    }
   ...
   *
   * <p>
   * Implements core calculation algorithm processing chronological position changes to determine accumulated costs
   * using precise date arithmetic and proportional cost allocation.
   * </p>
   *
   * <h4>Algorithm:</h4>
   * <ol>
   ...
   * <li>Exclude calendar days covered by previous financing payments</li>
   * </ol>
   *
   * @param openUnits        original units in opening position (for unit cost calculation)
   * @param dailyFinanceCost total daily finance cost for original position size, or null if unknown
   * @param dayUnitsMap      chronologically ordered position size changes (date → units)
   * @param lastPaidDay      opening date plus previously recorded financing days
  ...
  private static ProposedMarginFinanceCost calcFinanceCost(double openUnits, Double dailyFinanceCost,
      TreeMap<LocalDate, Double> dayUnitsMap, LocalDate lastPaidDay) {
    double rollingUnit = 0;
    double unitDailyCost = dailyFinanceCost == null ? 0 : dailyFinanceCost / openUnits;
    ProposedMarginFinanceCost pMFC = new ProposedMarginFinanceCost();
    for (Map.Entry<LocalDate, Double> dayUnit : dayUnitsMap.entrySet()) {
      if (pMFC.untilDate != null && rollingUnit > 0) {
        LocalDate start = pMFC.untilDate.isAfter(lastPaidDay) ? pMFC.untilDate : lastPaidDay;
        long days = Math.max(0, ChronoUnit.DAYS.between(start, dayUnit.getKey()));
