...
class StockexchangeResourceTest extends BaseIntegrationTest {
  ...
  @Order(4)
  @ParameterizedTest
  @CsvFileSource(resources = "/testdata/generated/stockexchanges.csv", encoding = "UTF-8", nullValues = {
      "\\N" }, delimiter = '|')
  ...
  void createTest(String mic, String name, String countryCode, boolean noMarketValue, boolean secondaryMarket,
      LocalTime timeOpen, LocalTime timeClose, String timeZone, String website, String e2e)
    ...
    Stockexchange sNew = authenticatedClient(RestTestHelper.getRadomUser()).post()
        .uri(RequestGTMappings.STOCKEXCHANGE_MAP).body(s).exchange().expectStatus().isOk()
  ...
  void deleteByIdTest() {
    Stockexchange[] stockexchanges = authenticatedClient(RestTestHelper.LIMIT1).get()
        .uri(uriBuilder -> uriBuilder.path(RequestGTMappings.STOCKEXCHANGE_MAP)
            .queryParam("includeNameOfCalendarIndex", false).build())
        .exchange().expectStatus().isOk().expectBody(Stockexchange[].class).returnResult().getResponseBody();
    ...
    authenticatedClient(RestTestHelper.LIMIT1).delete()
        .uri(RequestGTMappings.STOCKEXCHANGE_MAP + "/" + s.getIdStockexchange()).exchange().expectStatus()
