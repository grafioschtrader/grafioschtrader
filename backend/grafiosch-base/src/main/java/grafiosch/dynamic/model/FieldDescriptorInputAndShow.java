...
public class FieldDescriptorInputAndShow implements Serializable {
  ...
  /**
   * Creates a field descriptor by analyzing a Java class type. The data type is automatically determined from the
   * provided Java class.
   *
   * @param fieldName the name of the field
  ...
  /**
   * Determines whether the provided type is an enum and sets the appropriate data type and enum information
   * accordingly.
   *
  ...
  /**
   * Maps a Java class type to the corresponding DataType enum value. Handles primitive types, wrapper classes, and
   * common data types used in form generation.
   *
  ...
  private void setForEnum(Class<?> model) {
    // String-backed enums have no numeric value field, but still need a usable form descriptor.
    dataType = DataType.String;
    enumType = model.getSimpleName();
    enumValues = Arrays.stream(model.getEnumConstants()).map(value -> ((Enum<?>) value).name()).toArray(String[]::new);
