...
/**
 * Utility class that provides methods for analyzing Java model classes and generating corresponding dynamic form
 * metadata. This class uses reflection to examine class fields and their annotations to create
 * {@link FieldDescriptorInputAndShow} and {@link ClassDescriptorInputAndShow} instances for dynamic form generation.
 *
 * <p>
 * The helper processes standard Jakarta validation annotations and custom annotations to extract form configuration
 * including field types, constraints, validation rules, and UI hints.
 * </p>
 *
...
public abstract class DynamicModelHelper {
  ...
  /**
   * Creates a complete form definition from a model class, including both field descriptors and class-level
   * constraints. This method analyzes all non-static fields of the provided class and processes supported annotations.
   *
   * <p>
   * Class-level annotations such as {@code @DateRange} are processed and added as constraint validators to the
   * resulting descriptor.
   * </p>
   *
   * @param modelClass the Java class to analyze for form generation
   * @return a complete class descriptor containing field metadata and class constraints, or a descriptor with empty
   *         field list if modelClass is null
   */
  ...
  /**
   * Creates field descriptors for all non-static fields in the specified model class. This is a convenience method that
   * processes all fields without annotation filtering.
   *
  ...
  /**
   * Creates field descriptors for model class fields, with optional annotation filtering. This method uses reflection
   * to examine each non-static field and processes supported annotations to build comprehensive field metadata for
   * dynamic form generation.
   *
   * <p>
   * The method processes various validation and form annotations to set field properties:
   * </p>
   * <ul>
   ...
   *
   * <p>
   * For numeric fields without an explicit maximum constraint, a default maximum value is calculated based on
   * system-wide digit and fraction digit limits.
   * </p>
   *
   * @param modelClass            the Java class to analyze for field definitions
   * @param possibleAnnotationSet a set of annotation types to filter by; if empty, all fields are processed regardless
   *                              of annotations
   * @return a list of field descriptors containing metadata for form generation, empty list if modelClass is null
   */
  public static List<FieldDescriptorInputAndShow> getFormDefinitionOfModelClassMembers(Class<?> modelClass,
  ...
  /**
   * Creates a complete form definition for an entity that uses {@code @DynamicFormField} to mark its input fields.
   * Unlike {@link #getFormDefinitionOfModelClass(Class)} this walks the full class hierarchy (so inherited form fields
   * are included), keeps only the fields that belong to the requested dialog, and orders them by the position encoded
   * in {@code @DynamicFormField.uiOrder()}.
   *
   * <p>
   * Class-level annotations such as {@code @DateRange} are processed and added as constraint validators, identical to
   * {@link #getFormDefinitionOfModelClass(Class)}.
   * </p>
   *
   * @param entityClass the entity to analyse
   * @param dialogId    the dialog id whose fields should be returned (entities with a single form use dialog 1)
   * @return a class descriptor whose field list contains the ordered {@link FieldDescriptorInputAndShowExtendedEntity}
   *         instances for the requested dialog
   */
  ...
  /**
   * Collects, constrains and orders the {@code @DynamicFormField} annotated fields of an entity (including inherited
   * ones) for a single dialog.
   *
   * @param entityClass the entity to analyse
  ...
  /**
   * Returns the position of a field within a dialog from a {@code uiOrder} string, or null when the field does not
   * belong to the dialog. Tokens are {@code dialogId.position}; a bare number is interpreted as a position in dialog 1.
   *
  ...
  /**
   * Extracts the supported Bean Validation and dynamic-form annotations from a field into the given descriptor. Shared
   * by the generic member analysis and the entity form producer.
   *
  ...
  /**
   * Applies the system-wide default maximum to a numeric field that has neither an explicit maximum nor a
   * {@code @Digits} precision constraint.
   *
  ...
  /**
   * Creates a field descriptor from a Java field, handling special types like Set&lt;Enum&gt;. For Set&lt;Enum&gt;
   * fields, creates a descriptor with DataType.EnumSet and populates the available enum values for multi-select input
   * generation.
   *
   * <p>
   * Supports both direct enum type parameters ({@code Set<MyEnum>}) and wildcard bounds
   * ({@code Set<? extends MyInterface>}). For wildcard bounds where the upper bound is an interface, scans the
   * interface's known implementing classes for an enum implementation.
   * </p>
   *
  ...
  /**
   * Resolves a generic type argument to a concrete Class. Handles direct Class references and WildcardType bounds
   * (e.g., {@code ? extends SomeInterface}).
   *
  ...
  /**
   * Creates a field descriptor for Set fields with an interface element type. The descriptor uses DataType.EnumSet with
   * the interface name as enumType, but enumValues is left empty — it must be populated at runtime by the serving
   * endpoint.
   *
  ...
  /**
   * Creates a field descriptor for Set&lt;Enum&gt; fields with multi-select support. Extracts all enum constant names
   * to populate the available options.
   *
   * @param fieldName the name of the field
  ...
  private static FieldDescriptorInputAndShow createEnumSetDescriptor(String fieldName, Class<?> enumClass) {
    ...
    fd.enumValues = Arrays.stream(enumConstants).map(e -> ((Enum<?>) e).name()).toArray(String[]::new);
    return fd;
  }
}
