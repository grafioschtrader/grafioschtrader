...
import grafiosch.validation.WebUrlValidator;
...
public class UDFData {
  ...
  public void checkDataAgainstMetadata(List<UDFMetadata> udfMetadataList) {
    Map<String, UDFMetadata> udfMetadataMap = udfMetadataList.stream()
        .collect(Collectors.toMap(u -> BaseConstants.UDF_FIELD_PREFIX + u.getIdUDFMetadata(), Function.identity()));
  ...
  private void checkDataWithFiledSizeTypes(UDFMetadata udfMetadata, String value) {
    ...
    case UDF_Numeric:
      ...
      checkNumeric(value, max * -1, max);
      break;
    case UDF_NumericInteger:
      checkNumericInteger(value, prefixSuffix[0], prefixSuffix[1]);
      break;
    default:
      // UDF_String:
  ...
  private Object checkDataWithoutFieldSize(UDFMetadata udfMetadata, String value) {
    ...
    default:
      ...
      if (value.length() > BaseConstants.FIELD_SIZE_MAX_G_WEB_URL) {
        throw new IllegalArgumentException("URL starts with '" + value.substring(0, 20) + "' has more than "
            + BaseConstants.FIELD_SIZE_MAX_G_WEB_URL + " characters");
      }
      return value;
    }
  }
