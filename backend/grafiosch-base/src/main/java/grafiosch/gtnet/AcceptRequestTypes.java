 ...
 *
 * Each GTNet entity can be configured with one of these acceptance types to control how the local instance responds to
 * data queries from remote instances:
 *
 * <ul>
 ...
 * <li>{@link #AC_PUSH_OPEN} - Accepts requests AND actively receives pushed updates from remote instances. This mode is
 * only available for certain entity kinds as these instances maintain more active connections with more current
 * data.</li>
 * </ul>
 *
 * Note: AC_PUSH_OPEN is preferred for data exchange because instances in this mode typically have more active
 * connections and therefore more and newer data available.
 */
public enum AcceptRequestTypes {
  ...
  /**
   * Open for requests and also accepts pushed data updates from remote instances. Only supported for certain entity
   * kinds. These instances are preferred for data exchange as they maintain more active connections with more current
   * data.
   */
  AC_PUSH_OPEN((byte) 2);
