 ...
 *       via {@link #getMsgClassByMessageCode(GTNetMessageCode)}</li>
 * </ol>
 *
 * <h3>Core vs Application-Specific Messages</h3>
 * Core protocol messages (0-54) are registered in the static block using {@link GNetCoreMessageCode}.
 * Application-specific messages (60+) should be registered via {@link #registerModel} at application startup,
 * typically in a @PostConstruct method.
 *
 ...
 * @see #registerModel for adding application-specific messages at startup
 */
public abstract class GTNetModelHelper {
  ...
  private static Map<GTNetMessageCode, GTNetMsgRequest> msgFormMap;

  static {
    msgFormMap = new HashMap<>();

    // Core protocol messages (0-54) using GNetCoreMessageCode
    msgFormMap.put(GNetCoreMessageCode.GT_NET_FIRST_HANDSHAKE_SEL_RR_S,
        new GTNetMsgRequest(FirstHandshakeMsg.class, true, (byte) 1));
    msgFormMap.put(GNetCoreMessageCode.GT_NET_UPDATE_SERVERLIST_SEL_RR_C,
        new GTNetMsgRequest(UpdateServerlistRequestMsg.class, true, (byte) 1));
    msgFormMap.put(GNetCoreMessageCode.GT_NET_MAINTENANCE_ALL_C,
        new GTNetMsgRequest(MaintenanceMsg.class, false, (byte) 10));

    // Unified data exchange messages
    msgFormMap.put(GNetCoreMessageCode.GT_NET_DATA_REQUEST_SEL_RR_C,
    ...
    msgFormMap.put(GNetCoreMessageCode.GT_NET_OPERATION_DISCONTINUED_ALL_C,
        new GTNetMsgRequest(DiscontinuedMsg.class, false, (byte) 1));

    // Cancel announcements - no model, no response expected
    msgFormMap.put(GNetCoreMessageCode.GT_NET_MAINTENANCE_CANCEL_ALL_C,
        new GTNetMsgRequest(null, false, (byte) 1));
    msgFormMap.put(GNetCoreMessageCode.GT_NET_OPERATION_DISCONTINUED_CANCEL_ALL_C,
        new GTNetMsgRequest(null, false, (byte) 1));
  }

  /**
   * Registers an application-specific message model at application startup.
   *
   * This method allows application modules to register their own message codes (60+) without
   * modifying this base class. Call this from a @PostConstruct method in your startup component.
   *
   ...
   */
  public static void registerModel(GTNetMessageCode messageCode, Class<?> model,
                                   boolean responseExpected, byte repeatSendAsMany) {
    msgFormMap.put(messageCode, new GTNetMsgRequest(model, responseExpected, repeatSendAsMany));
  }
   ...
   */
  public static GTNetMsgRequest getMsgClassByMessageCode(GTNetMessageCode messageCode) {
