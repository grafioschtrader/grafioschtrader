 ...
 */
public sealed interface HandlerResult<M, E> permits HandlerResult.ImmediateResponse,
    HandlerResult.AwaitingManualResponse, HandlerResult.NoResponseNeeded, HandlerResult.ProcessingError {
   ...
   *
   * Used when a request message was received but no auto-response rule matched. The admin must review the request in
   * the UI and manually send an accept/reject response.
   *
