...
public class TaskDataChangeFormConstraints {
  ...
  @Schema(description = "Configuration mapping of task types to their allowed parameter lists. Each task type can have specific parameters that control its behavior and execution", example = "{\"TOKEN_USER_REGISTRATION_PURGE\": [\"batchSize\", \"maxAge\"], \"MOVE_CREATED_BY_USER_TO_OTHER_USER\": [\"sourceUserId\", \"targetUserId\"]}")
  public Map<ITaskType, List<String>> taskTypeConfig = new HashMap<>();

  @Schema(description = "List of task types that support interruption during execution. Tasks in this list can be safely stopped or cancelled while running", example = "[\"TOKEN_USER_REGISTRATION_PURGE\"]")
  ...
  @Schema(description = """
      ...
      different tasks.""", example = "{\"CREATE_STOCK_EXCHANGE_CALENDAR_BY_RULE_SET\": {\"Stockexchange\": [{\"key\": \"5\", \"value\": \"SIX Swiss Exchange\"}]}}")
  public Map<ITaskType, Map<String, List<ValueKeyHtmlSelectOptions>>> entityIdOptions = new HashMap<>();

  /**
   * Options that apply to every task allowing the entity. A provider registers them once here; the form builder copies
   * them into {@link #entityIdOptions} for each task whose allowed entities contain the entity. Not serialized; it is a
   * staging map composed into {@link #entityIdOptions}.
   */
  @JsonIgnore
  public Map<String, List<ValueKeyHtmlSelectOptions>> sharedEntityIdOptions = new HashMap<>();

  /**
   * Task-specific option overrides keyed by task type then entity name. Used when the same entity type needs a
   * different option list per task. Takes precedence over {@link #sharedEntityIdOptions} for the same entity. Not
   * serialized; it is a staging map composed into {@link #entityIdOptions}.
   */
  @JsonIgnore
