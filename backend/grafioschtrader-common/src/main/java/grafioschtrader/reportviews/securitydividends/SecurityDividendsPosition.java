...
public class SecurityDividendsPosition extends AccountDividendPosition {
   ...
   * transaction ID. This is a deep copy taken at the year boundary, NOT a reference to the report's live tracking map —
   * later-year opens/closes must not alter the state used for this year's valuation.
   */
  @JsonIgnore
   ...
   *
   * <p>
   * This method determines the year-end portfolio valuation by applying historical closing prices, stock split
   * adjustments, and currency conversions. The calculation only applies to positions with units held at year-end,
   * ensuring accurate portfolio valuations for reporting purposes.
   * </p>
   *
   * <p>
   * The method handles:
  ...
  public void attachHistoryquoteAndCalcPositionTotal(Map<Integer, Historyquote> historyquoteIdMap,
    ...
    if (security.isMarginInstrument()) {
      if (marginOpenPositions != null && !marginOpenPositions.isEmpty()) {
        ...
        if (historyquote != null) {
          ...
          for (MarginTracker mt : marginOpenPositions.values()) {
            ...
            totalUnrealizedPL += (yearEndPrice - adjustedOpenPrice) * Math.abs(mt.openUnits) * shortFactor
      ...
      getAndSetExchangeRateEndOfYear(historyquoteIdMap, dateCurrencyMap, security.getCurrency(), officialYearEndRates);
      var historyquote = historyquoteIdMap.get(security.getIdSecuritycurrency());
      if (historyquote != null) {
