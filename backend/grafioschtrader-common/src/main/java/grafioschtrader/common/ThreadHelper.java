...
public class ThreadHelper {
   ...
   *
   * <p>
   * This method creates a ForkJoinPool with a thread count determined by multiplying the number of available processors
   * by the provided coreMultiplier. It submits the task to the pool and waits for its completion. To prevent a resource
   * leak, the pool is shut down in a finally block after execution.
   * </p>
   *
   * @param task           the Runnable task to be executed
   * @param coreMultiplier the multiplier for the available processor count to determine the number of threads in the
   *                       pool
