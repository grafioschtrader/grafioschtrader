 ...
 * prices in a fraction of the official currency: the London Stock Exchange in pence (GBX or GBp instead of GBP) and the
 * Johannesburg Stock Exchange in cents (ZAc instead of ZAR). GT stores every security, price history and transaction
 * exclusively in the major currency, so values quoted in a minor unit must be divided by the factor defined here. See
 * GitHub issue #10.
 */
public abstract class MinorCurrencyUnit {
   ...
   */
  private static final Map<String, String> MINOR_TO_MAJOR = Map.of("GBp", GlobalConstants.MC_GBP, "GBX",
      GlobalConstants.MC_GBP, "ZAc", GlobalConstants.MC_ZAR, "ZAX", GlobalConstants.MC_ZAR);
   ...
   */
  private static final Map<String, String> MIC_MINOR_QUOTED = Map.of(GlobalConstants.STOCK_EX_MIC_UK,
      GlobalConstants.MC_GBP, GlobalConstants.STOCK_EX_MIC_JSE, GlobalConstants.MC_ZAR);
  ...
  public static double getExchangeDivider(String mic, String currency) {
    ...
    return mic != null && currency != null && currency.equals(MIC_MINOR_QUOTED.get(mic)) ? MINOR_TO_MAJOR_DIVIDER : 1.0;
  }
