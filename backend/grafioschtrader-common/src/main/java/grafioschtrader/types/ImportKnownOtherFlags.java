...
/**
 * Attention: The order must not be changed. Flags that start with "CAN" can be set be the user, others are set by the
 * system.
 *
 */
public enum ImportKnownOtherFlags implements StableEnum {
  ...
  /**
   * Bonds may pay interest two or more times in a year. But the document shows only the predetermined interest rate. In
   * this case the system may correct this interest rate for the frequency.
   */
  ...
  /**
   * Some times a dividend are not paid in the currency of the security paper, in such cases the assignment of the
   * security may be wrong it has to be checked against the holdings of security account
   */
  ...
  /**
   * Certain trading platforms show the value 1 in the units field of the CSV document for interest transaction for
   * bonds. GT wants the correct number as specified at purchase. This configuration allows GT to adjust the number and
   * interest using the units and quotation fields. This adjustment is not made until the transaction is created.
   */
  CAN_BOND_ADJUST_UNITS_AND_QUOTATION_WHEN_UNITS_EQUAL_ONE((byte) 5),

  /**
   * In the transaction, the Subject to tax property is not set with this configuration for dividend or interest, i.e.
   * the amount is tax free. Otherwise, Subject to Tax is set.
   */
