...
/**
 * Parses PDF text input against multiple transaction import templates simultaneously to extract financial transaction
 * data.
 *
 * <p>
 * This class attempts to match PDF document text against all available templates for a trading platform, determining
 * which template best fits the document structure. It handles both required and optional fields, and resolves conflicts
 * when multiple templates match the same document.
 * </p>
 *
 * <h3>Multi-Template Matching</h3>
 * <p>
 * The parser tries all templates simultaneously rather than sequentially, which enables:
 * </p>
 * <ul>
 ...
 * <p>
 * When parsing fails, the class provides detailed diagnostic information showing:
 * </p>
 * <ul>
 ...
 * <p>
 * Input text is automatically normalized by removing empty lines, standardizing line endings, and collapsing multiple
 * spaces to ensure consistent parsing.
 * </p>
 */
public class ParseFormInputPDFasTXT {
  ...
  /**
   * Parses the input text with an optional file number for tracking multiple documents. Processes the text through all
   * available templates and returns the best match.
   *
  ...
  /**
   * Returns the template that successfully parsed the given transaction properties. Used to identify which template
   * configuration matched the document.
   *
  ...
  /**
   * Generates failure records for transaction import tracking when parsing fails. Creates diagnostic information
   * showing the progress of each attempted template.
   *
  ...
  /**
   * Returns diagnostic information about parsing failures for troubleshooting. Shows the last successfully matched
   * property for each attempted template.
   *
  ...
  /**
   * Attempts to match required template properties for all templates against a specific text line. Each template
   * maintains its own parsing state as it progresses through the document.
   *
  ...
  /**
   * Processes optional properties for templates that successfully matched all required fields. Only templates that
   * completed required property matching participate in this phase.
   *
  ...
  /**
   * Selects the best matching template when multiple templates successfully parse the document. Uses template validity
   * dates to choose the most appropriate template for the transaction date.
   *
  ...
  private List<ImportProperties> getMatchingPropertyList(
    ...
    if (matchingTemplateMap.size() > 1) {
      for (Map.Entry<FormInputTemplateMatchState, List<ImportProperties>> entry : matchingTemplateMap.entrySet()) {
        ...
        if (importTransactionTemplate.getValidSince().atStartOfDay().isBefore(importProperties.getDatetime())) {
