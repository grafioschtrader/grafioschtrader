 ...
 *
 * <p>
 * This class encapsulates all the configuration needed to identify, match, and extract a specific property from
 * structured PDF text. It supports sophisticated matching strategies including line-based positioning, regex pattern
 * matching, and context-aware value extraction. Each instance represents one property to be parsed from a PDF template,
 * with flexible configuration options for handling various PDF document formats.
 * </p>
 *
 * <h3>Matching Strategies</h3>
 * <p>
 * The class supports multiple matching approaches that can be combined for robust property identification:
 * </p>
 * <ul>
...
public class PropertyWithOptionsConfiguration {
   ...
   *
   * <p>
   * This constructor establishes the fundamental characteristics of a property including its field name, data type,
   * value pattern, and position within the template structure. Additional configuration through setter methods and
   * createRegex() call is required before the property can be used for matching.
   * </p>
   *
   ...
   *
   * <p>
   * This method must be called after all regex components have been configured. It combines the base value pattern with
   * any concatenated strings and additional regex patterns to create the complete matching pattern. The resulting
   * compiled pattern is case-insensitive and ready for use in matching operations.
   * </p>
   */
   ...
   *
   * <p>
   * This method implements the core matching logic, trying different strategies based on the configured line selectors
   * and regex patterns. It first attempts line-based matching using previous, same, or next line context, then falls
   * back to direct regex matching if no line selectors are configured.
   * </p>
   *
   * <h4>Matching Strategy Selection</h4>
   * <p>
   * When line selectors (startPL, startSL, startNL) are configured, the method attempts context-based matching.
   * Otherwise, it performs direct regex matching against the current line. This flexible approach handles both
   * structured form layouts and free-form text scenarios.
   * </p>
   *
   ...
   *
   * <p>
   * This method implements the line-based matching strategy where properties are identified by context from surrounding
   * lines. It supports both single string matching and multiple option matching using bracket notation
   * [option1|option2]. The method ensures proper bounds checking for line access.
   * </p>
   *
   ...
   *
   * <p>
   * This method determines the extraction strategy based on available selectors. When regex selectors are available, it
   * performs full line regex matching. When only line beginning selectors are configured, it uses position-based
   * extraction based on word count in the template.
   * </p>
   *
   ...
   *
   * <p>
   * This method applies the compiled regex pattern to the input string and extracts the value from the first capture
   * group. If ImportProperties container is provided, the extracted value is converted to the appropriate data type and
   * stored. The method gracefully handles conversion failures by returning false.
   * </p>
   *
