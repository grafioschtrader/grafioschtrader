  ...
  public static void configureGlobalParameters(HttpSecurity http) {
    try {
      http.authorizeHttpRequests(authz -> authz
          ...
          .requestMatchers(HttpMethod.POST, RequestMappings.API + "login").permitAll().requestMatchers(HttpMethod.PATCH, RequestMappings.TASK_DATA_CHANGE_MAP + "/**").hasRole(Role.ADMIN)
