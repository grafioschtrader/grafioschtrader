 ...
 *
 * <p>
 * This interface contains common utility methods used across REST controllers and services for validation, error
 * handling, and security operations. All methods are static and designed to be used without instantiation, providing
 * centralized functionality for REST API processing.
 * </p>
 */
public interface RestHelper {
   ...
   *
   * <p>
   * This method protects demo accounts from certain operations that could compromise the demonstration environment or
   * expose security vulnerabilities. Demo accounts are typically used for evaluation purposes and should not be able to
   * perform sensitive operations like password changes, account modifications, or data exports.
   * </p>
   *
  ...
  public static void isDemoAccount(String demoAccountPattern, String userName) {
    ...
    if (pattern.matcher(userName).matches()) {
      throw new GeneralNotTranslatedWithArgumentsException("g.demo.func.not.available", null);
    }
  }
   ...
   *
   * <p>
   * This method transforms internal validation violations into user-friendly error responses with proper
   * internationalization support. It handles locale resolution, message translation, and field name localization to
   * provide comprehensive error information for REST API clients.
   * </p>
   *
   * <p>
   * <strong>Locale Resolution Strategy:</strong>
   * </p>
   * <ol>
   ...
   *
   * <p>
   * <strong>Message Translation Process:</strong>
   * </p>
   * <p>
   * For each validation violation in the exception:
   * </p>
   * <ol>
   ...
   *
   * <p>
   * <strong>Field Name Translation:</strong>
   * </p>
   * <p>
   * Field names can be optionally translated based on the translateFieldName flag in each violation. This allows
   * technical field names to be converted to user-friendly labels while preserving technical identifiers where
   * appropriate.
   * </p>
   *
   * <p>
   ...
   * </p>
   * <p>
   * The resulting ValidationError contains a collection of field-specific error messages that can be easily consumed by
   * REST API clients for form validation feedback and user interface error display.
   * </p>
   *
   * <p>
   ...
   * </p>
   * <p>
   * This method accesses the Spring Security context in a thread-safe manner and handles cases where the security
   * context may not be available or may contain non-User authentication details.
   * </p>
   *
  ...
  /**
   * Converts a camelCase field name to dot.separated.lowercase format for backend NLS lookup. Example: "readableName" →
   * "readable.name", "rateLimitType" → "rate.limit.type"
   *
