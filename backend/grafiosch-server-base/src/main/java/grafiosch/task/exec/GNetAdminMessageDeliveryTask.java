 ...
 */
@Component
public class GNetAdminMessageDeliveryTask implements ITask {
  ...
  public void doWork(TaskDataChange taskDataChange) throws TaskBackgroundException {
    if (!globalparametersJpaRepository.isGTNetOperational()) {
      log.debug("GTNet is disabled or has no own entry configured, skipping admin message delivery");
      return;
    }
  ...
  private int deliverPendingMessages(GTNet myGTNet) {
    ...
    for (Map.Entry<Integer, List<GTNetMessageAttempt>> entry : attemptsByMessage.entrySet()) {
      ...
      for (GTNetMessageAttempt attempt : attempts) {
        ...
        String failure = sendMessageToTarget(myGTNet, targetGTNet, message);
        if (failure == null) {
   ...
   */
  private String sendMessageToTarget(GTNet myGTNet, GTNet targetGTNet, GTNetMessage message) {
    try {
