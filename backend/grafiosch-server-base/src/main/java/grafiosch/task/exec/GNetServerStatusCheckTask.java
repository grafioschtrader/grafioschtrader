...
public class GNetServerStatusCheckTask implements ITask {
  ...
  private static final Logger log = LoggerFactory.getLogger(GNetServerStatusCheckTask.class);
  ...
  public void doWork(TaskDataChange taskDataChange) throws TaskBackgroundException {
    if (!globalparametersJpaRepository.isGTNetOperational()) {
      log.debug("GTNet is disabled or has no own entry configured, skipping server status check");
      return;
    }
