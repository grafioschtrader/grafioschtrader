...
public interface GTNetExchangeLogJpaRepositoryCustom {
   ...
   */
  void logExchange(GTNet gtNet, IExchangeKindType entityKind, boolean asSupplier, int entitiesSent, int entitiesUpdated,
      int entitiesInResponse);

  /**
   * Aggregates log entries from one period type to the next. Groups entries by (idGtNet, entityKind, logAsSupplier) and
   * sums statistics.
   *
  ...
  /**
   * Gets the exchange log tree structure for a specific GTNet and entity kind. Returns hierarchical data with supplier
   * and consumer totals.
   *
  ...
  /**
   * Gets all log entries for all GTNets that support communication for the specified entity kind. Only returns GTNets
   * where the corresponding GTNetEntity has exchange enabled.
   *
   * @param entityKind the entity kind to filter by
