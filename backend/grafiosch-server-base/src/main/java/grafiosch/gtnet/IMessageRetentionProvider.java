 ...
 *
 * Application modules implement this interface to register their message code groups and default retention periods. The
 * scheduled aggregation task discovers all registered providers via Spring dependency injection and deletes old
 ...
 *       return List.of((byte) 60, (byte) 61);
 *     }
 *
...
public interface IMessageRetentionProvider {
  ...
  /**
   * Returns the list of GTNet message code byte values that should be deleted when retention period expires.
   *
  ...
  /**
   * Returns the default retention period in days, used when the config key is not found in the global parameter.
   *
