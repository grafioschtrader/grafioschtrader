...
public abstract class AbstractRequestHandler extends AbstractGTNetMessageHandler {
  ...
  @Override
  public final HandlerResult<GTNetMessage, MessageEnvelope> handle(GTNetMessageContext context) throws Exception {
    ...
    Optional<? extends GTNetMessageCode> priorApproval = checkPriorApproval(context);
    if (priorApproval.isPresent()) {
      GTNetMessageCode responseCode = priorApproval.get();
    ...
    if (autoResponse.isPresent()) {
      ...
      GTNetResponseResolver.ResolvedResponse resolved = autoResponse.get();
