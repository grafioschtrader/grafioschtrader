 ...
 * to JSON serialization and persistent storage interactions.
 * <p>
 * The methods in this class allow inserting, updating, and reading user-defined values based on metadata descriptions.
 * It manages JSON serialization/deserialization and handles type-specific processing such as date-time formatting.
 * <p>
 *
...
public abstract class UDFFieldsHelper {
   ...
   * JSON string.
   * <p>
   * This method deserializes the given JSON string into a map, updates the map by adding or replacing the entry with a
   * key constructed from the UDF metadata, and serializes the map back into a JSON string. If the provided value
   * corresponds to a date-time numeric UDF type, it is formatted appropriately before insertion.
   *
   * @param jsonValuesAsString the original JSON string representing a map of values; may be {@code null} or empty.
   * @param udfMetadata        metadata describing the UDF, including its ID and data type.
   * @param value              the value to insert or update; if {@code null}, the key will be associated with
   *                           {@code null}. If the type is date-time numeric, this should be a {@link LocalDateTime}
   *                           instance.
   * @return the updated JSON string after inserting or updating the provided value.
   * @throws JsonMappingException    if the JSON string cannot be mapped correctly to a map.
  ...
  /**
   * Inserts or updates a value associated with a specific user into persistent storage based on the provided UDF
   * metadata.
   * <p>
   * Retrieves existing user-defined data (UDFData) or creates new data if it does not exist, then updates the data with
   * the given value. If the provided value corresponds to a date-time numeric UDF type, it is formatted appropriately
   * before insertion.
   *
   * @param udfMetadata          metadata describing the UDF, including its ID and data type.
   * @param uDFDataJpaRepository repository for managing UDF data persistence.
   * @param classz               the class reference identifying the context of the UDF data.
   * @param id                   UDF_ID_USER and the name of the class and the ID of the entity of this class are the
   *                             key for UDF data.
   * @param value                the value to store; if {@code null}, no action is performed.
   */
  ...
  /**
   * Reads a stored user-defined value from persistent storage based on provided UDF metadata and user identifier.
   * <p>
   * Retrieves the stored value associated with the provided metadata. If the stored value corresponds to a date-time
   * numeric UDF type, it will be parsed into a {@link LocalDateTime} object.
   *
   * @param udfMetadata          metadata describing the UDF, including its ID and data type.
   ...
   * @param id                   UDF_ID_USER and the name of the class and the ID of the entity of this class are the
   *                             key for UDF data.
   * @return the stored value; returns {@code null} if no data exists for the specified criteria.
   */
