 ...
 * being contacted from a watchlist. This interface is the generic contract the mailing service in the {@code grafiosch}
 * library dispatches to; the concrete, entity-specific rules live in the application layer ({@code grafioschtrader}).
 * The library stays free of application references and gains new contexts simply by an additional Spring bean
 * implementing this interface — no library change required.
 *
 * <p>
...
public interface IMailUserToUserContextVerifier {
  ...
  /**
   * Verifies that the sender may legitimately start a direct message to the given recipient from the referenced context
   * entity. Implementations throw a {@link SecurityException} when the context does not justify the message (e.g.
   * recipient is not the entity's creator, or the sender has no legitimate relationship to the entity).
   *
