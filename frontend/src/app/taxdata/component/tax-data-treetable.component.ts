...
export class TaxDataTreetableComponent extends TreeTableConfigBase implements OnInit, OnDestroy, IGlobalMenuAttach {
  ...
  constructor(
    ...
    this.countryNames = new Intl.DisplayNames([gps.getUserLang()], {
      type: 'region'
    });
    ...
    this.addColumnFeqH(DataType.NumericInteger, 'recordCount', true, false, {
      width: 150
    });
  ...
  private prepareEditMenu(): MenuItem[] {
    ...
    if (!this.selectedNode) {
      menuItems.push({
        label: 'CREATE_TAX_COUNTRY' + BaseSettings.DIALOG_MENU_SUFFIX,
        command: () => this.createCountry()
      });
    } else {
      ...
      ref.onClose.subscribe((taxYear: number) => {
        if (taxYear) {
          this.taxDataService
            .createYear({
              idTaxYear: null,
              idTaxCountry: country.idTaxCountry,
              taxYear
            })
