...
export class AlgoRuleStrategyCreateDynamicComponent
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  protected assetclassChangeSubList: Subscription[] = new Array(
    AlgoRuleStrategyCreateDynamicComponent.maxAssetclassRows
  );
  ...
  removeAssetclassPercentageRow(): void {
    ...
    this.config = [
      ...this.config.slice(0, this.config.length - AlgoRuleStrategyCreateDynamicComponent.NO_OF_BUTTONS - 2),
      ...this.config.slice(
        this.config.length - AlgoRuleStrategyCreateDynamicComponent.NO_OF_BUTTONS,
        this.config.length
      )
    ];
  }
  ...
  addAssetclassPercentageRow(addFirstElement: boolean): void {
    ...
    const fieldConfig: FieldConfig[] = [
      DynamicFieldHelper.createFieldSelectString(
        AlgoRuleStrategyCreateDynamicComponent.ASSETCLASS_FIELD + this.assetclassCounter,
    ...
    this.config = [
      ...this.config.slice(0, this.config.length - AlgoRuleStrategyCreateDynamicComponent.NO_OF_BUTTONS),
      ...fieldConfig,
      ...this.config.slice(
        this.config.length - AlgoRuleStrategyCreateDynamicComponent.NO_OF_BUTTONS,
        this.config.length
