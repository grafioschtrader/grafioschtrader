...
> {
  ...
  override getGroupRowFieldText(columnConfig: ColumnConfig, arrIndex: number, data: any, mapKey: any): string {
    ...
    const exchangeRate = this.datatableConfigBase.getFieldValueForFieldColumnTotal(
      columnConfig,
      arrIndex,
      data,
      mapKey,
  ...
  public override getChartDefinition(title: string, securityPositionGrandSummary: SecurityPositionGrandSummary): any {
    ...
    securityPositionGrandSummary.securityPositionGroupSummaryList.forEach(
      (spgs: SecurityPositionCurrenyGroupSummary) => {
        values.push(
          (spgs.groupAccountValueSecurityMC / securityPositionGrandSummary.grandAccountValueSecurityMC) * 100
        );
