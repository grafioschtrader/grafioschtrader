...
@Component({
  ...
  template: `
    <p-dialog header="{{i18nRecord | translate}}" [visible]="visibleDialog"
      ...
      <yaml-editor [height]="'380px'" [(value)]="ruleYamlValue" [schema]="ruleSchema"
      ...
      @if (idTradingCalendarRuleSet) {
        <p-fieldset [legend]="'PREVIEW_CLOSURES' | translate" [toggleable]="true" [collapsed]="true"
                    styleClass="mt-3" (onBeforeToggle)="loadPreview()">
          ...
          @if (previewError) {
            ...
            <trading-calendar-rule-set-preview [closures]="previewClosures">
            </trading-calendar-rule-set-preview>
          }
        </p-fieldset>
      }
    </p-dialog>`,
  standalone: true,
  imports: [DialogModule, DynamicFormModule, TranslateModule, FieldsetModule, ButtonModule, YamlEditorComponent,
    TradingCalendarRuleSetPreviewComponent]
})
export class TradingCalendarRuleSetEditComponent extends SimpleEntityEditBase<TradingCalendarRuleSet>
  implements OnInit {
  ...
  constructor(private tradingCalendarRuleSetService: TradingCalendarRuleSetService,
    ...
    super(HelpIds.HELP_BASEDATA_TRADING_CALENDAR_RULE,
      AppHelper.toUpperCaseWithUnderscore(AppSettings.TRADING_CALENDAR_RULE_SET), translateService,
      gps, messageToastService, tradingCalendarRuleSetService);
  }
  ...
  ngOnInit(): void {
    ...
    this.config = [
      DynamicFieldHelper.createFieldInputStringHeqF('name', 64, true, {minLength: 2}),
      DynamicFieldHelper.createFieldInputStringHeqF('mic', 4, false, {minLength: 4}),
  ...
  protected override initialize(): void {
    ...
    this.httpClient.get('assets/schemas/trading-calendar-rule-schema.json').subscribe(schema =>
    ...
    this.tradingCalendarRuleSetService.getRuleSetOptions().subscribe((options: ValueKeyHtmlSelectOptions[]) => {
      ...
      this.configObject.idExtendsRuleSet.valueKeyHtmlOptions = options.filter(option =>
        !this.idTradingCalendarRuleSet || +option.key !== this.idTradingCalendarRuleSet);
      this.form.setDefaultValuesAndEnableSubmit();
      AuditHelper.transferToFormAndChangeButtonForProposaleEdit(this.translateService, this.gps,
        this.callParam, this.form, this.configObject, this.proposeChangeEntityWithEntity);
    });
  }
  ...
  loadPreview(): void {
    ...
    this.tradingCalendarRuleSetService.previewClosures(this.idTradingCalendarRuleSet, this.previewYear,
      this.ruleYamlValue).subscribe({
   ...
   */
  private static buildRuleCompletions(): { [fieldName: string]: YamlFieldCompletion[] } {
    const completion = (detail: string) => (label: string, documentation: string): YamlFieldCompletion =>
      ({label, insertText: label, detail, documentation, kind: 'Keyword'});
    ...
    return {
      type: [
        ...
        ruleType('NTH_WEEKDAY', 'The nth weekday of a month, for instance the third Monday in January. Needs month, nth and dayOfWeek.'),
        ...
        ruleType('HIJRI', 'An Islamic date converted to the Gregorian calendar. Needs month and day, usually with offsetTolerance.'),
        ruleType('EXPLICIT_DATES', 'A fixed list of dates that follow no pattern. Needs dates.')
      ],
      observance: [
        observance('NONE', 'A closure falling on a weekend is dropped. Correct wherever holidays are not made up, for instance in Switzerland.'),
      ...
      dayOfWeek: ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY', 'SUNDAY']
        .map(day => weekday(day, 'Weekday in English and upper case.'))
    };
  }
}
