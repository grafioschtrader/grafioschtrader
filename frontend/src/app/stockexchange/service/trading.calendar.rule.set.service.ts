...
@Injectable()
export class TradingCalendarRuleSetService
  extends AuthServiceWithLogout<TradingCalendarRuleSet>
  implements ServiceEntityUpdate<TradingCalendarRuleSet>, DeleteService
{
  ...
  public getAllRuleSets(): Observable<TradingCalendarRuleSet[]> {
    return <Observable<TradingCalendarRuleSet[]>>(
      this.httpClient
        .get(`${BaseSettings.API_ENDPOINT}${AppSettings.TRADING_CALENDAR_RULE_SET_KEY}`, this.getHeaders())
  ...
  public getRuleSetOptions(): Observable<ValueKeyHtmlSelectOptions[]> {
    return <Observable<ValueKeyHtmlSelectOptions[]>>(
      this.httpClient
        .get(`${BaseSettings.API_ENDPOINT}${AppSettings.TRADING_CALENDAR_RULE_SET_KEY}/options`, this.getHeaders())
        .pipe(catchError(this.handleError.bind(this)))
    );
  }
  ...
  public getRuleSetIdByMic(): Observable<{ [mic: string]: number }> {
    return <Observable<{ [mic: string]: number }>>(
      this.httpClient
        .get(`${BaseSettings.API_ENDPOINT}${AppSettings.TRADING_CALENDAR_RULE_SET_KEY}/micmapping`, this.getHeaders())
        .pipe(catchError(this.handleError.bind(this)))
    );
  }
  ...
  public validateYaml(ruleYaml: string): Observable<string[]> {
    return <Observable<string[]>>this.httpClient
      .post(`${BaseSettings.API_ENDPOINT}${AppSettings.TRADING_CALENDAR_RULE_SET_KEY}/validateyaml`, ruleYaml, {
