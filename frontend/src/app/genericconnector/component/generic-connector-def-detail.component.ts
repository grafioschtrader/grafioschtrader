                ...
                @switch (field.templateName) {
                  @case ('check') {
                    <span
                      ><i
                        [ngClass]="{
                          'fa fa-check': getValueByPath(connectorDef, field)
                        }"></i
                    ></span>
                  }
                  @case ('yaml') {
                    @if (getValueByPath(connectorDef, field)) {
                      <pre
                        style="white-space: pre-wrap; font-family: monospace; font-size: 0.85em; max-height: 200px; overflow-y: auto; margin: 0;"
                        >{{ getValueByPath(connectorDef, field) }}</pre>
                    } @else {
...
export class GenericConnectorDefDetailComponent extends SingleRecordConfigBase implements OnChanges {
  ...
  constructor(translateService: TranslateService, gps: GlobalparameterService) {
    ...
    this.addFieldPropertyFeqH(DataType.String, 'shortId', {
      fieldsetName: 'CONNECTOR_SETTINGS'
    });
    this.addFieldPropertyFeqH(DataType.String, 'readableName', {
      fieldsetName: 'CONNECTOR_SETTINGS'
    });
    ...
    this.addFieldPropertyFeqH(DataType.String, 'domainUrl', {
      fieldsetName: 'CONNECTOR_SETTINGS'
    });
    ...
    this.addFieldPropertyFeqH(DataType.String, 'supportedCategories', {
      translateValues: TranslateValue.UPPER_CASE_ARRAY_TO_COMMA_SEPERATED,
      fieldsetName: 'ADVANCED_SETTINGS'
    });
