...
import { GTNetExchangeKindType, GTNetSupplierWithDetails } from '../../lib/gnet/model/gtnet';
...
export class GTNetSupplierDetailTableComponent extends TreeTableConfigBase implements OnInit {
  ...
  constructor(
    private gtNetExchangeService: GTNetExchangeService,
  ...
  private buildTreeNodes(data: GTNetSupplierWithDetails[]): TreeNode[] {
    ...
    if (histChildren.length > 0) {
      roots.push({
        data: {
          uniqueKey: 'root-hist',
          name: GTNetExchangeKindType[GTNetExchangeKindType.HISTORICAL_PRICES]
        },
    ...
    if (lastChildren.length > 0) {
      roots.push({
        data: {
          uniqueKey: 'root-last',
          name: GTNetExchangeKindType[GTNetExchangeKindType.LAST_PRICE]
        },
