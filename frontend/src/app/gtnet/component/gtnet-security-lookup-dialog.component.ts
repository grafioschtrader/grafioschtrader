...
import {SecurityGtnetLookupDTO, SecurityGtnetLookupRequest, SecurityGtnetLookupResponse} from '../model/gtnet-security-lookup';
...
export class GtnetSecurityLookupDialogComponent {
  ...
  @ViewChild('lookupTable') lookupTable: GtnetSecurityLookupTableComponent;
  ...
  private performSearch(): void {
    ...
    this.lookupService.lookupSecurity(request).subscribe({
      next: (response) => {
        ...
        this.loading = false;
        // Trigger translated value store update
        if (this.lookupTable) {
          this.lookupTable.onDataLoaded();
        }
      },
      error: (error) => {
        this.loading = false;
