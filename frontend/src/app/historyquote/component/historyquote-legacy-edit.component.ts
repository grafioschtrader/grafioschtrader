...
export class HistoryquoteLegacySecurityCurrency {
  constructor(
    public historyquoteLegacy: HistoryquoteLegacy,
...
@Component({
  ...
  template: ` <p-dialog
    header="{{ 'HISTORYQUOTE_LEGACY' | translate }} {{ callParam.showName }}"
...
export class HistoryquoteLegacyEditComponent extends HistoryquoteEditBase<HistoryquoteLegacy> {
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_WATCHLIST_HISTORYQUOTES_LEGACY,
      'HISTORYQUOTE_LEGACY',
      translateService,
      gps,
      messageToastService,
  ...
  protected override getAdditionalLeadingFields(): FieldConfig[] {
    return [
      DynamicFieldHelper.createFieldPcalendarHeqF(DataType.DateString, 'transferDate', false, { disabled: true })
    ];
  }
