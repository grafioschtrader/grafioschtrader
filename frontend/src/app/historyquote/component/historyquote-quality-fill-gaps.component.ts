...
@Component({
  ...
  template: `
    <p-dialog
      header="{{ 'HISTORYQUOTE_FILL_GAPS' | translate }}"
...
export class HistoryquoteQualityFillGapsComponent extends SimpleEditBase implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.config = [
      DynamicFieldHelper.createFieldPcalendarHeqF(DataType.DateString, this.fillUpToDate, true, {
        calendarConfig: {
          minDate: HistoryquoteQualityFillGapsComponent.toLocalDate(this.fillGapsBounds.minFillUpTo),
      ...
      DynamicFieldHelper.createFunctionButtonFieldName('execute', 'EXECUTE', (e) => this.submit(null))
    ];
  ...
  submit(value: { [name: string]: any }): void {
    ...
    this.securityService
      .fillHistoryquoteGapsLinear(this.securitycurrency.idSecuritycurrency, {
        moveWeekendToFriday: this.configObject[this.moveWeekendToFriday].formControl.value,
        fillUpToDate: moment(this.configObject[this.fillUpToDate].formControl.value).format(
          BaseSettings.FORMAT_DATE_SHORT_NATIVE
        )
      })
      .subscribe({
        next: (hlfs: HisotryqouteLinearFilledSummary) => {
          this.messageToastService.showMessageI18nEnableHtml(
            hlfs.warning ? InfoLevelType.WARNING : InfoLevelType.SUCCESS,
            'HISTORYQUOTE_FILL_RESULT',
            hlfs
          );
  ...
  protected override initialize(): void {
    ...
    this.configObject[this.fillUpToDate].formControl.setValue(
      HistoryquoteQualityFillGapsComponent.toLocalDate(this.fillGapsBounds.defaultFillUpTo)
    );
    this.prepareCalendarLimitMessage();
    const hasWeekendDays: boolean =
      (this.historyquoteQuality.quoteSaturday !== null && this.historyquoteQuality.quoteSaturday > 0) ||
  ...
  private prepareCalendarLimitMessage(): void {
    if (this.fillGapsBounds.calendarOutdated) {
      this.translateService
        .get('HISTORYQUOTE_FILL_CALENDAR_LIMIT', {
          calendarHorizon: moment(
            HistoryquoteQualityFillGapsComponent.toLocalDate(this.fillGapsBounds.calendarHorizon)
