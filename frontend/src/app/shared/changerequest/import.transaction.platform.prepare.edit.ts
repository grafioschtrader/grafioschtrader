 ...
 * in the edit dialog.
 */
export class ImportTransactionPlatformPrepareEdit
  extends BasePrepareEdit<ImportTransactionPlatform>
  implements PrepareCallParam<ImportTransactionPlatform>
{
  /**
  ...
  prepareForEditEntity(entity: ImportTransactionPlatform, entityMapping: EntityMapping): Observable<void> {
    return this.importTransactionPlatformService.getPlatformTransactionImport().pipe(
      map((platformTransactionImports: IPlatformTransactionImport[]) => {
        entityMapping.option = SelectOptionsHelper.createValueKeyHtmlSelectOptionsFromArray(
          'id',
          'readableName',
