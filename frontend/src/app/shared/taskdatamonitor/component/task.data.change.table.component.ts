...
@Component({
    template: `
      <div class="data-container" (click)="onComponentClick($event)" #cmDiv
        ...
        <p-table #table [columns]="fields" [value]="taskDataChangeSecurityCurrencyPairInfo.taskDataChangeList"
...
export class TaskDataChangeTableComponent extends TableCrudSupportMenu<TaskDataChange> {
    ...
    protected override readData(): void {
        combineLatest([this.taskDataChangeService.getAllTaskDataChangeSecurityInfo(), this.taskDataChangeService.getFormConstraints()]).subscribe(data => {
            this.taskDataChangeSecurityCurrencyPairInfo = data[0];
            this.tdcFormConstraints = data[1];
            this.prepareTableAndTranslate();
            this.createTranslatedValueStoreAndFilterField(this.taskDataChangeSecurityCurrencyPairInfo.taskDataChangeList);
            this.prepareFilter(this.taskDataChangeSecurityCurrencyPairInfo.taskDataChangeList);
        });
    }
    ...
    getTooltipValueByPath(dataobject: any, field: ColumnConfig) {
        if (field.field === 'idEntity' && (dataobject.entity === 'Security' || dataobject.entity === 'Currencypair' )) {
            return this.taskDataChangeSecurityCurrencyPairInfo.securityInfo[dataobject['idEntity']];
        } else {
