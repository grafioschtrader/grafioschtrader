...
@Injectable()
export class MailSettingForwardService
  extends AuthServiceWithLogout<MailSettingForward>
  implements ServiceEntityUpdate<MailSettingForward>
{
  ...
  public getMailSettingForwardByUser(): Observable<MailSettingForward[]> {
    return <Observable<MailSettingForward[]>>(
      this.httpClient
        .get(`${BaseSettings.API_ENDPOINT}${BaseSettings.MAIL_SETTING_FORWARD_KEY}/user`, this.getHeaders())
        .pipe(catchError(this.handleError.bind(this)))
    );
  }

  public getSendForwardDefault(): Observable<MailSendForwardDefault> {
    return <Observable<MailSendForwardDefault>>this.httpClient
      .get(
        `
  ...
  public deleteEntity(idMailSettingForward: number): Observable<any> {
    return this.httpClient
      .delete(
        `${BaseSettings.API_ENDPOINT}${BaseSettings.MAIL_SETTING_FORWARD_KEY}/${idMailSettingForward}`,
        this.getHeaders()
      )
      .pipe(catchError(this.handleError.bind(this)));
  }
  ...
  update(mailSettingForward: MailSettingForward): Observable<MailSettingForward> {
    return this.updateEntity(
      mailSettingForward,
      mailSettingForward.idMailSettingForward,
      BaseSettings.MAIL_SETTING_FORWARD_KEY
    );
  }
}
