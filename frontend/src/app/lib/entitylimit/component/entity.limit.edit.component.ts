...
@Component({
  ...
  template: `
    <p-dialog header="{{'ENTITY_LIMIT_INFO_CLASS' | translate}}" [visible]="visibleDialog"
              [style]="{width: '560px'}"
...
export class EntityLimitEditComponent extends SimpleEntityEditBase<EntityLimit> implements OnInit {
  ...
  constructor(translateService: TranslateService,
    ...
    super(HelpIds.HELP_ENTITY_LIMIT, AppHelper.toUpperCaseWithUnderscore(BaseSettings.ENTITY_LIMIT), translateService,
      gps, messageToastService, entityLimitService);
  }
  ...
  protected override initialize(): void {
    ...
    combineLatest([
      entityLimitService.getLimitKeyDefinitions(this.idUser,
      ...
      if (this.existingEntityLimit) {
        ...
        this.showReadableKeyParts(this.existingEntityLimit.limitTypeKey, this.existingEntityLimit.ownerScopeKey,
          this.existingEntityLimit.countScopeKey, this.existingEntityLimit.relationEntityName);
      }
      AuditHelper.transferToFormAndChangeButtonForProposaleEdit(this.translateService, this.gps,
        this.proposeChangeEntityWithEntity ? this.proposeChangeEntityWithEntity.proposedEntity
  ...
  private collectUsedScopes(): Map<string, Set<string>> {
    ...
    (this.existingEntityLimits ?? []).filter(entityLimit => entityLimit.idUser == null
      && entityLimit.idEntityLimit !== this.existingEntityLimit?.idEntityLimit).forEach(entityLimit => {
  ...
  private toKeyOptions(limitKeyDefinitions: LimitKeyDefinition[]): GroupItem[] {
    return limitKeyDefinitions.filter(definition => !this.isKeyExhausted(definition.keyId)).map(definition => {
      const parts: string[] = [this.translateService.instant(definition.labelKey)];
      if (definition.relationEntityName) {
        parts.push(this.translateService.instant(
          AppHelper.toUpperCaseWithUnderscore(definition.relationEntityName)));
      }
    ...
    }).sort((a, b) => a.optionsText.toLowerCase() < b.optionsText.toLowerCase() ? -1
      : a.optionsText.toLowerCase() > b.optionsText.toLowerCase() ? 1 : 0);
  }
  ...
  private isKeyExhausted(keyId: string): boolean {
    ...
    return usedScopes != null && usedScopes.has('')
      && this.roleOptions.every(role => usedScopes.has(String(role.key)));
  }
  ...
  private applyRoleOptions(keyId: string): void {
    ...
    this.configObject.idRole.valueKeyHtmlOptions = SelectOptionsHelper.translateExistingValueKeyHtmlSelectOptions(
      this.translateService, this.roleOptions.filter(role => !usedScopes.has(String(role.key))), !usedScopes.has(''));
  }
  ...
  private applyPickedKey(keyId: string): void {
    ...
    this.showReadableKeyParts('LIMIT_TYPE_' + definition.limitType,
      definition.ownerScope ? 'OWNER_SCOPE_' + definition.ownerScope : null,
      definition.countScope ? 'COUNT_SCOPE_' + definition.countScope : null, definition.relationEntityName);
    if (this.existingEntityLimit == null && definition.defaultValue != null
  ...
  private showReadableKeyParts(limitTypeKey: string, ownerScopeKey: string, countScopeKey: string,
    relationEntityName: string): void {
  ...
  protected override getNewOrExistingInstanceBeforeSave(value: { [name: string]: any }): EntityLimit {
    ...
    ['limitTypeReadable', 'scopeReadable', 'limitType', 'entityName', 'relationEntityName', 'countScope', 'ownerScope']
