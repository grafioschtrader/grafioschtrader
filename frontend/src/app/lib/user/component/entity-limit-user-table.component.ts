...
export class EntityLimitUserTableComponent extends TableConfigBase implements OnInit, OnChanges, OnDestroy, IGlobalMenuAttach {
  ...
  constructor(private activePanelService: ActivePanelService,
    ...
    this.addColumnFeqH(DataType.String, 'limitTypeKey', true, false, {translateValues: TranslateValue.NORMAL});
  ...
  private getEntityLabel(entityLimit: EntityLimit, field: ColumnConfig, valueField: any): string {
    return entityLimit.entityName ? this.translateService.instant(
      AppHelper.toUpperCaseWithUnderscore(entityLimit.entityName)) : '';
  }

  getLimitProposeIcon(entityLimit: EntityLimit, field: ColumnConfig): string {
    return !entityLimit.idEntityLimit || this.proposeMap.has(entityLimit.idEntityLimit) ?
      ...
      'MSG_CONFIRM_DELETE_RECORD|' + EntityLimitUserTableComponent.ENTITY_LIMIT, () => {
        this.entityLimitService.deleteEntity(selectedEntityLimit[this.entityKeyName]).subscribe(response => {
          this.messageToastService.showMessageI18n(InfoLevelType.SUCCESS,
            'MSG_DELETE_RECORD', {i18nRecord: EntityLimitUserTableComponent.ENTITY_LIMIT});
    ...
    if (this.user.userChangeLimitProposeList) {
      for (const proposeUserTask of this.user.userChangeLimitProposeList) {
        ...
        if (!existingLimit) {
          ...
          uecl.limitValue = proposeUserTask.proposeChangeFieldList.find(p => p.field === 'dayLimit').valueDesarialized;
          uecl.validUntil = proposeUserTask.proposeChangeFieldList.find(p => p.field === 'untilDate').valueDesarialized;
        ...
        this.proposeMap.set(existingLimit.idEntityLimit,
