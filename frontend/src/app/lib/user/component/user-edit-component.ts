...
export class UserEditComponent extends SimpleEntityEditBase<User> implements OnInit {
  ...
  constructor(
    ...
    super(
      HelpIds.HELP_USER,
      AppHelper.toUpperCaseWithUnderscore(BaseSettings.USER),
      translateService,
      gps,
      messageToastService,
      userService
  ...
  ngOnInit(): void {
    ...
    this.config = [
      ...
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(DataType.NumericInteger, 'timezoneOffset', true, -720, 720, {
        fieldSuffix: 'MINUTES'
      }),
      DynamicFieldHelper.createFieldMinMaxNumberHeqF(DataType.NumericInteger, 'securityBreachCount', true, 0, 99, null),
  ...
  protected initialize(): void {
    ...
    const proposeChangeEntityWithEntity: ProposeChangeEntityWithEntity =
      AuditHelper.convertToProposeChangeEntityWithEntity(this.callParam, this.callParam.userChangePropose);
    this.configObject.mostPrivilegedRole.valueKeyHtmlOptions = SelectOptionsHelper.translateArrayKeyEqualValue(
