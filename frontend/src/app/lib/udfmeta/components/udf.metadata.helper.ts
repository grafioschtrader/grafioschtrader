...
export class UDFMetadataHelper {
   ...
   * @static
   */
  public static createMetadataBaseFields(
    ...
    fdTypeSizeGroup.validation = [
      dataTypeFieldSizeGroup(
        configDataTypeFields[0].field,
        configDataTypeFields[1].field,
        configDataTypeFields[1].field
      )
    ];
    ...
    return <FieldConfig[]>[
      ...
      DynamicFieldHelper.createFieldInputStringVSParam(
        this.DESCRIPTION,
        'FIELD_DESCRIPTION',
        24,
        true,
  ...
  public static getFieldDescriptorByEntity(entityName: string): FieldDescriptorInputAndShowExtended[] {
    return JSON.parse(sessionStorage.getItem(GlobalSessionNames.UDF_FORM_DESCRIPTOR_GENERAL)).filter(
      (fd) => fd.entity === entityName
    );
  }
   ...
   * @static
   */
  public static prepareDataTypeFields(
    ...
    businessObject: UDFMetadata,
    configDataTypeFields: FieldConfig[]
  ): Subscription {
    configObject.uiOrder.valueKeyHtmlOptions = SelectOptionsHelper.createValueKeyHtmlSelectOptionsForNumberRange(
      1,
      99,
      excludeUiOrders
    );
    const fieldGroupSub: Subscription = configObject[UDFMetadataHelper.DATA_TYPE_FIELD_GROUP].formControl
      .get([UDFMetadataHelper.UDF_DATA_TYPE])
      .valueChanges.subscribe((udt) =>
   ...
   * @static
   */
  private static enableDisableFileSize(
    translateService: TranslateService,
    configObject: {
      [name: string]: FieldConfig;
    },
    ...
    const controlFieldSize = configObject[UDFMetadataHelper.DATA_TYPE_FIELD_GROUP].formControl.get([
      UDFMetadataHelper.FIELD_SIZE
    ]);
    if (
      udt === UDFDataType[UDFDataType.UDF_DateTimeNumeric] ||
      udt === UDFDataType[UDFDataType.UDF_DateString] ||
      udt === UDFDataType[UDFDataType.UDF_Boolean] ||
      udt === UDFDataType[UDFDataType.UDF_URLString]
    ) {
