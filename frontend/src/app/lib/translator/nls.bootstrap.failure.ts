...
export function showNlsBootstrapFailure(onRetry?: () => void): void {
  ...
  panel.style.cssText = [
    'max-width:34rem', 'margin:1rem', 'padding:1.75rem 2rem', 'background:#fff', 'border:1px solid #d0d0d0',
    'border-radius:6px', 'box-shadow:0 2px 10px rgba(0,0,0,.12)', 'text-align:center', 'line-height:1.5'
  ].join(';');
  ...
  message.textContent = 'Grafioschtrader could not load its texts from the server. Please check that the server is '
  ...
  button.style.cssText = [
    'padding:.55rem 1.4rem', 'font-size:1rem', 'cursor:pointer', 'color:#fff', 'background:#0b5ed7',
    'border:1px solid #0a58ca', 'border-radius:4px'
  ].join(';');
