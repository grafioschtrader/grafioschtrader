...
async function openInstrumentMenu(page: Page, instrument: Instrument): Promise<Locator> {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (await menu.getByRole('menuitem', {name: RX.chart}).first().isVisible().catch(() => false)) {
      return menu;
    }
...
async function plotState(page: Page): Promise<PlotState> {
  ...
  return page.locator('.fullChart .js-plotly-plot').first().evaluate((element: any) => ({
...
async function openChart(page: Page, instrument: Instrument): Promise<OpenChartResult> {
  ...
  await waitForPlot(page);
  await expect.poll(async () => (await plotState(page)).names.some(name => name.includes(instrument.name)),
    {timeout: 30_000, message: `${instrument.ticker} trace was not opened`}).toBe(true);
  return result;
}
...
async function addToChart(page: Page, instrument: Instrument): Promise<OpenChartResult> {
  ...
  const result = await responseChartResult(await responsePromise);
  await expect.poll(async () => (await plotState(page)).names.some(name => name.includes(instrument.name)),
    {timeout: 30_000, message: `${instrument.ticker} trace was not added`}).toBe(true);
  return result;
}

function chartCheckbox(page: Page, label: RegExp): Locator {
  return page.locator('.fullChart .input-row label').filter({hasText: label}).first()
...
async function editIndicator(page: Page, indicator: 'SMA' | 'EMA' | 'RSI', parentName: RegExp,
  ...
  const edit = page.locator('[role="menu"]:visible')
    .getByRole('menuitem', {name: new RegExp(`^(Bearbeiten|Edit)\\s+${indicator}$`, 'i')}).first();
  ...
  expect(response.request().postDataJSON()).toEqual(Object.fromEntries(
    Object.entries(values).map(([field, value]) => [field, String(value)])));
  await dialog.waitFor({state: 'hidden', timeout: 10_000});
  for (const period of Object.values(values)) {
    await expect.poll(async () => (await plotState(page)).names.some(name => name.includes(`(${period})`)),
      {timeout: 30_000, message: `${indicator} period ${period} was not plotted`}).toBe(true);
  }
}
...
function waitForShapeWrite(page: Page, expectedCount: number): Promise<Response> {
  return page.waitForResponse(response => {
    if (response.request().method() !== 'PUT'
        || !/\/api\/userchartshape$/.test(new URL(response.url()).pathname)) {
      return false;
    }
...
async function drawShape(page: Page, mode: 'drawline' | 'drawrect' | 'drawcircle',
    start: {x: number, y: number}, end: {x: number, y: number}, expectedCount: number): Promise<void> {
  await clickModeBarButton(page, `[data-val="${mode}"]`);
  await expect.poll(() => page.locator('.fullChart .js-plotly-plot').first()
    .evaluate((element: any) => element.layout?.dragmode), {timeout: 5_000}).toBe(mode);
...
test.describe.serial('time-series chart interaction', () => {
  ...
  test('reacts to date ranges, checkboxes, chart types, and all technical indicators', async ({page}) => {
    ...
    await editIndicator(page, 'SMA', RX.sma,
      {taShortPeriod: 10, taMediumPeriod: 30, taLongPeriod: 100});
    await editIndicator(page, 'EMA', RX.ema,
      {taShortPeriod: 12, taMediumPeriod: 26, taLongPeriod: 60});
  ...
  test('adds DIS and NESN and normalizes the comparison to USD and CHF', async ({page}) => {
    ...
    await expect.poll(async () => (await plotState(page)).names.filter(name =>
      [HPQ.name, DIS.name, NESN.name].some(expected => name.includes(expected))).length,
    ...
    for (const code of ['CHF', 'USD']) {
      ...
      await expect.poll(async () => (await plotState(page)).names.length, {timeout: 15_000}).toBeGreaterThanOrEqual(3);
  ...
  test('persists drawings and supports erase, undo, redo, reload, and delete all', async ({page}) => {
    ...
    await drawShape(page, 'drawline', {x: 0.12, y: 0.20}, {x: 0.27, y: 0.72}, 1);
    await drawShape(page, 'drawrect', {x: 0.36, y: 0.18}, {x: 0.49, y: 0.75}, 2);
    await drawShape(page, 'drawcircle', {x: 0.60, y: 0.20}, {x: 0.73, y: 0.72}, 3);

    await clickModeBarButton(page, '[data-val="zoom"]');
    await expect.poll(() => page.locator('.fullChart .js-plotly-plot').first()
      .evaluate((element: any) => element.layout?.dragmode), {timeout: 5_000}).toBe('zoom');
    const circlePath = page.locator(
      '.fullChart .js-plotly-plot .shapelayer .shape-group[data-index="2"] > path').first();
    await expect(circlePath).toBeVisible();
    await circlePath.dispatchEvent('click');
    await expect.poll(() => page.locator('.fullChart .js-plotly-plot').first()
      .evaluate((element: any) => element._fullLayout?._activeShapeIndex), {timeout: 5_000}).toBe(2);
    ...
    const loadPromise = page.waitForResponse(response => response.request().method() === 'GET'
