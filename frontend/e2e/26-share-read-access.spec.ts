...
import {findMailhogMessage, getCsvUser, loginAs, loginAsCsvUser} from './helpers';
import {
 ...
 *    e-mailed. The viewer is the users.csv row flagged 'er' (e2euser.r@test.local) — NOT registered
...
test.describe.serial('Share read access (issue #201)', () => {
  ...
  const owner = getCsvUser(OWNER_NICKNAME);
  const viewer = getCsvUser(VIEWER_NICKNAME);
  const grantee = getCsvUser(GRANTEE_NICKNAME);
  ...
  test('owner shares read access: new viewer login and READ grant for a registered user', async ({page}) => {
    ...
    await loginAsCsvUser(page, OWNER_NICKNAME);
  ...
  test('viewer login sees the owner portfolio read-only', async ({page}) => {
    test.setTimeout(120_000);
    await loginAsCsvUser(page, VIEWER_NICKNAME);
