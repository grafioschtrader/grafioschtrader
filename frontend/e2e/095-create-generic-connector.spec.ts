...
// No toast assertions: the shared Optimus UI toast runs with preventDuplicates=true and a 10s life, and every
...
async function createConnectorDef(page: Page, c: GenericConnectorDefData): Promise<void> {
  ...
  if (c.tokenConfigYaml) {
    ...
    await dialog
      .locator('.p-fieldset-legend, .p-fieldset legend')
      .first()
      .click()
      .catch(() => {});
...
test.describe.serial('generic connectors — create as alledit, activate as admin', () => {
  ...
  test('admin activates all generic connectors', async ({ page }) => {
    ...
    for (const c of connectors) {
      ...
      const activateResponse = page.waitForResponse((r) => r.url().includes('/genericconnector/activate/') && r.ok(), {
        timeout: 20_000
      });
