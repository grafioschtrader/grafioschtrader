...
test.describe.serial('reusable mail system', () => {
  ...
  test('admin sends an internal message to a user', async ({ page }) => {
    ...
    const recipientValue = await userSelect.evaluate(
      (select, nickname) =>
        Array.from((select as HTMLSelectElement).options).find((option) => option.textContent.trim().endsWith(nickname))
          ?.value ?? null,
      recipient.nickname
    );
    ...
    const sendResponsePromise = page.waitForResponse(
      (response) => response.url().endsWith('/api/mailsendrecv') && response.request().method() === 'POST'
    );
