...
function loadFixture(): EntityLimitFixtureRow[] {
  ...
  const rows = lines.map((line, index) => {
    ...
    if (
      !['MAX', 'DAY_CUD', 'DAY_READ'].includes(limitType) ||
      !entityName ||
      roleName !== 'ROLE_LIMITEDIT' ||
      !Number.isInteger(limitValue) ||
      limitValue < 1 ||
      !['i', 'e2e'].includes(routing)
    ) {
...
async function createLimit(
  page: Page,
  table: Locator,
  row: EntityLimitFixtureRow,
  idRole: number
): Promise<ApiEntityLimit> {
  ...
  const saved = page.waitForResponse(
    (response) => response.url().includes('/api/entitylimit') && response.request().method() === 'POST',
    { timeout: 20_000 }
  );
...
test('admin creates the E2E-owned ROLE_LIMITEDIT limits from CSV', async ({ page }) => {
  ...
  for (const row of LIMITS) {
    const matches = persisted.filter(
      (limit) => limit.idRole === idRole && limit.idUser == null && limit.keyId === keyId(row)
    );
