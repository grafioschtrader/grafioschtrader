...
test.describe.serial('connector API keys — created through the admin UI from the local CSV fixture', () => {
  ...
  test('removes pre-existing connector API keys', async ({ page }) => {
    ...
    for (let guard = 0; guard < 40; guard++) {
      ...
      const deleted = page.waitForResponse(
        (r) => r.url().includes('/connectorapikey') && r.request().method() === 'DELETE' && r.ok(),
        { timeout: 20_000 }
      );
  ...
  test('creates every connector API key from the CSV fixture', async ({ page }) => {
    ...
    for (const row of apiKeys) {
      ...
      await expect(
        option,
        `provider '${row.idProvider}' is not offered — is it already configured, ` + 'or unknown to this build?'
      ).toHaveCount(1);
      ...
      const saved = page.waitForResponse(
        (r) => r.url().includes('/connectorapikey') && ['POST', 'PUT'].includes(r.request().method()) && r.ok(),
        { timeout: 20_000 }
      );
