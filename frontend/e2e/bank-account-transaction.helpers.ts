...
export async function ensureBankTransactionExchangeRates(page: Page,
    fixtures: BankTransactionFixture[]): Promise<void> {
 ...
 */
export async function findMissingBankTransactions(page: Page,
    fixtures: BankTransactionFixture[]): Promise<BankTransactionFixture[]> {
    ...
    for (const fixture of fixtures) {
      if (!isFixturePersisted(transactions, fixture)) {
        ...
        for (const conflict of conflicts) {
          ...
          expect(response.ok(), `deleting conflicting transaction ${conflict.idTransaction}: `
...
export async function expectBankTransactions(page: Page, fixtures: BankTransactionFixture[]): Promise<void> {
  await test.step('verify persisted fixture transactions', async () => {
    ...
    for (const fixture of fixtures) {
      const debitOrSingle = transactions.find(transaction =>
        !matchedIds.has(transaction.idTransaction) && matchesFixtureDebitOrSingle(transaction, fixture));
      ...
      if (fixture.transaction.kind === 'single') {
        ...
        const deposit = transactions.find(transaction =>
          transaction.idTransaction === debitOrSingle.connectedIdTransaction);
...
async function openTransactionDialog(page: Page, portfolio: PortfolioFixture, cashAccount: string,
    menuItem: RegExp): Promise<void> {
  ...
  const row = summary.locator('p-table').first().locator('tbody > tr[data-p-selectable-row]')
    .filter({has: page.getByRole('cell', {name: cashAccount, exact: true})}).first();
...
async function ensureHistoryquote(page: Page, fromCurrency: string, toCurrency: string,
    date: string, close: number): Promise<void> {
  ...
  if (!currencypair) {
    ...
    expect(pairResponse.ok(), `creating currency pair ${fromCurrency}/${toCurrency}: ${await pairResponse.text()}`)
      .toBeTruthy();
  ...
  if (!createResponse.ok()) {
    ...
    lookup = await getCurrencypairWithHistoryquote(page, lookupUrl, headers);
    expect(lookup.historyquote?.date,
      `creating ${fromCurrency}/${toCurrency} quote on ${date}: ${await createResponse.text()}`).toBe(date);
  }
}

async function getCurrencypairWithHistoryquote(page: Page, url: string,
    headers: Record<string, string>): Promise<ApiCurrencypairWithHistoryquote> {
...
function matchesFixtureDebitOrSingle(transaction: ApiTransaction, fixture: BankTransactionFixture): boolean {
  ...
  if (data.kind === 'single') {
    return transaction.cashaccount?.name === data.cashAccount
  ...
  return transaction.cashaccount?.name === data.debitCashAccount
...
function isFixturePersisted(transactions: ApiTransaction[], fixture: BankTransactionFixture): boolean {
  ...
  const deposit = transactions.find(transaction => transaction.idTransaction === debitOrSingle.connectedIdTransaction);
  if (!deposit) {
    return false;
  }
  try {
...
function expectTransferDetails(withdrawal: ApiTransaction, deposit: ApiTransaction,
    expected: CashTransferData): void {
