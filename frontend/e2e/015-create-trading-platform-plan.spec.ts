...
/**
 * Creates the trading platform plans tagged e2e='e' in the shared CSV; the e2e='i' rows are seeded by
 * the backend TradingPlatformPlanResourceTest instead.
 *
 * Must run BEFORE 025-create-portfolio.spec.ts: the securities account dialog there picks its plan by
 * name from the very list this spec fills, and 'Saxo Trader' exists only once this spec has created
 * it. Spec files run in alphabetical order (workers: 1), so the number is what enforces that.
 */
interface TppRow {
...
test.describe.serial("Create trading platform plans (e2e='e' rows from shared CSV)", () => {
  ...
  for (const row of rows) {
    test(`creates TPP: ${row.platformPlanNameEN} (${row.importPlatformName})`, async ({ page }) => {
      ...
      // src/app/shared/contributor/basedata-main-tree.contributor.ts. 'alledit' has locale de-CH, so
      // every text selector must match the German and the English label.
      const tppTreeNode = page
        .locator('.p-tree-node-content', { hasText: /(Trading\s*platform\s*plan|Handelsplattform\s*Plan)/i })
        .first();
      ...
      const existingPlan = contentArea
        .locator('td')
        .filter({ hasText: planNameRegex(row) })
        .first();
      ...
      await menuList
        .getByText(/(Create.*Trading.*platform.*plan|Erstellen.*Handelsplattform\s*Plan)/i)
        .first()
        .click();
