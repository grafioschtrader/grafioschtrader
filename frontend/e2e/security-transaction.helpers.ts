...
export async function deleteSecurityTransactionGroup(page: Page, group: SecurityTransactionGroup): Promise<void> {
  await test.step(`delete leftovers of ${group.identifier}`, async () => {
    ...
    for (const transaction of transactions) {
      ...
      expect(
        response.ok(),
        `deleting ${transaction.transactionType} transaction ${transaction.idTransaction}: ` + (await response.text())
      ).toBeTruthy();
    }

    const remaining = (await getTenantTransactions(page, headers)).filter((transaction) =>
      belongsToGroup(transaction, group)
    );
...
export async function expectSecurityTransactionGroup(page: Page, group: SecurityTransactionGroup): Promise<void> {
  await test.step(`verify ${group.identifier}`, async () => {
    ...
    for (const expectedTransaction of group.transactions) {
      ...
      expect(
        actual,
        `${group.identifier} ${expectedTransaction.transactionType} on ${expectedTransaction.date}`
      ).toBeDefined();
...
async function createOpeningTransaction(
  page: Page,
  group: SecurityTransactionGroup,
  transaction: SecurityTransactionData
): Promise<void> {
  ...
  await selectWhenReady(dialog.locator('select#idSecurityaccount'), [group.securityAccountName, group.portfolioName]);
...
async function createConnectedTransaction(
  page: Page,
  group: SecurityTransactionGroup,
  transaction: SecurityTransactionData,
  menuItem: RegExp
...
function watchlistSecurityRow(page: Page, group: SecurityTransactionGroup): Locator {
  return page
    ...
    .filter({ hasText: group.securityName })
    .first();
}
...
async function openMarginTable(page: Page, group: SecurityTransactionGroup): Promise<Locator> {
  ...
  let marginTable = row.locator('xpath=following-sibling::tr[1]').locator('transaction-security-margin-treetable');
  if (!(await marginTable.isVisible().catch(() => false))) {
    await row.locator('td').first().locator('a').click();
    marginTable = row.locator('xpath=following-sibling::tr[1]').locator('transaction-security-margin-treetable');
  }
...
async function submitTransaction(page: Page, dialog: Locator): Promise<void> {
  ...
  const responsePromise = page.waitForResponse(
    (response) =>
      response.request().method() === 'POST' && new URL(response.url()).pathname === '/api/transaction/securitytrans'
  );
  await submit.click();
...
async function getTenantTransactions(page: Page, headers: Record<string, string>): Promise<ApiSecurityTransaction[]> {
...
function belongsToGroup(transaction: ApiSecurityTransaction, group: SecurityTransactionGroup): boolean {
  return (
    transaction.security?.name === group.securityName &&
    transaction.cashaccount?.name === group.cashAccountName &&
    group.transactions.some(
      (expected) =>
        expected.transactionType === transaction.transactionType && expected.date === transactionDate(transaction)
    )
  );
}
