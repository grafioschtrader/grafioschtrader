...
import {findMailhogMessage, getCsvUser, loginAsCsvUser} from './helpers';
import {
 ...
 *
 * Advisor = users.csv nickname 'alledit' (hugo.graf@grafiosch.com, ROLE_ALLEDIT, de-CH → German
 * UI). The managed client is the users.csv row flagged 'ec' (e2euser.c@test.local); that user is
...
test.describe.serial('Manage clients (issue #200)', () => {
  ...
  const client = getCsvUser(CLIENT_NICKNAME);

  test('advisor creates the managed client; credentials are e-mailed', async ({page}) => {
    test.setTimeout(120_000);
    await loginAsCsvUser(page, ADVISOR_NICKNAME);
  ...
  test('client login is read-only: data visible, no Client menu, writes rejected', async ({page}) => {
    test.setTimeout(120_000);
    await loginAsCsvUser(page, CLIENT_NICKNAME);
