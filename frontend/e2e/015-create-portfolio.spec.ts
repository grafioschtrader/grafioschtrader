...
for (const p of PORTFOLIOS) {
  test.describe.serial(`Create portfolio ${p.name} of '${p.loginNickname}'`, () => {
    ...
    test(`creates the portfolio ${p.name}`, async ({page}) => {
      ...
      await expect(page.locator('.p-tree-node-content', {hasText: portfolioLabel(p)}))
        .toBeVisible({timeout: 5_000});
    });

    for (const sa of p.securityAccounts) {
      test(`creates the securities account ${sa.name} in ${p.name}`, async ({page}) => {
        const creds = await loginAsCsvUser(page, p.loginNickname);
...
async function openSecurityAccountsFolder(page: Page, p: PortfolioFixture, itemRx: RegExp): Promise<void> {
  ...
  const folder = portfolioSubtree(page, p).locator('.p-tree-node-content')
 ...
 */
async function selectTradingPlatformPlan(page: Page, dialog: Locator,
    sa: SecurityAccountData): Promise<void> {
  ...
  if (await nativeSelect.isVisible().catch(() => false)) {
    ...
    if (sa.tradingPlatformPlan) {
      ...
      expect(index, `no trading platform plan matching '${sa.tradingPlatformPlan}' in [${labels.join(', ')}]`)
