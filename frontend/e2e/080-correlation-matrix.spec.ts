...
async function ensureSetSelected(page: Page, name: string): Promise<void> {
  const optionForName = page.locator('select#idCorrelationSet option').filter({ hasText: name });

  if ((await optionForName.count()) === 0) {
    const menu = await openContextMenu(page);
    await menu
      .getByText(/(Erstellen\s+Korrelationsset|Create\s+Correlation\s+set)/i)
      .first()
      .click();
...
async function runSearch(page: Page, addDialog: Locator): Promise<void> {
  ...
  await expect(addDialog.locator('correlation-set-add-instrument-table tbody tr').first()).toBeVisible({
    timeout: 15_000
  });
}
...
test.describe.serial('Correlation matrix', () => {
  test('builds a 6-instrument matrix and generates a rolling-correlation chart', async ({ page }) => {
    ...
    await expect(page.locator('correlation-table td[style*="background-color"]').first()).toBeVisible({
      timeout: 20_000
    });
    ...
    await expect(page.getByText(/Berechnung nicht m.glich|no overlapping closing prices/i)).toHaveCount(0);
    ...
    const graphItem = menu
      .getByText(/(als\s+Grafik|as\s+graph)/i)
  ...
  test('enforces the max-instruments upper limit', async ({ page }) => {
    ...
    if ((await matrixRowCount(page)) < MAX_INSTRUMENTS) {
      ...
      const exchangeValue = await addDialog
        .locator('select#idStockexchange option')
        .filter({ hasText: /SIX\s+Swiss\s+Exchange/i })
        .first()
        .getAttribute('value');
