...
export async function resolveTransferAccounts(
  page: Page,
  fixture: SecurityTransferFixture
): Promise<ResolvedSecurityTransferFixture> {
...
export async function ensureSecurityTransferQuote(page: Page, fixture: SecurityTransferFixture): Promise<void> {
  await test.step('ensure security-transfer closing price', async () => {
    ...
    expect(
      searchResponse.ok(),
      `searching security ${fixture.isin}/${fixture.currency}: ${await searchResponse.text()}`
    ).toBeTruthy();
    const matches = ((await searchResponse.json()) as ApiSecurity[]).filter(
      (security) => security.isin === fixture.isin && security.currency === fixture.currency
    );
    ...
    if (quote?.date === fixture.transferDate) {
      expect(quote.close, `closing price for ${fixture.isin} on ${fixture.transferDate}`).toBeCloseTo(
        fixture.quotation,
        8
      );
      return;
    ...
    if (!createResponse.ok()) {
      ...
      expect(quote?.date, `creating ${fixture.isin} quote on ${fixture.transferDate}: ${createError}`).toBe(
        fixture.transferDate
      );
    } else {
      quote = await getHistoryquoteDateClose(page, quoteUrl, headers);
    }
 ...
 */
export async function reconcileSecurityTransfer(
  page: Page,
  fixture: SecurityTransferFixture,
  resolved: ResolvedSecurityTransferFixture
): Promise<void> {
  await test.step('reconcile prior security transfer', async () => {
    const headers = await authHeaders(page);
    const tree = await getSecurityActionTree(page, headers);
    const matches = (tree.clientTransfers ?? []).filter((transfer) => matchesFixture(transfer, fixture, resolved));
    ...
    if (matches.length === 1) {
      ...
      expect(
        response.ok(),
        `reversing transfer ${matches[0].idSecurityTransfer}: ${await response.text()}`
      ).toBeTruthy();
    }
    ...
    await expect
      .poll(
        async () => {
          ...
          return (refreshed.clientTransfers ?? []).filter((transfer) => matchesFixture(transfer, fixture, resolved))
            .length;
        },
        { message: 'matching transfer after startup reconciliation', timeout: 15_000 }
      )
      .toBe(0);

    await expect
      .poll(async () => await getHoldingUnits(page, headers, resolved.source.idSecurityAccount, fixture), {
...
export async function openSecurityTransferDialog(
  ...
  return await test.step('open security-transfer form', async () => {
    ...
    const dialog = page
      ...
      .filter({ has: page.locator('select#idPortfolioTarget') })
      .first();
...
export async function submitSecurityTransfer(
  ...
  await test.step('submit security-transfer form from fixture', async () => {
    ...
    const responsePromise = page.waitForResponse(
      (response) =>
        response.request().method() === 'POST' && new URL(response.url()).pathname === '/api/securityaction/transfer',
      { timeout: 30_000 }
    );
...
export async function expectSecurityTransfer(
  page: Page,
  fixture: SecurityTransferFixture,
  resolved: ResolvedSecurityTransferFixture
): Promise<void> {
  await test.step('verify persisted security transfer', async () => {
    ...
    await expect
      .poll(async () => await getHoldingUnits(page, headers, resolved.source.idSecurityAccount, fixture), {
        message: 'source holding after transfer',
        timeout: 30_000
      })
      .toBe(0);
    await expect
      .poll(async () => await getHoldingUnits(page, headers, resolved.target.idSecurityAccount, fixture), {
        message: 'target holding after transfer',
        timeout: 30_000
      })
      .toBe(fixture.units);
  });
...
function resolveAccount(portfolios: ApiPortfolio[], portfolioName: string, accountName: string): ResolvedAccount {
  const matches = portfolios
    .filter((portfolio) => portfolio.name === portfolioName)
    .flatMap((portfolio) =>
...
async function getSecurityActionTree(page: Page, headers: Record<string, string>): Promise<ApiSecurityActionTreeData> {
...
async function getHistoryquoteDateClose(
  page: Page,
  url: string,
  headers: Record<string, string>
): Promise<ApiHistoryquoteDateClose | null> {
...
function matchesFixture(
  transfer: ApiSecurityTransfer,
  fixture: SecurityTransferFixture,
  resolved: ResolvedSecurityTransferFixture
): boolean {
  return (
    transfer.security?.name === fixture.securityName &&
...
async function getHoldingUnits(
  page: Page,
  headers: Record<string, string>,
  idSecurityAccount: number,
  fixture: SecurityTransferFixture
  ...
  const matches = (summary.securityPositionGroupSummaryList ?? [])
    ...
    .filter(
      (position) =>
        position.security?.name === fixture.securityName &&
        position.security?.isin === fixture.isin &&
        position.security?.currency === fixture.currency
    );
  expect(
    matches.length,
    `holding ${fixture.isin}/${fixture.currency} in account ${idSecurityAccount}`
