...
export function findSecurityAction(tree: ApiSecurityActionTreeData,
    fixture: IsinChangeFixture): ApiSecurityAction | undefined {
  return (tree.systemActions ?? []).find(action => action.isinOld === fixture.isinOld
    && action.isinNew === fixture.isinNew && action.actionDate === fixture.actionDate);
}
...
export async function createIsinChange(page: Page, fixture: IsinChangeFixture, locale: string): Promise<void> {
  await test.step(`create ${fixture.isinOld} -> ${fixture.isinNew}`, async () => {
    ...
    await createDialog.locator('form-input-button').filter({has: page.locator('#isinOld')})
    ...
    await expect(searchDialog.locator('select#idStockexchange option'))
      .not.toHaveCount(0, {timeout: 15_000});
    await typeText(searchDialog.locator('#isin'), fixture.isinOld);
    await searchDialog.locator('button[type="submit"]').click();

    const searchRow = searchDialog.locator('securitycurrency-search-and-set-table tbody tr')
    ...
    const responsePromise = page.waitForResponse(response => new URL(response.url()).pathname === '/api/securityaction'
...
export async function reverseActiveApplication(page: Page, action: ApiSecurityAction,
    application: ApiSecurityActionApplication | undefined): Promise<void> {
  ...
  const response = await page.request.post(`/api/securityaction/${action.idSecurityAction}/reverse`, {
    headers: await authHeaders(page), failOnStatusCode: false
  });
...
export async function expectIsinChangeMail(page: Page, fixture: IsinChangeFixture,
    sendRecv: 'S' | 'R'): Promise<void> {
  ...
  const message = `Security ISIN ${fixture.isinOld} changed to ${fixture.isinNew} on ${fixture.actionDate}. `
    + 'Apply in Security Actions.';
  const matches = inbox.mailSendRecvList.filter(mail => mail.sendRecv === sendRecv
    && mail.subject === subject && mail.message === message);
...
export function expectSecurityAction(action: ApiSecurityAction | undefined,
    fixture: IsinChangeFixture, appliedCount: number): void {
...
export function securityActionRow(page: Page, action: ApiSecurityAction): Locator {
  return securityActionContainer(page).locator('configurable-tree-table tbody tr')
...
function parseFixtureRow(line: string, lineNumber: number): IsinChangeFixture {
  ...
  return {
    createNickname: columns[0], applyNickname: columns[1], isinOld: columns[2], isinNew: columns[3],
    actionDate: columns[4], note: columns[5], fromFactor, toFactor, expectedAffectedCount,
...
function systemActionsRow(page: Page): Locator {
  return securityActionContainer(page).locator('configurable-tree-table tbody tr')
    .filter({has: page.locator('td').first().filter({hasText: RX.systemActions})}).first();
}

function securityActionContainer(page: Page): Locator {
  return page.locator('.data-container')
    .filter({has: page.getByRole('heading', {name: RX.securityAction})}).first();
}
