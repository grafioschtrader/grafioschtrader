 ...
 *
 * Must run BEFORE 015-create-portfolio.spec.ts: the securities account dialog there picks its plan by
...
test.describe.serial('Create trading platform plans (e2e=\'e\' rows from shared CSV)', () => {
  ...
  for (const row of rows) {
    test(`creates TPP: ${row.platformPlanNameEN} (${row.importPlatformName})`, async ({page}) => {
      ...
      const tppTreeNode = page.locator('.p-tree-node-content',
        {hasText: /(Trading\s*platform\s*plan|Handelsplattform\s*Plan)/i}).first();
      ...
      await menuList.getByText(
        /(Create.*Trading.*platform.*plan|Erstellen.*Handelsplattform\s*Plan)/i).first().click();
