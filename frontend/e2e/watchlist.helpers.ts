...
export function loadPlaywrightInstrumentWatchlists(): WatchlistFixture[] {
  return loadWatchlistFixture().watchlists.filter(
    (row) => (row.instrumentE2E ?? row.e2e) === 'e' && row.securities.length > 0
  );
}
...
export async function ensureWatchlist(page: Page, watchlist: WatchlistFixture): Promise<void> {
  ...
  if ((await watchlistNode.count()) > 0) {
    ...
    await menu
      .getByText(/(Create|Erstellen)\s*Watchlist/i)
      .first()
      .click();
...
async function ensureMainWatchlist(page: Page, watchlistNode: Locator): Promise<void> {
  ...
  await menu.waitFor({ state: 'visible', timeout: 5_000 });
  await menu
    .getByRole('menuitem', { name: /^(For performance|Für Performance)/i })
    .first()
    .click();
 ...
 */
export async function ensureWatchlistSecuritiesBySubCategorySearch(
  page: Page,
  ...
  await expect(subCategorySelect.locator('option', { hasText: exactText(subCategoryNLS) })).toHaveCount(1, {
    timeout: 15_000
  });
  await subCategorySelect.selectOption(subCategoryNLS);

  const searchResponsePromise = page.waitForResponse(
    (response) => response.request().method() === 'GET' && /\/api\/watchlist\/\d+\/search(?:\?|$)/.test(response.url())
  );
  ...
  await expect(watchlistRows.first(), `${watchlist.name} contains ${subCategoryNLS} securities`).toBeVisible({
    timeout: 15_000
  });
 ...
 */
export async function ensureWatchlistSecuritiesBySearch(page: Page, watchlist: WatchlistFixture): Promise<void> {
  ...
  for (const security of expectedSecurities) {
    if (
      !(await watchlistSecurityRow(page, container, security)
        .isVisible()
        .catch(() => false))
    ) {
  ...
  for (const security of expectedSecurities) {
    await expect(
      watchlistSecurityRow(page, container, security),
      `watchlist row ${security.name ?? security.isin}/${security.currency}`
    ).toBeVisible({ timeout: 15_000 });
  }
}

async function addSecuritiesThroughSearch(
  page: Page,
  container: Locator,
  securities: WatchlistSecurityFixture[],
  field: 'isin' | 'name',
  ...
  for (const security of securities) {
    ...
    await expect(resultRow, `exact search result ${key}/${security.currency}`).toHaveCount(1, { timeout: 15_000 });
...
async function openAddInstrumentDialog(page: Page, container: Locator): Promise<Locator> {
  ...
  await menu.waitFor({ state: 'visible', timeout: 5_000 });
  await menu
    .getByText(/(Bestehendes\s+Instrument\s+hinzuf.gen|Add\s+existing\s+instrument)/i)
    .first()
    .click();
